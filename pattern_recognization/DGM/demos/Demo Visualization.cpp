...
struct USER_DATA {
    ...
    int imgWidth;
    USER_DATA( CGraph& _graph, CMarkerHistogram& _marker, int _imgWidth )
        : graph( static_cast< CGraphPairwise& >( _graph ) )
        , marker( _marker )
        , imgWidth( _imgWidth ) {}
};
...
int main( int argc, char* argv[] ) {
    ...
    resize( img, img, imgSize, 0, 0, INTER_LANCZOS4 );   // image
    Mat fv = imread( argv[2], 1 );
    resize( fv, fv, imgSize, 0, 0, INTER_LANCZOS4 );   // feature vector
    Mat gt = imread( argv[3], 0 );
    resize( gt, gt, imgSize, 0, 0, INTER_NEAREST );   // groundtruth
