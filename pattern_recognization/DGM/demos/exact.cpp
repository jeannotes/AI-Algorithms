...
void CExact::fillGraph( CGraphPairwise& graph ) {
    ...
    edgePot.at< float >( 0, 0 ) = 2.0f;
    edgePot.at< float >( 0, 1 ) = 1.0f;
    edgePot.at< float >( 1, 0 ) = 1.0f;
    edgePot.at< float >( 1, 1 ) = 2.0f;
...
void CExact::Main( void ) {
    ...
    printf( "\t\t\t\tINFERENCE\n\t" );
    for ( i = 0; i < nNodes - 1; i++ ) printf( "   Node %zd\t|", i );
    printf( "   Node %zd\n\t", i );
    for ( i = 0; i < nNodes - 1; i++ ) printf( " False\tTrue\t|" );
    printf( " False\tTrue\n" );
    printf( "--------------------------------------------------------------------\n" );
    ...
    printf( "Node%zd\n", i );
    printf( "---------------------------------------------\n" );
    printf( "Exact decoder:\t" );
    for ( i = 0; i < nNodes; i++ ) printf( decoding_decoderExcact[i] ? "true\t" : "false\t" );
    printf( "\n" );
    printf( "Exact inferer:\t" );
    for ( i = 0; i < nNodes; i++ ) printf( decoding_infererExcact[i] ? "true\t" : "false\t" );
    printf( "\n" );
    printf( "LBP inferer:\t" );
    for ( i = 0; i < nNodes; i++ ) printf( decoding_infererLBP[i] ? "true\t" : "false\t" );
    printf( "\n" );
    printf( "Vtrbi inferer:\t" );
    for ( i = 0; i < nNodes; i++ ) printf( decoding_infererViterbi[i] ? "true\t" : "false\t" );
    printf( "\n" );
}
