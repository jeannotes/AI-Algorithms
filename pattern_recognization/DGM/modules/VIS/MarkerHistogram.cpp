...
Mat CMarkerHistogram::drawClassificationMap2D( float Z ) const {
    ...
    if ( nFeatures == 2 ) {
        ...
        DGM_WARNING( "The number of features (%d) is not 2", nFeatures );
...
Mat CMarkerHistogram::drawHistogram( Scalar color ) const {
    ...
    for ( word f = 0; f < nFeatures; f++ ) {   // freatures
        int dx = f / fMaxHeight;
        dx *= ( 256 + 2 * margin.width );
        int dy = f % fMaxHeight;
        dy *= ( 100 + margin.height );
...
Mat CMarkerHistogram::drawFeatureHistogram( word f, int activeState ) const {
    ...
    Mat res( margin.height + 100, 2 * margin.width + 256, CV_8UC3 );
    res.setTo( bkgIntencity );
    Mat tmp( res.size(), res.type() );
    tmp.setTo( 0 );
    ...
    // Feature Names
    if ( m_vFeatureNames.empty() )
        sprintf( str, "feature %d", f );
    else
        sprintf( str, "%s", m_vFeatureNames[f].c_str() );
...
Mat CMarkerHistogram::drawFeatureHistogram2D( word f, int activeState ) const {
    ...
    if ( ( typeid( m_nodeTrainer ) == typeid( CTrainNodeBayes ) ) && ( nFeatures == 2 ) ) {
        ...
        DGM_WARNING( "The node trainer (%s) is not Bayes or the number of features (%d) is not 2", typeid( m_nodeTrainer ).name(), nFeatures );
...
Mat CMarkerHistogram::drawLegend( int maxHeight, int activeState ) const {
    ...
    // White "all" color box
    int dx = nStates / sMaxHeight;
    dx *= 120;
    int dy = nStates % sMaxHeight;
    dy *= 2 * margin.height;
