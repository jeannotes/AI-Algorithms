...
CMarker::CMarker( default_pallete palette ) {
    switch ( palette ) {
        case DEF_PALETTE_3:
            for ( int h = 0; h < 360; h += 120 ) m_vPalette.push_back( std::make_pair( colorspaces::hsv2rgb( DGM_HSV( h, 255, 255 ) ), "" ) );
            break;
        case DEF_PALETTE_3_INV:
            for ( int h = 0; h < 360; h += 120 ) m_vPalette.push_back( std::make_pair( colorspaces::hsv2bgr( DGM_HSV( h, 255, 255 ) ), "" ) );
            break;
        case DEF_PALETTE_6:
            for ( int h = 0; h < 360; h += 60 ) m_vPalette.push_back( std::make_pair( colorspaces::hsv2rgb( DGM_HSV( h, 255, 255 ) ), "" ) );
            break;
        case DEF_PALETTE_6_INV:
            for ( int h = 0; h < 360; h += 60 ) m_vPalette.push_back( std::make_pair( colorspaces::hsv2bgr( DGM_HSV( h, 255, 255 ) ), "" ) );
            break;
        case DEF_PALETTE_12:
            for ( int h = 0; h < 360; h += 30 ) m_vPalette.push_back( std::make_pair( colorspaces::hsv2rgb( DGM_HSV( h, 255, 255 ) ), "" ) );
            break;
        case DEF_PALETTE_12_INV:
            for ( int h = 0; h < 360; h += 30 ) m_vPalette.push_back( std::make_pair( colorspaces::hsv2bgr( DGM_HSV( h, 255, 255 ) ), "" ) );
            break;
        case DEF_PALETTE_24:
            for ( int h = 0; h < 360; h += 15 ) m_vPalette.push_back( std::make_pair( colorspaces::hsv2rgb( DGM_HSV( h, 255, 255 ) ), "" ) );
            break;
        case DEF_PALETTE_24_INV:
            for ( int h = 0; h < 360; h += 15 ) m_vPalette.push_back( std::make_pair( colorspaces::hsv2bgr( DGM_HSV( h, 255, 255 ) ), "" ) );
            break;
        case DEF_PALETTE_24_M:
            for ( int i = 0; i < 24; i++ ) m_vPalette.push_back( std::make_pair( colors24[i], "" ) );
            break;
        case DEF_PALETTE_36:
            for ( int h = 0; h < 360; h += 10 ) m_vPalette.push_back( std::make_pair( colorspaces::hsv2rgb( DGM_HSV( h, 255, 255 ) ), "" ) );
            break;
        case DEF_PALETTE_36_INV:
            for ( int h = 0; h < 360; h += 10 ) m_vPalette.push_back( std::make_pair( colorspaces::hsv2bgr( DGM_HSV( h, 255, 255 ) ), "" ) );
            break;
        case DEF_PALETTE_72:
            for ( int h = 0; h < 360; h += 5 ) m_vPalette.push_back( std::make_pair( colorspaces::hsv2rgb( DGM_HSV( h, 255, 255 ) ), "" ) );
            break;
        case DEF_PALETTE_72_INV:
            for ( int h = 0; h < 360; h += 5 ) m_vPalette.push_back( std::make_pair( colorspaces::hsv2bgr( DGM_HSV( h, 255, 255 ) ), "" ) );
            break;
    }
}

// Constuctor
CMarker::CMarker( const vec_nColor_t& vPalette )
    : m_vPalette( vPalette ) {}
...
void CMarker::markClasses( Mat& base, const Mat& classes, byte flag ) const {
    ...
    for ( int y = 0; y < base.rows; y++ ) {
        ...
        for ( int x = 0; x < base.cols; x++ ) {
            ...
            for ( int c = 0; c < 3; c++ )
                if ( mapping && !( ( flag & MARK_OVER ) == MARK_OVER ) )
                    pBase[3 * x + c] = static_cast< byte >( ( pBase[3 * x + c] + m_vPalette.at( pClass[x] % n ).first.val[c] ) / 2 );
                else
                    pBase[3 * x + c] = static_cast< byte >( m_vPalette.at( pClass[x] % n ).first.val[c] );
        }   // x
    }       // y
}
...
Mat CMarker::drawPotentials( const Mat& potential, byte flag ) const {
    ...
    if ( potential.dims == 2 ) {
        if ( potential.cols == 1 )
            return drawVector( potential, flag );   // Node potential
        else
            return drawMatrix( potential, flag );   // Edge potential
    } else
        return drawVoxel( potential, flag );   // Triplet potential
}
...
Mat CMarker::drawConfusionMatrix( const Mat& confusionMat, byte flag ) const {
    ...
    if ( perClass ) {
        ...
        cMat = drawMatrix( confusionMat, flag | TP_PERCENT );
    ...
    tmp = Mat( 25 - 1, nStates * ds - 1, CV_8UC3 );
    tmp.setTo( bkgIntencity );
...
Mat CMarker::drawVector( const Mat& potential, byte flag ) const {
    ...
    Mat res( ds + 1, ds * nStates + 1, CV_8UC3 );
    res.setTo( bkgIntencity );
    ...
    for ( byte s = 0; s < nStates; s++ ) {
        ...
        if ( m_vPalette.at( s % n ).second.empty() )
            sprintf( str, "c%d", s );
        else
            sprintf( str, "%s", m_vPalette.at( s % n ).second.c_str() );
...
Mat CMarker::drawMatrix( const Mat& potential, byte flag ) const {
    ...
    Mat res( ds * ( nStates + 1 ) + 1, ds * ( nStates + 1 ) + 1, CV_8UC3 );
    res.setTo( bkgIntencity );

    drawSquare( res, 0, 0, frgColor, "" );

    for ( byte x = 1; x < nStates + 1; x++ ) {
        if ( m_vPalette.at( ( x - 1 ) % n ).second.empty() )
            sprintf( str, "W%d", x - 1 );
        else
            sprintf( str, "%s", m_vPalette.at( ( x - 1 ) % n ).second.c_str() );
        if ( x == 1 ) sprintf( str, "Bkgrd." );
        drawSquare( res, x, 0, frgColor, str, 0.45, TP_BOTTOM );
    }   // x

    for ( byte y = 1; y < nStates + 1; y++ ) {
        if ( m_vPalette.at( ( y - 1 ) % n ).second.empty() )
            sprintf( str, "W%d", y - 1 );
        else
            sprintf( str, "%s", m_vPalette.at( ( y - 1 ) % n ).second.c_str() );
...
void CMarker::drawRectangle( Mat& img, Point pt1, Point pt2, const Scalar& color, float val, double fontScale, byte textProp ) const {
    ...
    if ( procent ) {
        ...
        if ( std::isnan( val ) )
            ...
            sprintf( str, "O" );
        else if ( ( val < 0.01f ) || ( val > 9999.99f ) )
            sprintf( str, "%1.1E", val );
        else
