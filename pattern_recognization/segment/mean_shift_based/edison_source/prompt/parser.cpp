...
int CmCParser::Parse( char* filename ) {
    ...
    while ( cur != EOF ) {
        ...
        if ( blockComment ) {
            ptr1                                                     = strchr( buf, '*' );
            if ( ( ptr1 ) && ( *( ptr1 + 1 ) == '/' ) ) blockComment = false;
            ptr1 += 2;
        }
        ...
        while ( !blockComment ) {
            ...
            // get token
            ptr2 = ptr1;
            while ( ( !IsDelimiter( *ptr2 ) ) && ( *ptr2 != ' ' ) && ( *ptr2 != '\t' ) &&
                    ( *ptr2 != 0 ) )
                ptr2++;
            ch    = *ptr2;
            *ptr2 = 0;
            ...
            if ( ( storeDelimiters_ ) && ( IsDelimiter( ch ) ) ) InsertToken( ch, curLineNumber );
