...
int GetTokenSet( int cmdId, CmCToken** tokenSet, CmCParser* synParser ) {
    ...
    for ( i = 0; i < tokenCount; i++ ) {
        if ( !( tokenSet[i] = synParser->GetToken() ) ) return SYN_ERROR;
    }
...
int checkInteger( char* data ) {
    char *dptr = data, ch;
    while ( ch = *( dptr++ ) )
        if ( ( ( ch < 48 ) || ( ch > 57 ) ) && ( ch != '-' ) ) return SYN_ASSIGN_INTEGER;
    return NO_ERRORS;
}
...
int checkBounds( int paramId, char* data ) {
    ...
    if ( UBOUND_LIST[paramId] > 0 ) {
        if ( ( dataVal <= 0 ) || ( dataVal < LBOUND_LIST[paramId] ) ||
             ( dataVal > UBOUND_LIST[paramId] ) )
            return SYN_BOUNDS_ERROR;
    } else {
        if ( ( dataVal <= 0 ) || ( dataVal < LBOUND_LIST[paramId] ) ) return SYN_BOUNDS_ERROR;
    }
    return NO_ERRORS;
}
...
int CheckParameter( int paramId, char* data ) {
    ...
    switch ( PARAMTYPE_LIST[paramId] ) {
        ...
        case PARAM_FLOAT:
            ...
            if ( !error ) error = checkBounds( paramId, data );
            break;
        case PARAM_SPEEDUP_TYPE: error = checkSpeedup( data ); break;
        case PARAM_CURVE_TYPE: error   = checkCurveType( paramId, data ); break;
    }
    return error;
}
...
int CheckList( CmCParser* synParser, CmCToken** token ) {
    ...
    do {
        ...
        ( *token ) = synParser->GetToken();
        if ( ( ( *token )->token_[0] != ',' ) && ( ( *token )->token_[0] != '}' ) )
            return SYN_MISSING_RIGHT_CURL;
    ...
    while ( token = synParser->GetToken() ) {
        ...
        switch ( commandTypeId ) {
            case CMD_IO:
                switch ( commandId ) {
                    case CMD_SAVE:
                        // check structure
                        if ( strcmp( tokenSet[0]->token_, "(" ) )
                            return CmCError( tokenSet[0], SYN_MISSING_LEFT_PARENTHESIS );
                        if ( strcmp( tokenSet[1]->token_, "'" ) )
                            return CmCError( tokenSet[1], SYN_MISSING_QUOTATION );
                        if ( strcmp( tokenSet[3]->token_, "'" ) )
                            return CmCError( tokenSet[3], SYN_MISSING_QUOTATION );
                        if ( strcmp( tokenSet[4]->token_, "," ) )
                            return CmCError( tokenSet[4], SYN_MISSING_COMMA );
                        if ( strcmp( tokenSet[6]->token_, "," ) )
                            return CmCError( tokenSet[6], SYN_MISSING_COMMA );
                        if ( strcmp( tokenSet[8]->token_, ")" ) )
                            return CmCError( tokenSet[8], SYN_MISSING_RIGHT_PARENTHESIS );
                        if ( strcmp( tokenSet[9]->token_, ";" ) )
                            return CmCError( tokenSet[9], SYN_MISSING_SEMICOLON );

                        // check constants
                        if ( !validFileType( tokenSet[5]->token_ ) )
                            return CmCError( tokenSet[5], SYN_INVALID_FILETYPE );
                        if ( !validOutputType( tokenSet[7]->token_ ) )
                            return CmCError( tokenSet[7], SYN_INVALID_OUTPUTTYPE );
                    ...
                    case CMD_LOAD:
                        ...
                        if ( strcmp( tokenSet[4]->token_, "," ) )
                            return CmCError( tokenSet[4], SYN_MISSING_COMMA );
                        if ( strcmp( tokenSet[6]->token_, ")" ) )
                            return CmCError( tokenSet[6], SYN_MISSING_RIGHT_PARENTHESIS );
                        if ( strcmp( tokenSet[7]->token_, ";" ) )
                            return CmCError( tokenSet[7], SYN_MISSING_SEMICOLON );

                        // check constants
                        if ( !validInputType( tokenSet[5]->token_ ) )
                            return CmCError( tokenSet[5], SYN_INVALID_INPUTTYPE );
                        break;

                    case CMD_USE_RESULT:
                        if ( strcmp( tokenSet[0]->token_, "(" ) )
                            return CmCError( tokenSet[0], SYN_MISSING_LEFT_PARENTHESIS );
                        if ( strcmp( tokenSet[2]->token_, ")" ) )
                            return CmCError( tokenSet[2], SYN_MISSING_RIGHT_PARENTHESIS );
                        if ( strcmp( tokenSet[3]->token_, ";" ) )
                            return CmCError( tokenSet[3], SYN_MISSING_SEMICOLON );
            ...
            case CMD_EXECUTION:
                if ( tokenSet[0]->token_[0] != ';' )
                    return CmCError( tokenSet[0], SYN_MISSING_SEMICOLON );
                break;
            case CMD_FLAGS:
                if ( !validFlag( tokenSet[0]->token_ ) )
                    return CmCError( tokenSet[0], SYN_INVALID_FLAG );
                if ( tokenSet[1]->token_[0] != ';' )
                    return CmCError( tokenSet[1], SYN_MISSING_SEMICOLON );
                break;
            // unknown command
            default: break;
        }
        ...
        if ( commandTypeId == UNKNOWN_COMMAND ) {
            ...
            if ( parameterId != UNKNOWN_PARAMETER ) {
                ...
                if ( strcmp( tokenSet[0]->token_, "=" ) )
                    return CmCError( tokenSet[0], SYN_MISSING_EQUALS );
                if ( strcmp( tokenSet[2]->token_, ";" ) )
                    return CmCError( tokenSet[2], SYN_MISSING_SEMICOLON );
...
int findMissing( void** paramTable, int cmd ) {
    switch ( cmd ) {
        ...
        case CMD_EDGE_DETECT: {
            ...
            int paramId;
            for ( paramId = PARAM_MINIMUM_LENGTH; paramId <= PARAM_HYSTERISIS_LOW_TYPE;
                  paramId++ ) {
    ...
    if ( CmCSynergistic ) {
        int paramId;
        for ( paramId = PARAM_GRADIENT_WINDOW_RADIUS; paramId <= PARAM_EDGE_STRENGTH_THRESHOLD;
              paramId++ ) {
            if ( !paramTable[paramId] ) return paramId;
        }
    }
...
CmCError Run( CmCParser* script ) {
    ...
    int i;
    for ( i = 0; i < PARAM_NUM + 2 * CUST_CURVE_NUM; i++ ) paramTable[i] = (void*) NULL;
        ...
        if ( tokenId != UNKNOWN_COMMAND ) {
            ...
            if ( error ) {
                if ( error == EXE_MISSING_PARAM ) {
                    CmCToken* ertok = new CmCToken(
                        (char*) PARAM_LIST[findMissing( paramTable, Command( token->token_ ) )],
                        token->lineNumber_ );
...
float* CmCConvertToFloat( unsigned char* image, int height, int width, int dim ) {
    ...
    int i;
    for ( i = 0; i < height * width * dim; i++ ) { data[i] = (float) ( image[i] ); }
    return data;
}
...
int CmCWriteImage( char* filename, unsigned char* image, int height, int width, int dim,
    ...
    switch ( filetype ) {
        ...
        default: return EXE_UNSUPPORTED_FILE_FORMAT;
    }
    if ( !error )
        return NO_ERRORS;
    else
...
int CmCWriteMFile( char* filename, float* data, int rows, int cols, int n ) {
    ...
    for ( i = 0; i < rows; i++ ) {
        for ( j = 0; j < cols - 1; j++ ) {
            for ( k = 0; k < n; k++ ) { fprintf( fp, "%f ", data[n * ( rows * i + j ) + k] ); }
        }
        for ( k = 0; k < n - 1; k++ ) {
