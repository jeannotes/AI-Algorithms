...
int EDISON::Save( char* filename, int filetype, int outputtype ) {
    ...
    switch ( outputtype ) {
        case OUTPUT_SEGM_BOUNDARIES:
            ...
            error = writeImage( filename, (unsigned char*) NULL, boundaries_, numBoundaries_,
                                filetype );
            break;
...
int EDISON::Load( char* filename, int inputtype ) {
    ...
    switch ( inputtype ) {
        case INPUT_IMAGE:
            ...
            if ( !error ) Refresh();
            break;
        case INPUT_MAP: error = loadMap( filename ); break;
    }
    return error;
}
...
int EDISON::writeImage( char* filename, unsigned char* image, int* dataPoints, int n,
                        int filetype ) {
    ...
    if ( dataPoints ) {
        int i, j, x, y;
        for ( i = 0; i < n; i++ ) {
            for ( j = 0; j < dim_; j++ ) { data[dim_ * dataPoints[i] + j] = 255; }
        }
    }
...
int EDISON::saveData( char* filename, float* data, int filetype ) {
    if ( filetype != FILE_MATLAB_ASCII ) {
        ...
        int i;
        for ( i = 0; i < height_ * width_; i++ ) { imData[i] = (int) ( 255 * data[i] + 0.5 ); }
...
int EDISON::EdgeDetect( void ) {
    ...
    edgeDetector.DoEdgeDetect( &inputImage, &edgeList, nmxRank, nmxConf, hystHighRank, hystHighConf,
                               hystLowRank, hystLowConf, minLength, nmxType, hystHighType,
                               hystLowType );
    ...
    for ( i = 0; i < numEdges_; i++ ) { edges_[i] = edgey[i] * width_ + edgex[i]; }
    edgesDefined_ = true;
    delete[] edgex;
    delete[] edgey;
...
int EDISON::meanShift( int action ) {
    ...
    if ( iProc.ErrorStatus ) { return EXE_ERROR; }
    ...
    if ( CmCSynergistic ) {
        if ( CmCUseCustomWeightMap ) {
            ...
            iProc.SetWeightMap( custMap_,
                                *( (float*) ( parameterList_[PARAM_EDGE_STRENGTH_THRESHOLD] ) ) );
        } else {
            int error = ComputeWeightMap();
            if ( error ) return error;
            iProc.SetWeightMap( weightMap_,
                                *( (float*) ( parameterList_[PARAM_EDGE_STRENGTH_THRESHOLD] ) ) );
        }
        if ( iProc.ErrorStatus ) { return EXE_ERROR; }
    }
    ...
    switch ( action ) {
        case EDISON_FILTER:
            ...
            iProc.Filter( sigmaS, sigmaR,
                          ( SpeedUpLevel )( *(int*) ( parameterList_[PARAM_SPEEDUP] ) ) );
            if ( iProc.ErrorStatus ) { return EXE_ERROR; }
    ...
    numBoundaries_         = 0;
    int i;
    for ( i = 0; i < numRegions; i++ ) { numBoundaries_ += regionList->GetRegionCount( i ); }
    if ( boundariesDefined_ ) delete[] boundaries_;
    boundaries_ = new int[numBoundaries_];
    for ( i = 0; i < numBoundaries_; i++ ) { boundaries_[i] = regionIndeces[i]; }
...
int EDISON::ComputeWeightMap( void ) {
    ...
    // compute weight map
    if ( !weightMapDefined_ || !gradMapDefined_ || !confMapDefined_ || CmCGradWinChanged ||
         CmCMixtureChanged ) {
