...
void BgImage::SetImage( unsigned char* im, int x, int y, bool colorIm ) {
    ...
    if ( colorIm_ == false ) {
        for ( i = 0, its = im, itd = im_; i < x * y; i++ ) { *( itd++ ) = *( its++ ); }
    } else {
        for ( i = 0, its = im, itd = im_; i < x * y * 3; i++ ) { *( itd++ ) = *( its++ ); }
    }
}
...
void BgImage::SetSameImageFromRGB( unsigned char* im ) {
    ...
    if ( colorIm_ == false ) {
        ...
        for ( i = 0, its = im, itd = im_; i < x_ * y_ * 3; i++ ) *( itd++ ) = *( its++ );
    }
}
...
void BgImage::GetImage( unsigned char* im ) {
    ...
    int i;
    if ( colorIm_ == false ) {
        for ( i = 0, its = im_, itd = im; i < x_ * y_; i++ ) *( itd++ ) = *( its++ );
    } else {
        for ( i = 0, its = im_, itd = im; i < x_ * y_ * 3; i++ ) *( itd++ ) = *( its++ );
    }
}
...
inline unsigned char gBgImPt( BgImage* in_im, int in_r, int in_c ) {
    assert( in_im->hasIm_ && ( in_r >= 0 ) && ( in_r < in_im->y_ ) && ( in_c >= 0 ) &&
            ( in_c < in_im->x_ ) );
...
void BgImage::PrivateCopyToThis( const BgImage& im ) {
    ...
    int ncopy, i;
    ncopy = x_ * y_;
    if ( colorIm_ == true ) ncopy *= 3;

    unsigned char* src;
    src = im.im_;
    for ( i = 0; i < ncopy; i++ ) im_[i] = src[i];
}
