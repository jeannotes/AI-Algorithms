...
int write_pgm_image( const char* outfilename, unsigned char* image, int rows, int cols,
...
void write_MATLAB_ASCII( char* filename, float* data, int rows, int cols ) {
    ...
    int i, j;
    for ( i = 0; i < rows; i++ ) {
        for ( j = 0; j < cols - 1; j++ ) { fprintf( fp, "%10.6f ", data[i * rows + j] ); }
        fprintf( fp, "%10.6f\n", data[i * rows + cols - 1] );
    }
    fclose( fp );
}
...
void bgZoomIn( unsigned char** dest, unsigned char* src, int w, int h, int zconst,
               bool interpolate ) {
    // if dest or src is NULL or zconst is < 1 exit
    if ( ( !( *dest ) ) || ( !src ) || ( zconst < 1 ) ) return;
...
void bgZoomOut( unsigned char** dest, unsigned char* src, int w, int h, int zconst,
                bool interpolate ) {
    // if dest or src is NULL or zconst is <= 1 exit
    if ( ( !( *dest ) ) || ( !src ) || ( zconst <= 1 ) ) return;
