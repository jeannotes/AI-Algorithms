...
float BgEdgeDetect::CustomRegionEval( float r, float c ) {
    ...
    for ( i = 0; i < ( ncust_ - 1 ); i++ ) {
        if ( ( tcusty_[i] > 0 && tcusty_[i + 1] <= 0 ) ||
            ...
            x = ( tcustx_[i] * tcusty_[i + 1] - tcustx_[i + 1] * tcusty_[i] ) /
                ( tcusty_[i + 1] - tcusty_[i] );
            if ( x > 0 ) crossings++;
        }
    }
...
void BgEdgeDetect::GenerateMaskAngle( double* a, double theta ) {
    ...
    for ( i = WL_; i >= -WL_; i-- ) {
        for ( j = -WL_; j <= WL_; j++ ) {
            ...
            sind                              = 0;
            if ( scor[0] != 0 ) nscor[sind++] = scor[0];
            if ( scor[1] != 0 ) nscor[sind++] = scor[1];
            if ( scor[2] != 0 ) nscor[sind++] = scor[2];
            if ( scor[3] != 0 ) nscor[sind++] = scor[3];

            sflag = 0;
            for ( k = 1; k < sind; k++ ) {
                if ( nscor[k] != nscor[0] ) sflag++;
            }
            ...
            if ( sflag != 0 ) {
                for ( k = 0; k < 4; k++ ) intrs[0][k] = intrs[1][k] = 0.0;
                ...
                lsigind                              = 0;
                if ( scor[0] > 0 ) sigind[lsigind++] = 0;
                if ( scor[1] > 0 ) sigind[lsigind++] = 1;
                if ( scor[2] > 0 ) sigind[lsigind++] = 2;
                if ( scor[3] > 0 ) sigind[lsigind++] = 3;

                if ( lsigind == 1 ) {
                    a[colind + rowind * WW_] =
                        0.5 * fabs( cordi[0][0] - cordi[0][1] ) * fabs( cordi[1][0] - cordi[1][1] );
                }
                if ( lsigind == 2 ) {
                    ...
                    if ( diffin[0] == 1 ) {
                        ...
                        a[colind + rowind * WW_] =
                            0.5 * ( fabs( comcoor - cordi[1][0] ) + fabs( comcoor - cordi[1][1] ) );
                    }
                    if ( diffin[1] == 1 ) {
                        comcoor = corner[0][sigind[0]];
                        a[colind + rowind * WW_] =
                            0.5 * ( fabs( comcoor - cordi[0][0] ) + fabs( comcoor - cordi[0][1] ) );
                    }
                }
                if ( lsigind == 3 ) {
                    a[colind + rowind * WW_] =
    ...
    // A=A-mean(mean(A));
    comcoor = 0;
    for ( i = 0; i < WW_ * WW_; i++ ) comcoor += a[i];
    comcoor /= WW_ * WW_;
    for ( i = 0; i < WW_ * WW_; i++ ) a[i] -= comcoor;

    // A=A/norm(A,'fro')
    comcoor = 0;
    for ( i = 0; i < WW_ * WW_; i++ ) comcoor += a[i] * a[i];
...
void BgEdgeDetect::DeleteLookTable() {
    int i;
    for ( i = 0; i < NO_ANGLES; i++ ) { delete[] lookTable_[i]; }
}

void BgEdgeDetect::GetPixels( int* nopix, int* pixx, int* pixy, double x1, double x2, double y1,
                              double y2 ) {
...
void BgEdgeDetect::GetNmxPixels( int* nopix, int* pixx, int* pixy, double x1, double x2, double y1,
                                 double y2 ) {
    double minx, maxx, miny, maxy;
    if ( x1 < x2 ) {
        minx = x1;
...
void BgEdgeDetect::DoRecompute( BgEdgeList* cel, double nmxr, double nmxc, double rh, double ch,
                                double rl, double cl, int nMin, int nmxType, int hystTypeHigh,
                                int hystTypeLow ) {
    float *tr, *tc, *tdh, *tdl;
...
void BgEdgeDetect::SaveNmxValues() {
    ...
    for ( j = 0; j < y_; j++ ) {
        for ( i = 0; i < x_; i++ ) { fprintf( fd, "%f ", *( permNmxRank_ + j * x_ + i ) ); }
        fprintf( fd, "\n" );
    }
    fclose( fd );
    ...
    for ( j = 0; j < y_; j++ ) {
        for ( i = 0; i < x_; i++ ) { fprintf( fd, "%f ", *( permNmxConf_ + j * x_ + i ) ); }
        fprintf( fd, "\n" );
    }
    fclose( fd );
}
...
*/
void BgEdgeDetect::DoEdgeDetect( BgImage* cim, BgEdgeList* cel, double nmxr, double nmxc, double rh,
                                 double ch, double rl, double cl, int nMin, int nmxType,
                                 int hystTypeHigh, int hystTypeLow ) {
    x_ = cim->x_;
    y_ = cim->y_;
    bgLog( "Start edge detection...\n" );
    ...
    for ( j = WL_; j < ( y_ - WL_ ); j++ ) {
        ...
        for ( i = WL_; i < ( x_ - WL_ ); i++, itim++, itgx++, itgy++, itcee++ ) {
            if ( ( fabs( *itgx ) + fabs( *itgy ) ) > TOL_E ) {
                ...
                v1 /= WW2;
                for ( l = 0; l < WW2; l++ ) ti[l] -= v1;

                // A/norm(A,'fro')
                v1 = 0;
                for ( l = 0; l < WW2; l++ ) v1 += ti[l] * ti[l];
                v1      = sqrt( v1 );
                for ( l = 0; l < WW2; l++ ) ti[l] /= v1;

                // global
                v1 = 0;
                for ( l = 0; l < WW2; l++ ) v1 += tae[l] * ti[l];
                v1      = fabs( v1 );
                *itcee  = (float) v1;
            } else {
                *itcee = 0;
...
void BgEdgeDetect::GaussFilter( BgImage* cim, float* fim, double sigma, int width ) {
    ...
    im = cim->im_;
    if ( width == -2 ) {
        for ( i = 0; i < x_ * y_; i++ ) fim[i] = im[i];
        return;
    }

    if ( width < 3 ) width = (int) ( 1 + 2 * ceil( 2.5 * sigma ) );
    int tail               = width / 2;
    width                  = 2 * tail + 1;
    ...
    for ( i = 0; i < width; i++ ) filter[i] /= sum;
    ...
    for ( j = 0; j < y_; j++ ) {
        for ( i = tail; i < ( x_ - tail ); i++ ) {
            sum = 0;
            for ( k         = -tail; k <= tail; k++ ) sum += im[j * x_ + i + k] * filter[k + tail];
            tim[j * x_ + i] = sum;
        }
    ...
    for ( i = 0; i < x_; i++ ) {
        for ( j = tail; j < ( y_ - tail ); j++ ) {
            sum = 0;
            for ( k = -tail; k <= tail; k++ ) sum += tim[( j + k ) * x_ + i] * filter[k + tail];
...
void BgEdgeDetect::GaussDiffFilter( BgImage* cim, float* grx, float* gry, float* rezIm ) {
    ...
    for ( i = 0; i < x_; i++ ) {
        for ( j = WL_; j < ( y_ - WL_ ); j++ ) {
            sum = 0;
            for ( k         = -WL_; k <= WL_; k++ ) sum += im[( j + k ) * x_ + i] * sf[k + WL_];
            tim[j * x_ + i] = sum;
        }
    }
    // diff on x
    for ( j = 0; j < y_; j++ ) {
        for ( i = WL_; i < ( x_ - WL_ ); i++ ) {
            sum = 0;
            for ( k         = -WL_; k <= WL_; k++ ) sum += tim[j * x_ + i + k] * df[k + WL_];
            grx[j * x_ + i] = (float) ( sum );
        }
    }

    // filter image y
    for ( i = 0; i < x_ * y_; i++ ) tim[i] = 0;
    im                                     = cim->im_;
    // smooth on x
    for ( j = 0; j < y_; j++ ) {
        for ( i = WL_; i < ( x_ - WL_ ); i++ ) {
            sum = 0;
            for ( k         = -WL_; k <= WL_; k++ ) sum += im[j * x_ + i + k] * sf[k + WL_];
            tim[j * x_ + i] = sum;
        }
    }
    // diff on y
    for ( i = 0; i < x_; i++ ) {
        for ( j = WL_; j < ( y_ - WL_ ); j++ ) {
            sum = 0;
            for ( k         = -WL_; k <= WL_; k++ ) sum += tim[( j + k ) * x_ + i] * df[k + WL_];
...
void BgEdgeDetect::CompRanks( float* strength, float* ranks ) {
    ...
    for ( ii = 1; ii < x_ * y_; ii++ ) {
        if ( ra[ii] > ZERO_TRESH ) {
            ...
            if ( ra[ii] > ra[ii - 1] ) irra++;
        }
    }
    irra--;
    for ( ii = 0; ii < x_ * y_; ii++ ) ranks[ii] /= irra;
...
void BgEdgeDetect::StrConfEstim( float* ranks, float* confidence, float* rezult,
                                 float ( BgEdgeDetect::*feval )( float, float ) ) {
    int i;
    for ( i = 0; i < x_ * y_; i++ ) { rezult[i] = ( this->*feval )( ranks[i], confidence[i] ); }
}
...
void BgEdgeDetect::Strength( float* grx, float* gry, float* strength ) {
    ...
    for ( j = 0; j < y_; j++ )
        for ( i = 0; i < x_; i++ ) {
            val = sqrt( ( (double) ( *itgx * ( *( itgx++ ) ) ) ) +
                        ( (double) ( *itgy * ( *( itgy++ ) ) ) ) );
            *( its++ ) = (float) ( val );
        }
}

void BgEdgeDetect::NewNonMaxSupress( float* rank, float* conf, float* grx, float* gry,
                                     float* nmxRank, float* nmxConf,
                                     float ( BgEdgeDetect::*feval )( float, float, float,
                                                                     float ) ) {
    int i, j;
    float* itr;
    float* itc;
    ...
    for ( i = 0; i < x_ * y_; i++ ) { itnmxr[i] = itnmxc[i] = 0; }
    for ( i = 0; i < x_; i++ ) {
        itr[i] = itc[i]          = 0;
...
void BgEdgeDetect::NewHysteresisTr( float* edge, float* low, BgEdgeList* cel, int nMin, float* mark,
                                    float* coord ) {
    float* tm;
    float* te;
    float* tl;
    ...
    n = 0;
    for ( i = 0, tm = mark; i < x_ * y_; i++, tm++ ) *tm = 0;
...
void BgEdgeDetect::SetCustomHigh( int* x, int* y, int n, int sx, int sy ) {
    ...
    for ( i = 0; i <= n; i++ ) bgLog( " %f", hcustx_[i] );
    bgLog( "\n" );
    bgLog( " hist high custom y: " );
    for ( i = 0; i <= n; i++ ) bgLog( " %f", hcusty_[i] );
    bgLog( "\n" );
}
...
void BgEdgeDetect::SetCustomLow( int* x, int* y, int n, int sx, int sy ) {
    ...
    for ( i = 0; i <= n; i++ ) bgLog( " %f", lcustx_[i] );
    bgLog( "\n" );
    bgLog( " low custom y: " );
    for ( i = 0; i <= n; i++ ) bgLog( " %f", lcusty_[i] );
    bgLog( "\n" );
}
