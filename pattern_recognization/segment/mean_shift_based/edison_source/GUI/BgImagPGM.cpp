...
bool bgPGMHandler::LoadFile( wxImage* image, wxInputStream& stream, bool verbose,
                             int WXUNUSED( index ) ) {
    ...
    if ( !ptr ) {
        if ( verbose ) wxLogError( _( "PGM: Couldn't allocate memory." ) );
        return FALSE;
    }
...
bool bgPGMHandler::SaveFile( wxImage* image, wxOutputStream& stream, bool WXUNUSED( verbose ) ) {
    ...
    for ( i = 0; i < width * height; i++ ) {
        tptr[i] = (unsigned char) ( RED_WEIGHT * ptr[idx] + GREEN_WEIGHT * ptr[idx + 1] +
                                    BLUE_WEIGHT * ptr[idx + 2] );
        idx += 3;
    }
...
bool bgPGMHandler::DoCanRead( wxInputStream& stream ) {
    ...
    if ( stream.GetC() == 'P' ) {
        switch ( stream.GetC() ) {
            case '2':
            case '5': stream.SeekI( pos ); return TRUE;
        }
    }
    stream.SeekI( pos );
    return FALSE;
