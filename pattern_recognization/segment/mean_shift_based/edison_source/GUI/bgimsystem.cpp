...
void bgLogFile( const char* szFormat, ... ) {
    if ( glogfile == 0 ) glogfile = fopen( "filelog.txt", "w" );
    va_list argptr;
    va_start( argptr, szFormat );
...
bool BgApp::OnInit() {
    ...
    g_frame = new BgMdiFrame(
        (wxFrame*) NULL, -1, "Edge Detection and Image SegmentatiON System (EDISON)",
        wxPoint( 10, 10 ), wxSize( 1024, 768 ), wxDEFAULT_FRAME_STYLE | wxHSCROLL | wxVSCROLL );
#ifdef __WXMSW__
#if 0
...
// Give it an icon
    ...
    file_menu->Append( BG_NEW_EDGE_WINDOW, "New &edge window\tAlt-E", "Create a new edge detect "
                                                                      "window" );
    file_menu->Append( BG_NEW_SEGM_WINDOW, "New &segment window\tAlt-S", "Create a new "
                                                                         "segmentation window" );
    file_menu->Append( BG_LOAD_IMAGE_EDGE, "&Load edge image\tCtrl-L", "Load a new image to "
                                                                       "perform edge detection" );
    file_menu->Append( BG_SEGM_LOAD_IMAGE, "&Load segment image\tShift-L", "Load a new image to "
...
BgMdiFrame::BgMdiFrame( wxWindow* parent, const wxWindowID id, const wxString& title,
...
void BgMdiFrame::OnClose( wxCloseEvent& event ) {
    ...
    // bgFileLog
    if ( glogfile != 0 ) fclose( glogfile );

    event.Skip();
}

// sets the title of the active child frame
void BgMdiFrame::SetChildTitle( wxMDIChildFrame* activeChild, int zconst, int maxZoom,
                                int minZoom ) {
    wxString title;
...
void BgMdiFrame::OnAbout( wxCommandEvent& WXUNUSED( event ) ) {
    ...
    BgDialog aboutDialog( this, -1, "About EDISON", wxPoint( -1, -1 ), wxSize( 450, 300 ),
                          wxDEFAULT_DIALOG_STYLE | wxDIALOG_MODAL | wxBORDER | wxSYSTEM_MENU,
                          "aboutDialog" );
    wxFont myFont( 9, wxSWISS, wxNORMAL, wxBOLD, false );
    BgText bgText( 0, "Edge Detection and Image SegmentatiON System (EDISON) v1.1", myFont,
                   BG_DIALOG_INDENT, BG_DIALOG_TOP_MARGIN );
    ...
    aboutDialog.AddText( &bgText );
    wxBitmap riul_logo( "riul_logo", wxBITMAP_TYPE_RESOURCE ),
        caip_logo( "caip_logo", wxBITMAP_TYPE_RESOURCE ),
        rutgers_logo( "rutgers_logo", wxBITMAP_TYPE_RESOURCE );
...
void BgMdiFrame::OnNewEdgeWindow( wxCommandEvent& WXUNUSED( event ) ) {
    ...
    BgMdiEdgeChild* subframe =
        new BgMdiEdgeChild( g_frame, "Edge Detection Frame", wxPoint( -1, -1 ), wxSize( -1, -1 ),
...
// Give it an icon
    ...
    file_menu->Append( BG_NEW_EDGE_WINDOW, "New &edge window\tAlt-E", "Create a new edge detection "
                                                                      "window" );
    file_menu->Append( BG_NEW_SEGM_WINDOW, "New &segment window\tAlt-S", "Create a new "
                                                                         "segmentation window" );
    file_menu->Append( BG_LOAD_IMAGE_EDGE, "&Load image\tCtrl-L", "Load image to perform edge "
                                                                  "detection" );
    ...
    subframe->miViewOrig_ =
        new wxMenuItem( view_menu, BG_EDGE_VIEW_ORIG, "Original", "View original", TRUE );
    subframe->miViewEdge_ =
        new wxMenuItem( view_menu, BG_EDGE_VIEW_EDGE, "Edge map", "View edge map", TRUE );
...
void BgMdiFrame::OnNewSegmWindow( wxCommandEvent& WXUNUSED( event ) ) {
    ...
    // Make another frame, containing a edge processing window
    BgMdiSegmentChild* subframe = new BgMdiSegmentChild(
        g_frame, "Segmentation Frame", wxPoint( -1, -1 ), wxSize( -1, -1 ), wxDEFAULT_FRAME_STYLE );
...
// Give it an icon
    ...
    file_menu->Append( BG_NEW_SEGM_WINDOW, "New &segment window\tAlt-S", "Create a new "
                                                                         "segmentation window" );
    file_menu->Append( BG_SEGM_LOAD_IMAGE, "&Load image\tShift-L", "Load image to perform "
                                                                   "segmentation" );
    ...
    wxMenuItem* segm_menu_item =
        new wxMenuItem( segm_menu, BG_SEGM_SPEEDUP, "Speedup", "", false, speedup_menu );
...
// get the file name
#if defined( __WXGTK__ ) || defined( __WXMOTIF__ )
    wxFileDialog filedialog( this, "Choose an image file", "", "", "*", wxOPEN );
#else
    wxFileDialog filedialog( this, "Choose an image file", "", "",
                             "Common image "
                             "files|*.png;*.bmp;*.gif;*.tif;*.tiff;*.jpg;*.pnm;*.pgm;*.ppm|PNG "
                             "files (*.png)|*.png|BMP files (*.bmp)|*.bmp|GIF files "
                             "(*.gif)|*.gif|TIFF files (*.tif)|*.tif|JPEG files (*.jpg)|*.jpg|PNM "
                             "files (*.pnm)|*.pnm|PGM/PPM files (*.pgm,*.ppm)|*.pgm;*.ppm",
                             wxOPEN );
#endif
...
void BgMdiFrame::ZoomControl( wxCommandEvent& event ) {
    ...
    if ( activeChild->GetId() == BG_EDGE_WINDOW ) {
        switch ( event.m_id ) {
            case BG_CROSS: ( (BgMdiEdgeChild*) activeChild )->ZoomWindow(); break;
            case BG_ZOOM_IN: ( (BgMdiEdgeChild*) activeChild )->ZoomIn(); break;
            case BG_ZOOM_OUT:
                ( (BgMdiEdgeChild*) activeChild )->ZoomOut();
                break;
            // BG_POINTER:
            default: ( (BgMdiEdgeChild*) activeChild )->NoZoom(); break;
        }
    } else if ( activeChild->GetId() == BG_SEGM_WINDOW ) {
        switch ( event.m_id ) {
            case BG_CROSS: ( (BgMdiSegmentChild*) activeChild )->ZoomWindow(); break;
            case BG_ZOOM_IN: ( (BgMdiSegmentChild*) activeChild )->ZoomIn(); break;
            case BG_ZOOM_OUT:
                ( (BgMdiSegmentChild*) activeChild )->ZoomOut();
                break;
            // BG_POINTER:
            default: ( (BgMdiSegmentChild*) activeChild )->NoZoom(); break;
        }
    }
...
void BgMdiFrame::InitToolBar( wxToolBar* toolBar ) {
    ...
    toolBar->AddTool( BG_NEW_EDGE_WINDOW, *( bitmaps[0] ), wxNullBitmap, FALSE, currentX, -1,
                      (wxObject*) NULL, "New edge window" );
    currentX += width + 5;
    toolBar->AddTool( BG_NEW_SEGM_WINDOW, *( bitmaps[1] ), wxNullBitmap, FALSE, currentX, -1,
                      (wxObject*) NULL, "New segment window" );
    currentX += width + 5;
    toolBar->AddTool( BG_LOAD_IMAGE, *( bitmaps[3] ), wxNullBitmap, FALSE, currentX, -1,
                      (wxObject*) NULL, "Load image for processing" );
    currentX += width + 5;
    toolBar->AddTool( BG_SAVE_RESULT, *( bitmaps[4] ), wxNullBitmap, FALSE, currentX, -1,
                      (wxObject*) NULL, "Save result" );
    currentX += width + 5;
    toolBar->AddSeparator();
    toolBar->AddTool( BG_CROSS, *( bitmaps[5] ), wxNullBitmap, TRUE, currentX, -1, (wxObject*) NULL,
                      "Zoom Window" );
    currentX += width + 5;
    ...
    currentX += width + 5;
    toolBar->AddSeparator();
    toolBar->AddTool( BG_HELP, *bitmaps[2], wxNullBitmap, FALSE, currentX, -1, (wxObject*) NULL,
                      "Help" );
    ...
    int i;
    for ( i = 0; i < BITMAP_COUNT; i++ ) delete bitmaps[i];
}
...
void BgCurveSet::DrawYourself( unsigned char* buf, int val ) {
    ...
    switch ( type_ ) {
        ...
        case FC_ELLIPSE:
            ...
            break;
        case FC_VERT_LINE: DrawLine( buf, val, x_[0], 0, x_[0], ys_ ); break;
        case FC_HORIZ_LINE: DrawLine( buf, val, 0, y_[0], xs_, y_[0] ); break;
        case FC_LINE: DrawLine( buf, val, 0, y_[0], x_[0], ys_ ); break;
        case FC_SQUARE_BOX:
        ...
        case FC_CUSTOM:
            ...
            // control points
            for ( j = 0; j < n_; j++ ) DrawPoint( buf, val, x_[j], y_[j] );
            break;
    }
}
...
void BgCurveSet::DrawEllipticArc( unsigned char* buf, int val, int x, int y, int w, int h, int sa,
                                  int ea ) {
    double xc, yc, rx, ry;
    rx = w / 2;
    ry = h / 2;
    ...
    switch ( type_ ) {
        ...
        case FC_ELLIPSE:
            if ( abs( x ) < 3 )
                ...
                ltodrag_ = 0;
            break;
        case FC_VERT_LINE: ltodrag_  = 0; break;
        case FC_HORIZ_LINE: ltodrag_ = 0; break;
        case FC_LINE:
            if ( abs( x ) < 3 )
                ltodrag_ = 1;
            else if ( abs( y - ys_ ) < 3 )
...
void BgCurveSet::DragTo( int x, int y ) {
    ...
    switch ( type_ ) {
        ...
        case FC_ELLIPSE:
            ...
            break;
        case FC_VERT_LINE: x_[0]  = x; break;
        case FC_HORIZ_LINE: y_[0] = y; break;
        case FC_LINE:
            // modify line to drag
            if ( ltodrag_ == 0 ) {
...
BgParameterHistoryBox::BgParameterHistoryBox( wxWindow* parent, wxWindowID id,
                                              ...
                                              const wxSize& size, int n, long style,
...
int BgTextList::AddText( BgText* text ) {
    ...
    int id                                                = text->id_;
    cur_                                                  = head_;
    while ( ( cur_ ) && ( cur_->text_->id_ != id ) ) cur_ = cur_->next_;
    ...
    // otherwise add it to existing list
        ...
        BgTextObj* temp;
        if ( ( temp = new BgTextObj( text ) ) == NULL ) return 1;

        temp->next_               = head_;
        if ( head_ == NULL ) cur_ = temp;
        head_                     = temp;
        itemcount_++;
    }
...
int BgTextList::RemoveText( int textId ) {
    cur_                                                      = head_;
    while ( ( cur_ ) && ( cur_->text_->id_ != textId ) ) cur_ = cur_->next_;
...
BgBitmapObj::BgBitmapObj( BgBitmap* bitmap ) {
    bitmap_ =
        new BgBitmap( bitmap->bitmap_, bitmap->id_, bitmap->location_x_, bitmap->location_y_ );
    next_ = (BgBitmapObj*) NULL;
}
...
int BgBitmapList::AddBitmap( BgBitmap* bitmap ) {
    BgBitmapObj* temp;
    if ( ( temp = new BgBitmapObj( bitmap ) ) == NULL ) return 1;
...
void BgBitmapList::RemoveBitmap( BgBitmap* bitmap ) {
    int id                                                  = bitmap->id_;
    cur_                                                    = head_;
    while ( ( cur_ ) && ( cur_->bitmap_->id_ != id ) ) cur_ = cur_->next_;
    DeleteBitmap();
}
...
// overloaded constructor
BgAxis::BgAxis( int start_x, int start_y, int length, int ticknum, int direction, float start_val,
                float stop_val ) {
    start_x_   = start_x;
    start_y_   = start_y;
    length_    = length;
...
BgDialog::BgDialog( wxWindow* parent, wxWindowID id, const wxString& title, const wxPoint& pos,
...
BgHoverBar::BgHoverBar( wxWindow* parent, wxWindowID id, int gradViewId, int confViewId,
                        int weitViewId, int custViewId, int gradSaveId, int confSaveId,
                        int weitSaveId )
    ...
    menuButton1_ = new wxBitmapButton( this, BG_CANVAS_VIEW_BUTTON,
                                       wxBitmap( "down_arrow", wxBITMAP_TYPE_RESOURCE ),
                                       wxPoint( 33, 2 ), wxSize( 18, 20 ) );
    menuButton2_ = new wxBitmapButton( this, BG_CANVAS_SAVE_BUTTON,
                                       wxBitmap( "down_arrow", wxBITMAP_TYPE_RESOURCE ),
                                       wxPoint( 92, 2 ), wxSize( 18, 20 ) );
...
BgMenuPanel::BgMenuPanel( wxWindow* parent, wxWindowID id, int gradViewId, int confViewId,
                          int weitViewId, int custViewId, int gradSaveId, int confSaveId,
                          int weitSaveId )
...
void BgLineSet::SetLines( int* xs, int* xe, int* ys, int* ye, double* lineParam, int n ) {
    ...
    for ( i = 0; i < ( 3 * n ); i++ ) lineParam_[i] = lineParam[i];
}
...
BgImCanvas::BgImCanvas( wxWindow* child_frame, wxWindow* parent, const wxPoint& pos,
                        const wxSize& size )
    : wxScrolledWindow( parent, -1, pos, size, wxSIMPLE_BORDER | wxVSCROLL | wxHSCROLL ) {
...
void BgImCanvas::OnCustomDeleteNode( wxCommandEvent& WXUNUSED( event ) ) {
    if ( ( lmEventCurve_ < 0 ) || ( lmEventCurve_ >= nCurveSets_ ) || ( lmEventNode_ < 0 ) ||
         ( lmEventNode_ >= ( curveSet_[lmEventCurve_]->n_ - 1 ) ) ) {
        return;
    }
    if ( lmEventNode_ == 0 ) {
...
void BgImCanvas::RemovePointSet( BgPointSet* ps ) {
    int i = 0;
    while ( i < nPointSets_ && pointSet_[i] != ps ) i++;
    if ( i < nPointSets_ ) {
        i++;
        while ( i < nPointSets_ ) {
    ...
    if ( ( point_map ) && ( ps->type_ == 1 ) ) {   // point
        ...
        tx = ps->x_;
        ty = ps->y_;
        for ( i = 0; i < nt; i++ ) point_map[ty[i] * width + tx[i]] = false;
    }
    ...
    if ( zImg ) {
        ...
        if ( zoom_level > 1 )
            bgZoomIn( &zImg, pimage->GetData(), pimage->GetWidth(), pimage->GetHeight(), zoom_level,
                      false );
...
void BgImCanvas::RemoveLineSet( BgLineSet* ls ) {
    int i = 0;
    while ( i < nLineSets_ && lineSet_[i] != ls ) i++;
    if ( i < nLineSets_ ) {
        i++;
        while ( i < nLineSets_ ) {
...
void BgImCanvas::RemoveCurveSet( BgCurveSet* cs ) {
    int i = 0;
    while ( i < nCurveSets_ && curveSet_[i] != cs ) i++;
    if ( i < nCurveSets_ ) {
        i++;
        while ( i < nCurveSets_ ) {
...
void BgImCanvas::AddHorizontalAxis( int start_x, int start_y, int length, int ticknum,
...
// adds horizontal axis
void BgImCanvas::AddVerticalAxis( int start_x, int start_y, int length, int ticknum,
                                  float start_val, float stop_val ) {
    if ( yAxis ) delete yAxis;
...
void BgImCanvas::SetImage( unsigned char* data, int w, int h, bool colorim ) {
    ...
    if ( colorim == false ) {
        ...
        for ( i = 0; i < ( 3 * w * h ); i++ ) { *( itTData++ ) = data[i]; }
    }
    SetScrollbars( 1, 1, ( pimage->GetWidth() ) / 1, ( pimage->GetHeight() ) / 1 );
...
void BgImCanvas::ShowBitmap( bool showbitmap ) {
    ...
    if ( hasbitmap ) {
        ...
        if ( !showbitmap_ )
            memset( zImg, 255,
                    3 * zoom_level * zoom_level * width * height * sizeof( unsigned char ) );
        else
            ...
            if ( pointSet_[i]->type_ == 0 ) {   // circle
                ...
                for ( j = 0; j < nt; j++ ) {
                    ...
                    for ( y = 0; y < zoom_level; y++ ) {
                        for ( x = 0; x < zoom_level; x++ ) { dc.DrawPoint( xc + x, yc + y ); }
                    }
                }
            }
        }
        ...
        for ( i = 0; i < nCurveSets_; i++ ) {
            ...
            switch ( curveSet_[i]->type_ ) {
                ...
                case FC_SQUARE_BOX:
                    ...
                    dc.DrawLine( txs + x_offset_, tys + y_offset_, txs + x_offset_,
                                 ys + y_offset_ );
                    break;
                case FC_CUSTOM:
                    for ( j = 0; j < ( nt - 1 ); j++ )
...
int BgImCanvas::Zoom( int zconst, int m_x, int m_y ) {
    ...
    if ( hasbitmap ) {
        ...
        // zoomed image
        if ( !showbitmap_ ) memset( zImg, 255, 3 * width * height * sizeof( unsigned char ) );
...
int BgImCanvas::ZoomIn( int m_x, int m_y ) {
    ...
    if ( hasbitmap ) {
        ...
        if ( ( max_zoom_level ) && ( zoom_level == max_zoom_level ) ) {
            ...
            ( (BgMdiFrame*) parent )
                ->UpdateZoomControl( (wxMDIChildFrame*) child_frame_, true, false );
            return 1;
        }
        ...
        wxWindow* parent = child_frame_->GetParent();
        ( (BgMdiFrame*) parent )
            ->SetChildTitle( (wxMDIChildFrame*) child_frame_, zoom_level, maxZoom, false );

        // update zoom control of child frame
        ( (BgMdiFrame*) parent )
            ->UpdateZoomControl( (wxMDIChildFrame*) child_frame_, maxZoom, false );
...
int BgImCanvas::ZoomOut( int m_x, int m_y ) {
    ...
    if ( hasbitmap ) {
        ...
        if ( ( min_zoom_level ) && ( zoom_level == min_zoom_level ) ) {
            wxWindow* parent = child_frame_->GetParent();
            ( (BgMdiFrame*) parent )
                ->UpdateZoomControl( (wxMDIChildFrame*) child_frame_, false, true );
            return 1;
        }
        ...
        wxWindow* parent = child_frame_->GetParent();
        ( (BgMdiFrame*) parent )
            ->SetChildTitle( (wxMDIChildFrame*) child_frame_, zoom_level, false, minZoom );

        // update zoom control of child frame
        ( (BgMdiFrame*) parent )
            ->UpdateZoomControl( (wxMDIChildFrame*) child_frame_, false, minZoom );
    ...
    if ( nPointSets_ > 0 ) {
        ...
        for ( i = 0; i < nPointSets_; i++ ) {
            ...
            if ( pointSet_[i]->type_ == 1 ) {   // point
                for ( j = 0; j < nt; j++ ) point_map[ty[j] * w + tx[j]] = true;
            }
        }
    }
}
...
void BgImCanvas::DisplayZoomWindow( int m_x, int m_y ) {
    ...
    if ( ( m_x < 0 ) || ( m_x >= w ) || ( m_y < 0 ) || ( m_y >= h ) ) return;
...
void BgImCanvas::FillCurveClick() {
    int i;
    for ( i = 0; i < ccx_ * ccy_; i++ ) curveClick_[i] = 0;

    for ( i = 0; i < nCurveSets_; i++ ) curveSet_[i]->DrawYourself( curveClick_, i + 1 );
    ...
    if ( x >= 0 && x < ccx_ && y >= 0 && y < ccy_ ) {
        if ( curveClick_[x + y * ccx_] > 0 ) {
            ...
            if ( curveSet_[lmEventCurve_]->type_ == FC_CUSTOM ) {
                ...
                mindist = sqrt( ( x - tempx[0] ) * ( x - tempx[0] ) +
                                ( y - tempy[0] ) * ( y - tempy[0] ) );
                minnode = 0;
                for ( i = 1; i < tempn; i++ ) {
                    crtdist = sqrt( ( x - tempx[i] ) * ( x - tempx[i] ) +
                                    ( y - tempy[i] ) * ( y - tempy[i] ) );
                    if ( crtdist < mindist ) {
                        mindist = crtdist;
                        minnode = i;
...
void BgImCanvas::OnEvent( wxMouseEvent& event ) {
    ...
    if ( event.Entering() ) {
        ...
        if ( activeChild == (wxMDIChildFrame*) child_frame_ ) current_child = true;
        ...
        if ( ( menuWindow ) && ( popup ) ) menuWindow->Show( TRUE );
    }
    ...
    if ( event.Leaving() ) {
        ...
        if ( zoom_window ) Refresh( false );

        // hide the menu window
        if ( ( hideWindow ) && ( popup ) && ( menuWindow ) ) menuWindow->Show( FALSE );
    }

    // check if the left mouse button has been clicked and zoom is activated
    // if so zoom the image
    if ( event.LeftDown() && zoom_in ) ZoomIn( m_x_, m_y_ );

    if ( ( event.LeftDown() ) && zoom_out ) ZoomOut( m_x_, m_y_ );

    if ( zoom_window ) DisplayZoomWindow( m_x_, m_y_ );
}
...
BgMdiEdgeChild::BgMdiEdgeChild( wxMDIParentFrame* parent, const wxString& title, const wxPoint& pos,
    ...
    imagePlotSplitter_ = new wxSplitterWindow( this, -1, wxPoint( bpsize_, 0 ), wxDefaultSize,
                                               wxSP_NOBORDER | wxSP_3DSASH );
    plotSplitter_ = new wxSplitterWindow( imagePlotSplitter_, -1, wxDefaultPosition, wxDefaultSize,
                                          wxSP_NOBORDER | wxSP_3DSASH );
    ...
    wxStaticBox* viewSB =
        new wxStaticBox( buttonPanel_, -1, "View", wxPoint( 35, 45 ), wxSize( 85, 65 ) );
    ...
    txtKernelSize_ =
        new wxStaticText( buttonPanel_, -1, "Grad Win.",
                          wxPoint( C_PARAMX - 5, deltal + 125 + C_PARAMY + 0 * C_PARAMDY ) );
    valKernelSize_ = new wxStaticText(
        buttonPanel_, -1, "NA     ",
        wxPoint( C_PARAMX + C_PARAMDX + 35, deltal + 125 + C_PARAMY + 0 * C_PARAMDY ),
        wxSize( C_PARAMSX - 20, C_PARAMSY ) );

    txtMinPt_ =
        new wxStaticText( buttonPanel_, -1, "Min. length",
                          wxPoint( C_PARAMX - 5, deltal + 125 + C_PARAMY + 1 * C_PARAMDY ) );
    valMinPt_ = new wxStaticText(
        buttonPanel_, -1, "NA     ",
        wxPoint( C_PARAMX + C_PARAMDX + 35, deltal + 125 + C_PARAMY + 1 * C_PARAMDY ),
        wxSize( C_PARAMSX - 20, C_PARAMSY ) );

    wxStaticBox* nmxSB =
        new wxStaticBox( buttonPanel_, -1, "Nonmaxima supp.", wxPoint( 5, deltal + 205 + 0 ),
                         wxSize( 140, 4 * C_PARAMDY - 5 ) );
    txtNmxType_ = new wxStaticText( buttonPanel_, -1, "Type",
                                    wxPoint( C_PARAMX, deltal + 205 + C_PARAMY + 0 * C_PARAMDY ) );
    valNmxType_ =
        new wxStaticText( buttonPanel_, -1, "NA    ",
                          wxPoint( C_PARAMX + C_PARAMDX, deltal + 205 + C_PARAMY + 0 * C_PARAMDY ),
                          wxSize( C_PARAMSX, C_PARAMSY ) );
    txtNmxR_ = new wxStaticText( buttonPanel_, -1, "Rank",
                                 wxPoint( C_PARAMX, deltal + 205 + C_PARAMY + 1 * C_PARAMDY ) );
    valNmxR_ =
        new wxStaticText( buttonPanel_, -1, "NA     ",
                          wxPoint( C_PARAMX + C_PARAMDX, deltal + 205 + C_PARAMY + 1 * C_PARAMDY ),
                          wxSize( C_PARAMSX, C_PARAMSY ) );
    txtNmxC_ = new wxStaticText( buttonPanel_, -1, "Conf",
                                 wxPoint( C_PARAMX, deltal + 205 + C_PARAMY + 2 * C_PARAMDY ) );
    valNmxC_ =
        new wxStaticText( buttonPanel_, -1, "NA     ",
    ...
    wxStaticBox* hhSB =
        new wxStaticBox( buttonPanel_, -1, "Hyst. High Tr.", wxPoint( 5, deltal + 325 + 0 ),
                         wxSize( 140, 4 * C_PARAMDY - 5 ) );
    txtHHType_ = new wxStaticText( buttonPanel_, -1, "Type",
                                   wxPoint( C_PARAMX, deltal + 325 + C_PARAMY + 0 * C_PARAMDY ) );
    txtHHR_ = new wxStaticText( buttonPanel_, -1, "Rank",
                                wxPoint( C_PARAMX, deltal + 325 + C_PARAMY + 1 * C_PARAMDY ) );
    txtHHC_ = new wxStaticText( buttonPanel_, -1, "Conf",
                                wxPoint( C_PARAMX, deltal + 325 + C_PARAMY + 2 * C_PARAMDY ) );
    valHHType_ =
        new wxStaticText( buttonPanel_, -1, "NA    ",
    ...
    wxStaticBox* hlSB =
        new wxStaticBox( buttonPanel_, -1, "Hyst. Low Tr.", wxPoint( 5, deltal + 445 + 0 ),
                         wxSize( 140, 4 * C_PARAMDY - 5 ) );
    txtHLType_ = new wxStaticText( buttonPanel_, -1, "Type",
                                   wxPoint( C_PARAMX, deltal + 445 + C_PARAMY + 0 * C_PARAMDY ) );
    txtHLR_ = new wxStaticText( buttonPanel_, -1, "Rank",
                                wxPoint( C_PARAMX, deltal + 445 + C_PARAMY + 1 * C_PARAMDY ) );
    txtHLC_ = new wxStaticText( buttonPanel_, -1, "Conf",
                                wxPoint( C_PARAMX, deltal + 445 + C_PARAMY + 2 * C_PARAMDY ) );
    valHLType_ =
        new wxStaticText( buttonPanel_, -1, "NA    ",
    ...
    // add x and y axis
    plotNmxImage_->AddHorizontalAxis( RANK_CONF_MARGINX - 2, RANK_CONF_IMSIZEY + RANK_CONF_MARGINY,
                                      RANK_CONF_IMSIZEX + 2, 10, 0.0, 1.0 );
    plotNmxImage_->AddVerticalAxis( RANK_CONF_MARGINX - 1, RANK_CONF_IMSIZEY + RANK_CONF_MARGINY,
                                    RANK_CONF_IMSIZEX, 10, 0.0, 1.0 );
    plotTotImage_->AddHorizontalAxis( RANK_CONF_MARGINX - 2, RANK_CONF_IMSIZEY + RANK_CONF_MARGINY,
                                      RANK_CONF_IMSIZEX + 2, 10, 0.0, 1.0 );
    plotTotImage_->AddVerticalAxis( RANK_CONF_MARGINX - 1, RANK_CONF_IMSIZEY + RANK_CONF_MARGINY,
                                    RANK_CONF_IMSIZEX, 10, 0.0, 1.0 );
                           ...
                           : wxMDIChildFrame(parent, BG_EDGE_WINDOW, title, pos, size, style)
{
   ...
   bpsize_ = 100;
   imagePlotSplitter_ = new wxSplitterWindow(this, -1,wxDefaultPosition, wxDefaultSize,
wxSP_NOBORDER | wxSP_3DSASH);
   plotSplitter_ = new wxSplitterWindow(imagePlotSplitter_, -1, wxDefaultPosition, wxDefaultSize,
wxSP_NOBORDER | wxSP_3DSASH);
   origEdgeImage_ = new BgImCanvas(this, imagePlotSplitter_, wxDefaultPosition, wxDefaultSize);
...
//   cbgEdgeDetect_ = new BgEdgeDetect(kernelSize_);
   ...
   //add title to each individual plot
   BgText bgText(1, "      Diagram after Non-Maxima Supression", *wxSWISS_FONT, RANK_CONF_MARGINX -
10, RANK_CONF_MARGINY/2-10);
   ...
   plotNmxImage_->AddVerticalAxis
(RANK_CONF_MARGINX-1,RANK_CONF_IMSIZEY+RANK_CONF_MARGINY,RANK_CONF_IMSIZEX,10,0.0,1.0);
   plotTotImage_->AddHorizontalAxis(RANK_CONF_MARGINX-2,RANK_CONF_IMSIZEY+RANK_CONF_MARGINY,RANK_CONF_IMSIZEX+2,10,0.0,1.0);
   plotTotImage_->AddVerticalAxis
(RANK_CONF_MARGINX-1,RANK_CONF_IMSIZEY+RANK_CONF_MARGINY,RANK_CONF_IMSIZEX,10,0.0,1.0);
   ...
   plotTotImage_->AddBitmap(&ro_bmp);
   wxBitmap rotated_eta("rotated_eta", wxBITMAP_TYPE_RESOURCE);
   BgBitmap rotated_eta_bmp(&rotated_eta, 4,
RANK_CONF_MARGINX-55,RANK_CONF_IMSIZEY+RANK_CONF_MARGINY-162);
   plotNmxImage_->AddBitmap(&rotated_eta_bmp);
   plotTotImage_->AddBitmap(&rotated_eta_bmp);
...
BgMdiEdgeChild::~BgMdiEdgeChild() {
    ...
    if ( hasEdge_ == 1 ) origEdgeImage_->RemovePointSet( &cbgPointSet_ );

    if ( cbgEdgeList_ != 0 ) delete cbgEdgeList_;
    delete cbgImage_;
    if ( cbgEdgeDetect_ != 0 ) delete cbgEdgeDetect_;
...
void BgMdiEdgeChild::SetParametersStr() {
    switch ( hystHighType_ ) {
        case FC_ELLIPSE: valHHType_->SetLabel( "arc" ); break;
        case FC_VERT_LINE: valHHType_->SetLabel( "vertical line" ); break;
        case FC_HORIZ_LINE: valHHType_->SetLabel( "horizontal line" ); break;
        case FC_SQUARE_BOX: valHHType_->SetLabel( "box" ); break;
        case FC_LINE: valHHType_->SetLabel( "line" ); break;
        case FC_CUSTOM: valHHType_->SetLabel( "custom" ); break;
    }

    switch ( hystLowType_ ) {
        case FC_ELLIPSE: valHLType_->SetLabel( "arc" ); break;
        case FC_VERT_LINE: valHLType_->SetLabel( "vertical line" ); break;
        case FC_HORIZ_LINE: valHLType_->SetLabel( "horizontal line" ); break;
        case FC_SQUARE_BOX: valHLType_->SetLabel( "box" ); break;
        case FC_LINE: valHLType_->SetLabel( "line" ); break;
        case FC_CUSTOM: valHLType_->SetLabel( "custom" ); break;
    }

    switch ( nmxType_ ) {
        case FC_ELLIPSE: valNmxType_->SetLabel( "arc" ); break;
        case FC_VERT_LINE: valNmxType_->SetLabel( "vertical line" ); break;
        case FC_HORIZ_LINE: valNmxType_->SetLabel( "horizontal line" ); break;
        case FC_SQUARE_BOX: valNmxType_->SetLabel( "box" ); break;
        case FC_LINE: valNmxType_->SetLabel( "line" ); break;
        case FC_CUSTOM: valNmxType_->SetLabel( "custom" ); break;
    }
    wxString ts;
...
void BgMdiEdgeChild::ReadImage( char* pathname, char* filename ) {
    ...
    if ( origEdgeImage_->SetImage( pathname ) == 0 ) return;
    bgLog( "Image %s loaded\n", pathname );
    ...
    cbgImage_->SetImageFromRGB( origEdgeImage_->pimage->GetData(),
                                origEdgeImage_->pimage->GetWidth(),
                                origEdgeImage_->pimage->GetHeight() );
    if ( cbgEdgeDetect_ != 0 ) delete cbgEdgeDetect_;
    cbgEdgeDetect_ = new BgEdgeDetect( kernelSize_ );
    hasImage_      = 1;
    hasEdge_       = 0;
...
// get the file name
//   wxFileDialog filedialog(this,"Choose an image file","","",
//      "All files (*.*)|*.*|PNG files (*.png)|*.png|BMP files (*.bmp)|*.bmp|GIF files
//      (*.gif)|*.gif|TIFF files (*.tif)|*.tif|JPEG files (*.jpg)|*.jpg|PNM files (*.pnm)|*.pnm",
//      wxOPEN);
#if defined( __WXGTK__ ) || defined( __WXMOTIF__ )
    wxFileDialog filedialog( this, "Choose an image file", "", "", "*", wxOPEN );
#else
    ...
    if ( filedialog.ShowModal() == wxID_OK ) {
        ...
        if ( origEdgeImage_->SetImage( filedialog.GetPath().c_str() ) == 0 ) return;
        bgLog( "Image %s loaded\n", filedialog.GetPath().c_str() );
...
void BgMdiEdgeChild::OnSaveEdgeMap( wxCommandEvent& WXUNUSED( event ) ) {
    ...
    // get the file name
    wxFileDialog filedialog( this, "Choose an image file", "", "", "PGM files (*.pgm)|*.pgm",
                             wxSAVE );

    if ( filedialog.ShowModal() == wxID_OK ) {
        BgImage tempImage( cbgImage_->x_, cbgImage_->y_ );
        cbgEdgeList_->SetBinImage( &tempImage );
        write_pgm_image( filedialog.GetPath().c_str(), tempImage.im_, tempImage.y_, tempImage.x_,
                         "", 255 );
...
void BgMdiEdgeChild::OnEdgeDetect( wxCommandEvent& WXUNUSED( event ) ) {
    ...
    if ( hystHighType_ == FC_CUSTOM ) cbgEdgeDetect_->SetCustomHigh( custHx_, custHy_, nCustH_ );
    if ( hystLowType_ == FC_CUSTOM ) cbgEdgeDetect_->SetCustomLow( custLx_, custLy_, nCustL_ );

    // determine if we have permanent data
    if ( cbgEdgeDetect_->havePerm_ == true ) {
        // compute only nmx and hyst
        cbgEdgeDetect_->DoRecompute( cbgEdgeList_, rankNmx_, confNmx_, rankH_, confH_, rankL_,
                                     confL_, nMin_, nmxType_, hystHighType_, hystLowType_ );
        // set only nmx image
        SetNmxImage();
    } else {
        // compute all steps
        cbgEdgeDetect_->DoEdgeDetect( cbgImage_, cbgEdgeList_, rankNmx_, confNmx_, rankH_, confH_,
                                      rankL_, confL_, nMin_, nmxType_, hystHighType_,
                                      hystLowType_ );
    ...
    // update image canvas
    if ( miViewEdge_->IsChecked() ) origEdgeImage_->AddPointSet( &cbgPointSet_ );
...
void BgMdiEdgeChild::OnChangeParam( wxCommandEvent& WXUNUSED( event ) ) {
    ...
    paramDialog.SetValues( rankNmx_, confNmx_, rankH_, confH_, rankL_, confL_, nMin_, nmxType_,
                           hystHighType_, hystLowType_, kernelSize_ );
    if ( paramDialog.ShowModal() == wxID_OK ) {
        // do change param stuff
        int tempKernelSize;
        tempKernelSize = kernelSize_;
        paramDialog.GetValues( rankNmx_, confNmx_, rankH_, confH_, rankL_, confL_, nMin_, nmxType_,
                               hystHighType_, hystLowType_, tempKernelSize );
...
BgParamDialog::BgParamDialog( wxWindow* parent, wxWindowID id, const wxString& title,
    ...
    txtHHR_ = new wxStaticText(this, -1, "Hyst. high rank: ",
    wxPoint(C_PARAMX,C_PARAMY+2*C_PARAMDY));
    txtHHC_ = new wxStaticText(this, -1, "Hyst. high conf: ",
    wxPoint(C_PARAMX,C_PARAMY+3*C_PARAMDY));
    txtHLR_ = new wxStaticText(this, -1, "Hyst. low rank: ",
    wxPoint(C_PARAMX,C_PARAMY+4*C_PARAMDY));
    txtHLC_ = new wxStaticText(this, -1, "Hyst. low conf: ",
    wxPoint(C_PARAMX,C_PARAMY+5*C_PARAMDY));
    ...
    txtHHType_ = new wxStaticText(this, -1, "Hyst. high type: ",
    wxPoint(C_PARAMX,C_PARAMY+8*C_PARAMDY));
    txtHLType_ = new wxStaticText(this, -1, "Hyst. low: ", wxPoint(C_PARAMX,C_PARAMY+9*C_PARAMDY));
    txtKernelSize_ = new wxStaticText(this, -1, "Kernel radius: ",
    wxPoint(C_PARAMX,C_PARAMY+10*C_PARAMDY));

    valNmxR_ = new wxTextCtrl(this, -1, "NA     ",
    wxPoint(C_PARAMX+C_PARAMDX,C_PARAMY+0*C_PARAMDY));
    valNmxC_ = new wxTextCtrl(this, -1, "NA     ",
    wxPoint(C_PARAMX+C_PARAMDX,C_PARAMY+1*C_PARAMDY));
    ...
    wxPoint(C_PARAMX+C_PARAMDX,C_PARAMY+6*C_PARAMDY));
    valNmxType_ = new wxChoice(this, -1, wxPoint(C_PARAMX+C_PARAMDX,C_PARAMY+7*C_PARAMDY),
    wxDefaultSize);
    valHHType_ = new wxChoice(this, -1, wxPoint(C_PARAMX+C_PARAMDX,C_PARAMY+8*C_PARAMDY),
    wxDefaultSize);
    valHLType_ = new wxChoice(this, -1, wxPoint(C_PARAMX+C_PARAMDX,C_PARAMY+9*C_PARAMDY),
    wxDefaultSize);
    valKernelSize_ = new wxTextCtrl(this, -1, "NA     ",
    wxPoint(C_PARAMX+C_PARAMDX,C_PARAMY+10*C_PARAMDY));
    */

    txtKernelSize_ = new wxStaticText( this, -1, "Grad Win.",
                                       wxPoint( 20 + C_PARAMX - 5, 0 + C_PARAMY + 0 * C_PARAMDY ) );
    valKernelSize_ = new wxTextCtrl( this, -1, "NA     ", wxPoint( 20 + C_PARAMX + C_PARAMDX + 35,
                                                                   0 + C_PARAMY + 0 * C_PARAMDY ),
                                     wxSize( C_PARAMSX - 20, C_PARAMSY ) );

    txtMinPt_ = new wxStaticText( this, -1, "Min. length",
                                  wxPoint( 20 + C_PARAMX - 5, 0 + C_PARAMY + 1 * C_PARAMDY ) );
    valMinPt_ = new wxTextCtrl( this, -1, "NA     ", wxPoint( 20 + C_PARAMX + C_PARAMDX + 35,
                                                              0 + C_PARAMY + 1 * C_PARAMDY ),
                                wxSize( C_PARAMSX - 20, C_PARAMSY ) );

    wxStaticBox* nmxSB = new wxStaticBox( this, -1, "Nonmaxima supp.", wxPoint( 20 + 5, 80 + 0 ),
                                          wxSize( 140, 4 * C_PARAMDY - 5 ) );
    txtNmxType_ = new wxStaticText( this, -1, "Type",
                                    wxPoint( 20 + C_PARAMX, 80 + C_PARAMY + 0 * C_PARAMDY ) );
    valNmxType_ =
        new wxChoice( this, -1, wxPoint( 20 + C_PARAMX + C_PARAMDX, 80 + C_PARAMY + 0 * C_PARAMDY ),
                      wxSize( C_PARAMSX, C_PARAMSY ) );
    txtNmxR_ = new wxStaticText( this, -1, "Rank",
                                 wxPoint( 20 + C_PARAMX, 80 + C_PARAMY + 1 * C_PARAMDY ) );
    valNmxR_ = new wxTextCtrl( this, -1, "NA     ",
                               wxPoint( 20 + C_PARAMX + C_PARAMDX, 80 + C_PARAMY + 1 * C_PARAMDY ),
                               wxSize( C_PARAMSX, C_PARAMSY ) );
    txtNmxC_ = new wxStaticText( this, -1, "Conf",
                                 wxPoint( 20 + C_PARAMX, 80 + C_PARAMY + 2 * C_PARAMDY ) );
    valNmxC_ = new wxTextCtrl( this, -1, "NA     ",
                               wxPoint( 20 + C_PARAMX + C_PARAMDX, 80 + C_PARAMY + 2 * C_PARAMDY ),
                               wxSize( C_PARAMSX, C_PARAMSY ) );

    wxStaticBox* hhSB = new wxStaticBox( this, -1, "Hyst. High Tr.", wxPoint( 20 + 5, 200 + 0 ),
                                         wxSize( 140, 4 * C_PARAMDY - 5 ) );
    txtHHType_ = new wxStaticText( this, -1, "Type",
                                   wxPoint( 20 + C_PARAMX, 200 + C_PARAMY + 0 * C_PARAMDY ) );
    txtHHR_ = new wxStaticText( this, -1, "Rank",
                                wxPoint( 20 + C_PARAMX, 200 + C_PARAMY + 1 * C_PARAMDY ) );
    txtHHC_ = new wxStaticText( this, -1, "Conf",
                                wxPoint( 20 + C_PARAMX, 200 + C_PARAMY + 2 * C_PARAMDY ) );
    valHHType_ = new wxChoice( this, -1,
                               wxPoint( 20 + C_PARAMX + C_PARAMDX, 200 + C_PARAMY + 0 * C_PARAMDY ),
                               wxSize( C_PARAMSX, C_PARAMSY ) );
    valHHR_ = new wxTextCtrl( this, -1, "NA     ",
                              wxPoint( 20 + C_PARAMX + C_PARAMDX, 200 + C_PARAMY + 1 * C_PARAMDY ),
                              wxSize( C_PARAMSX, C_PARAMSY ) );
    valHHC_ = new wxTextCtrl( this, -1, "NA     ",
                              wxPoint( 20 + C_PARAMX + C_PARAMDX, 200 + C_PARAMY + 2 * C_PARAMDY ),
                              wxSize( C_PARAMSX, C_PARAMSY ) );

    wxStaticBox* hlSB = new wxStaticBox( this, -1, "Hyst. Low Tr.", wxPoint( 20 + 5, 320 + 0 ),
                                         wxSize( 140, 4 * C_PARAMDY - 5 ) );
    txtHLType_ = new wxStaticText( this, -1, "Type",
                                   wxPoint( 20 + C_PARAMX, 320 + C_PARAMY + 0 * C_PARAMDY ) );
    txtHLR_ = new wxStaticText( this, -1, "Rank",
                                wxPoint( 20 + C_PARAMX, 320 + C_PARAMY + 1 * C_PARAMDY ) );
    txtHLC_ = new wxStaticText( this, -1, "Conf",
                                wxPoint( 20 + C_PARAMX, 320 + C_PARAMY + 2 * C_PARAMDY ) );
    valHLType_ = new wxChoice( this, -1,
                               wxPoint( 20 + C_PARAMX + C_PARAMDX, 320 + C_PARAMY + 0 * C_PARAMDY ),
                               wxSize( C_PARAMSX, C_PARAMSY ) );
    valHLR_ = new wxTextCtrl( this, -1, "NA     ",
                              wxPoint( 20 + C_PARAMX + C_PARAMDX, 320 + C_PARAMY + 1 * C_PARAMDY ),
                              wxSize( C_PARAMSX, C_PARAMSY ) );
    valHLC_ = new wxTextCtrl( this, -1, "NA     ",
                              wxPoint( 20 + C_PARAMX + C_PARAMDX, 320 + C_PARAMY + 2 * C_PARAMDY ),
                              wxSize( C_PARAMSX, C_PARAMSY ) );
...
void BgParamDialog::SetValues( double nmxR, double nmxC, double hhR, double hhC, double hlR,
                               double hlC, int nMin, int nmxT, int hhT, int hlT, int ks ) {
    wxString ts;
...
void BgParamDialog::GetValues( double& nmxR, double& nmxC, double& hhR, double& hhC, double& hlR,
                               double& hlC, int& nMin, int& nmxT, int& hhT, int& hlT, int& ks ) {
    double td;
    long tl;
    int ti;
    ...
    if ( ( ti = valNmxType_->GetSelection() ) != -1 ) nmxT = ti;

    if ( ( ti = valHHType_->GetSelection() ) != -1 ) hhT = ti;

    if ( ( ti = valHLType_->GetSelection() ) != -1 ) hlT = ti;
...
BgSpeedSelect::BgSpeedSelect( wxWindow* parent, wxWindowID id, const wxString& title,
    ...
    sldSpeed_ = new wxSlider( this, BG_SPEEDSEL_SLD, 0, 0, 100, wxPoint( 18, 40 ),
                              wxSize( 155, -1 ), wxSL_AUTOTICKS | wxSL_LABELS );
...
BgMdiSegmentChild::BgMdiSegmentChild( wxMDIParentFrame* parent, const wxString& title,
    ...
    imagePlotSplitter_ = new wxSplitterWindow( this, -1, wxDefaultPosition, wxDefaultSize,
                                               wxSP_NOBORDER | wxSP_3DSASH );
    plotMapSplitter_ = new wxSplitterWindow( imagePlotSplitter_, -1, wxDefaultPosition,
                                             wxDefaultSize, wxSP_NOBORDER | wxSP_3DSASH );
    mapSplitter_ = new wxSplitterWindow( plotMapSplitter_, -1, wxDefaultPosition, wxDefaultSize,
                                         wxSP_NOBORDER | wxSP_3DSASH );

    // create option panels
    winPanel1_ =
        new BgMenuPanel( mapSplitter_, -1, BG_CANVAS_VIEW1_GRADMAP, BG_CANVAS_VIEW1_CONFMAP,
                         BG_CANVAS_VIEW1_WEITMAP, BG_CANVAS_VIEW1_CUSTMAP, BG_CANVAS_SAVE_GRADMAP,
                         BG_CANVAS_SAVE_CONFMAP, BG_CANVAS_SAVE_WEITMAP );
    winPanel2_ =
        new BgMenuPanel( mapSplitter_, -1, BG_CANVAS_VIEW2_GRADMAP, BG_CANVAS_VIEW2_CONFMAP,
                         BG_CANVAS_VIEW2_WEITMAP, BG_CANVAS_VIEW2_CUSTMAP, BG_CANVAS_SAVE_GRADMAP,
                         BG_CANVAS_SAVE_CONFMAP, BG_CANVAS_SAVE_WEITMAP );
    ...
    plotWindow1_ =
        new BgImCanvas( this, winPanel1_, wxPoint( 0, PLOT_MENU_HEIGHT ), wxDefaultSize );
    plotWindow1_->SetScrollbars( 20, 20, 50, 50 );
    plotWindow2_ =
        new BgImCanvas( this, winPanel2_, wxPoint( 0, PLOT_MENU_HEIGHT ), wxDefaultSize );
    ...
    optionsPanel_ =
        new wxPanel( this, -1, wxPoint( 0, 0 ), wxSize( BG_SEGM_OP_SIZEX, BG_SEGM_OP_SIZEY ) );
    ...
    operationRadio_ =
        new wxRadioBox( optionsPanel_, BG_SEGM_OPERATION, "Operation", wxDefaultPosition,
                        wxSize( BG_SP_WIDTH, -1 ), 3, operations, 3, wxRA_SPECIFY_ROWS );
    ...
    viewImSegRadio_ =
        new wxRadioBox( optionsPanel_, BG_SEGM_VIEW_IMSEG, "View Image", wxDefaultPosition,
                        wxSize( BG_SP_WIDTH, -1 ), 4, choices, 4, wxRA_SPECIFY_ROWS );
    toplayout->Add( viewImSegRadio_, 0, wxCENTER | wxBOTTOM, 5 );

    viewBoundariesCheck_ =
        new wxCheckBox( optionsPanel_, BG_SEGM_VIEW_EDGES, "Overlay Boundaries" );
    toplayout->Add( viewBoundariesCheck_, 0, wxALIGN_LEFT | wxLEFT | wxBOTTOM, 10 );

    // add bandwidth parameters...
    subPanel1_ =
        new wxPanel( optionsPanel_, -1, wxDefaultPosition, wxSize( BG_SP_WIDTH, BG_SP_HEIGHT ) );
    subPanelBox1_ =
        new wxStaticBox( subPanel1_, -1, "Bandwidth", wxPoint( 0, 0 ),
                         wxSize( BG_SP_WIDTH, BG_SP_TOP_HEIGHT ), wxSIMPLE_BORDER, "staticBox" );
    subPanelBox2_ = new wxStaticBox( subPanel1_, -1, "", wxPoint( 0, BG_SP_TOP_HEIGHT - 8 ),
                                     wxSize( BG_SP_WIDTH, BG_SP_HEIGHT - BG_SP_TOP_HEIGHT ),
                                     wxSIMPLE_BORDER, "staticBox" );
    textSigmaS_ = new wxStaticText( subPanel1_, -1, "Spatial [2h+1]", wxPoint( BG_LEFT_CELL, 18 ) );
    txtSigmaS_ = new wxTextCtrl( subPanel1_, BG_SEGM_TEXT_SIGMAS, "7", wxPoint( BG_RIGHT_CELL, 15 ),
                                 txtSize );
    ...
    textMinRegion_ =
        new wxStaticText( subPanel1_, -1, "Minimum Region", wxPoint( BG_LEFT_CELL, 78 ) );
    ...
    subPanel2_ =
        new wxPanel( optionsPanel_, -1, wxDefaultPosition, wxSize( BG_SP_WIDTH, BG_SP_HEIGHT_2 ) );
    subPanelBox3_ =
        new wxStaticBox( subPanel2_, -1, "", wxPoint( 0, 0 ), wxSize( BG_SP_WIDTH, BG_SP_HEIGHT_2 ),
                         wxSIMPLE_BORDER, "staticBox" );
    textKernelSize_ =
        new wxStaticText( subPanel2_, -1, "Grad. Window (2n+1)", wxPoint( BG_LEFT_CELL, 18 ) );
    ...
    txtA_  = new wxTextCtrl( subPanel2_, BG_SEGM_TEXT_AIJ, "0.3", wxPoint( BG_RIGHT_CELL, 40 ),
                            txtSize );
    ...
    subPanelBox4_ = new wxStaticBox( subPanel3_, -1, "", wxPoint( 0, 0 ), wxSize( BG_SP_WIDTH, 43 ),
                                     wxSIMPLE_BORDER, "staticBox" );
    textParamBox_  = new wxStaticText( subPanel3_, -1, "History", wxPoint( BG_LEFT_CELL, 18 ) );
    paramComboBox_ = new BgParameterHistoryBox(
        subPanel3_, BG_SEGM_CHANGE_PARAMS, "", wxPoint( BG_RIGHT_CELL - 50, 15 ), wxSize( 100, 20 ),
        5, 0, wxDefaultValidator, "comboBox" );
    ...
    // add x and y axis
    phDiagram_->AddHorizontalAxis( RANK_CONF_MARGINX - 2, RANK_CONF_IMSIZEY + RANK_CONF_MARGINY,
                                   RANK_CONF_IMSIZEX + 2, 10, 0.0, 1.0 );
    phDiagram_->AddVerticalAxis( RANK_CONF_MARGINX - 1, RANK_CONF_IMSIZEY + RANK_CONF_MARGINY,
                                 RANK_CONF_IMSIZEX, 10, 0.0, 1.0 );
...
BgMdiSegmentChild::~BgMdiSegmentChild() {
    if ( hasBoundaries_ == 1 ) displayImage_->RemovePointSet( boundaries_ );
...
void BgMdiSegmentChild::OnChangeOperation( wxCommandEvent& WXUNUSED( event ) ) {
    ...
    switch ( operationRadio_->GetSelection() ) {
        ...
        case 1:
            ...
            break;
        // fuse regions
        default: menubar->SetLabel( BG_SEGM_SEGMENT, "Fuse Regions\tShift-R" ); break;
    }
...
void BgMdiSegmentChild::OnViewBoundaries( wxCommandEvent& WXUNUSED( event ) ) {
    if ( hasBoundaries_ == false ) return;
...
void BgMdiSegmentChild::OnSaveEdgeInformation( wxCommandEvent& event ) {
#if defined( __WXGTK__ ) || defined( __WXMOTIF__ )
    wxFileDialog filedialog( this, "Save Edge Information", "", "", "*", wxSAVE );
#else
    ...
    if ( filedialog.ShowModal() == wxID_OK ) {
        ...
        float* myData;
        switch ( event.GetId() ) {
            case BG_CANVAS_SAVE_GRADMAP: myData = gradMap_; break;
            case BG_CANVAS_SAVE_CONFMAP: myData = confMap_; break;
            case BG_CANVAS_SAVE_WEITMAP: myData = weightMap_; break;
        }
        ...
        if ( dtype ) {
            ...
            int i;
            for ( i = 0; i < height_ * width_; i++ ) buf[i] = myData[i] * 255 + 0.5;
...
void BgMdiSegmentChild::ReadImage( char* pathname, char* filename ) {
    ...
    if ( displayImage_->SetImage( pathname ) == 0 ) return;
    ...
    // adjust titles
    BgText title( 1, "Confidence Map", *wxSWISS_FONT, RANK_CONF_MARGINX + width_ / 2 - 45,
                  RANK_CONF_MARGINY / 2 - 10 );
    ...
    plotWindow1_->AddHorizontalAxis( RANK_CONF_MARGINX, RANK_CONF_MARGINY + height_, width_, 5, 0,
                                     width_ - 1 );
    plotWindow1_->AddVerticalAxis( RANK_CONF_MARGINX, RANK_CONF_MARGINY + height_, height_, 5, 0,
                                   height_ - 1 );
    plotWindow2_->AddHorizontalAxis( RANK_CONF_MARGINX, RANK_CONF_MARGINY + height_, width_, 5, 0,
                                     width_ - 1 );
    plotWindow2_->AddVerticalAxis( RANK_CONF_MARGINX, RANK_CONF_MARGINY + height_, height_, 5, 0,
                                   height_ - 1 );
    ...
    // set window title
    wxString statusname;
    statusname.Printf( _T("Segmentation Frame %d - %s (%d x %d) [Original Image]"), window_number_,
                       filename_, width_, height_ );
    SetTitle( statusname );
...
//      wxOPEN);
    ...
    if ( filedialog.ShowModal() == wxID_OK ) {
        ...
        if ( displayImage_->SetImage( filedialog.GetPath().c_str() ) == 0 ) return;
        bgLog( "Image %s loaded\n", filedialog.GetPath().c_str() );
        ...
        plotWindow1_->ClearAxis();
        plotWindow2_->ClearAxis();
        plotWindow1_->AddHorizontalAxis( RANK_CONF_MARGINX, RANK_CONF_MARGINY + height_, width_, 5,
                                         0, width_ - 1 );
        plotWindow1_->AddVerticalAxis( RANK_CONF_MARGINX, RANK_CONF_MARGINY + height_, height_, 5,
                                       0, height_ - 1 );
        plotWindow2_->AddHorizontalAxis( RANK_CONF_MARGINX, RANK_CONF_MARGINY + height_, width_, 5,
                                         0, width_ - 1 );
        plotWindow2_->AddVerticalAxis( RANK_CONF_MARGINX, RANK_CONF_MARGINY + height_, height_, 5,
                                       0, height_ - 1 );
...
// get the file name
#if defined( __WXGTK__ ) || defined( __WXMOTIF__ )
    wxFileDialog filedialog( this, "Upload Custom Weight Map", "", "", "*", wxOPEN );
#else
    wxFileDialog filedialog( this, "Upload Custom Weight Map", "", "",
                             "Matlab ASCII data files (*.dat)|*.dat|All Files (*.*)|*.*", wxOPEN );
#endif
    if ( filedialog.ShowModal() == wxID_OK ) {
...
#if defined( __WXGTK__ ) || defined( __WXMOTIF__ )
    wxFileDialog filedialog( this, "Choose an image file", "", "", "*", wxSAVE );
#else
    wxFileDialog filedialog( this, "Choose an image file", "", "",
                             "PNM files (*.pnm)|*.pnm|PNG files (*.png)|*.png|PCX files "
                             "(*.pcx)|*.pcx|PGM files (*.pgm)|*.pgm|JPEG files (*.jpg) (not "
                             "recommended)|*.jpg",
                             wxSAVE );
#endif
    if ( filedialog.ShowModal() == wxID_OK ) {
        ...
        switch ( filedialog.GetFilterIndex() ) {
            case 0: imtype = wxBITMAP_TYPE_PNM; break;
            case 1: imtype = wxBITMAP_TYPE_PNG; break;
            case 2: imtype = wxBITMAP_TYPE_PCX; break;
            case 3: imtype = wxBITMAP_TYPE_ANY; break;
            case 4: imtype = wxBITMAP_TYPE_JPEG; break;
            default: return;
        }
...
void BgMdiSegmentChild::Segment( void ) {
    ...
    // class
    int i;
    for ( i = 0; i < numRegions; i++ ) boundaryPointCount += regionList->GetRegionCount( i );
...
int BgMdiSegmentChild::GetParameters( int& sigmaS, float& sigmaR, float& aij, float& epsilon,
                                      int& minRegion, int& kernelSize, int dataEntryCheck ) {
    double tempd;
    char str[10];
