...
bool bgPNMHandler::LoadFile( wxImage* image, wxInputStream& stream, bool verbose,
                             int WXUNUSED( index ) ) {
    ...
    if ( !ptr ) {
        if ( verbose ) wxLogError( _( "PNM: Couldn't allocate memory." ) );
        return FALSE;
    }
...
bool bgPNMHandler::DoCanRead( wxInputStream& stream ) {
    ...
    if ( stream.GetC() == 'P' ) {
        switch ( stream.GetC() ) {
            case '2':
            case '5':
            case '6': stream.SeekI( pos ); return TRUE;
        }
    }
    stream.SeekI( pos );
    return FALSE;
