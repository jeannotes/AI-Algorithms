...
void msImageProcessor::DefineImage( byte* data_, imageType type, int height_, int width_ ) {
    ...
    if ( dim == 1 ) {
        for ( i = 0; i < height_ * width_; i++ ) luv[i] = (float) ( data_[i] );
    } else {
        for ( i = 0; i < height_ * width_; i++ ) { RGBtoLUV( &data_[dim * i], &luv[dim * i] ); }
    }
...
void msImageProcessor::Filter( int sigmaS, float sigmaR, SpeedUpLevel speedUpLevel ) {
    ...
    if ( ErrorStatus == EL_ERROR ) return;

    // If the algorithm has been halted, then exit
    if ( ( ErrorStatus = msSys.Progress( (float) ( 0.0 ) ) ) == EL_HALT ) { return; }
...
// start timer
    ...
    */
    int i;
    for ( i = 0; i < L * N; i++ ) { LUV_data[i] = msRawData[i]; }
...
void msImageProcessor::FuseRegions( float sigmaS, int minRegion ) {
    ...
    // flag an error)
    if ( ( h[1] = sigmaS ) <= 0 ) {
        ErrorHandler( "msImageProcessor", "FuseRegions", "The feature radius must be greater than "
                                                         "or equal to zero." );
        return;
    }
    ...
    if ( !( class_state.OUTPUT_DEFINED ) ) {
        ...
        */
        int i;
        for ( i = 0; i < L * N; i++ ) { LUV_data[i] = data[i]; }
...
#ifdef PROMPT
    double timer = msSys.ElapsedTime();
    msSys.Prompt( "done. (%6.2f seconds, numRegions = %6d)\nPruning spurious regions   ...", timer,
                  regionCount );
    msSys.StartTimer();
#endif
    ...
    for ( i = 0; i < L; i++ ) {
        label = labels[i];
        for ( j = 0; j < N; j++ ) { msRawData[N * i + j] = modes[N * label + j]; }
    }
...
void msImageProcessor::GetRawData( float* outputImageData ) {
    ...
    int i;
    for ( i = 0; i < L * N; i++ ) outputImageData[i] = msRawData[i];
...
void msImageProcessor::GetResults( byte* outputImage ) {
    ...
    if ( N == 1 ) {
        ...
        int i;
        for ( i = 0; i < L; i++ ) LUVtoRGB( &msRawData[N * i], &outputImage[N * i] );

    } else
        // Unknown image type: should use MeanShift::GetRawData()...
        ErrorHandler( "msImageProcessor", "GetResults", "Unknown image type. Try using "
                                                        "MeanShift::GetRawData()." );
...
RegionList* msImageProcessor::GetBoundaries( void ) {
    ...
    if ( class_state.OUTPUT_DEFINED ) DefineBoundaries();
...
int msImageProcessor::GetRegions( int** labels_out, float** modes_out, int** MPC_out ) {
    ...
    if ( class_state.OUTPUT_DEFINED == false ) return -1;
    ...
    int i;
    for ( i = 0; i < L; i++ ) labels_[i] = labels[i];

    // populate modes_out and MPC_out with the color and point
    // count of each region
    for ( i = 0; i < regionCount * N; i++ ) modes_[i] = modes[i];
    for ( i = 0; i < regionCount; i++ ) MPC_out_[i] = modePointCounts[i];
...
// proceed ...
    ...
    for ( i = 0; i < L; i++ ) {
        ...
        yk[0] = i % width;
        yk[1] = i / width;
        for ( j = 0; j < N; j++ ) yk[j + 2] = data[N * i + j];
        ...
        mvAbs = 0;
        for ( j = 0; j < lN; j++ ) mvAbs += Mh[j] * Mh[j];
        ...
        for ( j = 0; j < N; j++ ) msRawData[N * i + j] = (float) ( yk[j + 2] );
...
// Prompt user on progress
        ...
        if ( ( i % PROGRESS_RATE == 0 ) &&
             ( ( ErrorStatus = msSys.Progress( (float) ( i / (float) ( L ) ) *
                                               (float) ( 0.8 ) ) ) ) == EL_HALT )
            break;
    }
    ...
    for ( i = 0; i < L; i++ ) {
        ...
        while ( ( mvAbs >= EPSILON ) && ( iterationCount < LIMIT ) ) {
            ...
            if ( ( modeTable[modeCandidate_i] != 2 ) && ( modeCandidate_i != i ) ) {
                ...
                // of yk
                for ( j = 0; j < N; j++ ) modeCandidatePoint[j] = data[N * modeCandidate_i + j];
                ...
                if ( diff < TC_DIST_FACTOR ) {
                    ...
                    if ( modeTable[modeCandidate_i] == 0 ) {
                        ...
                        for ( j = 0; j < N; j++ ) yk[j + 2] = msRawData[modeCandidate_i * N + j];
        ...
        for ( j = 0; j < pointCount; j++ ) {
            ...
            // store result into msRawData...
            for ( k = 0; k < N; k++ ) msRawData[N * modeCandidate_i + k] = (float) ( yk[k + 2] );
        }
...
void msImageProcessor::Connect( void ) {
    ...
    for ( i = 0; i < height * width; i++ ) {
        ...
        if ( labels[i] < 0 ) {
            ...
            for ( k = 0; k < N; k++ ) modes[( N * label ) + k] = LUV_data[( N * i ) + k];
            ...
            */
            ...
            if ( ( neighLoc >= 0 ) && ( neighLoc < imageSize ) && ( labels[neighLoc] < 0 ) ) {
                for ( k = 0; k < N; k++ ) {
                    ...
                    if ( fabs( LUV_data[( regionLoc * N ) + k] - LUV_data[( neighLoc * N ) + k] ) >=
                         LUV_treshold )
                        break;
                }
...
void msImageProcessor::BuildRAM( void ) {
    ...
    if ( ( !raList ) && ( ( !( raList = new RAList[regionCount] ) ) ||
    ...
    for ( i = 0; i < height - 1; i++ ) {
        ...
        for ( j = 0; j < width - 1; j++ ) {
            ...
            if ( curLabel != rightLabel ) {
                ...
                // free list
                if ( exists ) freeRAList = oldRAFreeList;
            }
...
void msImageProcessor::TransitiveClosure( void ) {
    ...
    for ( i = 0; i < regionCount; i++ ) MPC_buffer[i] = 0;
    for ( i = 0; i < N * regionCount; i++ ) modes_buffer[i] = 0;
    ...
    for ( i = 0; i < regionCount; i++ ) {
        ...
        for ( k = 0; k < N; k++ ) modes_buffer[( N * iCanEl ) + k] += iMPC * modes[( N * i ) + k];
    ...
    for ( i = 0; i < regionCount; i++ ) label_buffer[i] = -1;
    ...
    for ( i = 0; i < height * width; i++ ) labels[i] = label_buffer[raList[labels[i]].label];
    ...
    for ( x = 0; x < regionCount; x++ ) {
        ...
        while ( curRegion ) {
            ...
            if ( curLabel > x ) {
                ...
                if ( ( edgePixelCount =
                           curRegion->edgePixelCount + neighborRegion->edgePixelCount ) != 0 ) {
...
void msImageProcessor::DefineBoundaries( void ) {
    ...
    int i;
    for ( i = 0; i < L; i++ ) boundaryMap[i] = -1;
    for ( i = 0; i < regionCount; i++ ) boundaryCount[i] = 0;
...
void msImageProcessor::InitializeOutput( void ) {
    ...
    if ( ( !( modes = new float[L * ( N + 2 )] ) ) || ( !( labels = new int[L] ) ) ||
...
void msImageProcessor::NewOptimizedFilter1( float sigmaS, float sigmaR ) {
    ...
    if ( N == 3 ) {
        ...
        for ( i = 0; i < L; i++ ) {
            sdata[idxs++] = ( i % width ) / sigmaS;
            sdata[idxs++] = ( i / width ) / sigmaS;
            for ( j = 0; j < N; j++ ) sdata[idxs++] = data[idxd++] / sigmaR;
        }
    }
...
// proceed ...
    ...
    for ( i = 0; i < L; i++ ) {
        ...
        // data[i])
        idxs = i * lN;
        for ( j = 0; j < lN; j++ ) yk[j] = sdata[idxs + j];

        // Calculate the mean shift vector using the lattice
        // LatticeMSVector(Mh, yk); // modify to new
        /*****************************************************/
        // Initialize mean shift vector
        for ( j = 0; j < lN; j++ ) Mh[j] = 0;
        wsuml                            = 0;
            ...
            while ( idxd >= 0 ) {
                ...
                if ( diff < 1.0 ) {
                    ...
                    if ( diff < 1.0 ) {
                        ...
                        for ( k = 0; k < lN; k++ ) Mh[k] += weight * sdata[idxs + k];
                        wsuml += weight;
                    }
                }
                idxd = slist[idxd];
        ...
        if ( wsuml > 0 ) {
            for ( j = 0; j < lN; j++ ) Mh[j] = Mh[j] / wsuml - yk[j];
        } else {
            for ( j = 0; j < lN; j++ ) Mh[j] = 0;
        }
        /*****************************************************/
...
// proceed ...
    ...
    for ( i = 0; i < L; i++ ) {
        ...
        // store result into msRawData...
        for ( j = 0; j < N; j++ ) msRawData[N * i + j] = (float) ( yk[j + 2] * sigmaR );
