...
static image< uchar >* loadPBM( const char* name ) {
    ...
    pnm_read( file, buf );
    if ( strncmp( buf, "P4", 2 ) ) throw pnm_error();
    ...
    for ( int i = 0; i < height; i++ ) read_packed( imPtr( im, 0, i ), width, file );
...
static void savePBM( image< uchar >* im, const char* name ) {
    ...
    file << "P4\n" << width << " " << height << "\n";
    for ( int i = 0; i < height; i++ ) write_packed( imPtr( im, 0, i ), width, file );
}
...
static image< uchar >* loadPGM( const char* name ) {
    ...
    std::ifstream file( name, std::ios::in | std::ios::binary );
    pnm_read( file, buf );
    if ( strncmp( buf, "P5", 2 ) ) throw pnm_error();
    ...
    pnm_read( file, buf );
    if ( atoi( buf ) > UCHAR_MAX ) throw pnm_error();
...
static image< rgb >* loadPPM( const char* name ) {
    ...
    std::ifstream file( name, std::ios::in | std::ios::binary );
    pnm_read( file, buf );
    if ( strncmp( buf, "P6", 2 ) ) throw pnm_error();
...
void load_image( image< T >** im, const char* name ) {
    ...
    std::ifstream file( name, std::ios::in | std::ios::binary );
    pnm_read( file, buf );
    if ( strncmp( buf, "VLIB", 9 ) ) throw pnm_error();
