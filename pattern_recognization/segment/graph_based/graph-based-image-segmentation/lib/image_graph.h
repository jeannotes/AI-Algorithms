...
class ImageEdgeSorter {
  public:
    inline bool operator()( const ImageEdge& g, const ImageEdge h ) { return ( h.w > g.w ); }
};

class ImageNode {
  public:
    ImageNode()
        : b( 0 ), g( 0 ), r( 0 ), x( 0 ), y( 0 ), z( 0 ), l( 0 ), n( 1 ), id( 0 ), max_w( 0 ){};
    unsigned char b, g, r;
...
class ImageGraph {
  public:
    ...
    void setNode( int n, ImageNode& node ) { nodes[n] = node; }
    ...
    void addEdge( ImageEdge& edge ) { edges.push_back( edge ); }
    ...
    int getNumComponents() { return K; }

    void sortEdges() { std::sort( edges.begin(), edges.end(), ImageEdgeSorter() ); }
