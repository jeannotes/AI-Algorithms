 ...
 * Information Filter.
 */
#include "bayes_tracking/BayesFilter/infFlt.hpp"
...
Information_scheme::Linear_predict_byproducts::Linear_predict_byproducts( std::size_t x_size,
                                                                          std::size_t q_size )
...
{
    // Postconditions
    if ( !isPSD( Y ) ) error( Numeric_exception( "Initial Y not PSD" ) );
...
Bayes_base::Float Information_scheme::predict( Linrz_predict_model& f )
...
Float Information_scheme::predict( Linear_invertable_predict_model& f,
...
Bayes_base::Float Information_scheme::observe_innovation( Linrz_correlated_observe_model& h,
                                                          const FM::Vec& s )
...
Bayes_base::Float Information_scheme::observe_innovation( Linrz_uncorrelated_observe_model& h,
                                                          const FM::Vec& s )
 ...
 */
{
    ...
    for ( std::size_t w = 0; w < h.Zv.size(); ++w ) column( HxT, w ) *= 1 / h.Zv[w];
    // Calculate EIF i = Hx'*ZI*zz
