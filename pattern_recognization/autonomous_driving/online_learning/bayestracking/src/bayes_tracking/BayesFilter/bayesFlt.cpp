...
const Bayes_base::Float Numerical_rcond::limit_PD_init =
    std::numeric_limits< Bayes_base::Float >::epsilon() * Bayes_base::Float( 1e5 );
...
Linear_invertable_predict_model::Linear_invertable_predict_model( std::size_t x_size,
                                                                  std::size_t q_size )
    : /*
       * Set the size of things we know about
       */
 ...
 */
{
    if ( x_size < 1 ) error( Logic_exception( "Zero state filter constructed" ) );
}
...
Bayes_base::Float Extended_kalman_filter::observe( Linrz_correlated_observe_model& h,
                                                   const FM::Vec& z )
...
Bayes_base::Float Extended_kalman_filter::observe( Linrz_uncorrelated_observe_model& h,
                                                   const FM::Vec& z )
 ...
 */
{
    if ( s_size < 1 ) error( Logic_exception( "Zero sample filter constructed" ) );
}
