 ...
 *   strict_upper_triangle(UD) = strict_upper_triangle(U), diagonal(UD) = d,
 * strict_lower_triangle(UD) ignored or zeroed
 *  LD(LTriMatrix) format of LdL' factor
 *   strict_lower_triangle(LD) = strict_lower_triangle(L), diagonal(LD) = d,
 * strict_upper_triangle(LD) ignored or zeroed
 */
#include <cassert>
#include <cmath>
 ...
 */
{
    ...
    for ( std::size_t i = 0; i < n; ++i ) { det *= UD( i, i ); }
    return det;
}
 ...
 */
{
    ...
    if ( n > 0 ) {
        ...
        do {
            ...
            } else if ( d == 0 ) {   // Possibly semi-definite, check not negative
                for ( i = 0; i < j; ++i ) {
                    if ( M( i, j ) != 0 ) goto Negative;
                }
            } else {   // Negative
                goto Negative;
            }
        ...
        do {
            ...
            if ( d > 0 ) {   // Positive definite
                ...
                do {
                    ...
                    e = Mi[j];
                    for ( k = j + 1; k < n; ++k ) { e -= Mi[k] * M( k, k ) * Mj[k]; }
                    if ( i == j ) {
            ...
            } else if ( d == 0 ) {   // Possibly semi-definite, check not negative, whole row must
                                     // be identically zero
                for ( k = j + 1; k < n; ++k ) {
                    if ( Mj[k] != 0 ) goto Negative;
                }
            } else {   // Negative
                goto Negative;
            }
 ...
 * Only diagonal part d can be singular (zero elements), inverse is computed of all elements other
 * then singular
 * Reference: A+G p.223
 *
 * Output:
    ...
    if ( n > 1 ) {
        ...
        do {
            ...
            for ( j = n - 1; j > i; --j ) {
                ...
                for ( k = i + 1; k < j; ++k ) UDij -= UDi[k] * UD( k, j );
                UDi[j]  = UDij;
            }
        } while ( i-- > 0 );
    }
    ...
    if ( n > 0 ) {
        ...
        do {
            ...
            for ( std::size_t j = n - 1; j > i; --j ) {
                ...
                for ( std::size_t k = i + 1; k <= j; ++k ) e -= Ui[k] * U( k, j );
                Ui[j]               = e * d;
            }
        } while ( i-- > 0 );
    }
 ...
 */
{
    ...
    if ( n > 0 ) {
        ...
        do {
            ...
            for ( j = 0; j < i; ++j ) Mi[j] = M( j, i ) * M( j, j );
            // (U' d) U in place
            j = n - 1;
            do {   // j>=i
 ...
 */
{
    ...
    RowMatrix& M_matrix =
        M.asRowMatrix();   // Must use variant1, variant2 cannot deal with infinity
    ...
    assert( rcond == orig_rcond || orig_rcond == 0 );
    (void) orig_rcond;
