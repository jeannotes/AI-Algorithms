...
#include <cassert>
#include "bayes_tracking/BayesFilter/bayesFlt.hpp"   // Exceptions required
#ifndef NDEBUG
 ...
 */
{
    ...
    if ( !bPSD ) {
        // Display Non PSD
        std::cerr.flags( std::ios::scientific );
 ...
 */
{
    ...
    if ( bUpperToLower ) {
        ...
        for ( std::size_t r = 1; r < size; ++r ) {
            for ( std::size_t c = 0; c < r; ++c ) { M( c, r ) = M( r, c ); }
        }
    } else {
        // Copy Upper to Lower
        for ( std::size_t r = 1; r < size; ++r ) {
