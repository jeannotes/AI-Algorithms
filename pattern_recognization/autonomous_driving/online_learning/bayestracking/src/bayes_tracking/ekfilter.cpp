...
EKFilter::EKFilter( std::size_t x_size )
    : Kalman_state_filter( x_size ), Covariance_scheme( x_size ), s( Empty ), z_p( Empty ) {
...
EKFilter::EKFilter( const FM::Vec& x0, const FM::SymMatrix& P0 )
    : Kalman_state_filter( x0.size() ), Covariance_scheme( x0.size() ), s( Empty ), z_p( Empty ) {
...
void EKFilter::update( Linrz_predict_model& predict_model,
                       Linrz_correlated_observe_model& observe_model, const FM::Vec& z ) {
    dynamic_cast< JacobianModel& >( predict_model ).updateJacobian( x );   // update model
                                                                           // linearization
    predict( predict_model );

    dynamic_cast< JacobianModel& >( observe_model ).updateJacobian( x );   // update model
                                                                           // linearization
    observe( observe_model, z );
}

void EKFilter::predict_observation( Linrz_correlated_observe_model& observe_model, FM::Vec& z_pred,
                                    FM::SymMatrix& R_pred ) {
    dynamic_cast< JacobianModel& >( observe_model ).updateJacobian( x );   // update model
                                                                           // linearization
...
Bayes_base::Float EKFilter::observeInnovation( Linrz_correlated_observe_model& h,
 ...
 */
{
    ...
    if ( last_z_size != si.size() ) { s.resize( si.size() ); }
    FM::noalias( s ) = si;
...
Bayes_base::Float EKFilter::observe( Linrz_correlated_observe_model& h, const FM::Vec& z )
/*
 * Extended linrz correlated observe, compute innovation for observe_innovation
 */
{
    ...
    if ( last_z_size != z.size() ) { s.resize( z.size() ); }
    s = z;
    h.normalise( s, zp );
    FM::noalias( s ) -= zp;
...
Bayes_base::Float EKFilter::observe( Linrz_uncorrelated_observe_model& h, const FM::Vec& z )
/*
