...
const FM::Vec& CVModel::f( const FM::Vec& x ) const {   // human model
    fx[0] = x[0] + x[1] * dt;                           // x
    fx[1] = x[1];                                       // dx
    fx[2] = x[2] + x[3] * dt;                           // y
    fx[3] = x[3];                                       // dy
   ...
   */
{
    ...
    for ( FM::DenseVec::iterator ni = n.begin(); ni != n.end(); ++ni ) { *ni *= rootq[ni.index()]; }
...
Bayes_base::Float CartesianModel::Likelihood_correlated::L(
 ...
 */
{
    if ( !zset ) Bayes_base::error( Logic_exception( "BGSubModel used without Lz set" ) );
    ...
    using namespace std;
    return exp( Float( -0.5 ) *
                ( logL + z.size() * log( 2 * M_PI ) + logdetZ ) );   // normalized likelihood
}
...
Bayes_base::Float CartesianModel::Likelihood_correlated::scaled_vector_square(
...
Bayes_base::Float PolarModel::Likelihood_correlated::L(
    const Correlated_additive_observe_model& model, const FM::Vec& z, const FM::Vec& zp ) const
/*
 * Definition of likelihood given an additive Gaussian observation model:
 *  p(z|x) = exp(-0.5*(z-h(x))'*inv(Z)*(z-h(x))) / sqrt(2pi^nz*det(Z));
 ...
 */
{
    if ( !zset ) Bayes_base::error( Logic_exception( "PolarModel used without Lz set" ) );
    // Normalised innovation
    zInnov = z;
    model.normalise( zInnov, zp );
    FM::noalias( zInnov ) -= zp;
...
Bayes_base::Float PolarModel::Likelihood_correlated::scaled_vector_square( const FM::Vec& v,
                                                                           const FM::SymMatrix& V )
/*
 * Compute covariance scaled square inner product of a Vector: v'*V*v
 */
{
...
const FM::Vec& CVModel3D::f( const FM::Vec& x ) const {   // human model
    fx[0] = x[0] + x[1] * dt;                             // x
    fx[1] = x[1];                                         // dx
    fx[2] = x[2] + x[3] * dt;                             // y
    fx[3] = x[3];                                         // dy
...
Bayes_base::Float CartesianModel3D::Likelihood_correlated::L(
    const Correlated_additive_observe_model& model, const FM::Vec& z, const FM::Vec& zp ) const
/*
 * Definition of likelihood given an additive Gaussian observation model:
 *  p(z|x) = exp(-0.5*(z-h(x))'*inv(Z)*(z-h(x))) / sqrt(2pi^nz*det(Z));
...
Bayes_base::Float CartesianModel3D::Likelihood_correlated::scaled_vector_square(
    const FM::Vec& v, const FM::SymMatrix& V )
/*
 * Compute covariance scaled square inner product of a Vector: v'*V*v
 */
