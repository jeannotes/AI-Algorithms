...
UKFilter::UKFilter( std::size_t x_size )
    : Kalman_state_filter( x_size ), Unscented_scheme( x_size ), z_p( Empty ) {
...
UKFilter::UKFilter( const FM::Vec& x0, const FM::SymMatrix& P0 )
    : Kalman_state_filter( x0.size() ), Unscented_scheme( x0.size() ), z_p( Empty ) {
...
void UKFilter::predict_observation( Correlated_additive_observe_model& observe_model,
    ...
    // Predict points of XX using supplied observation model
    {
        ...
        zXX0 =
            static_cast< Correlated_additive_observe_model& >( observe_model ).h( column( XX, 0 ) );
        column( zXX, 0 ) = zXX0;
        for ( std::size_t i = 1; i < XX.size2(); ++i ) {
            zXXi = static_cast< Correlated_additive_observe_model& >( observe_model )
                       .h( column( XX, i ) );
    ...
    for ( std::size_t i = 0; i < XX_size; ++i ) { column( zXX, i ).minus_assign( z_p ); }
...
Bayes_base::Float UKFilter::observeInnovation( Correlated_additive_observe_model& h, const Vec& si,
                                               const SymMatrix& Si )
/*
 * Observation fusion
 *  Pre : x,X
...
void UKFilter::unscented( FM::ColMatrix& XX, const FM::Vec& x, const FM::SymMatrix& X,
                          Float scale ) {
