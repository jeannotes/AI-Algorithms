...
TrackWin::TrackWin( const char* name, int width, int height, double scale ) {
    ...
    _textColor                    = BLACK;
    _textPos                      = cvPoint( (int) ( 20 * scale / SCALE_FACTOR ),
                        (int) ( m_winHeight - 20 * scale / SCALE_FACTOR ) );
    cvInitFont( &_font, CV_FONT_HERSHEY_PLAIN, 0.5 * scale / SCALE_FACTOR,
                0.5 * scale / SCALE_FACTOR, 0, 1, CV_AA );
}
...
void TrackWin::setObject( const char* label, double xpos, double ypos, double orientation,
                          color_t color, double varx, double vary, double covxy,
...
void TrackWin::update( int delay, double cameraView, double cx, double cy ) {
    ...
    cvLine( _canvas,   // X axis
            cvPoint( m_xoffset, m_yoffset ), cvPoint( m_scale + m_xoffset, m_yoffset ),
            CV_RGB( 192, 192, 192 ), 1 );
    cvLine( _canvas,   // Y axis
            cvPoint( m_xoffset, m_yoffset ), cvPoint( m_xoffset, -m_scale + m_yoffset ),
            CV_RGB( 192, 192, 192 ), 1 );
    ...
    for ( uint i = 0; i < _objVector.size(); i++ ) {
        ...
        if ( ( _objVector[i].varx > 0 ) && ( _objVector[i].vary > 0 ) ) {
            ...
            Mat covmat    = ( Mat_< double >( 2, 2 ) << _objVector[i].varx * scale2,
                           _objVector[i].covxy * scale2, _objVector[i].covxy * scale2,
                           _objVector[i].vary * scale2 );
        ...
        if ( strlen( _objVector[i].label.c_str() ) > 0 )
            cvPutText( _canvas, _objVector[i].label.c_str(), cvPoint( x0, y0 - r - 5 ), &_font,
                       color );
    }
    _objVector.clear();
    // draw the text
...
CvScalar TrackWin::getColor( color_t color ) {
    switch ( color ) {
        case BLACK: return cvScalar( 0, 0, 0 ); break;
        case WHITE: return cvScalar( 255, 255, 255 ); break;
        case GREY: return cvScalar( 128, 128, 128 ); break;
        case BLUE: return cvScalar( 255, 0, 0 ); break;
        case GREEN: return cvScalar( 0, 255, 0 ); break;
        case DARK_GREEN: return cvScalar( 0, 127, 0 ); break;
        case RED: return cvScalar( 0, 0, 255 ); break;
        case PURPLE: return cvScalar( 196, 0, 196 ); break;
        case YELLOW: return cvScalar( 0, 255, 255 ); break;
        case ORANGE: return cvScalar( 0, 128, 255 ); break;
        case DARK_ORANGE: return cvScalar( 0, 100, 200 ); break;
        default:
