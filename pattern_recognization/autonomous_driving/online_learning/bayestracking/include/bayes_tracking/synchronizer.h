...
class Synchronizer {
  ...
  public:
    ...
    bool addOdometry( odometry_t data, double timeStamp ) {
        if ( timeStamp > m_odometryTimeStamp ) {   // queue new odometry with timestamp
    ...
    bool addSonar( sonar_t data, double timeStamp ) {
        if ( timeStamp > m_sonarTimeStamp ) {   // queue new frames with timestamp
    ...
    bool addLaser( laser_t data, double timeStamp ) {
        if ( timeStamp > m_laserTimeStamp ) {   // queue new laser data with timestamp
    ...
    bool addCamera( camera_t data, double timeStamp ) {
        if ( timeStamp > m_cameraTimeStamp ) {   // queue new frames with timestamp
    ...
    bool synchronize() {
        ...
        double ref_timestamp = std::min( std::min( m_odometryTimeStamp, m_sonarTimeStamp ),
        ...
        if ( m_odometryQueue.empty() || m_sonarQueue.empty() || m_laserQueue.empty() ||
             m_cameraQueue.empty() ) {
        ...
        typename deque< odometry_pair >::iterator oi = m_odometryQueue.begin(), prev_oi = oi;
        while ( ( oi != m_odometryQueue.end() ) && ( oi->second > ref_timestamp ) ) prev_oi = oi++;
        {
        ...
        typename deque< sonar_pair >::iterator si = m_sonarQueue.begin(), prev_si = si;
        while ( ( si != m_sonarQueue.end() ) && ( si->second > ref_timestamp ) ) prev_si = si++;
        {
        ...
        typename deque< laser_pair >::iterator li = m_laserQueue.begin(), prev_li = li;
        while ( ( li != m_laserQueue.end() ) && ( li->second > ref_timestamp ) ) prev_li = li++;
        {
        ...
        typename deque< camera_pair >::iterator ci = m_cameraQueue.begin(), prev_ci = ci;
        while ( ( ci != m_cameraQueue.end() ) && ( ci->second > ref_timestamp ) ) prev_ci = ci++;
        {
