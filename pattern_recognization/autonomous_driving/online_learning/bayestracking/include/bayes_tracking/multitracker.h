...
struct observation_t {
    ...
    observation_t( FM::Vec v, double t, string f, string n )
        : vec( v ), time( t ), flag( f ), name( n ) {}
};
...
class MultiTracker {
  ...
  public:
    ...
    ~MultiTracker() {
        ...
        for ( fi = m_filters.begin(); fi != fiEnd; fi++ ) delete fi->filter;
    }
    ...
    void predict( PredictionModelType& pm ) {
        typename std::vector< filter_t >::iterator fi, fiEnd = m_filters.end();
        for ( fi = m_filters.begin(); fi != fiEnd; fi++ ) { fi->filter->predict( pm ); }
    }

    /**
     * Perform data association and update step for all the current filters, create new ones and
     * remove those which are no more necessary
    ...
    void process( ObservationModelType& om, observ_model_t om_flag = CARTESIAN,
                  association_t alg = NN, unsigned int seqSize = 5, double seqTime = 0.2,
                  double stdLimit = 1.0 ) {
        // data association
        ...
        if ( m_observations.size() ) createTracks( om, om_flag, seqSize, seqTime );
        // finished
        cleanup();
    }
  ...
  private:
    ...
    bool dataAssociation( ObservationModelType& om, association_t alg = NN ) {
        ...
        if ( N != 0 ) {   // observations and tracks, associate
            jpda::JPDA* jpda;
            vector< size_t > znum;     // this would contain the number of observations for each
                                       // sensor
    ...
    template < class ObservationModelType >
    void createTracks( ObservationModelType& om, observ_model_t om_flag, unsigned int seqSize,
                       double seqTime ) {
        ...
        while ( ui != m_unmatched.end() ) {
            ...
            while ( si != m_sequences.end() ) {
                if ( m_observations[*ui].time - si->back().time >
                     seqTime ) {   // erase old unmatched observations
                    ...
                    FilterType* filter;
                    if ( si->size() >= seqSize &&
                         initialize( filter, *si, om_flag ) ) {   // there's a minimum number of
