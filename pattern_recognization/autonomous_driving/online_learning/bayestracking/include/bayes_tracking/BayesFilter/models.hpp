...
// Additive predict model initialised from function and model matricies
{
  ...
  public:
    Simple_additive_predict_model( State_function f_init, const FM::Matrix& G_init,
                                   const FM::Vec& q_init );
    // Precondition: G, q are conformantly dimensioned (not checked)

    // No default assignment operator

    virtual const FM::Vec& f( const FM::Vec& x ) const { return ff( x ); }
};
...
// Linrz predict model initialised from function and model matrices
{
  ...
  public:
    Simple_linrz_predict_model( State_function f_init, const FM::Matrix& Fx_init,
                                const FM::Matrix& G_init, const FM::Vec& q_init );
...
{
  public:
    Simple_linear_predict_model( const FM::Matrix& Fx_init, const FM::Matrix& G_init,
                                 const FM::Vec& q_init );
...
// Linrz observe model initialised from function and model matrices
{
  ...
  public:
    ...
    virtual const FM::Vec& h( const FM::Vec& x ) const { return ff( x ); }
};
 ...
 */
{
  public:
    ...
    const FM::Vec& h( const FM::Vec& x ) const { return unc.h( x ); }
...
class General_LzUnAd_observe_model : public Linrz_uncorrelated_observe_model,
                                     public Likelihood_observe_model {
  public:
  ...
  private:
    ...
    struct Likelihood_uncorrelated {
        Likelihood_uncorrelated( std::size_t z_size ) : zInnov( z_size ), Zv_inv( z_size ) {
            zset = false;
        }
        ...
        bool zset;
        Float L( const Uncorrelated_additive_observe_model& model, const FM::Vec& z,
                 const FM::Vec& zp ) const;
        // Definition of likelihood for additive noise model given zz
...
class General_LiUnAd_observe_model : public Linear_uncorrelated_observe_model,
                                     public Likelihood_observe_model {
  public:
...
class General_LzCoAd_observe_model : public Linrz_correlated_observe_model,
                                     public Likelihood_observe_model {
  public:
  ...
  private:
    ...
    struct Likelihood_correlated {
        Likelihood_correlated( std::size_t z_size ) : zInnov( z_size ), Z_inv( z_size, z_size ) {
            zset = false;
        }
        ...
        Float L( const Correlated_additive_observe_model& model, const FM::Vec& z,
                 const FM::Vec& zp ) const;
        // Definition of likelihood for additive noise model given zz
...
class General_LiCoAd_observe_model : public Linear_correlated_observe_model,
                                     public Likelihood_observe_model {
  public:
