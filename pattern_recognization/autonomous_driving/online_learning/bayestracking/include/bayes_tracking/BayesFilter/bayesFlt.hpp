...
#include "bayes_tracking/BayesFilter/bayesException.hpp"   // exception types
#include "bayes_tracking/BayesFilter/matSupSub.hpp"        // matrix support subsystem
...
{
  public:
    Numerical_rcond() { limit_PD = limit_PD_init; }
    void set_limit_PD( Bayes_base::Float nl ) { limit_PD = nl; }
     ...
     */
    {
        if ( !( rcond >= 0 ) ) Bayes_base::error( Numeric_exception( error_description ) );
    }
     ...
     */
    {
        if ( !( rcond >= limit_PD ) ) Bayes_base::error( Numeric_exception( error_description ) );
    }
 ...
 */
{
  public:
    ...
    const FM::Vec& operator()( const FM::Vec& x ) const {   // Operator form of functional model
        return fx( x );
    }
};
 ...
 */
{
  public:
    ...
    const FM::Vec& f( const FM::Vec& x ) const {   // Provide the linear implementation of
                                                   // functional f
...
class Functional_observe_model : virtual public Observe_model_base,
                                 public Observe_function
 ...
 */
{
  public:
    ...
    const FM::Vec& operator()( const FM::Vec& x ) const { return h( x ); }
};
...
/* Linrz observation model Hx, h about state x (fixed size)
    ...
    Normalisation consistency Hx: Assume normalise will be from h(x(k|k-1)) so result is consistent
   with Hx
 */
{
  public:
  ...
  protected:         // Jacobian model is not sufficient, it is used to build Linrz observe model's
    Jacobian_observe_model( std::size_t x_size, std::size_t z_size ) : Hx( z_size, x_size ) {}
};

class Linrz_correlated_observe_model : public Correlated_additive_observe_model,
                                       public Jacobian_observe_model
...
class Linrz_uncorrelated_observe_model : public Uncorrelated_additive_observe_model,
                                         public Jacobian_observe_model
/* Linrz observation model Hx, h with respect to state x (fixed size)
 ...
 */
{
  public:
    ...
    const FM::Vec& h( const FM::Vec& x ) const {   // Provide a linear implementation of functional
                                                   // h assumes model is already Linrz for Hx
 ...
 *  This allows considerable flexibility so filter implementations can use different numerical
 * representations
 *
 ...
 * Linear filter information space representation for 1st (mean) and 2nd (covariance) moments of a
 * distribution
 *   Y = inv(X)   Information
...
class Linrz_filter : virtual public Bayes_filter_base {
  public:
    ...
    /* Predict state using a Linrz model
        ...
        Returns: Reciprocal condition number of primary matrix used in predict computation (1. if
       none)
    */
    ...
    /* Observation z(k) and with (Un)correlated observation noise model
        Requires x(k|k), X(k|k) or internal equivalent
        Returns: Reciprocal condition number of primary matrix used in observe computation (1. if
       none)
    */
};
...
class Linrz_kalman_filter : public Linrz_filter, virtual public Kalman_state_filter {
  protected:
    Linrz_kalman_filter()
        : Kalman_state_filter( 0 )   // define a default constructor
    {}
};
...
class Sample_state_filter : virtual public Bayes_filter_base {
  public:
    ...
    */
    ...
    /* Resampling update
        Returns lcond, Smallest normalised likelihood weight, represents conditioning of resampling
       solution
                lcond == 1. if no resampling performed
                This should by multiplied by the number of samples to get the Likelihood function
       conditioning
     */
 ...
 * 	predict, observe
 * cycle derived from the bayes_filter. observe Likelihoods are merged into a single combined
 * weight.
...
class Sample_filter : public Likelihood_filter,
                      public Functional_filter,
                      virtual public Sample_state_filter {
  public:
    ...
    */
    ...
    /* Observation fusion directly from likelihood weights
        lw may be smaller then the state sampling. Weights for additional particles are assumed to
       be 1
    */
};
