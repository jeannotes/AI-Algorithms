...
double Gauss( const Vec& s, const SymMatrix& S ) {
    ...
    return exp( -0.5 * ( inner_prod( trans( s ), prod( Si, s ) ) ) ) /
           sqrt( pow( 2 * M_PI, (double) s.size() ) * detS );
}
...
double logGauss( const Vec& s, const SymMatrix& S ) {
    ...
    return -0.5 * ( inner_prod( trans( s ), prod( Si, s ) ) ) -
           0.5 * ( (double) s.size() * ( log( 2 * M_PI ) ) + log( detS ) );
}
  ...
  public:
    ...
    void set( Element e ) {
        for ( size_t i = 0; i < m_rows; i++ ) {
            for ( size_t j = 0; j < m_cols; j++ ) { ( *this )[i][j] = e; }
        }
    }
    ...
    void print() {
        for ( size_t i = 0; i < m_rows; i++ ) {
            for ( size_t j = 0; j < m_cols; j++ ) { cout << ( *this )[i][j] << " "; }
            cout << endl;
        }
    }
};
...
class MultiMatrix : public vector< MultiMatrix< Element > > {
  ...
  public:
    ...
    MultiMatrix( const MultiMatrix& mm ) {
        ...
        if ( m_dim ) {
            size_t s = mm.size();
            for ( size_t i = 0; i < s; i++ ) { ( *this ).push_back( mm[i] ); }
        }
    }

    MultiMatrix( const vector< size_t >& sizes ) { init( sizes ); }
    ...
    void init( const vector< size_t >& sizes ) {
        ...
        if ( s > 1 ) {
            ...
            for ( size_t i = 0; i < extent; i++ ) { this->push_back( m ); }
        } else {
...
class Association : public vector< association_t > {
  public:
    void print() {
        ...
        for ( it = begin(); it != itEnd; it++ ) { cout << "(" << it->z << "," << it->t << ") "; }
        cout << endl;
    }
};
...
class JPDA {
  public:
     ...
     */
    JPDA( const vector< size_t >& zNum, size_t tNum ) : m_beta( Empty ) { init( zNum, tNum ); }
    ...
    void init( const vector< size_t >& zNum, size_t tNum ) {
        ...
        for ( size_t m = 0; m < m_mNum; m++ ) {
            ...
            for ( size_t i = 0; i < zNum[m]; i++ ) { omega[i][0] = true; }
    ...
    int getAssociations() {
        ...
        for ( size_t j = 0; j < m_tNum + 1; j++ ) { colVec.push_back( j ); }
    ...
    void getProbabilities( double Pd = 0.8, double C = 0.2 ) {
        ...
        for ( size_t m = 0; m < m_mNum; m++ ) {   // for each sensor
            ...
            for ( pi = logP[m].begin(); pi != piEnd; pi++ ) { *pi -= logSum; }
            ...
            for ( size_t t = 1; t <= m_tNum; t++ ) {
                ...
                double betaSum = 0;
                for ( size_t j = 1; j < rows; j++ ) { betaSum += Beta[m][j][t]; }
    ...
    void getMultiNNJPDA( vector< Association >& association, double minP = 0.1 ) {
        ...
        for ( size_t i = 0; i < dims; i++ ) {
            size_t s = l[i];
            for ( size_t j = 0; j < s; j++ ) { planes[i].push_back( j ); }
        }
        ...
        while ( !done ) {
            ...
            while ( pit[last] != planes[last].end() ) {
                // get position vector
                for ( size_t i = 0; i <= last; i++ ) { pos[i] = *pit[i]; }
                // check for maximum
                if ( m_beta( pos ) > max ) {
                    max    = m_beta( pos );
                    maxPos = pos;
            ...
            for ( size_t m = 0; m < m_mNum; m++ ) {
                if ( maxPos[m + 1] ) {               // it's a proper measurement
                    ...
                    planes[m + 1].erase(
                        maxPit[m + 1] );   // remove plane from further consideration
                }
            ...
            if ( done |=
                 allEmpty ) {   // if finished, throw out the remaining measurements as clutters
                for ( size_t m = 0; m < m_mNum; m++ ) {
    ...
    /**
     * Compute NNJPDA (independent sensors) and return the resulting vector of associations, for
     * each sensor
     * @param association Vector of associations
     * @param minP Minimum association probability (default 0.1)
     */
     ...
     */
    size_t getBestXsi() { return m_bestXsi; }
    ...
    void fill( size_t start, size_t end ) {
        if ( start < end ) {
            ...
            for ( vi = m_assocVec.begin(); vi != viEnd; vi++ ) {
                ...
                for ( ai = vi->begin(); ai != aiEnd; ai++ ) { a[ai->z][ai->t] = true; }
                av.push_back( a );
            }
