...
bool MTRK::isLost( const FilterType* filter, double stdLimit = 1.0 ) {
    ...
    if ( filter->X( 0, 0 ) + filter->X( 2, 2 ) > sqr( stdLimit ) ) return true;
    return false;
}
...
class SimpleTracking {
  public:
    ...
    void addDetectorModel( std::string name, association_t alg, observ_model_t om_flag,
                           double pos_noise_x, double pos_noise_y, unsigned int seqSize = 5,
                           double seqTime = 0.2 ) {
        ROS_INFO( "Adding detector model for: %s.", name.c_str() );
        detector_model det;
        det.om_flag                         = om_flag;
        det.alg                             = alg;
        if ( om_flag == CARTESIAN ) det.ctm = new CartesianModel( pos_noise_x, pos_noise_y );
        if ( om_flag == POLAR ) det.plm     = new PolarModel( pos_noise_x, pos_noise_y );
        det.seqSize                         = seqSize;
        det.seqTime                         = seqTime;
        detectors[name]                     = det;
    }
    ...
    std::map< long, std::vector< geometry_msgs::Pose > > track( double* track_time = NULL ) {
        ...
        //
        //        for(typename std::map<std::string, detector_model>::const_iterator it =
        //        detectors.begin();
        //            it != detectors.end();
        //            ++it) {
        //            // process observations (if available) and update tracks
        //            mtrk.process(*(it->second.ctm), it->second.alg,  it->second.seqSize,
        //            it->second.seqTime);
        //        }
        //==========================================================================//
...
// std::cerr << "track ID = " << mtrk[i].id << ", pose ID = " << mtrk[i].pose_id << std::endl;
    ...
    void addObservation( std::string detector_name, std::vector< geometry_msgs::Point > obsv,
                         double obsv_time, geometry_msgs::Pose& robot_pose ) {
        boost::mutex::scoped_lock lock( mutex );
        ...
        for ( std::vector< geometry_msgs::Point >::iterator li = obsv.begin(); li != obsv.end();
              ++li ) {
            ...
            mtrk.addObservation( *observation, obsv_time, boost::to_string( (uint32_t) li->z ),
                                 detector_name );
#else
