...
def segmentation(fname):
    ...
    for i in selected_labels:
        ...
        new_points = np.matmul(new_eigen_vecs.transpose(),
                               twod_points.transpose())
        points = np.concatenate(
            (new_points.transpose(), np.zeros((4, 1))), axis=1)
    ...
    name = 'fol' + str(1) + str(int(os.path.basename(fname)[:-4]))
    fix.savefig(name)
...
def main():
    ...
    fname = '/home/jean/Documents/AI-Algorithms/script/kitti_vis/data/object/training/velodyne/000002.bin'
    segmentation(fname)
