...
def occupancy_grid(fname):
    ...
    # Creating an empty occupancy grid, 100m *  100m
    # actually, not 668*668, should be (668*2)*(668*2)
    ...
    def conversion(in_val): return np.floor((in_val*100)/15)
    ...
    for i in range(len(unique_rows)):
        ...
        collect_z = np.where((temp[:, 0] == unique_rows[i, 0]) & (
            temp[:, 1] == unique_rows[i, 1]))
#         print(collect_z)
        ...
        '''
        here, adding 344 just means that left part belogs to 3-rd part
        ...
        if translated_coord[0] >= 0 and translated_coord[0] < 668 and translated_coord[1] >= 0 and translated_coord[1] < 668:
            occupancy_grid[translated_coord[0],
                           translated_coord[1]] = max_abs_dist
