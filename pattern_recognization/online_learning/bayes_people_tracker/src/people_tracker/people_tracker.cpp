...
PeopleTracker::PeopleTracker() : detect_seq( 0 ), marker_seq( 0 ) {
    ...
    ros::SubscriberStatusCallback con_cb =
        boost::bind( &PeopleTracker::connectCallback, this, boost::ref( n ) );

    private_node_handle.param( "positions", pub_topic, std::string( "/people_tracker/positions" ) );
    pub_detect = n.advertise< bayes_people_tracker::PeopleTracker >( pub_topic.c_str(), 100, con_cb,
                                                                     con_cb );
    private_node_handle.param( "pose", pub_topic_pose, std::string( "/people_tracker/pose" ) );
    pub_pose =
        n.advertise< geometry_msgs::PoseStamped >( pub_topic_pose.c_str(), 100, con_cb, con_cb );
    private_node_handle.param( "pose_array", pub_topic_pose_array,
                               std::string( "/people_tracker/pose_array" ) );
    pub_pose_array = n.advertise< geometry_msgs::PoseArray >( pub_topic_pose_array.c_str(), 100,
                                                              con_cb, con_cb );
    private_node_handle.param( "people", pub_topic_people,
                               std::string( "/people_tracker/people" ) );
    pub_people =
        n.advertise< people_msgs::People >( pub_topic_people.c_str(), 100, con_cb, con_cb );
    private_node_handle.param( "trajectory", pub_topic_trajectory,
                               std::string( "/people_tracker/trajectory" ) );
    pub_trajectory = n.advertise< geometry_msgs::PoseArray >( pub_topic_trajectory.c_str(), 100,
                                                              con_cb, con_cb );
    private_node_handle.param( "marker", pub_topic_marker,
                               std::string( "/people_tracker/marker_array" ) );
    pub_marker = n.advertise< visualization_msgs::MarkerArray >( pub_topic_marker.c_str(), 100,
                                                                 con_cb, con_cb );
...
void PeopleTracker::parseParams( ros::NodeHandle n ) {
    ...
    if ( filter == "EKF" ) {
        ...
        ROS_FATAL_STREAM( "Filter type " << filter << " is not specified. Unable to create the "
                                                      "tracker. Please use either EKF, UKF or "
                                                      "PF." );
        return;
    }
    ...
    ROS_INFO_STREAM( "Created " << filter << " based tracker using constant velocity prediction "
                                             "model." );
    ...
    for ( XmlRpc::XmlRpcValue::ValueStruct::const_iterator it = detectors.begin();
            ...
            if ( ekf == NULL ) {
                if ( ukf == NULL ) {
                    if ( detectors[it->first].hasMember( "seq_size" ) &&
                         detectors[it->first].hasMember( "seq_time" ) ) {
                        int seq_size = detectors[it->first]["seq_size"];
                        pf->addDetectorModel(
                            it->first, detectors[it->first]["matching_algorithm"] == "NN" ?
                                           NN :
                            ...
                            detectors[it->first]["noise_params"]["y"], (unsigned int) seq_size,
                            detectors[it->first]["seq_time"] );
                    } else {
                        pf->addDetectorModel(
                            it->first, detectors[it->first]["matching_algorithm"] == "NN" ?
                                           NN :
        ...
        ros::Subscriber sub;
        subscribers[std::pair< std::string, std::string >( it->first,
                                                           detectors[it->first]["topic"] )] = sub;
    }
}
...
void PeopleTracker::trackingThread() {
    ...
    while ( ros::ok() ) {
        ...
        if ( ppl.size() ) {
            ...
            for ( std::map< long, std::vector< geometry_msgs::Pose > >::const_iterator it =
                      ppl.begin();
                  it != ppl.end(); ++it ) {
                ...
                if ( pub_detect.getNumSubscribers() || pub_pose.getNumSubscribers() ||
                     pub_pose_array.getNumSubscribers() || pub_people.getNumSubscribers() ) {
                    std::vector< double > polar =
                        cartesianToPolar( poseInRobotCoords.pose.position );
                    distances.push_back( polar[0] );
                    angles.push_back( polar[1] );
                    angle = polar[0] < min_dist ? polar[1] : angle;
                    closest_person_point =
                        polar[0] < min_dist ? it->second[0] : closest_person_point;
            ...
            if ( pub_detect.getNumSubscribers() || pub_pose.getNumSubscribers() ||
                 pub_pose_array.getNumSubscribers() || pub_people.getNumSubscribers() )
                publishDetections( time_sec, closest_person_point, poses, vels, uuids, distances,
                                   angles, min_dist, angle );

            if ( pub_marker.getNumSubscribers() ) createVisualisation( poses, pids, pub_marker );
...
void PeopleTracker::publishDetections(
    double time_sec, geometry_msgs::Pose closest, std::vector< geometry_msgs::Pose > ppl,
    std::vector< geometry_msgs::Pose > vels, std::vector< std::string > uuids,
    std::vector< double > distances, std::vector< double > angles, double min_dist, double angle ) {
...
void PeopleTracker::PN_experts( geometry_msgs::PoseArray& variance,
    ...
    // std::cerr << "path_length = " << path_length << ", avg_velocity = " << avg_velocity << ",
    // avg_variance = " << avg_variance << std::endl;
    if ( path_length >= human_path_min && avg_velocity >= human_velo_min &&
         avg_velocity <= human_velo_max )
        trajectory.header.frame_id = "human_trajectory";
    if ( path_length <= static_path_max && avg_velocity <= static_velo_max &&
         avg_variance <= static_vari_max )
...
void PeopleTracker::publishTrajectory( std::vector< geometry_msgs::Pose > poses,
                                       std::vector< geometry_msgs::Pose > vels,
                                       std::vector< geometry_msgs::Pose > vars,
                                       std::vector< long > pids, ros::Publisher& pub ) {
#ifdef ONLINE_LEARNING
    ...
    for ( int i = 0; i < previous_poses.size(); i++ ) checked[i] = false;
    for ( int i = 0; i < previous_poses.size(); i++ ) {
        if ( !checked[i] && boost::get< 0 >( previous_poses[i] ) != INVALID_ID ) {
            ...
            for ( int j = i; j < previous_poses.size(); j++ ) {
                if ( boost::get< 0 >( previous_poses[j] ) ==
                     boost::get< 0 >( previous_poses[i] ) ) {
                    n++;
                    checked[j] = true;
                }
            }
            ...
            if ( n > repeated_poses_max ) {
                ...
                if ( trajectory.header.frame_id == "static_trajectory" ) publish_and_remove = true;
            }
            if ( publish_and_remove ) {   //@todo or n > 999 for memory safety
                // std::cerr << "ID = " << boost::get<0>(previous_poses[i]) << ", repeated " << n <<
                // std::endl;
    ...
    for ( int i = 0; i < previous_poses.size(); i++ ) {
        if ( boost::get< 0 >( previous_poses[i] ) != INVALID_ID ) {
            ...
            if ( last_pose ) {
                ...
                PN_experts( variance, velocity, trajectory );
#endif
                // trajectory.poses.insert(trajectory.poses.end(), variance.poses.begin(),
                // variance.poses.end()); //@NB_test
                pub.publish( trajectory );
                // std::cerr << "[people_tracker] trajectory ID = " << trajectory.header.seq << ",
                // timestamp = " << trajectory.header.stamp << ", poses size = " <<
                // trajectory.poses.size() << std::endl;
                if ( log_trajectories ) {
                    for ( int k = 0; k < trajectory.poses.size(); k++ )
                        std::cerr << trajectory.poses[k].position.x << " "
                                  << trajectory.poses[k].position.y << std::endl;
                }
            }
        }
    }
...
#ifdef ONLINE_LEARNING
        for ( int j = 0; j < previous_poses.size(); j++ ) {
            if ( poses[i].position.z >= 0.0 &&
                 boost::get< 3 >( previous_poses[j] ).position.z == poses[i].position.z ) {
                new_pose = false;
                break;
            }
        }
#endif
        if ( new_pose ) {
            // if(vars[i].position.x+vars[i].position.y <= human_vari_max) // only use this for
            // learning!
...
void PeopleTracker::detectorCallback( const geometry_msgs::PoseArray::ConstPtr& pta,
                                      std::string detector ) {
// std::cerr << "[people_tacker] got " << pta->poses.size() << " poses, from " << detector <<
// std::endl;

#ifdef ONLINE_LEARNING
    double tmp[pta->poses.size()];
    for ( int i = 0; i < pta->poses.size(); ++i ) tmp[i] = pta->poses[i].position.z;
#endif
    ...
    for ( int i = 0; i < pta->poses.size(); i++ ) {
        ...
        try {
            ...
            listener->waitForTransform( poseInCamCoords.header.frame_id, target_frame,
                                        poseInCamCoords.header.stamp, ros::Duration( 1.0 ) );
...
#ifdef ONLINE_LEARNING
    for ( int i = 0; i < ppl.size(); ++i ) ppl[i].z = tmp[i];
#endif
