 ...
 */
{
  public:
    ...
    virtual Float resample( Resamples_t& presamples, std::size_t& uresamples, FM::DenseVec& w,
                            SIR_random& r ) const = 0;
    /*
     ...
     *	presamples: posterior resample, the number of times each sample should appear posterior baes
     *on it weight
     *  uresample: the number of unique resamples in the posterior == number of non zero elements in
     *presamples
     * Preconditions
...
{
  public:
    Float resample( Resamples_t& presamples, std::size_t& uresamples, FM::DenseVec& w,
                    SIR_random& r ) const;
};
...
class Sampled_general_predict_model : public Predict_model,
...
{
  public:
    Sampled_general_predict_model( std::size_t x_size, std::size_t q_size,
     ...
     */
    {
        ...
        for ( FM::Vec::const_iterator qi = this->q.begin(); qi != this->q.end(); ++qi ) {
            if ( *qi < 0 ) error( Numeric_exception( "Negative q in init_GqG" ) );
...
typedef Sampled_general_predict_model< Linear_invertable_predict_model >
 ...
 *  A stochastic sample is defined as a sample with a unqiue stochastic history other then
 * roughening
 */
{
  ...
  public:
    ...
    {
        if ( rougheningK != 0 ) roughen_minmax( S, rougheningK );
    }

    static void copy_resamples( FM::ColMatrix& P,
  ...
  protected:
    void roughen_minmax( FM::ColMatrix& P,
                         Float K ) const;          // roughening using minmax of P distribution
...
class SIR_kalman_scheme : public SIR_scheme,
                          virtual public Kalman_state_filter
/*
 ...
 */
{
  public:
    ...
    void roughen() {   // Specialised correlated roughening
        if ( rougheningK != 0 ) roughen_correlated( S, rougheningK );
    }

  protected:
    void roughen_correlated( FM::ColMatrix& P,
                             Float K );         // Roughening using covariance of P distribution
