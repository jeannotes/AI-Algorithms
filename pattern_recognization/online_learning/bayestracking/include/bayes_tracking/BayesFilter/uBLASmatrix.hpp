...
class FMVec : public VecBase {
  public:
    ...
    explicit FMVec( EmptyTag ) : VecBase() {}                 // Empty constructor
    explicit FMVec( std::size_t size ) : VecBase( size ) {}   // Normal sized constructor
    FMVec( const FMVec& c ) : VecBase( static_cast< const VecBase& >( c ) ) {}   // Copy constructor
    template < class E >
    explicit FMVec( const ublas::vector_expression< E >& e )
        : VecBase( e ) {}   // vector_expression copy constructor
    template < class E >
    ...
    template < class E >
    FMVec& operator=( const ublas::vector_expression< E >& r ) {   // Expression assignment; may be
                                                                   // dependent on r
        VecBase::operator=( r );
        return *this;
    }
    FMVec& operator=( const FMVec& r ) {   // Vector assignment; independent
        VecBase::assign( r );
        return *this;
    }
...
class FMMatrix : public MatrixBase {
  public:
    ...
    // No Default Constructor. Empty creation is very error prone
    explicit FMMatrix( EmptyTag ) : MatrixBase() {}   // Empty constructor
    FMMatrix( std::size_t size1, std::size_t size2 )
        : MatrixBase( size1, size2 ) {}   // Normal sized constructor
    FMMatrix( const FMMatrix& c )
        : MatrixBase( static_cast< const MatrixBase& >( c ) ) {}   // Copy constructor
    template < class E >
    explicit FMMatrix( const ublas::matrix_expression< E >& e )
        : MatrixBase( e ) {}   // matrix_expression copy constructor

    template < class E >
    FMMatrix& operator=( const ublas::matrix_expression< E >& r ) {   // Expression assignment; may
                                                                      // be dependent on r
        MatrixBase::operator=( r );
        return *this;
    }
    FMMatrix& operator=( const FMMatrix& r ) {   // Matrix assignment; independent
        MatrixBase::assign( r );
        return *this;
    }
    ...
    // ri() returns container associated with iterator. static_cast required as typeof(ri()) may not
    // be typeof(MM)
    ...
    // Sub-range selection operators
    ublas::matrix_range< const MatrixBase > sub_matrix( std::size_t s1, std::size_t e1,
                                                        std::size_t s2, std::size_t e2 ) const {
        return ublas::matrix_range< const MatrixBase >( *this, ublas::range( s1, e1 ),
                                                        ublas::range( s2, e2 ) );
    }
    ublas::matrix_range< MatrixBase > sub_matrix( std::size_t s1, std::size_t e1, std::size_t s2,
                                                  std::size_t e2 ) {
        return ublas::matrix_range< MatrixBase >( *this, ublas::range( s1, e1 ),
                                                  ublas::range( s2, e2 ) );
    }

    // Requires boost_1.30.0 which has a generalised matrix_vector_slice
    ublas::matrix_vector_slice< const MatrixBase > sub_column( std::size_t s1, std::size_t e1,
                                                               std::size_t s2 ) const
    // Column vector s2 with rows [s1,e1)
    {
        return ublas::matrix_vector_slice< const MatrixBase >(
            *this, ublas::slice( s1, 1, e1 - s1 ), ublas::slice( s2, 0, e1 - s1 ) );
    }
    ublas::matrix_vector_slice< MatrixBase > sub_column( std::size_t s1, std::size_t e1,
                                                         std::size_t s2 )
    // Column vector s2 with rows [s1,e1)
    {
        return ublas::matrix_vector_slice< MatrixBase >( *this, ublas::slice( s1, 1, e1 - s1 ),
                                                         ublas::slice( s2, 0, e1 - s1 ) );
    }
};
 ...
 */
template < class MatrixBase >
class SymMatrixWrapper : private BaseFromMember< MatrixBase >,   // allow construction of MatrixBase
                                                                 // member before symmetric_adaptor
  ...
  public:
    ...
    SymMatrixWrapper( std::size_t size1, std::size_t size2 )
        : matrix_type( size1, size2 ),
          symadaptor_type( matrix_type::member ) {}   // Normal sized constructor
    explicit SymMatrixWrapper( const SymMatrixWrapper& r )
        : matrix_type( reinterpret_cast< const MatrixBase& >( r ) ),
          symadaptor_type( matrix_type::member ) {
    }   // Explicit copy construction referencing the copy reinterpreted as a MatrixBase
    template < class E >
    explicit SymMatrixWrapper( const ublas::matrix_expression< E >& e )
        : matrix_type( e ),
          symadaptor_type( matrix_type::member ) {
    ...
    void clear() { matrix_type::member.clear(); }
...
diag( FMMatrix< Base >& M,
      std::size_t n ) {   // Return a vector proxy to the first n diagonal elements of M
    return ublas::matrix_vector_range< FMMatrix< Base > >( M, ublas::range( 0, n ),
                                                           ublas::range( 0, n ) );
}
...
diag( const FMMatrix< Base >& M,
      std::size_t n ) {   // Return a vector proxy to the first n diagonal elements of M
    return ublas::matrix_vector_range< const FMMatrix< Base > >( M, ublas::range( 0, n ),
                                                                 ublas::range( 0, n ) );
}

template < class Base >
ublas::matrix_vector_range< FMMatrix< Base > >
diag( FMMatrix< Base >& M ) {   // Return a vector proxy to the diagonal elements of M
    const std::size_t common_size = std::min( M.size1(), M.size2() );
    return ublas::matrix_vector_range< FMMatrix< Base > >( M, ublas::range( 0, common_size ),
                                                           ublas::range( 0, common_size ) );
}

template < class Base >
const ublas::matrix_vector_range< const FMMatrix< Base > >
diag( const FMMatrix< Base >& M ) {   // Return a vector proxy to the diagonal elements of M
    const std::size_t common_size = std::min( M.size1(), M.size2() );
    return ublas::matrix_vector_range< const FMMatrix< Base > >( M, ublas::range( 0, common_size ),
                                                                 ublas::range( 0, common_size ) );
}

template < class Base >
void identity( FMMatrix< Base >& I ) {   // Set I to generalised Identity matrix. Clear I and set
                                         // diag(I) to one
    I.clear();
    ...
    diag( I ).assign(
        ublas::scalar_vector< Base_value_type >( common_size, Base_value_type( 1 ) ) );
}
...
namespace detail
{   // mult_SPD now an implementation detail
 ...
 */
{
    ...
    for ( ; Xa != Xend; ++Xa )   // Iterate vectors
    {
        ...
        Xb = Xa;   // Start at the row Xa only one triangle of symmetric result required
        for ( ; Xb != Xend; ++Xb ) {   // Triple vector inner product
...
struct prod_expression_result {   // Provide ET result E1E2T_type of
                                  // prod(matrix_expression<E1>,trans(matrix_expression<E2>)
    typedef BOOST_UBLAS_TYPENAME ublas::matrix_unary2_traits<
        E2, ublas::scalar_identity< BOOST_UBLAS_TYPENAME E2::value_type > >::result_type E2T_type;
    typedef BOOST_UBLAS_TYPENAME ublas::matrix_matrix_binary_traits<
        BOOST_UBLAS_TYPENAME E1::value_type, E1, BOOST_UBLAS_TYPENAME E2T_type::value_type,
        E2T_type >::result_type E1E2T_type;

    // Provide ET result E1TE2_type of prod(trans(matrix_expression<E1>),matrix_expression<E2>)
    typedef BOOST_UBLAS_TYPENAME ublas::matrix_unary2_traits<
        E1, ublas::scalar_identity< BOOST_UBLAS_TYPENAME E1::value_type > >::result_type E1T_type;
    typedef BOOST_UBLAS_TYPENAME ublas::matrix_matrix_binary_traits<
        BOOST_UBLAS_TYPENAME E1T_type::value_type, E1T_type, BOOST_UBLAS_TYPENAME E2::value_type,
        E2 >::result_type E1TE2_type;
};

template < class E >
inline typename prod_expression_result< E, E >::E1E2T_type
prod_SPD( const ublas::matrix_expression< E >& X )
/*
 * Symmetric Positive (Semi) Definite product: X*X'
 */
{
    // ISSUE: uBLAS post Boost 1_31_0 introduces a trans(const matrix_expression<E>& e) which
    // propagates the const expression type
...
inline typename prod_expression_result< EX, ET >::E1E2T_type
prod_SPD( const ublas::matrix_expression< EX >& X, const ublas::matrix_expression< ES >& S,
...
template < class EX, class ES, class ET >
inline ET prod_SPD( const ublas::matrix_expression< EX >& X,
...
inline SymMatrix prod_SPD( const ublas::matrix_expression< EX >& X,
...
template < class E >
inline typename prod_expression_result< E, E >::E1TE2_type
...
template < class EX, class ES, class ET >
inline typename prod_expression_result< ET, EX >::E1TE2_type
prod_SPDT( const ublas::matrix_expression< EX >& X, const ublas::matrix_expression< ES >& S,
...
template < class EX, class ES, class ET >
inline ET prod_SPDT( const ublas::matrix_expression< EX >& X,
                     const ublas::vector_expression< ES >& s,
...
template < class EX, class ES >
inline SymMatrix prod_SPDT( const ublas::matrix_expression< EX >& X,
                            const ublas::vector_expression< ES >& s )
/*
 * Symmetric Positive (Semi) Definite product: X'*diag_matrix(s)*X
 * TODO requires a prod_diag(X,s)
