...
void AssociationMatrix::setSize( size_t row, size_t col ) {
    if ( ( row > 0 ) && ( col >= 0 ) ) {
        ...
        cerr << "ERROR! - Wrong size for AssociationMatrix (row = " << row << ", col = " << col
             << ")\n";
        exit( 1 );
    }
}
...
void AssociationMatrix::print() {
    for ( size_t i = 0; i < RowSize; i++ ) {
        for ( size_t j = 0; j < ColSize; j++ ) std::cout << ( *this )[i][j] << " ";
        std::cout << std::endl;
    }
}

double
AssociationMatrix::mahalanobis( const FM::Vec& v1, const FM::SymMatrix& R1, const FM::Vec& v2,
                                const FM::SymMatrix& R2 ) {   // sqrt((v1-v2)' * Si * (v1-v2))
    return mahalanobis( Vec( v1 - v2 ), SymMatrix( R1 + R2 ) );
}

Float AssociationMatrix::detS = 0.;

double AssociationMatrix::mahalanobis( const FM::Vec& s, const FM::SymMatrix& S ) {
...
double AssociationMatrix::gate( int dof ) {
    switch ( dof ) {
        ...
        case 4:   // 4 dof @ P=0.01
            return 3.644;
            break;
        default: cerr << "ERROR - undefined value for gate(" << dof << ")\n"; exit( EXIT_FAILURE );
    }
}
