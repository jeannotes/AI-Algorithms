...
Standard_resampler::Float Standard_resampler::resample( Resamples_t& presamples,
                                                        std::size_t& uresamples, FM::DenseVec& w,
                                                        SIR_random& r ) const
/* Standard resampler from [1]
 * Algorithm:
 *	A particle is chosen once for each time its cumulative weight intersects with a uniform random
 *draw.
...
Systematic_resampler::Float Systematic_resampler::resample( Resamples_t& presamples,
                                                            std::size_t& uresamples,
                                                            FM::DenseVec& w, SIR_random& r ) const
/* Systematic resample algorithm from [2]
 * Algorithm:
 *	A particle is chosen once for each time its cumulative weight intersects with an equidistant
 *grid.
 ...
 */
{
    ...
    // Any numerical failure should cascade into cumulative sum
    if ( wcum != wcum ) error( Numeric_exception( "total likelihood numerical error" ) );
...
SIR_scheme::Float SIR_scheme::update_resample( const Importance_resampler& resampler )
...
void SIR_scheme::predict( Sampled_predict_model& f )
...
void SIR_scheme::observe( Likelihood_observe_model& h, const Vec& z )
 ...
 */
{
    ...
    const std::size_t nSamples = S.size2();
    for ( std::size_t i = 0; i != nSamples; ++i ) { wir[i] *= h.L( FM::column( S, i ) ); }
    wir_update          = true;
}

void SIR_scheme::observe_likelihood( const Vec& lw )
/* Observation fusion directly from likelihood weights
 * lw may be smaller then the number of particles. Weights for additional particles are assumed to
 * be 1
 * Pre : wir previous particle likelihood weights
 * Post: wir fused (multiplicative) particle likelihood weights
 */
 ...
 */
{
    ...
    while ( pi != P.end2() )   // Loop includes 0 to simplify code
    {
        ...
        for ( ColMatrix::iterator1 xpi = begin( pi, ublas::iterator2_tag() );
              xpi != end( pi, ublas::iterator2_tag() ); ++xpi )
#else
    ...
    for ( pi = P.begin2(); pi != P.end2(); ++pi ) {
        ...
        for ( DenseVec::iterator ni = n.begin(); ni != n.end(); ++ni ) { *ni *= rootq[ni.index()]; }
 ...
 */
SIR_kalman_scheme::SIR_kalman_scheme( std::size_t x_size, std::size_t s_size,
...
Bayes_base::Float SIR_kalman_scheme::update_resample( const Importance_resampler& resampler )
 ...
 *  The definition is the Maximum Likelihood (biased) estimate of covariance given samples with
 * unknown (estimated) mean
 * Numerics
