 ...
 * ISSUES:
 *  observe functions: returned rcond is the minimum of each sequential update, an overall
...
UD_scheme::UD_scheme( std::size_t x_size, std::size_t q_maxsize, std::size_t z_initialsize )
...
UD_scheme& UD_scheme::operator=( const UD_scheme& a )
...
UD_scheme::Float UD_scheme::predict( Linrz_predict_model& f )
...
UD_scheme::Float UD_scheme::predictGq( const Matrix& Fx, const Matrix& G, const FM::Vec& q )
 ...
 */
{
    ...
    if ( Nq > q_max ) error( Logic_exception( "Predict model q larger than preallocated space" ) );
...
void UD_scheme::observe_size( std::size_t z_size )
...
Bayes_base::Float UD_scheme::observe( Linrz_uncorrelated_observe_model& h, const Vec& z )
 ...
 */
{
    ...
    for ( std::size_t o = 0; o < z_size; ++o ) {
        ...
        if ( h.Zv[o] < 0 ) error( Numeric_exception( "Zv not PSD in observe" ) );
...
Bayes_base::Float UD_scheme::observe( Linrz_correlated_observe_model& /*h*/, const Vec& /*z*/ )
...
Bayes_base::Float UD_scheme::observe( Linear_correlated_observe_model& h, const Vec& z )
 ...
 */
{
    ...
    if ( z_size > 0 ) {   // Solve G* GIHx = Hx for GIHx in-place
        GIHx = h.Hx;
        for ( j = 0; j < x_size; ++j ) {
            i = z_size - 1;
            do {
                for ( k = i + 1; k < z_size; ++k ) { GIHx( i, j ) -= Gz( i, k ) * GIHx( k, j ); }
            } while ( i-- > 0 );
        }
...
Bayes_base::Float UD_scheme::observe( UD_sequential_observe_model& h, const Vec& z )
...
UD_scheme::Float UD_scheme::observeUD( FM::Vec& gain, Float& alpha, const Vec& h, const Float r )
