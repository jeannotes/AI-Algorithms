...
void Unscented_scheme::unscented( FM::ColMatrix& XX, const FM::Vec& x, const FM::SymMatrix& X,
                                  Float scale )
/*
...
{
    // Postconditions
    if ( !isPSD( X ) ) error( Numeric_exception( "Initial X not PSD" ) );
}
 ...
 */
{
    ...
    for ( std::size_t i = 1; i < XX_size; ++i ) {
        noalias( x ) +=
            column( fXX, i ) / Float( 2 );   // ISSUE uBlas may not be able to promote integer 2
    }
    x /= x_kappa;
    // Covariance of distribution: X
    // Subtract mean from each point in fXX
    for ( std::size_t i = 0; i < XX_size; ++i ) { column( fXX, i ).minus_assign( x ); }
...
class Adapted_zero_model : public Unscented_predict_model {
  public:
    Adapted_zero_model( Functional_predict_model& fm )
        : Unscented_predict_model( 0 ), fmodel( fm ), zeroQ( 0, 0 ) {}
    const Vec& f( const Vec& x ) const { return fmodel.fx( x ); }
    const SymMatrix& Q( const FM::Vec& /*x*/ ) const { return zeroQ; }
  ...
  public:
    Adapted_model( Additive_predict_model& am )
        : Unscented_predict_model( am.G.size1() ),
          amodel( am ),
          QGqG( am.G.size1(), am.G.size1() )   // Q gets size from GqG'
    {
        RowMatrix temp( am.G.size1(), am.G.size1() );
        noalias( QGqG ) = prod_SPD( am.G, am.q, temp );
    }
    const Vec& f( const Vec& x ) const { return amodel.f( x ); }
    const SymMatrix& Q( const FM::Vec& /*x*/ ) const { return QGqG; }
...
Bayes_base::Float Unscented_scheme::observe( Uncorrelated_additive_observe_model& h,
                                             const FM::Vec& z )
/* Observation fusion
 *  Pre : x,X
 *  Post: x,X is PSD
...
Bayes_base::Float Unscented_scheme::observe( Correlated_additive_observe_model& h,
                                             const FM::Vec& z )
/* Observation fusion
 *  Pre : x,X
 *  Post: x,X is PSD
 */
{
    ...
    for ( std::size_t i = 1; i < zXX.size2(); ++i ) {
        noalias( zp ) +=
            column( zXX, i ) / Float( 2 );   // ISSUE uBlas may not be able to promote integer 2
    }
    zp /= x_kappa;

    // Covariance of observation predict: Xzz
    // Subtract mean from each point in zXX
    for ( std::size_t i = 0; i < XX_size; ++i ) { column( zXX, i ).minus_assign( zp ); }
    // Center point, premult here by 2 for efficiency
    {
