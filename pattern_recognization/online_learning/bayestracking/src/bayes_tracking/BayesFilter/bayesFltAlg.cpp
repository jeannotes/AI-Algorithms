...
{
Simple_additive_predict_model::Simple_additive_predict_model( State_function f_init,
...
Simple_linrz_predict_model::Simple_linrz_predict_model( State_function f_init,
...
Simple_linear_predict_model::Simple_linear_predict_model( const FM::Matrix& Fx_init,
                                                          const FM::Matrix& G_init,
                                                          const FM::Vec& q_init )
...
Simple_linrz_correlated_observe_model::Simple_linrz_correlated_observe_model(
    State_function f_init, const FM::Matrix& Hx_init, const FM::SymMatrix& Z_init )
...
Simple_linrz_uncorrelated_observe_model::Simple_linrz_uncorrelated_observe_model(
    State_function f_init, const FM::Matrix& Hx_init, const FM::Vec& Zv_init )
...
Simple_linear_correlated_observe_model::Simple_linear_correlated_observe_model(
    const FM::Matrix& Hx_init, const FM::SymMatrix& Z_init )
...
Simple_linear_uncorrelated_observe_model::Simple_linear_uncorrelated_observe_model(
    const FM::Matrix& Hx_init, const FM::Vec& Zv_init )
...
Bayes_base::Float General_LzUnAd_observe_model::Likelihood_uncorrelated::L(
    const Uncorrelated_additive_observe_model& model, const FM::Vec& z, const FM::Vec& zp ) const
 ...
 */
{
    ...
    // distribution with variance Zd. Thus, the likelihood can be determined directly from the
    // Gaussian

    FM::Vec::iterator zi = zInnov.begin(), zi_end = zInnov.end();
    for ( ; zi != zi_end; ++zi ) { *zi *= *zi; }
...
void General_LzUnAd_observe_model::Likelihood_uncorrelated::Lz(
    const Uncorrelated_additive_observe_model& model )
 ...
 */
{
    ...
    for ( FM::Vec::const_iterator zi = model.Zv.begin(), zi_end = model.Zv.end(); zi != zi_end;
          ++zi ) {
        detZ *= *zi;
...
Bayes_base::Float General_LzCoAd_observe_model::Likelihood_correlated::L(
    const Correlated_additive_observe_model& model, const FM::Vec& z, const FM::Vec& zp ) const
/* Definition of likelihood given an additive Gaussian observation model:
 *  p(z|x) = exp(-0.5*(z-h(x))'*inv(Z)*(z-h(x))) / sqrt(2pi^nz*det(Z));
 *  L(x) the the Likelihood L(x) doesn't depend on / sqrt(2pi^nz) for constant z size
...
void General_LzCoAd_observe_model::Likelihood_correlated::Lz(
    const Correlated_additive_observe_model& model )
...
Bayes_base::Float General_LzCoAd_observe_model::Likelihood_correlated::scaled_vector_square(
    const FM::Vec& v, const FM::SymMatrix& V )
...
Adapted_Correlated_additive_observe_model::Adapted_Correlated_additive_observe_model(
    Uncorrelated_additive_observe_model& adapt )
    : Correlated_additive_observe_model( adapt.Zv.size() ), unc( adapt ) {
    Z.clear();
    for ( std::size_t i = 0; i < unc.Zv.size(); ++i )
        Z( i, i ) = Float( unc.Zv[i] );   // ISSUE mixed type proxy assignment
}

Adapted_Linrz_correlated_observe_model::Adapted_Linrz_correlated_observe_model(
    Linrz_uncorrelated_observe_model& adapt )
    : Linrz_correlated_observe_model( adapt.Hx.size2(), adapt.Hx.size1() ), unc( adapt ) {
    Hx = unc.Hx;
    Z.clear();
    for ( std::size_t i = 0; i < unc.Zv.size(); ++i )
        Z( i, i ) = Float( unc.Zv[i] );   // ISSUE mixed type proxy assignment
