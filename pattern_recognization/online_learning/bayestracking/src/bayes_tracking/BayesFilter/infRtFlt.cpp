 ...
 */
#include "bayes_tracking/BayesFilter/infRtFlt.hpp"
#include <algorithm>
#include "bayes_tracking/BayesFilter/matSup.hpp"
#include "bayes_tracking/BayesFilter/uLAPACK.hpp"   // Common LAPACK interface
 ...
 */
{
    ...
    bool singular = UTinverse( RI );
    if ( singular ) error( Numeric_exception( "R not PD" ) );
 ...
 */
{
    ...
    if ( info < 0 ) error( Numeric_exception( "Fx not LU factorisable" ) );

    FM::identity( invFx );   // Invert
    info = LAPACK::getrs( 'N', FxLU, ipivot, invFx );
    if ( info != 0 ) error( Numeric_exception( "Predict Fx not LU invertable" ) );
}

Bayes_base::Float Information_root_scheme::predict( Linrz_predict_model& f,
                                                    const FM::ColMatrix& invFx, bool linear_r )
 ...
 */
{
    ...
    if ( linear_r ) A.sub_column( q_size, q_size + x_size, q_size + x_size ).assign( r );

    // Calculate factorisation so we have and upper triangular R
    DenseVec tau( q_size + x_size );
    int info = LAPACK::geqrf( A, tau );
    if ( info != 0 ) error( Numeric_exception( "Predict no QR factor" ) );
...
Bayes_base::Float Information_root_scheme::predict( Linear_predict_model& f )
...
Bayes_base::Float Information_root_scheme::observe_innovation( Linrz_correlated_observe_model& h,
                                                               const FM::Vec& s )
 ...
 */
{
    ...
    (void) singular;
    // Form Augmented matrix for factorisation
    DenseColMatrix A( x_size + z_size, x_size + 1 );   // Column major required for LAPACK, also
                                                       // this property is using in indexing
    ...
    int info = LAPACK::geqrf( A, tau );
    if ( info != 0 ) error( Numeric_exception( "Observe no QR factor" ) );
    // Extract the roots, junk in strict lower triangle
...
Bayes_base::Float Information_root_scheme::observe_innovation( Linrz_uncorrelated_observe_model& h,
                                                               const FM::Vec& s )
 ...
 */
{
    ...
    Zir.clear();
    for ( std::size_t i = 0; i < z_size; ++i ) { Zir( i, i ) = 1 / std::sqrt( h.Zv[i] ); }
    // Form Augmented matrix for factorisation
    DenseColMatrix A( x_size + z_size, x_size + 1 );   // Column major required for LAPACK, also
                                                       // this property is using in indexing
    A.sub_matrix( 0, x_size, 0, x_size ).assign( R );
