...
bool MTRK::isLost( const FilterType* filter, double varLimit = 1.0 ) {
    ...
    if ( filter->X( 0, 0 ) + filter->X( 2, 2 ) > sqr( varLimit ) ) return true;
    return false;
}
    ...
    for ( ;; ) {
        ...
        if ( time - obsvTime > 30e-3 ) {
            ...
            for ( li = obsvBuffer.begin(); li != liEnd; li++ ) {
                ...
                if ( winDebug ) {
                    ...
                    if ( om_flag == CARTESIAN )
                        trkwin->setObject( label, observation[0], observation[1],
                                           TrackWin::NO_ORIENTATION, TrackWin::GREY );
                    if ( om_flag == POLAR )
                        trkwin->setObject( label, observation[1] * cos( observation[0] ),
                                           observation[1] * sin( observation[0] ),
                                           TrackWin::NO_ORIENTATION, TrackWin::GREY );
                }
            }

            // process observations (if available) and update tracks
            if ( om_flag == CARTESIAN ) { mtrk.process( ctm, CARTESIAN, alg ); }
            if ( om_flag == POLAR ) {
                plm.update( 0, 0, 0 );
        ...
        if ( winDebug ) {
            ...
            for ( int i = 0; i < n; i++ ) {
                ...
                trkwin->setObject(
                    label, mtrk[i].filter->x[0], mtrk[i].filter->x[2],
                    atan2( mtrk[i].filter->x[3], mtrk[i].filter->x[1] ) /*orientation*/,
                    TrackWin::RED, mtrk[i].filter->X( 0, 0 ), mtrk[i].filter->X( 2, 2 ),
                    mtrk[i].filter->X( 1, 2 ) /*(co)variance*/ );
            }
            trkwin->update( 20 );
        }
