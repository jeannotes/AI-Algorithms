...
void Object3dDetector::pointCloudCallback(const sensor_msgs::PointCloud2::ConstPtr &ros_pc2)
{
  ...
  if (print_fps_)
    if (++frames > 10)
    {
      std::cerr << "[object3d_detector_ol]: fps = " << float(frames) / (float(clock() - start_time) / CLOCKS_PER_SEC) << ", timestamp = " << clock() / CLOCKS_PER_SEC << ", positive = " << positive_ << ", negative = " << negative_ << std::endl;
      reset = true;
    } //fps
}
...
void Object3dDetector::classify()
{
  ...
  {// features_.push_back(f); in xtractCluster
