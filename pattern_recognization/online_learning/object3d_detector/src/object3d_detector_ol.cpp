...
Object3dDetector::Object3dDetector() {
    ...
    private_nh.param< bool >( "find_the_best_training_parameters",
                              find_the_best_training_parameters_, true );
    ...
    svm_node_ = (struct svm_node*) malloc(
        ( FEATURE_SIZE + 1 ) * sizeof( struct svm_node ) );   // 1 more size for end index (-1)
    ...
    svm_node_size_ = ( round_positives_ + round_negatives_ ) * ( max_trains_ - 1 ) +
                     init_positives_ + init_negatives_;
    svm_problem_.l = 0;
    ...
    for ( int i = 0; i < svm_node_size_; i++ )
        svm_problem_.x[i] =
            (struct svm_node*) malloc( ( FEATURE_SIZE + 1 ) * sizeof( struct svm_node ) );
    // one-line malloc
    // svm_problem_.x = (struct svm_node **)malloc((max_positives_+max_negatives_)*sizeof(struct
    // svm_node *)+((max_positives_+max_negatives_)*((FEATURE_SIZE+1)*sizeof(struct svm_node))));
    ...
    /*** Subscribers ***/
    point_cloud_sub_ = node_handle_.subscribe< sensor_msgs::PointCloud2 >(
        "velodyne_points", 100, &Object3dDetector::pointCloudCallback, this );
    trajectory_sub_ = node_handle_.subscribe< geometry_msgs::PoseArray >(
        "people_tracker/trajectory", 100, &Object3dDetector::trajectoryCallback, this );
}
...
Object3dDetector::~Object3dDetector() {
    ...
    free( svm_problem_.y );
    for ( int i = 0; i < svm_node_size_; i++ ) free( svm_problem_.x[i] );
    free( svm_problem_.x );
}
...
void Object3dDetector::trajectoryCallback( const geometry_msgs::PoseArray::ConstPtr& trajectory ) {
    if ( train_round_ == max_trains_ ||
         ( positive_ + negative_ ) == ( max_positives_ + max_negatives_ ) )
        return;
    ...
    if ( train_round_ > 0 ) {
        ...
        for ( int i = 0; i < trajectory->poses.size(); i++ ) {
            for ( int j = 0; j < learnable_clusters_.size(); j++ ) {
                if ( (uint32_t) trajectory->poses[i].position.z ==
                         learnable_clusters_[j]->header.seq &&
                     learnable_clusters_[j]->header.frame_id == "human" ) {
                    learn_it = true;
                    break;
                }
            }
        ...
        for ( int i = 0; i < trajectory->poses.size(); i++ ) {
            for ( int j = 0; j < learnable_clusters_.size(); j++ ) {
                if ( (uint32_t) trajectory->poses[i].position.z ==
                    ...
                    if ( trajectory->header.frame_id == "human_trajectory" &&
                         positive_ < max_positives_ ) {
...
void Object3dDetector::pointCloudCallback( const sensor_msgs::PointCloud2::ConstPtr& ros_pc2 ) {
    ...
    if ( print_fps_ )
        if ( ++frames > 10 ) {
            std::cerr << "[object3d_detector_ol]: fps = "
                      << float( frames ) / ( float( clock() - start_time ) / CLOCKS_PER_SEC )
                      << ", timestamp = " << clock() / CLOCKS_PER_SEC
                      << ", positive = " << positive_ << ", negative = " << negative_ << std::endl;
            reset = true;
        }   // fps
}

const int nested_regions_  = 14;
int zone_[nested_regions_] = { 2, 3, 3, 3, 3, 3, 3, 2, 3, 3, 3, 3, 3, 3 };   // for more details,
                                                                             // see our IROS'17
                                                                             // paper.
        ...
        if ( indices_array[i].size() > cluster_size_min_ ) {
            ...
            for ( std::vector< pcl::PointIndices >::const_iterator it = cluster_indices.begin();
                ...
                if ( max[0] - min[0] >= vfilter_min_x_ && max[0] - min[0] <= vfilter_max_x_ &&
                    ...
                    /*
                     * in the above, it already examines (with bbox threshold) it must
                     * not be a human, so next just add a negative sample in to classifier
                     */
                    if ( positive_ == max_positives_ &&
                         negative_ <
                             max_negatives_ ) {   // @todo condition test only, to be removed
                        Feature f;
                        extractFeature( cluster, f );
                        saveFeature( f, svm_problem_.x[svm_problem_.l] );
                        svm_problem_.y[svm_problem_.l++] = -1;   // -1, the negative label
 ...
 * f5 (9d): 2D covariance matrix in 3 zones, which are the upper half, and the left and right lower
 * halves.
...
void computeProjectedPlane( pcl::PointCloud< pcl::PointXYZI >::Ptr pc,
                            Eigen::Matrix3f& eigenvectors, int axe, Eigen::Vector4f& centroid,
    ...
    for ( size_t i = 0; i < pc->size(); i++ ) {
        float distance_to_plane = coefficients[0] * pc->points[i].x +
...
void compute3ZoneCovarianceMatrix( pcl::PointCloud< pcl::PointXYZI >::Ptr plane,
                                   Eigen::Vector4f& mean, float* partial_covariance_2d ) {
    pcl::PointCloud< pcl::PointXYZI >::Ptr zone_decomposed[3];
    for ( int i = 0; i < 3; i++ ) zone_decomposed[i].reset( new pcl::PointCloud< pcl::PointXYZI > );
...
void computeHistogramNormalized( pcl::PointCloud< pcl::PointXYZI >::Ptr pc, int horiz_bins,
                                 int verti_bins, float* histogram ) {
    Eigen::Vector4f min, max, min_box, max_box;
    pcl::getMinMax3D( *pc, min, max );
    float horiz_itv, verti_itv;
    horiz_itv = ( max[0] - min[0] > max[1] - min[1] ) ? ( max[0] - min[0] ) / horiz_bins :
    ...
    for ( int i = 0; i < horiz_bins; i++ ) {
        for ( int j = 0; j < verti_bins; j++ ) {
            if ( max[0] - min[0] > max[1] - min[1] ) {
                ...
                max_box << min[0] + horiz_itv * ( i + 1 ), max[1], min[2] + verti_itv * ( j + 1 ),
                    0;
            } else {
                min_box << min[0], min[1] + horiz_itv * i, min[2] + verti_itv * j, 0;
                max_box << max[0], min[1] + horiz_itv * ( i + 1 ), min[2] + verti_itv * ( j + 1 ),
                    0;
            }
...
void computeSlice( pcl::PointCloud< pcl::PointXYZI >::Ptr pc, int n, float* slice ) {
    ...
    if ( itv > 0 ) {
        for ( int i = 0; i < n; i++ ) { blocks[i].reset( new pcl::PointCloud< pcl::PointXYZI > ); }
...
void computeIntensity( pcl::PointCloud< pcl::PointXYZI >::Ptr pc, int bins, float* intensity ) {
    ...
    for ( int i = 0; i < 27; i++ ) intensity[i] = 0;
    ...
    for ( size_t i = 0; i < pc->size(); i++ ) {
        ...
        int ii = std::min( float( bins - 1 ), std::floor( ( pc->points[i].intensity - min ) /
                                                          ( ( max - min ) / bins ) ) );
        intensity[ii]++;
    }
...
void Object3dDetector::extractFeature( pcl::PointCloud< pcl::PointXYZI >::Ptr pc, Feature& f ) {
    ...
    for ( int i = 0; i < pc->size(); i++ ) {
        d2 = pc->points[i].x * pc->points[i].x + pc->points[i].y * pc->points[i].y +
    ...
    // pcl::PointCloud<pcl::PointXYZI>::Ptr main_plane(new pcl::PointCloud<pcl::PointXYZI>),
    // secondary_plane(new pcl::PointCloud<pcl::PointXYZI>);
    ...
    // // f5: 2D covariance matrix in 3 zones, which are the upper half, and the left and right
    // lower halves.
...
void Object3dDetector::classify() {
    ...
    for ( std::vector< Feature >::iterator it = features_.begin(); it != features_.end();
          it++ ) {   // features_.push_back(f); in xtractCluster
        ...
        if ( train_round_ > 0 ) {
            ...
            for ( int i = 0; i < FEATURE_SIZE; i++ ) {
                ...
                if ( svm_node_[i].value == svm_range_[i][0] )
                    ...
                    svm_node_[i].value = svm_xlower_ +
                                         ( svm_xupper_ - svm_xlower_ ) *
                                             ( svm_node_[i].value - svm_range_[i][0] ) /
                                             ( svm_range_[i][1] - svm_range_[i][0] );
            }
            ...
            if ( svm_check_probability_model( svm_model_ ) ) {
                ...
                if ( prob_estimates[0] > human_probability_ ) svm_find_human = true;
            } else {
                if ( svm_predict( svm_model_, svm_node_ ) == 1 ) svm_find_human = true;
            }
        }
        ...
        if ( train_round_ < max_trains_ ) {
            ...
            if ( svm_find_human ) learnable_clusters_[i]->header.frame_id = "human";
    ...
    if ( marker_array.markers.size() ) { marker_array_pub_.publish( marker_array ); }
}
...
void Object3dDetector::train() {
    ...
    //   std::cerr << "svm scale range [attribute " << i << "]: " << svm_range_[i][0] << ", " <<
    //   svm_range_[i][1] << std::endl;
    for ( int i = 0; i < svm_problem_.l; i++ ) {
        for ( int j = 0; j < FEATURE_SIZE; j++ ) {
            if ( svm_range_[j][0] == svm_range_[j][1] )   // skip single-valued attribute
                continue;
            if ( svm_problem_.x[i][j].value == svm_range_[j][0] )
                ...
                svm_problem_.x[i][j].value = svm_xlower_ +
                                             ( svm_xupper_ - svm_xlower_ ) *
                                                 ( svm_problem_.x[i][j].value - svm_range_[j][0] ) /
                                                 ( svm_range_[j][1] - svm_range_[j][0] );
            // std::cerr << "training data " << i << " [attribute " << j << "]: " <<
            // svm_problem_.x[i][j].value << std::endl;
        }
    }
        ...
        if ( svm_check_parameter( &svm_problem_, &svm_parameter_ ) == NULL ) {
            ...
            if ( fp == NULL ) {
                ...
                if ( fgets( result, 100, fp ) != NULL ) {
                    ...
                    pch                  = strtok( NULL, " " );
                    float rate           = atof( pch );
                    std::cerr << "Best c=" << svm_parameter_.C << ", g=" << svm_parameter_.gamma
                              << " CV rate=" << rate << std::endl;
                }
            }
            pclose( fp );
        }
    ...
    std::cerr << "Training round " << train_round_ << " finished with "
              << float( clock() - t ) / CLOCKS_PER_SEC << " seconds" << std::endl;
