...
Object3dDetector::Object3dDetector() {
    point_cloud_sub_ = node_handle_.subscribe< sensor_msgs::PointCloud2 >(
        "velodyne_points", 1, &Object3dDetector::pointCloudCallback, this );
    ...
    if ( ( svm_model_ = svm_load_model( model_file_name_.c_str() ) ) == NULL ) {
        ...
        if ( !range_file.is_open() ) {
            ...
            while ( std::getline( range_file, line ) ) {
                ...
                i++;
                // std::cerr << i << " " <<  svm_scale_range_[i][0] << " " << svm_scale_range_[i][1]
                // << std::endl;
            }
            use_svm_model_ = true;
        }
        ...
        if ( indices_array[i].size() > cluster_size_min_ ) {
            ...
            for ( std::vector< pcl::PointIndices >::const_iterator it = cluster_indices.begin();
                ...
                if ( human_size_limit_ &&
                     ( max[0] - min[0] < 0.2 || max[0] - min[0] > 1.0 || max[1] - min[1] < 0.2 ||
                       max[1] - min[1] > 1.0 || max[2] - min[2] < 0.5 || max[2] - min[2] > 2.0 ) )
                    continue;
...
void Object3dDetector::classify() {
    ...
    for ( std::vector< Feature >::iterator it = features_.begin(); it != features_.end(); ++it ) {
        if ( use_svm_model_ ) {
            ...
            // std::cerr << "test_id = " << it->id << ", number_points = " << it->number_points <<
            // ", min_distance = " << it->min_distance << std::endl;
            ...
            if ( is_probability_model_ ) {
                ...
                if ( prob_estimates[0] < human_probability_ ) continue;
            } else {
                if ( svm_predict( svm_model_, svm_node_ ) != 1 ) continue;
            }
        }
