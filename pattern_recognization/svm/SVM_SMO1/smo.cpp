...
int main()
{
    ...
    //存放训练后的参数
    {
        ...
        outClientFile<<"rate="<<(double)n_support_vectors/first_test_i<<endl;//支持向量在样本数量中的占比
        for(int i=0;i<end_support_i;i++)
        {
            if(alph[i]>0)
            {
...
int examineExample(int i1)
{
    ...
    if(alph1>0&&alph1<C)//支持向量
        E1=error_cache[i1];
        /*
        A cached error value E is kept for every non-bound training example from which example can be chosed for maximizing the step size.
        */
    else
        E1=learned_func(i1)-y1;//learned_func为计算输出函数（书中127页的7.105）
    /*
    When an error E is required by SMO, it will look up the error in the error cache if the corresponding Lagrange multiplier is not at bound. Otherwise, it will evaluate the current SVM decision function based on the current alph vector.
    */
    r1=y1*E1;
    ...
    */
    if((r1>tolerance&&alph1>0)||(r1<-tolerance&&alph1<C))//如果找到点（xi,yi）不满足kkt条件,那么开始选择第二个点
    {
        /*使用三种方法选择第二个乘子:
...
int examineNonBound(int i1)
{
 ...
 //If there is no positive progress in hierarchy one, hierarchy two will iterate through the non-bound example starting at a random position.
     int i2;
...
int takeStep(int i1,int i2)
{
    ...
    ***********************************/
    ...
    //After each step, b is re-computed.
    {
        ...
        {
            if(a2>0&&a2<C)
             bnew=b+E2 + y1*(a1-alph1)*k12 + y2*(a2-alph2)*k22;//同上，根据公式7.116
            else
            {
...
//更新error_cache，对取得进展的a1,a2,所对应的i1,i2的error_cache[i1]=error_cache[i2]=0
    {
        ...
        error_cache[i2]=0.0;
        /*
        When a Lagrange multiplier is non-bound and is unvolved in a joint optimization,        its cached error is set to zero.
        */
    }
