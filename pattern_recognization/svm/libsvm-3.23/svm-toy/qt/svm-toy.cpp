...
QRgb colors[] = { qRgb( 0, 0, 0 ),     qRgb( 0, 120, 120 ), qRgb( 120, 120, 0 ),
                  qRgb( 120, 0, 120 ), qRgb( 0, 200, 200 ), qRgb( 200, 200, 0 ),
                  qRgb( 200, 0, 200 ) };
  ...
  private slots:
    ...
    void button_run_clicked() {
        ...
        if ( param.kernel_type == PRECOMPUTED ) {
            ...
            i = 0;
            for ( list< point >::iterator q = point_list.begin(); q != point_list.end();
                  q++, i++ ) {
                x_space[2 * i].index     = 1;
    ...
    void button_clear_clicked() { clear_all(); }
        ...
        if ( !filename.isNull() ) {
            ...
            if ( fp ) {
                if ( svm_type == EPSILON_SVR || svm_type == NU_SVR ) {
                    for ( list< point >::iterator p = point_list.begin(); p != point_list.end();
                          p++ )
                        fprintf( fp, "%f 1:%f\n", p->y, p->x );
                } else {
                    for ( list< point >::iterator p = point_list.begin(); p != point_list.end();
...
SvmToyWindow::SvmToyWindow()
    ...
    QObject::connect( &button_run, SIGNAL( clicked() ), this, SLOT( button_run_clicked() ) );
    QObject::connect( &button_clear, SIGNAL( clicked() ), this, SLOT( button_clear_clicked() ) );
    QObject::connect( &button_save, SIGNAL( clicked() ), this, SLOT( button_save_clicked() ) );
    QObject::connect( &button_load, SIGNAL( clicked() ), this, SLOT( button_load_clicked() ) );
    QObject::connect( &input_line, SIGNAL( returnPressed() ), this, SLOT( button_run_clicked() ) );
