...
class Kernel : public QMatrix {
  ...
  private:
    ...
    double kernel_precomputed( int i, int j ) const { return x[i][(int) ( x[j][0].value )].value; }
};
...
Kernel::Kernel( int l, svm_node* const* x_, const svm_parameter& param )
    : kernel_type( param.kernel_type ),
      ...
      coef0( param.coef0 ) {
    switch ( kernel_type ) {
        case LINEAR: kernel_function      = &Kernel::kernel_linear; break;
        case POLY: kernel_function        = &Kernel::kernel_poly; break;
        case RBF: kernel_function         = &Kernel::kernel_rbf; break;
        case SIGMOID: kernel_function     = &Kernel::kernel_sigmoid; break;
        case PRECOMPUTED: kernel_function = &Kernel::kernel_precomputed; break;
    }
...
double Kernel::k_function( const svm_node* x, const svm_node* y, const svm_parameter& param ) {
    switch ( param.kernel_type ) {
        case LINEAR: return dot( x, y );
        case POLY: return powi( param.gamma * dot( x, y ) + param.coef0, param.degree );
        case RBF: {
            double sum = 0;
        ...
        case SIGMOID: return tanh( param.gamma * dot( x, y ) + param.coef0 );
...
void Solver::reconstruct_gradient() {
    ...
    if ( nr_free * l > 2 * active_size * ( l - active_size ) ) {
        for ( i = active_size; i < l; i++ ) {
            ...
            for ( j = 0; j < active_size; j++ )
                if ( is_free( j ) ) G[i] += alpha[j] * Q_i[j];
        }
    } else {
...
static void solve_epsilon_svr( const svm_problem* prob, const svm_parameter* param, double* alpha,
    ...
    Solver s;
    s.Solve( 2 * l, SVR_Q( *prob, *param ), linear_term, y, alpha2, param->C, param->C, param->eps,
             si, param->shrinking );
...
static decision_function svm_train_one( const svm_problem* prob, const svm_parameter* param,
    ...
    switch ( param->svm_type ) {
        case C_SVC: solve_c_svc( prob, param, alpha, &si, Cp, Cn ); break;
        case NU_SVC: solve_nu_svc( prob, param, alpha, &si ); break;
        case ONE_CLASS: solve_one_class( prob, param, alpha, &si ); break;
        case EPSILON_SVR: solve_epsilon_svr( prob, param, alpha, &si ); break;
        case NU_SVR: solve_nu_svr( prob, param, alpha, &si ); break;
    }
...
static void svm_group_classes( const svm_problem* prob, int* nr_class_ret, int** label_ret,
                               int** start_ret, int** count_ret,
                               int* perm ) {   //(prob, &nr_class, &label, &start, &count, perm);
    int l            = prob->l;
    int max_nr_class = 16;
    int nr_class     = 0;
    int* label       = Malloc( int, max_nr_class );
    int* count       = Malloc( int, max_nr_class );
    int* data_label  = Malloc( int, l );   // stores which label
    int i;
    ...
    for ( i = 0; i < l; i++ ) {
        ...
        for ( j = 0; j < nr_class; j++ ) {
            ...
            // if all the iteration does not applt this, j= nr_class, and set new label
        }
        data_label[i] = j;
        if ( j == nr_class ) {
    ...
    // we swap labels to ensure that internally the binary SVM has positive data corresponding to
    // the +1 instances.
    //
...
void svm_cross_validation( const svm_problem* prob, const svm_parameter* param, int nr_fold,
    ...
    if ( nr_fold > l ) {
        nr_fold = l;
        fprintf( stderr, "WARNING: # folds > # data. Will use # folds = # data instead (i.e., "
...
void svm_get_sv_indices( const svm_model* model, int* indices ) {
    if ( model->sv_indices != NULL )
        for ( int i = 0; i < model->l; i++ ) indices[i] = model->sv_indices[i];
}
...
double svm_predict_probability( const svm_model* model, const svm_node* x,
    ...
    if ( ( model->param.svm_type == C_SVC || model->param.svm_type == NU_SVC ) &&
        ...
        int prob_max_idx = 0;
        for ( i = 1; i < nr_class; i++ )
            if ( prob_estimates[i] > prob_estimates[prob_max_idx] ) prob_max_idx = i;
...
void svm_free_model_content( svm_model* model_ptr ) {
    ...
    if ( model_ptr->sv_coef ) {
        for ( int i = 0; i < model_ptr->nr_class - 1; i++ ) free( model_ptr->sv_coef[i] );
    }
...
const char* svm_check_parameter( const svm_problem* prob, const svm_parameter* param ) {
    ...
    if ( svm_type == NU_SVC || svm_type == ONE_CLASS || svm_type == NU_SVR )
        if ( param->nu <= 0 || param->nu > 1 ) return "nu <= 0 or nu > 1";
