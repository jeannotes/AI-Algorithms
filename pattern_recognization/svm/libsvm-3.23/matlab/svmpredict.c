...
void predict( int nlhs, mxArray* plhs[], const mxArray* prhs[], struct svm_model* model,
    ...
    if ( predict_probability ) {
        ...
        if ( svm_type == ONE_CLASS || svm_type == EPSILON_SVR || svm_type == NU_SVR ||
             nr_class ==
                 1 )   // if only one class in training data, decision values are still returned.
            tplhs[2] = mxCreateDoubleMatrix( testing_instance_number, 1, mxREAL );
        else
            tplhs[2] = mxCreateDoubleMatrix( testing_instance_number,
                                             nr_class * ( nr_class - 1 ) / 2, mxREAL );
    }
    ...
    for ( instance_index = 0; instance_index < testing_instance_number; instance_index++ ) {
        ...
        if ( mxIsSparse( prhs[1] ) &&
             model->param.kernel_type != PRECOMPUTED )   // prhs[1]^T is still sparse
        ...
        if ( predict_probability ) {
            if ( svm_type == C_SVC || svm_type == NU_SVC ) {
                ...
                for ( i = 0; i < nr_class; i++ )
                    ptr_prob_estimates[instance_index + i * testing_instance_number] =
                        prob_estimates[i];
            } else {
            ...
            if ( svm_type == ONE_CLASS || svm_type == EPSILON_SVR || svm_type == NU_SVR ) {
                ...
                double* dec_values =
                    (double*) malloc( sizeof( double ) * nr_class * ( nr_class - 1 ) / 2 );
                predict_label = svm_predict_values( model, x, dec_values );
                if ( nr_class == 1 )
                    ptr_dec_values[instance_index] = 1;
                else
                    for ( i = 0; i < ( nr_class * ( nr_class - 1 ) ) / 2; i++ )
                        ptr_dec_values[instance_index + i * testing_instance_number] =
                            dec_values[i];
                free( dec_values );
            }
            ptr_predict_label[instance_index] = predict_label;
    ...
    free( x );
    if ( prob_estimates != NULL ) free( prob_estimates );

    switch ( nlhs ) {
        case 3: plhs[2] = tplhs[2]; plhs[1] = tplhs[1];
        case 1:
    ...
    if ( mxIsStruct( prhs[2] ) ) {
        ...
        if ( nrhs == 4 ) {
            ...
            for ( i = 1; i < argc; i++ ) {
                ...
                switch ( argv[i - 1][1] ) {
                    case 'b': prob_estimate_flag = atoi( argv[i] ); break;
                    case 'q':
                        i--;
                        info = &print_null;
                        break;
