...
static const char* field_names[] = { "Parameters", "nr_class",   "totalSV", "rho",
                                     "Label",      "sv_indices", "ProbA",   "ProbB",
                                     "nSV",        "sv_coef",    "SVs" };

const char* model_to_matlab_structure( mxArray* plhs[], int num_of_feature,
    ...
    ptr         = mxGetPr( rhs[out_id] );
    for ( i = 0; i < n; i++ ) ptr[i] = model->rho[i];
    out_id++;
    ...
    if ( model->label ) {
        ...
        ptr         = mxGetPr( rhs[out_id] );
        for ( i = 0; i < model->nr_class; i++ ) ptr[i] = model->label[i];
    } else
    ...
    if ( model->sv_indices ) {
        ...
        ptr         = mxGetPr( rhs[out_id] );
        for ( i = 0; i < model->l; i++ ) ptr[i] = model->sv_indices[i];
    } else
        rhs[out_id] = mxCreateDoubleMatrix( 0, 0, mxREAL );
    out_id++;
    ...
    if ( model->probA != NULL ) {
        rhs[out_id] = mxCreateDoubleMatrix( n, 1, mxREAL );
        ptr         = mxGetPr( rhs[out_id] );
        for ( i = 0; i < n; i++ ) ptr[i] = model->probA[i];
    } else
        rhs[out_id] = mxCreateDoubleMatrix( 0, 0, mxREAL );
    out_id++;
    ...
    if ( model->probB != NULL ) {
        rhs[out_id] = mxCreateDoubleMatrix( n, 1, mxREAL );
        ptr         = mxGetPr( rhs[out_id] );
        for ( i = 0; i < n; i++ ) ptr[i] = model->probB[i];
    } else
        rhs[out_id] = mxCreateDoubleMatrix( 0, 0, mxREAL );
    out_id++;
    ...
    if ( model->nSV ) {
        rhs[out_id] = mxCreateDoubleMatrix( model->nr_class, 1, mxREAL );
        ptr         = mxGetPr( rhs[out_id] );
        for ( i = 0; i < model->nr_class; i++ ) ptr[i] = model->nSV[i];
    } else
        rhs[out_id] = mxCreateDoubleMatrix( 0, 0, mxREAL );
    out_id++;
    ...
    for ( i = 0; i < model->nr_class - 1; i++ )
        for ( j = 0; j < model->l; j++ ) ptr[( i * ( model->l ) ) + j] = model->sv_coef[i][j];
    out_id++;
    ...
    // SVs
    {
        ...
        pprhs[0] = rhs[out_id];
        if ( mexCallMATLAB( 1, pplhs, 1, pprhs, "transpose" ) ) return "cannot transpose SV matrix";
        rhs[out_id] = pplhs[0];
        out_id++;
    }
...
struct svm_model* matlab_matrix_to_model( const mxArray* matlab_struct, const char** msg ) {
    ...
    for ( i = 0; i < num_of_fields; i++ ) rhs[i] = mxGetFieldByNumber( matlab_struct, 0, i );
    ...
    ptr        = mxGetPr( rhs[id] );
    for ( i = 0; i < n; i++ ) model->rho[i] = ptr[i];
    id++;
    ...
    if ( mxIsEmpty( rhs[id] ) == 0 ) {
        ...
        ptr          = mxGetPr( rhs[id] );
        for ( i = 0; i < model->nr_class; i++ ) model->label[i] = (int) ptr[i];
    }
    id++;
    ...
    if ( mxIsEmpty( rhs[id] ) == 0 ) {
        ...
        ptr               = mxGetPr( rhs[id] );
        for ( i = 0; i < model->l; i++ ) model->sv_indices[i] = (int) ptr[i];
    }
    id++;
    ...
    if ( mxIsEmpty( rhs[id] ) == 0 ) {
        ...
        ptr          = mxGetPr( rhs[id] );
        for ( i = 0; i < n; i++ ) model->probA[i] = ptr[i];
    }
    id++;
    ...
    if ( mxIsEmpty( rhs[id] ) == 0 ) {
        ...
        ptr          = mxGetPr( rhs[id] );
        for ( i = 0; i < n; i++ ) model->probB[i] = ptr[i];
    }
    id++;
    ...
    if ( mxIsEmpty( rhs[id] ) == 0 ) {
        ...
        ptr        = mxGetPr( rhs[id] );
        for ( i = 0; i < model->nr_class; i++ ) model->nSV[i] = (int) ptr[i];
    }
    id++;
    ...
    for ( i = 0; i < model->nr_class - 1; i++ )
        for ( j = 0; j < model->l; j++ ) model->sv_coef[i][j] = ptr[i * ( model->l ) + j];
    id++;
