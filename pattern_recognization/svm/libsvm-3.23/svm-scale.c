...
void exit_with_help() {
    printf( "Usage: svm-scale [options] data_filename\n"
            "options:\n"
...
#define SKIP_ELEMENT             \
    ...
    if ( restore_filename ) {
        ...
        while ( fscanf( fp_restore, "%d %*f %*f\n", &idx ) == 1 ) max_index = max( idx, max_index );
        rewind( fp_restore );
    }
    ...
    if ( restore_filename ) {
        ...
        if ( fgetc( fp_restore ) == 'x' ) {
            ...
            while ( fscanf( fp_restore, "%d %lf %lf\n", &idx, &fmin, &fmax ) == 3 ) {
                for ( i = next_index; i < idx; i++ )
                    if ( feature_min[i] != feature_max[i] ) {
                        fprintf( stderr, "WARNING: feature index %d appeared in file %s was not "
                                         "seen in the scaling factor file %s. The feature is "
                                         "scaled to 0.\n",
            ...
            for ( i = next_index; i <= max_index; i++ )
                if ( feature_min[i] != feature_max[i] ) {
                    fprintf( stderr, "WARNING: feature index %d appeared in file %s was not seen "
                                     "in the scaling factor file %s. The feature is scaled to 0.\n",
                             i, argv[argc - 1], restore_filename );
                    feature_min[i] = 0;
                    feature_max[i] = 0;
                }
    ...
    if ( save_filename ) {
        ...
        if ( min_index < 1 )
            fprintf( stderr, "WARNING: scaling factors with indices smaller than 1 are not stored "
                             "to the file %s.\n",
                     save_filename );
    ...
    if ( new_num_nonzeros > num_nonzeros )
        fprintf( stderr, "WARNING: original #nonzeros %ld\n"
                         "       > new      #nonzeros %ld\n"
                         "If feature values are non-negative and sparse, use -l 0 rather than the "
                         "default -l -1\n",
...
void output( int index, double value ) {
    ...
    if ( value == feature_min[index] )
        ...
        value = lower +
                ( upper - lower ) * ( value - feature_min[index] ) /
...
int clean_up( FILE* fp_restore, FILE* fp, const char* msg ) {
    ...
    free( feature_min );
    fclose( fp );
    if ( fp_restore ) fclose( fp_restore );
    return -1;
}
