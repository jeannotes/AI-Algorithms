...
void predict( FILE* input, FILE* output ) {
    ...
    if ( predict_probability ) {
        if ( svm_type == NU_SVR || svm_type == EPSILON_SVR )
            ...
            for ( j = 0; j < nr_class; j++ ) fprintf( output, " %d", labels[j] );
            fprintf( output, "\n" );
            free( labels );
        }
    }
    ...
    while ( readline( input ) != NULL ) {
        ...
        if ( endptr == label || *endptr != '\0' ) exit_input_error( total + 1 );
        ...
        if ( predict_probability && ( svm_type == C_SVC || svm_type == NU_SVC ) ) {
            ...
            for ( j = 0; j < nr_class; j++ ) fprintf( output, " %g", prob_estimates[j] );
            fprintf( output, "\n" );
        } else {
...
int main( int argc, char** argv ) {
    ...
    for ( i = 1; i < argc; i++ ) {
        ...
        ++i;
        switch ( argv[i - 1][1] ) {
            case 'b': predict_probability = atoi( argv[i] ); break;
            case 'q':
                info = &print_null;
                i--;
                break;
