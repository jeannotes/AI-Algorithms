...
vector< int > generateLabels( int labelsSize ) {
    ...
    for ( int i = 0; i < labelsSize; ++i ) { labels.push_back( i % 2 + 1 ); }
    return labels;
}

vector< vector< double > > generateData( int problemSize, int featureNum ) {   // 30, 10
...
int main() {
    ...
    x_space =
        Malloc( struct svm_node, ( elements + 1 ) * prob.l );   // memory for pairs of index/value
