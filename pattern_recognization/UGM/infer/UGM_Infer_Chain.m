...
if edgeStruct.useMex
    [nodeBel,edgeBel,logZ] = UGM_Infer_ChainC(nodePot,edgePot,edgeStruct.nStates);
else
    ...
    for n = nNodes-1:-1:1
        tmp = repmat(nodePot(n+1,1:nStates(n+1)),nStates(n),1).*edgePot(1:nStates(n),1:nStates(n+1),n);
        tmp2 = repmat(beta(n+1,1:nStates(n+1)),nStates(n),1);
