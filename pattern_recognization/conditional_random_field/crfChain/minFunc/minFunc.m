...
%   TolFun - Termination tolerance on the first-order optimality (1e-4)
%   TolX - Termination tolerance on progress in terms of function/parameter changes (1e-5)
%   Method - [ newton | bfgs | {lbfgs} | newton0 | cg | bb | sd ]
...
%       SR1 - after 2nd iteration, use SR1 instead of BFGS when it maintains positive definiteness (default: 1)
...
[verbose,verboseI,debug,doPlot,maxFunEvals,maxIter,tolFun,tolX,method,...
    ...
    HessianIter,outputFcn,useMex,useNegCurv] = ...
...
for i = 1:maxIter
    ...
    switch method
        ...
        case NEWTON0 % Hessian-Free Newton
            ...
            if isempty(HvFunc)
                ...
                if useNegCurv
                    [d,cgIter,cgRes,negCurv] = conjGrad([],-g,cgForce,cgMaxIter,debug,precondFunc,precondArgs,@autoHv,HvArgs);
                else
                ...
                HvArgs = {x,varargin{:}};
                if useNegCurv
                    [d,cgIter,cgRes,negCurv] = conjGrad([],-g,cgForce,cgMaxIter,debug,precondFunc,precondArgs,HvFunc,HvArgs);
                else
    ...
    if LS < 3 % Use Armijo Bactracking
        fprintf('Perform Backtracking line search')
    ...
    elseif LS < 6
        ...
        fprintf('Find Point satisfying Wolfe\n')
        ...
        fprintf('use matlab optim toolbox ')
        if isempty(t)
            exitflag = -2;
