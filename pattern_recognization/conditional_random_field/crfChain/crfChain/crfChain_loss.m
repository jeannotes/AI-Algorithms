        ...
        features = X(sentences(s,1)+n-1,:); % features for word w in sentence s
         for feat = 1:length(nFeatures)
             if features(feat) ~= 0 % we ignore features that are 0
                 ...
                 % iterate through all states, sometimes it's 0, but we
                 % guess some probability, so it's wrong, sometimes it's
                 % 100 percentage, but we also guess some other prob, this
                 % is stohastic gradient decent methods
             end
         end
    end
