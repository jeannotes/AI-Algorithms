...
class FastSearch {
  public:
    ...
    void create( Vocabulary& voc ) {
        ...
        _child_off_start =
            _feature_off_start + voc.m_k * _desc_size_bytes;   // where do children information
                                                               // start from the start of the block
        ...
        cerr << "creating done _total_size=" << _total_size / ( 1024 * 1024 ) << " " << _total_size
             << endl;
        ...
        for ( const auto& Block : parent_children ) {
            ...
            *reinterpret_cast< uint32_t* >( _data + block_offset_bytes ) = Block.second.size();
            for ( const auto& c : Block.second ) {
                ...
                memcpy( _data + block_offset_bytes + _feature_off_start + idx * _desc_size_bytes,
    ...
    node_info* getBestOfBlock( int block, const cv::Mat& desc ) {
        ...
        for ( int i = 0; i < n; i++ ) {
            uchar* ptr =
                (uchar*) ( _data + block_start + _feature_off_start + _desc_size_bytes * i );
            uint32_t d = 0;
    ...
    inline bool getNodeInfo( int block, int id ) {
        return (node_info*) ( _data + ( block * _block_size_bytes ) + _child_off_start +
                              sizeof( node_info ) * id );
    }
    ...
    void saveToFile( const std::string& filepath ) throw( std::exception ) {
        ...
        cout << "save:" << int( _data[0] ) << " " << int( _data[1] ) << " " << int( _data[2] )
             << " " << int( _data[3] ) << endl;
