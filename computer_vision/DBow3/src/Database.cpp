...
EntryId Database::add( const std::vector< cv::Mat >& features, BowVector* bowvec,
                       FeatureVector* fvec ) {   // std::map<NodeId, std::vector<unsigned int> >
    ...
    if ( m_use_di && fvec != NULL ) {
        ...
        return add( v, *fvec );                               // v is already a image
    } else if ( m_use_di ) {
        FeatureVector fv;
...
EntryId Database::add( const BowVector& v, const FeatureVector& fv ) {
    /*
     * featurevector: a picture has many corners, stores all wordID,
     * and which time it is add in.
     */
    EntryId entry_id = m_nentries++;   // return old one
    ...
    if ( m_use_di ) {
        ...
        if ( entry_id == m_dfile.size() ) {
            ...
            /*
             * that is fv, always, it push (corner, i++) map(wordID, vector)
             * if conter appears twice, push in
             * otherwise, insert
             */
        } else {
            m_dfile[entry_id] = fv;   // entry_id is sequence number of all images
        }
    }

    // update inverted file
    // vector of words to represent image
    for ( vit = v.begin(); vit != v.end(); ++vit ) {
        const WordId& word_id        = vit->first;
        const WordValue& word_weight = vit->second;

        IFRow& ifrow = m_ifile[word_id];   // std::vector<IFRow> ;typedef std::list<IFPair> IFRow;
        ifrow.push_back( IFPair( entry_id, word_weight ) );
        /*
         * stores those images reach the same nodeID, entry_id is the sequence,
         * so when sompare the score, only need to compare the same node, we have those images's
         * weight
...
void Database::query( const cv::Mat& features, QueryResults& ret, int max_results,
                      int max_id ) const {
    std::vector< cv::Mat > vf( features.rows );
    for ( int r = 0; r < features.rows; r++ ) vf[r] = features.rowRange( r, r + 1 );
...
void Database::query( const std::vector< cv::Mat >& features, QueryResults& ret, int max_results,
                      int max_id ) const {
    BowVector vec;
    m_voc->transform( features, vec );   // Vector of words to represent images
...
void Database::query( const BowVector& vec, QueryResults& ret, int max_results, int max_id ) const {
    ...
    switch ( m_voc->getScoringType() ) {
        case L1_NORM: queryL1( vec, ret, max_results, max_id ); break;

        case L2_NORM: queryL2( vec, ret, max_results, max_id ); break;

        case CHI_SQUARE: queryChiSquare( vec, ret, max_results, max_id ); break;

        case KL: queryKL( vec, ret, max_results, max_id ); break;

        case BHATTACHARYYA: queryBhattacharyya( vec, ret, max_results, max_id ); break;

        case DOT_PRODUCT: queryDotProduct( vec, ret, max_results, max_id ); break;
    }
}
...
void Database::queryL1( const BowVector& vec, QueryResults& ret, int max_results,
    ...
    for ( vit = vec.begin(); vit != vec.end(); ++vit ) {
        const WordId word_id    = vit->first;    // sequence number of images
        const WordValue& qvalue = vit->second;   // corresponding weight

        const IFRow& row = m_ifile[word_id];   // ifrow.push_back(IFPair(entry_id, word_weight));
        ...
        for ( auto rit = row.begin(); rit != row.end(); ++rit ) {
            ...
            if ( (int) entry_id < max_id || max_id == -1 ) {
                ...
                if ( pit != pairs.end() && !( pairs.key_comp()( entry_id, pit->first ) ) ) {
                    ...
                    // the score comparing with i-th image
                }
            }
    ...
    if ( max_results > 0 && (int) ret.size() > max_results )
        ret.resize( max_results );   // get the first max_results part
...
void Database::queryChiSquare( const BowVector& vec, QueryResults& ret, int max_results,
                               int max_id ) const {
    BowVector::const_iterator vit;
        ...
        for ( auto rit = row.begin(); rit != row.end(); ++rit ) {
            ...
            if ( (int) entry_id < max_id || max_id == -1 ) {
                ...
                if ( pit != pairs.end() && !( pairs.key_comp()( entry_id, pit->first ) ) ) {
                    ...
                    pairs.insert( pit, std::map< EntryId, std::pair< double, int > >::value_type(
                                           entry_id, std::make_pair( value, 1 ) ) );
                    // expected.insert(eit,
                    //  map<EntryId, double>::value_type(entry_id, dvalue));

                    sums.insert( sit, std::map< EntryId, std::pair< double, double > >::value_type(
...
void Database::queryBhattacharyya( const BowVector& vec, QueryResults& ret, int max_results,
                                   int max_id ) const {
    BowVector::const_iterator vit;
...
void Database::queryDotProduct( const BowVector& vec, QueryResults& ret, int max_results,
                                int max_id ) const {
    BowVector::const_iterator vit;
...
std::ostream& operator<<( std::ostream& os, const Database& db ) {
    ...
    if ( db.usingDirectIndex() ) os << ", Direct index levels = " << db.getDirectIndexLevels();
