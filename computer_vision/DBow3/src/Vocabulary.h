...
class DBOW_API Vocabulary {
  ...
  public:
     ...
     * @param L depth levels
     */
    virtual void create( const std::vector< std::vector< cv::Mat > >& training_features, int k,
                         int L );
     ...
     */
    virtual void transform( const std::vector< cv::Mat >& features, BowVector& v, FeatureVector& fv,
                            int levelsup ) const;
    ...
    /**
     * Saves the vocabulary into a file. If filename extension contains .yml, opencv YALM format is
     * used. Otherwise, binary format is employed
     * @param filename
     */
  ...
  protected:
     ...
     * @param levelsup
     */
    virtual void transform( const cv::Mat& feature, WordId& id, WordValue& weight, NodeId* nid,
