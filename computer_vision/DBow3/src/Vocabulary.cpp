...
void Vocabulary::createScoringObject() {
    ...
    switch ( m_scoring ) {
        case L1_NORM: m_scoring_object = new L1Scoring; break;

        case L2_NORM: m_scoring_object = new L2Scoring; break;

        case CHI_SQUARE: m_scoring_object = new ChiSquareScoring; break;

        case KL: m_scoring_object = new KLScoring; break;

        case BHATTACHARYYA: m_scoring_object = new BhattacharyyaScoring; break;

        case DOT_PRODUCT: m_scoring_object = new DotProductScoring; break;
    }
}
...
void Vocabulary::create( const std::vector< cv::Mat >& training_features ) {
    ...
    for ( size_t i = 0; i < training_features.size(); i++ ) {
        vtf[i].resize( training_features[i].rows );   // how many corner points in this picture
        for ( int r = 0; r < training_features[i].rows; r++ )
            vtf[i][r] =
                training_features[i].rowRange( r, r + 1 );   // from row r to row (r+1) [r, R+1)
    }
    create( vtf );
    /*
     * figure 1 detect some points, every points has 256-dim vector,
     * the rows is just how many corner points, now we have some figures,
     * alot of vectors(corner points), clasify them.
     */
}
...
void Vocabulary::create( const std::vector< std::vector< cv::Mat > >& training_features ) {
    ...
    getFeatures( training_features, features );   // features: all the features!
...
void Vocabulary::create( const std::vector< std::vector< cv::Mat > >& training_features, int k,
                         int L ) {
    m_k = k;
    m_L = L;
...
void Vocabulary::getFeatures( const std::vector< std::vector< cv::Mat > >& training_features,
...
void Vocabulary::HKmeansStep( NodeId parent_id, const std::vector< cv::Mat >& descriptors,
                              int current_level ) {
        ...
        while ( goon ) {
            ...
            for ( auto fit = descriptors.begin(); fit != descriptors.end(); ++fit ) {   //, ++d)
                ...
                for ( unsigned int c = 1; c < clusters.size(); ++c ) {
                    ...
                    if ( dist < best_dist ) {
                        best_dist = dist;
                        icluster  = c;   // this time, which cluster
                    }
                }
...
void Vocabulary::initiateClustersKMpp( const std::vector< cv::Mat >& pfeatures,
     ...
     * attention, here, the nearest just means the latest center we choose.
    ...
    < < < < < < < HEAD
                  ...
                  =
                      // 1. Choose one center uniformly at random from among the data points.
                  > > > > > > > a23cacd75fbfab2c85d16b6eb19f221ffd2e44f6

        int ifeature =
            rand() % pfeatures.size();   // DUtils::Random::RandomInt(0, pfeatures.size()-1);
...
void Vocabulary::setNodeWeights( const std::vector< std::vector< cv::Mat > >& training_features ) {
    ...
    const unsigned int NDocs  = training_features.size();   // number of all images
    ...
    if ( m_weighting == TF || m_weighting == BINARY ) {
        ...
        for ( unsigned int i = 0; i < NWords; i++ ) {
            if ( Ni[i] > 0 ) {
                // Ni: number of image reach k-th leaf node
...
void Vocabulary::transform( const cv::Mat& features, BowVector& v ) const {
    ...
    if ( m_weighting == TF || m_weighting == TF_IDF ) {
        ...
        if ( !v.empty() && !must ) {
            // unnecessary when normalizing,  just do some naive normalizing
...
void Vocabulary::transform( const std::vector< cv::Mat >& features, BowVector& v, FeatureVector& fv,
                            int levelsup ) const {
    v.clear();
    fv.clear();
    ...
    if ( m_weighting == TF || m_weighting == TF_IDF ) {
        ...
        for ( auto fit = features.begin(); fit < features.end(); ++fit, ++i_feature ) {
            ...
            // nid is actually final_id( m_nodes ), the same as id, the leaf node

            if ( w > 0 ) {   // not stopped
                v.addWeight( id, w );
                fv.addFeature( nid, i_feature );
                /*
                 * which node in m_nodes, we may have some alike corners could appearing twice
                 * stores some features appears which time, attention: i_feature is alwayes plusing
                 * 1
                 */
            }
        }
        ...
        if ( !v.empty() && !must ) {
            // unnecessary when normalizing, just (/nd), really means unneceaasry
            const double nd = v.size();
            for ( BowVector::iterator vit = v.begin(); vit != v.end(); vit++ ) vit->second /= nd;
        }
...
void Vocabulary::transform( const cv::Mat& feature, WordId& word_id, WordValue& weight, NodeId* nid,
                            int levelsup ) const {
...
void Vocabulary::transform( const cv::Mat& feature, WordId& word_id, WordValue& weight ) const {
    ...
    if ( feature.type() == CV_8U ) {
        do {
            ...
            // std::cout<<bestidx<<" "<<final_id<<" d:"<<best_d<<" "<<m_nodes[final_id].descriptor<<
            // std::endl<<std::endl;
        } while ( !m_nodes[final_id].isLeaf() );
    } else {
        do {
...
void Vocabulary::save( const std::string& filename, bool binary_compressed ) const {
    if ( filename.find( ".yml" ) == std::string::npos ) {
        ...
        if ( !file_out )
            throw std::runtime_error( "Vocabulary::saveBinary Could not open file :" + filename +
                                      " for writing" );
...
void Vocabulary::load( const std::string& filename ) {
    ...
    if ( !ifile )
        throw std::runtime_error( "Vocabulary::load Could not open file :" + filename + " for "
                                                                                        "reading" );
    if ( !load( ifile ) ) {
...
void Vocabulary::load_fromtxt( const std::string& filename ) throw( std::runtime_error ) {
    ...
    if ( !ifile )
        throw std::runtime_error( "Vocabulary:: load_fromtxt  Could not open file for reading:" +
                                  filename );
    int n1, n2;
    {
        std::string str;
...
std::ostream& operator<<( std::ostream& os, const Vocabulary& voc ) {
    os << "Vocabulary: k = " << voc.getBranchingFactor() << ", L = " << voc.getDepthLevels()
       << ", Weighting = ";
