...
void Vocabulary::create(
    ...
    for (size_t i = 0; i < training_features.size(); i++) {
        ...
        for (int r = 0; r < training_features[i].rows; r++)
            vtf[i][r] = training_features[i].rowRange(r, r + 1);// from row r to row (r+1) [r, R+1)
    }
    create(vtf);
    /*
    ...
    if (m_weighting == TF || m_weighting == TF_IDF) {
        ...
        for (auto fit = features.begin(); fit < features.end(); ++fit, ++i_feature) {
            ...
            if (w > 0) { // not stopped
                 ...
                 * stores some features appears which time, attention: i_feature is alwayes plusing 1
                 */
            }
        }
