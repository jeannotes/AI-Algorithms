...
class DBOW_API Database {
  public:
     ...
     *   < 0 means all
     */
    void query( const std::vector< cv::Mat >& features, QueryResults& ret, int max_results = 1,
                int max_id = -1 ) const;
    /**
     * Queries the database with some features
     ...
     * @param db
     */
    DBOW_API friend std::ostream& operator<<( std::ostream& os, const Database& db );
  ...
  protected:
    ...
    void queryBhattacharyya( const BowVector& vec, QueryResults& ret, int max_results,
                             int max_id ) const;
