...
static void reset_table_decompress( qlz_state_decompress* state ) {
    ...
    for ( i = 0; i < QLZ_HASH_VALUES; i++ ) { state->hash_counter[i] = 0; }
#endif
}
...
static __inline ui32 fast_read( void const* src, ui32 bytes ) {
    ...
    switch ( bytes ) {
        case 4: return ( *p | *( p + 1 ) << 8 | *( p + 2 ) << 16 | *( p + 3 ) << 24 );
        case 3: return ( *p | *( p + 1 ) << 8 | *( p + 2 ) << 16 );
        case 2: return ( *p | *( p + 1 ) << 8 );
        case 1: return ( *p );
    }
    return 0;
#else
...
static __inline void fast_write( ui32 f, void* dst, size_t bytes ) {
    ...
    switch ( bytes ) {
        ...
        case 2:
            ...
            *( p + 1 ) = (unsigned char) ( f >> 8 );
            return;
        case 1: *p = (unsigned char) f; return;
    }
#else
    switch ( bytes ) {
        case 4: *( (ui32*) dst )          = f; return;
        case 3: *( (ui32*) dst )          = f; return;
        case 2: *( (ui16*) dst )          = (ui16) f; return;
        case 1: *( (unsigned char*) dst ) = (unsigned char) f; return;
    }
#endif
}
...
static size_t qlz_compress_core( const unsigned char* source, unsigned char* destination,
                                 size_t size, qlz_state_compress* state ) {
    ...
    if ( src <= last_matchstart ) fetch = fast_read( src, 3 );
...
#ifdef X86X64
            if ( ( cached & 0xffffff ) == 0 && o != OFFSET_BASE &&
                 ( src - o > MINOFFSET ||
                   ( src == o + 1 && lits >= 3 && src > source + 3 && same( src - 3, 6 ) ) ) ) {
                if ( cached != 0 ) {
#else
            if ( cached == 0 && o != OFFSET_BASE &&
                 ( src - o > MINOFFSET ||
                   ( src == o + 1 && lits >= 3 && src > source + 3 && same( src - 3, 6 ) ) ) ) {
                if ( *( o + 3 ) != *( src + 3 ) ) {
#endif
...
static size_t qlz_decompress_core( const unsigned char* source, unsigned char* destination,
    ...
    for ( ;; ) {
        ...
        if ( cword_val == 1 ) {
#ifdef QLZ_MEMORY_SAFE
            if ( src + CWORD_LEN - 1 > last_source_byte ) return 0;
#endif
...
#ifdef QLZ_MEMORY_SAFE
        if ( src + 4 - 1 > last_source_byte ) return 0;
#endif
...
#ifdef QLZ_MEMORY_SAFE
            if ( offset2 < history || offset2 > dst - MINOFFSET - 1 ) return 0;
...
#ifdef QLZ_MEMORY_SAFE
                    if ( src >= last_source_byte + 1 ) return 0;
#endif
                    *dst = *src;
                    dst++;
                    src++;
...
size_t qlz_compress( const void* source, char* destination, size_t size,
    ...
    if ( size == 0 || size > 0xffffffff - 400 ) return 0;
...
#endif
    {
        ...
        r = base + qlz_compress_core( (const unsigned char*) source,
...
size_t qlz_decompress( const char* source, void* destination, qlz_state_decompress* state ) {
...
#endif
    {
        if ( ( *source & 1 ) == 1 ) {
            ...
            dsiz = qlz_decompress_core( (const unsigned char*) source, (unsigned char*) destination,
                                        dsiz, state, (const unsigned char*) destination );
        } else {
        ...
        if ( ( *source & 1 ) == 1 ) {
            dsiz = qlz_decompress_core( (const unsigned char*) source, dst, dsiz, state,
                                        (const unsigned char*) state->stream_buffer );
        } else {
