...
void ellipse2param( double* p, double param[] ) {
    ...
    e = 2 * p[5];
    f = p[8];
    double thetarad, cost, sint, cos_squared, sin_squared, cos_sin, Ao, Au, Av, Auu, Avv, tuCentre,
        tvCentre, wCentre, uCentre, vCentre, Ru, Rv;
...
void mean( double* v, int sz, double* m, int dim ) {
    ...
    for ( i = 0; i < sz; i++ )
        for ( j = 0; j < dim; j++ ) sum[j] += v[i * dim + j];
...
void fit_equations( struct point* reg, int reg_size, image_double gradx, image_double grady,
    ...
    for ( i = 0; i < reg_size; i++ ) {
        ...
        gBufferDouble[idx]          = -K[0];
        gBufferDouble[idx + 6]      = -K[1];
        gBufferDouble[idx + 12]     = -K[2];
        gBufferDouble[idx + 1]      = -( K[3] + K[9] );
        gBufferDouble[idx + 6 + 1]  = -( K[4] + K[10] );
        gBufferDouble[idx + 12 + 1] = -( K[5] + K[11] );
        gBufferDouble[idx + 2]      = -( K[6] + K[18] );
        gBufferDouble[idx + 6 + 2]  = -( K[7] + K[19] );
        gBufferDouble[idx + 12 + 2] = -( K[8] + K[20] );
        gBufferDouble[idx + 3]      = -K[12];
        gBufferDouble[idx + 6 + 3]  = -K[13];
        gBufferDouble[idx + 12 + 3] = -K[14];
        gBufferDouble[idx + 4]      = -( K[15] + K[21] );
        gBufferDouble[idx + 6 + 4]  = -( K[16] + K[22] );
        gBufferDouble[idx + 12 + 4] = -( K[17] + K[23] );
        gBufferDouble[idx + 5]      = -K[24];
        gBufferDouble[idx + 6 + 5]  = -K[25];
        gBufferDouble[idx + 12 + 5] = -K[26];
...
#define SIZE6 6
    ...
    double C[9];
    C[0] = vgg[0] * vgg[0] * s[0] + vgg[0] * vgg[3] * s[3] + vgg[0] * vgg[3] * s[1] +
           vgg[3] * vgg[3] * s[4];
    C[1] = vgg[0] * vgg[1] * s[0] + vgg[1] * vgg[3] * s[3] + vgg[0] * vgg[4] * s[1] +
           vgg[3] * vgg[4] * s[4];
    C[2] = vgg[0] * vgg[2] * s[0] + vgg[2] * vgg[3] * s[3] + vgg[0] * vgg[5] * s[1] +
           vgg[3] * vgg[5] * s[4] + vgg[0] * s[2] + vgg[3] * s[5];

    C[3] = vgg[0] * vgg[1] * s[0] + vgg[0] * vgg[4] * s[3] + vgg[1] * vgg[3] * s[1] +
           vgg[3] * vgg[4] * s[4];
    C[4] = vgg[1] * vgg[1] * s[0] + vgg[1] * vgg[4] * s[3] + vgg[1] * vgg[4] * s[1] +
           vgg[4] * vgg[4] * s[4];
    C[5] = vgg[1] * vgg[2] * s[0] + vgg[2] * vgg[4] * s[3] + vgg[1] * vgg[5] * s[1] +
           vgg[4] * vgg[5] * s[4] + vgg[1] * s[2] + vgg[4] * s[5];

    C[6] = vgg[0] * vgg[2] * s[0] + vgg[0] * vgg[5] * s[3] + vgg[0] * s[6] +
           vgg[2] * vgg[3] * s[1] + vgg[3] * vgg[5] * s[4] + vgg[3] * s[7];
    C[7] = vgg[1] * vgg[2] * s[0] + vgg[1] * vgg[5] * s[3] + vgg[1] * s[6] +
           vgg[2] * vgg[4] * s[1] + vgg[4] * vgg[5] * s[4] + vgg[4] * s[7];
...
int int_ext( double xc, double yc, int px, int py, image_double angles ) {
    ...
    int dir = 0;
    a       = atan2( py - yc, px - xc );
    if ( angle_diff( a, angles->data[py * angles->xsize + px] ) < M_1_2_PI ) dir = 1;
    return dir;
}
/*---------------------------------------------------------------------------*/
 ...
 */
void px_seed( struct rect rec, struct point* reg, int reg_size, int reg_size0, image_char used,
              image_double angles, image_double grad, int* pext, int* ps, double prec ) {
    int xx, yy, i;
    int adr;
        ...
        for ( yy = pext[1] - 1; yy <= pext[1] + 1; yy++ ) {
            if ( xx >= 0 && xx < (int) used->xsize && yy >= 0 && yy < (int) used->ysize ) {
                ...
                if ( used->data[adr] != USED &&
                     angle_diff( angles->data[adr], rec.theta - M_1_2_PI ) < 2 * prec )
 ...
 */
void subcurve( struct point* reg, int* reg_size, double prec, double p, image_double angles,
               image_char used, image_double grad, image_double gradx, image_double grady,
               double logNT, double mlog10eps, double density_th, int* pext, int* ps, int* sgn,
               double* ang0, double* spir ) {
    int convex    = 1;
    int reg_size0 = 0;
    struct rect rec;
    double reg_angle;
    ...
    while ( *reg_size != reg_size0 && convex ) {
        ...
        if ( *reg_size - reg_size0 > 1 ) {
            ...
            if ( dist( rec.x1, rec.y1, rec.x2, rec.y2 ) < 1 ) {
                rec.x1 += ( rec.dx + rec.dy );
                rec.y1 += ( rec.dx + rec.dy );
                rec.x2 -= ( rec.dx + rec.dy );
                rec.y2 -= ( rec.dx + rec.dy );
            }

            if ( *sgn == 0 ) *sgn = sign( angle_diff_signed( rec.theta, *ang0 ) );
...
/*---------------------------------------------------------------------------*/
void curve_grow( struct point* reg, int* reg_size, struct rect rec, double prec, double p,
                 image_double angles, image_char used, image_double grad, image_double gradx,
                 image_double grady, double logNT, double mlog10eps, double density_th ) {
    int ps1[2], ps2[2];
    int pext1[2], pext2[2];
    int sgn     = 0;
    ...
    subcurve( reg, reg_size, prec, p, angles, used, grad, gradx, grady, logNT, mlog10eps,
              density_th, pext2, ps2, &sgn, &ang0, &spir );
    ...
    if ( spir < M_2__PI )
        subcurve( reg, reg_size, prec, p, angles, used, grad, gradx, grady, logNT, mlog10eps,
                  density_th, pext1, ps1, &sgn, &ang0, &spir );
}
/*---------------------------------------------------------------------------*/
...
void valid_curve( struct point* reg, int* reg_size, double prec, double p, image_double angles,
    ...
    curve_grow( reg, reg_size, rec, prec, p, angles, used, grad, gradx, grady, logNT[0], mlog10eps,
                density_th );
    if ( *reg_size > min_size[1] ) {
        fit_equations( reg, *reg_size, gradx, grady, vgg );
        /* perform first ellipse fitting, and then circle fitting because circle fitting modifies
         * the equations 'eq' */
        if ( *reg_size > min_size[2] ) { fitellipse( *reg_size, vgg, parame ); }
        ...
        if ( *reg_size > min_size[2] ) {
            ...
            if ( ell_ok )
                nfa[2] = valid_ellipse( reg, *reg_size, used, prec, p, angles, grad, gradx, grady,
                                        parame, logNT[2], dir, pext, rege, &regp_size[2],
                                        min_size[2], mlog10eps );
        }
    }
}
/*---------------------------------------------------------------------------*/
