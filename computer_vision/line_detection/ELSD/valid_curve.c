...
void clean_reg( struct point* reg, int reg_size, image_char used ) {
    int i = 0;
    for ( i = 0; i < reg_size; i++ ) used->data[reg[i].y * used->xsize + reg[i].x] = NOTUSED;
}
/*----------------------------------------------------------------------------*/
...
double valid_circle( struct point* reg, int reg_size, image_char used, double prec, double p,
    ...
    /* refine: search for connected aligned points on the given circle starting from the same seed
     * point */
    ...
    *regc_size                                    = 1;
    ...
    if ( *regc_size > min_size ) {
        ...
        for ( i = 0; i < *regc_size; i++ ) {
            ...
            d = sqrt( ( reg[i].x - param[0] ) * ( reg[i].x - param[0] ) +
                      ( reg[i].y - param[1] ) * ( reg[i].y - param[1] ) ) -
                param[2];
            if ( d < d_min ) d_min = d;
            if ( d > d_max ) d_max = d;
            ...
            for ( xx = regc[i].x - 1; xx <= regc[i].x + 1; xx++ )
                for ( yy = regc[i].y - 1; yy <= regc[i].y + 1; yy++ )
                    if ( xx >= 0 && yy >= 0 && xx < (int) used->xsize && yy < (int) used->ysize &&
                        ...
                        d = sqrt( ( xx - param[0] ) * ( xx - param[0] ) +
                                  ( yy - param[1] ) * ( yy - param[1] ) ) -
                            param[2];
                        theta0 = atan2( (double) yy - param[1], (double) xx - param[0] );
                        if ( theta0 < 0 )
                            theta = theta0 + M_2__PI;
                        else
                            theta = theta0;
...
double valid_ellipse( struct point* reg, int reg_size, image_char used, double prec, double p,
    ...
    /* refine: search for connected aligned points on the given ellipse starting from the same seed
     * point */
    *rege_size                                    = 1;
    used->data[reg[0].y * used->xsize + reg[0].x] = USED;
    ...
    if ( *rege_size > min_size ) {
        ...
        if ( ell_ok ) {
            ...
            for ( i = 0; i < *rege_size; i++ ) {
                ...
                if ( d > d_max ) d_max = d;
                gBufferDouble[i] =
                    atan2( (double) reg[i].y - param[1], (double) reg[i].x - param[0] );
                if ( gBufferDouble[i] < 0 ) gBufferDouble[i] += M_2__PI;
                used->data[reg[i].y * grad->xsize + reg[i].x] = NOTUSED;
            }
            ...
            /* scan the elliptical ring and count the number of points and the number of aligned
             * points */
            *rege_size                                    = 1;
                ...
                for ( xx = rege[i].x - 1; xx <= rege[i].x + 1; xx++ )
                    for ( yy = rege[i].y - 1; yy <= rege[i].y + 1; yy++ )
                        if ( xx >= 0 && yy >= 0 && xx < (int) used->xsize &&
                             yy < (int) used->ysize && used->data[xx + yy * used->xsize] != USED &&
                            ...
                            if ( theta0 < 0 )
                                ...
                                theta = theta0;
                            if ( d >= d_min && d <= d_max &&
                                 isInAng( theta, ang_start, ang_end ) ) {
                                theta = angle( (double) xx, (double) yy, foci );
                                if ( dir == 0 ) {
                                    if ( theta > 0 )
                                        theta = -( M_PI - theta );
