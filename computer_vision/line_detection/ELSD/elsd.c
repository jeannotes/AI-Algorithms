...
static image_double read_pgm_image_double( char* name ) {
    ...
    for ( y = 0; y < ysize; y++ )
        for ( x                        = 0; x < xsize; x++ )
            image->data[x + y * xsize] = bin ? (double) getc( f ) : (double) get_num( f );
...
void free_image_int( image_int i ) {
    if ( i == NULL || i->data == NULL ) error( "free_image_int: invalid input image." );
    free( (void*) i->data );
    free( (void*) i );
}
/*----------------------------------------------------------------------------*/
...
void free_gauss_filter( gauss_filter in ) {
    if ( in == NULL || in->values == NULL ) error( "free_gauss_filter: invalid filter input." );
    free( (void*) in->values );
    free( (void*) in );
}
/*----------------------------------------------------------------------------*/
 ...
 */
image_double ll_angle( image_double in, double threshold, struct coorlist** list_p, void** mem_p,
                       image_double* gradx, image_double* grady, image_double* grad,
                       unsigned int n_bins, double max_grad ) {
    /* check parameters */
    if ( in == NULL || in->data == NULL || in->xsize <= 0 || in->ysize <= 0 )
...
/**
 */
void EllipseDetection( image_double image, double rho, double prec, double p, double eps,
                       int smooth, int* ell_count, int* circ_count, int* line_count, char* fstr ) {
    ...
    logNT[2] = 4.0 * ( log10( (double) xsize ) + log10( (double) ysize ) ) + log10( 9.0 ) +
               log10( 3.0 ); /* N^8 */
    /* number of tests for circular arcs */
    logNT[1] = 3.0 * ( log10( (double) xsize ) + log10( (double) ysize ) ) + log10( 9.0 ) +
               log10( 3.0 ); /* N^6 */
    /* number of tests for line-segments */
    logNT[0] = 5.0 * ( log10( (double) xsize ) + log10( (double) ysize ) ) / 2.0 + log10( 11 ) +
               log10( 3.0 ); /* N^5 */
    ...
    for ( ; list_p; list_p = list_p->next ) {
        ...
        if ( used->data[list_p->y * used->xsize + list_p->x] == NOTUSED &&
            ...
            reg_size                 = 1;
            regp_size[0] = regp_size[1] = regp_size[2] = 0;
            region_grow( list_p->x, list_p->y, angles, reg, &reg_size, &reg_angle, used, prec );

            /*-------- FIT A LINEAR SEGMENT AND VERIFY IF VALID ------------------- */
            valid_line( reg, &reg_size, reg_angle, prec, p, &rec, lin, grad, gradx, grady, used,
                        angles, density_th, logNT[0], mlog10eps, &nfa[0] );
            ...
            if ( reg_size > 2 ) {
                ...
                valid_curve( reg, &reg_size, prec, p, angles, used, grad, gradx, grady, paramc,
                             parame, rec, logNT, mlog10eps, density_th, min_size, nfa, pext, regc,
                             rege, regp_size );

                /* ------ DECIDE IF LINEAR SEGMENT OR CIRCLE OR ELLIPSE BY COMPARING THEIR NFAs
                 * -------*/
                if ( nfa[2] > mlog10eps && nfa[2] > nfa[0] && nfa[2] > nfa[1] &&
                     regp_size[2] > min_size[2] ) { /* ellipse */
                    ( *ell_count )++;
                    /*if (smooth)
                ...
                } else if ( nfa[1] > mlog10eps && nfa[1] > nfa[0] && nfa[1] > nfa[2] &&
                            regp_size[1] > min_size[1] ) { /* circle */
                    ( *circ_count )++;
                    /*if (smooth)
                      fprintf(fe,"%f %f %f %f %f
                ...
                } else if ( nfa[0] > mlog10eps && regp_size[0] > min_size[0] && nfa[0] > nfa[1] &&
                            nfa[0] > nfa[2] ) { /* line */
                    ( *line_count )++;
...
int check_ellipse( double* param ) {
    ...
    if ( param[4] > M_1_2_PI ) param[4]  = param[4] - M_PI;
    if ( param[4] > M_PI ) param[4]      = param[4] - M_PI;
    if ( param[4] < -M_1_2_PI ) param[4] = param[4] + M_PI;
    return 1;
}
...
int main( int argc, char** argv ) {
    ...
    EllipseDetection( image, rho, prec, p, eps, smooth, &ell_count, &circ_count, &line_count,
                      argv[1] );
    printf( "%s\n", argv[1] );
    printf( "%d elliptical arcs, %d circular arcs, %d line segments\n", ell_count, circ_count,
            line_count );
    return 0;
}
/*----------------------------------------------------------------------------*/
