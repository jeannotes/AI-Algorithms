...
double rect_nfa( struct rect* rec, image_double angles, double logNT ) {
    ...
    for ( i = ri_ini( rec ); !ri_end( i ); ri_inc( i ) )
        if ( i->x >= 0 && i->y >= 0 && i->x < (int) angles->xsize && i->y < (int) angles->ysize ) {
            ++pts;
            if ( isaligned( i->x, i->y, angles, theta, rec->prec ) ) { ++alg; }
        }
    ri_del( i );
 ...
 */
int reduce_region_radius( struct point* reg, int reg_size0, int* reg_size, image_double modgrad,
                          image_double gradx, image_double grady, double reg_angle, double prec,
                          double p, struct rect* rec, image_char used, image_double angles,
...
int refine( struct point* reg, int reg_size0, int* reg_size, image_double modgrad,
    ...
    *reg_size = reg_size0 + 1;
    ...
    if ( density < density_th )
        return reduce_region_radius( reg, reg_size0, reg_size, modgrad, gradx, grady, reg_angle,
                                     prec, p, rec, used, angles, density_th, logNT, eps );
...
void valid_line( struct point* reg, int* reg_size, double reg_angle, double prec, double p,
    ...
    *nfaline = mlog10eps;
    ...
    if ( *reg_size > 2 ) {
        ...
        if ( !refine( reg, 0, reg_size, grad, gradx, grady, prec, p, rec, used, angles, density_th,
                      logNT, mlog10eps ) ) {
            l[4] = 0;
        } else {
            *nfaline = rect_improve( rec, angles, logNT, mlog10eps );
            if ( ( *nfaline ) <= mlog10eps )
                l[4] = 0;
            else {
                l[0] = rec->x1 + 0.5;
                l[1] = rec->y1 + 0.5;
