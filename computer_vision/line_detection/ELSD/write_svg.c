...
void fclose_svg( FILE* svg ) {
    /* close SVG file */
    fprintf( svg, "</svg>\n" );
    if ( fclose( svg ) == EOF ) error( "Error: unable to close file while writing SVG file." );
}
/*---------------------------------------------------------------------------*/
...
void write_svg_line( FILE* svg, double* lin, int smooth ) {
    ...
    fprintf( svg, "<line x1=\"%f\" y1=\"%f\" x2=\"%f\" y2=\"%f\" ", lin[0], lin[1], lin[2],
             lin[3] );
...
void write_svg_circle( FILE* fe, FILE* svg, double* param, int* pext, int smooth ) {
    ...
    if ( angle_diff( ang_start, ang_end ) < M_2__PI * SQRT2 / C &&
        ...
        fprintf( svg, "<ellipse cx=\"%f\" cy=\"%f\" rx=\"%f\" ry=\"%f\" stroke=\"blue\" "
        ...
        fprintf( svg, "<path d=\"M %f,%f A%f,%f %f %d,%d %f,%f\"", x1, y1, param[2], param[2], 0.0,
                 fa, fs, x2, y2 );
...
void write_svg_ellipse( FILE* fe, FILE* svg, double* param, int* pext, int smooth ) {
    ...
    if ( angle_diff( ang_start, ang_end ) < M_2__PI * SQRT2 / C &&
         angle_diff_signed( ang_start, ang_end ) > 0 ) {
        fprintf( svg, "<ellipse transform=\"translate(%f %f) rotate(%f)\" rx=\"%f\" ry=\"%f\" "
                      "stroke=\"red\" fill=\"none\" stroke-width= \"%d\" /> \n",
        ...
        fprintf( svg, "<path d=\"M %f,%f A%f,%f %f %d,%d %f,%f\"", x1, y1, param[2], param[3],
                 param[4] * 180 / M_PI, fa, fs, x2, y2 );
