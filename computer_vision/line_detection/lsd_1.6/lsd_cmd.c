...
static void free_arguments( struct arguments* arg ) {
    if ( arg == NULL || arg->args == NULL ) error( "Error: NULL pointer at 'free_arguments'." );
    free( (void*) arg->args );
    free( (void*) arg );
}
...
static char* get_next_token( char* p, char div, char* value ) {
    ...
    /* check input */
    if ( p == NULL || value == NULL ) error( "Error: invalid input to 'get_next_token'." );

    if ( *p == '\0' ) error( "Error: argument token expected in 'use description'." );
...
static void process_new_argument( char* id, char* value, struct arguments* arg ) {
    ...
    if ( strcmp( id, "opt" ) == 0 ) {
        ...
        if ( strlen( token ) != 1 ) error( "Error: invalid option letter in 'use description'." );
        ...
        for ( i = 0; i < arg->arg_num; i++ )
            if ( !( arg->args[i].required ) && arg->args[i].id == arg->args[arg->arg_num].id )
        ...
        if ( strlen( token ) > 0 ) error( "Error: unused option letter in 'use description'." );
    ...
    if ( strcmp( token, "int" ) == 0 )
        arg->args[arg->arg_num].type = 'i';
    else if ( strcmp( token, "double" ) == 0 )
        arg->args[arg->arg_num].type = 'd';
    else if ( strcmp( token, "str" ) == 0 )
        arg->args[arg->arg_num].type = 's';
    else if ( strcmp( token, "bool" ) == 0 )
        arg->args[arg->arg_num].type = 'b';
    else
        error( "Error: unknown argument type in 'use description'." );
    ...
    if ( strlen( token ) > 0 ) {
        ...
        if ( arg->args[arg->arg_num].type == 'i' ) arg->args[arg->arg_num].i_value = atoi( token );
        if ( arg->args[arg->arg_num].type == 'd' ) arg->args[arg->arg_num].f_value = atof( token );
    } else {
    ...
    if ( *p != '\0' ) error( "Error: too many tokens in one argument in 'use description'." );
...
static void process_argument_description( char* desc, struct arguments* arg ) {
    ...
    arg->arg_num       = 0;
    arg->arg_allocated = 2;
    arg->args = (struct argument*) malloc( arg->arg_allocated * sizeof( struct argument ) );
    if ( arg->args == NULL ) error( "Error: not enough memory." );
    ...
    if ( arg->name[0] == '\0' ) error( "Error: program name is required in 'use description'." );
    if ( arg->author[0] == '\0' ) error( "Error: author name is required in 'use description'." );
    if ( arg->version[0] == '\0' ) error( "Error: version is required in 'use description'." );
    if ( arg->desc[0] == '\0' )
        error( "Error: program description is required in 'use description'." );
    if ( arg->year[0] == '\0' ) error( "Error: year is required in 'use description'." );
}
...
static void print_version( struct arguments* arg, FILE* f ) {
    if ( arg == NULL || f == NULL ) error( "Error: invalid input to 'print_version'." );
...
static void use( struct arguments* arg ) {
    ...
    for ( i = 0; i < arg->arg_num; i++ )
        if ( arg->args[i].required ) fprintf( stderr, " %s", arg->args[i].name );
    ...
    for ( i = 0; i < arg->arg_num; i++ )
        if ( !( arg->args[i].required ) ) {
            ...
            if ( arg->args[i].type == 'd' ) {
                ...
                fprintf( stderr, ", range [" );
                if ( arg->args[i].min_set )
                    fprintf( stderr, "%g,", arg->args[i].min );
                else
                    fprintf( stderr, "-inf," );
                if ( arg->args[i].max_set )
                    fprintf( stderr, "%g]", arg->args[i].max );
                else
                    fprintf( stderr, "inf]" );
                if ( arg->args[i].def_value )
    ...
    for ( i = 0; i < arg->arg_num; i++ )
        if ( arg->args[i].required ) {
            ...
            if ( arg->args[i].type == 'd' ) {
                ...
                fprintf( stderr, ", range [" );
                if ( arg->args[i].min_set )
                    fprintf( stderr, "%f,", arg->args[i].min );
                else
                    fprintf( stderr, "-inf," );
                if ( arg->args[i].max_set )
                    fprintf( stderr, "%f]", arg->args[i].max );
                else
                    fprintf( stderr, "inf]" );
                fprintf( stderr, "\n" );
            }
...
static void evaluate_arguments( int argc, char** argv, struct arguments* arg ) {
    ...
    if ( argv == NULL || arg == NULL ) error( "Error: invalid input to 'evaluate_arguments'." );
            ...
            if ( !( arg->args[i].required ) && arg->args[i].id == argv[n][1] ) {
                ...
                if ( arg->args[i].type != 'b' ) {
                    ...
                    if ( arg->args[i].type == 'd' ) {
                        ...
                        if ( arg->args[i].min_set && arg->args[i].f_value < arg->args[i].min ) {
                            fprintf( stderr, "Error: in '%s': ", argv[n - 1] );
                            error( "value out of range." );
                        }
                        if ( arg->args[i].max_set && arg->args[i].f_value > arg->args[i].max ) {
                            fprintf( stderr, "Error: in '%s': ", argv[n - 1] );
                            error( "value out of range." );
                        }
                    }
    ...
    for ( i = 0; n < argc && i < arg->arg_num; i++ )
        if ( arg->args[i].required ) {
            ...
            if ( arg->args[i].type == 'i' ) {
                arg->args[i].i_value = atoi( argv[n] );
                if ( arg->args[i].min_set && arg->args[i].i_value < (int) arg->args[i].min ) {
                    fprintf( stderr, "Error: in '%s': ", arg->args[i].name );
                    error( "value out of range." );
                }
                if ( arg->args[i].max_set && arg->args[i].i_value > (int) arg->args[i].max ) {
                    fprintf( stderr, "Error: in '%s': ", arg->args[i].name );
                    error( "value out of range." );
                }
            }
...
static struct arguments* process_arguments( char* desc, int argc, char** argv ) {
    ...
    /* check input */
    if ( desc == NULL || argv == NULL ) error( "Error: invalid input to 'process_arguments'." );
...
static int is_assigned( struct arguments* arg, char* name ) {
    int i;
    if ( arg == NULL || name == NULL ) error( "Error: invalid input to 'is_assigned'." );
    for ( i = 0; i < arg->arg_num; i++ )
...
static char* get_str( struct arguments* arg, char* name ) {
    ...
    if ( arg == NULL || name == NULL ) error( "Error: invalid input to 'get_str'." );
    ...
    for ( i = 0; i < arg->arg_num; i++ )
        if ( strcmp( name, arg->args[i].name ) == 0 ) {
            if ( arg->args[i].type == 's' ) {
                ...
                error( "Error: get_str: the parameter is not a double." );
        }
...
static int get_int( struct arguments* arg, char* name ) {
    ...
    if ( arg == NULL || name == NULL ) error( "Error: invalid input to 'get_int'." );

    for ( i = 0; i < arg->arg_num; i++ )
        if ( strcmp( name, arg->args[i].name ) == 0 ) {
            if ( !( arg->args[i].assigned ) ) error( "Error: get_int: parameter not assigned." );
            if ( arg->args[i].type == 'i' )
                return arg->args[i].i_value;
            else
                error( "Error: get_int: the parameter is not an integer." );
        }
...
static double get_double( struct arguments* arg, char* name ) {
    ...
    if ( arg == NULL || name == NULL ) error( "Error: invalid input to 'get_double'." );

    for ( i = 0; i < arg->arg_num; i++ )
        if ( strcmp( name, arg->args[i].name ) == 0 ) {
            if ( !( arg->args[i].assigned ) ) error( "Error: get_double: parameter not assigned." );
            if ( arg->args[i].type == 'd' )
                return arg->args[i].f_value;
            else
                error( "Error: get_double: the parameter is not a double." );
        }
...
static double* read_pgm_image_double( int* X, int* Y, char* name ) {
    ...
    for ( y = 0; y < ysize; y++ )
        for ( x                  = 0; x < xsize; x++ )
            image[x + y * xsize] = bin ? (double) getc( f ) : (double) get_num( f );
 ...
 */
static void write_eps( double* segs, int n, int dim, char* filename, int xsize, int ysize,
                       double width ) {
    FILE* eps;
    int i;
    ...
    if ( xsize <= 0 || ysize <= 0 ) error( "Error: invalid image size in write_eps." );
    ...
    for ( i = 0; i < n; i++ ) {
        fprintf( eps, "newpath %f %f moveto %f %f lineto %f setlinewidth stroke\n",
                 segs[i * dim + 0], (double) ysize - segs[i * dim + 1], segs[i * dim + 2],
                 (double) ysize - segs[i * dim + 3], width <= 0.0 ? segs[i * dim + 4] : width );
    }
...
/** Write line segments into a SVG file.
    If the name is "-" the file is written to standard output.
*/
static void write_svg( double* segs, int n, int dim, char* filename, int xsize, int ysize,
                       double width ) {
    FILE* svg;
    int i;
    ...
    if ( xsize <= 0 || ysize <= 0 ) error( "Error: invalid image size in write_svg." );
    ...
    /* write line segments */
    for ( i = 0; i < n; i++ ) {
        fprintf( svg, "<line x1=\"%f\" y1=\"%f\" x2=\"%f\" y2=\"%f\" ", segs[i * dim + 0],
                 segs[i * dim + 1], segs[i * dim + 2], segs[i * dim + 3] );
...
int main( int argc, char** argv ) {
    ...
    /* execute LSD */
    segs = LineSegmentDetection( &n, image, X, Y, get_double( arg, "scale" ),
                                 get_double( arg, "sigma_coef" ), get_double( arg, "quant" ),
                                 get_double( arg, "ang_th" ), get_double( arg, "log_eps" ),
                                 get_double( arg, "density_th" ), get_int( arg, "n_bins" ),
                                 is_assigned( arg, "reg" ) ? &region : NULL, &regX, &regY );

    /* output */
    if ( strcmp( get_str( arg, "out" ), "-" ) == 0 )
        output = stdout;
    else
        output = fopen( get_str( arg, "out" ), "w" );
    if ( output == NULL ) error( "Error: unable to open ASCII output file." );
    for ( i = 0; i < n; i++ ) {
        for ( j = 0; j < dim; j++ ) fprintf( output, "%f ", segs[i * dim + j] );
        fprintf( output, "\n" );
    }
