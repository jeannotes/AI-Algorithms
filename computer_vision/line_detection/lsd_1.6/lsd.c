 ...
 */
static void add_7tuple( ntuple_list out, double v1, double v2, double v3, double v4, double v5,
                        double v6, double v7 ) {
    /* check parameters */
...
static image_char new_image_char_ini( unsigned int xsize, unsigned int ysize,
    ...
    /* check parameters */
    if ( image == NULL || image->data == NULL ) error( "new_image_char_ini: invalid image." );
...
/** Scale the input image 'in' by a factor 'scale' by Gaussian sub-sampling.
    ...
    G(x,y)  =  --------- e
                       2
               2pisigma   (some sthing like this)
...
static image_double gaussian_sampler( image_double in, double scale, double sigma_scale ) {
    ...
    /*
       ...
       Then,
         x = sigma * sqrt( 2 * prec * ln(10) ). //don't understand
     */
    prec   = 3.0;
    h      = (unsigned int) ceil( sigma * sqrt( 2.0 * prec * log( 30.0 ) ) );   // change 10 to 30
    n      = 1 + 2 * h; /* kernel size */   // change here ,origin 2
 ...
 */
static image_double ll_angle( image_double in, double threshold, struct coorlist** list_p,
                              void** mem_p, image_double* modgrad, unsigned int n_bins ) {
    image_double g;
    ...
    *modgrad = new_image_double( in->xsize, in->ysize );
    ...
    list = (struct coorlist*) calloc( ( size_t )( n * p ),
                                      sizeof( struct coorlist ) );   // n*p numbers of something
    *mem_p    = (void*) list;
    ...
    for ( x = 0; x < p - 1; x++ )
        for ( y = 0; y < n - 1; y++ ) {
            ...
            if ( range_l_e[i] == NULL )
                ...
                /*
                * first, range_l_s and range_l_e point to some frid, after that,
                * range_l_e's next is pointed to (list + list_count), so
                * we can see that, finally "range_l_e[i] = list + list_count++;"
                * attention, here list_count means the index of a paticular cell ,
                * first (0,0). (0,1), (0,2), in every range_l_s or range_l_e, it
                * stores the position of a cell(from (0,0), to (0,1) .... (0, cols - 1))
                * and next is (1, 0), (1, 1) ......
                */
            }
            range_l_e[i]->x    = (int) x;
    ...
    @f[
      \log\Gamma(x) = \log\left( \sum_{n=0}^{N} q_n x^n \right)
                      + (x+0.5) \log(x+5.5) - (x+5.5) - \sum_{n=0}^{N} \log(x+n)
                       ...
                       /  __ N		   n \
    logGamma(x)  =	log| \		   q  x  | +  (x + 0.5) log(x + 5.5)  -  (x +
                       \ /__ n = 0	n	 /
...
static double nfa( int n, int k, double p, double logNT ) {
    ...
    for ( i = k + 1; i <= n; i++ ) {
        ...
        if ( bin_term < 1.0 ) {
            ...
            err =
                term *
                ( ( 1.0 - pow( mult_term, (double) ( n - i + 1 ) ) ) / ( 1.0 - mult_term ) - 1.0 );
 ...
 */
static double get_theta( struct point* reg, int reg_size, double x, double y, image_double modgrad,
 ...
 */
static void region2rect( struct point* reg, int reg_size, image_double modgrad, double reg_angle,
 ...
 */
static void region_grow( int x, int y, image_double angles, struct point* reg, int* reg_size,
    ...
    *reg_angle                      = angles->data[x + y * angles->xsize]; /* region's angle */
    ...
    for ( i = 0; i < *reg_size; i++ )
        for ( xx = reg[i].x - diff; xx <= reg[i].x + diff; xx++ )
            for ( yy = reg[i].y - diff; yy <= reg[i].y + diff; yy++ )
 ...
 */
static int reduce_region_radius( struct point* reg, int* reg_size, image_double modgrad,
 ...
 */
static int refine( struct point* reg, int* reg_size, image_double modgrad, double reg_angle,
                   double prec, double p, struct rect* rec, image_char used, image_double angles,
                   double density_th ) {
    ...
    if ( density < density_th )
        return reduce_region_radius( reg, reg_size, modgrad, reg_angle, prec, p, rec, used, angles,
                                     density_th );
...
/** LSD full interface.
 */
double* LineSegmentDetection( int* n_out, double* img, int X, int Y, double scale,
                              double sigma_scale, double quant, double ang_th, double log_eps,
                              double density_th, int n_bins, int** reg_img, int* reg_x,
                              int* reg_y ) {
    image_double image;
    ...
    /*
     * modgrad stores gradient magnitude
     * list_p stores starting gradient magnitude
     * mem_p: copied list of *coorlist
     * angles: stores angle
     */
    xsize = angles->xsize;
    ysize = angles->ysize;
    ...
    */
    ...
    for ( ; list_p != NULL; list_p = list_p->next )
               ...
               because we are only interested in the exact NOTDEF value */
        {
             ...
             * reg: stores point in the tolerate region
             */
             ...
             */
            if ( !refine( reg, &reg_size, modgrad, reg_angle, prec, p, &rec, used, angles,
                          density_th ) )
                continue;
 ...
 */
double* lsd_scale_region( int* n_out, double* img, int X, int Y, double scale, int** reg_img,
                          int* reg_x, int* reg_y ) {
    /* LSD parameters */
    ...
    return LineSegmentDetection( n_out, img, X, Y, scale, sigma_scale, quant, ang_th, log_eps,
                                 density_th, n_bins, reg_img, reg_x, reg_y );
}
