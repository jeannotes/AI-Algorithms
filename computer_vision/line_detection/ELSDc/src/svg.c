...
void write_svg_poly( FILE* fsvg, Polygon* poly ) {
    ...
    for ( i = 0; i < poly->dim; i += 2 ) {
        fprintf( fsvg, "<line x1=\"%f\" y1=\"%f\" x2=\"%f\" y2=\"%f\" ", poly->pts[i].x,
                 poly->pts[i].y, poly->pts[i + 1].x, poly->pts[i + 1].y );
...
void write_svg_circ_arc( FILE* fsvg, Ring* cring ) {
    ...
    C = M_2__PI * cring->ax;
    if ( ( cring->full ) || ( ( angle_diff( ang_start, ang_end ) < M_2__PI * SQRT2 / C ) &&
        ...
        if ( ( double_equal( x1, x2 ) && double_equal( y1, y2 ) ) ||
        ...
        fprintf( fsvg, "<path d=\"M %f,%f A%f,%f %f %d,%d %f,%f\"", x1, y1, cring->ax, cring->ax,
                 0.0, fa, fs, x2, y2 );
        /* define style: color and width */
...
void write_svg_ell_arc( FILE* fsvg, Ring* ering ) {
    ...
    if ( ering->full ) { /* if complete ellipse, write full ellipse, not an arc */
        ...
        /* write starting and ending points, axes, orientation, fa, fs */
        fprintf( fsvg, "<path d=\"M %f,%f A%f,%f %f %d,%d %f,%f\"", x1, y1, ering->ax, ering->bx,
                 ering->theta * 180 / M_PI, fa, fs, x2, y2 );
        /* define style: color and width */
        fprintf( fsvg, " fill=\"none\" stroke =\"red\" stroke-width=\"%d\" />\n", 1 );
    }
}
