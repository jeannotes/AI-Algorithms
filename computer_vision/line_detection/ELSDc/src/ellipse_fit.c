...
/** Compute equations for circle/ellipse fitting. Needs buffer memory zone to do
    the computations.
 */
static void fit_equations( PImageDouble gradx, PImageDouble grady, double* vgg, Point* reg,
    ...
    if ( reg_size * 24 > *size_buff_max ) {
        *buff = (double*) realloc( *buff, sizeof( double ) * 2 * reg_size * 24 );
...
static void fit_circle( double* buff, double* vgg, int reg_size, double* param ) {
    ...
    for ( i = 0; i < 4; i++ )
        for ( j = 0; j < 4; j++ )
            for ( k = 0; k < 4 * reg_size; k++ ) A[i * 4 + j] += buff[k * 6 + i] * buff[k * 6 + j];
 ...
 */
static void fit_ellipse( double* buff, double* vgg, int reg_size, double* param ) {
    int i, j, k;
    double A[36];
    /* check parameters */
    ...
    for ( i = 0; i < 6; i++ )
        for ( j = 0; j < 6; j++ )
            for ( k = 0; k < 4 * reg_size; k++ ) A[i * 6 + j] += buff[k * 6 + i] * buff[k * 6 + j];
