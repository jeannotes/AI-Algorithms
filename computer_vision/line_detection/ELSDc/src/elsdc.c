...
static double NFAc( int n, double k, double logNT ) {
    ...
    k = max( k, 0.01 );
    logNFAC =
        logNT + n * log10( k ) - LOG10_1_2_M_2__PI - ( n + 0.5 ) * log10( n ) + n * LOG10_EXP1;
 ...
 */
static void rect_count_error( PImageDouble angles, PImageInt used, Rectangle* rec, int* pts,
                              double* err, Point* buff, int* size_buff ) {
    RectIter* i;
    int adr;
    double theta;
    ...
    if ( size_buff == NULL ) error( "rect_count_error: size_buff must be non null." );
...
/** Given a list of points, estimate segment. One segment is a particular case
    of a polygon.
 */
static void get_seg( PImageDouble angles, PImageDouble gradmag, PImageInt used, PolyRect* poly,
                     Point* reg, int reg_size, double prec ) {
...
/** Compute additive normalised per-pixel error inside a polygon, and stack
    ...
    meaningful primitive.
 */
static void poly_count_error( PImageDouble angles, PImageInt used, PolyRect* poly, double polywmin,
                              double polywmax, int* pts, double* err, Point* buff,
                              int* size_buff ) {
    int pts_rect;
    double err_rect;
    int i;
    ...
    if ( size_buff == NULL ) error( "poly_count_error: size_buff must be non null." );
    ...
    *size_buff = 0;
    ...
    for ( i = 0; i < poly->dim; i++ ) {
        ...
        rect_count_error( angles, used, &( poly->rectlist[i] ), &pts_rect, &err_rect, buff,
                          size_buff );
        ( *pts ) += pts_rect;
        ( *err ) += err_rect;
    }
 ...
 */
static double poly_nfa( PImageDouble angles, PImageInt used, PolyRect* poly, double polywmin,
                        double polywmax, Point* buff, int* size_buff, double logNT_poly ) {
    int pts;
    double err;
    ...
    if ( size_buff == NULL ) error( "poly_nfa: size_buff must be non null." );

    /* count additive error */
    poly_count_error( angles, used, poly, polywmin, polywmax, &pts, &err, buff, size_buff );
    /* return computed NFA */
 ...
 */
static double poly_improve( PImageDouble angles, PImageInt used, PolyRect* poly, Point** local_buff,
                            int* size_local_buff, Point** tmp_buff, double logNT_poly ) {
    double delta = 0.5;
    ...
    if ( size_local_buff == NULL ) error( "poly_improve: 'size_local_buff' must be non null." );
    ...
    *size_local_buff = 0;
    ...
    log_nfa = poly_nfa( angles, used, poly, wmin, wmax, *local_buff, size_local_buff, logNT_poly );
    ...
    while ( wmax - delta > 0.0 ) {
        ...
        log_nfa_new =
            poly_nfa( angles, used, poly, wmin, wmax, *tmp_buff, &size_tmp_buff, logNT_poly );
    ...
    while ( wmin + delta < 0.0 ) {
        ...
        log_nfa_new =
            poly_nfa( angles, used, poly, wmin, poly->wmax, *tmp_buff, &size_tmp_buff, logNT_poly );
        if ( log_nfa_new > log_nfa ) {
            log_nfa = log_nfa_new;
            /* switch addresses of the pointers to the local and tmp buffer */
            /* swap_ptr_pts( local_buff, tmp_buff );
 ...
 */
static void circ_count_error( PImageDouble angles, PImageInt used, Ring* cring, int x, int y,
                              int grad_dir, Point* buff, int* size_buff, int label, int* pts,
                              double* err ) {
    int xx, yy, i, adr;
    double theta;
    ...
    if ( size_buff == NULL ) error( "circ_count_error: 'size_local_buff' must be non null." );
                ...
                if ( in_image( xx, yy, used->xsize, used->ysize ) ) {
                    ...
                    if ( ( used->data[adr] != USED ) && ( used->data[adr] != label ) ) {
                        ...
                        if ( is_in_circ_ring( cring, xx, yy ) ) {
                            ...
                            theta = atan2( (double) yy - cring->cy, (double) xx - cring->cx );
                            if ( grad_dir == 0 ) {
                                if ( theta > 0 )
                                    theta = -( M_PI - theta );
                                else
...
/** Iteration to improve NFA: count additive error and compute NFA.
 */
static double circ_ring_nfa( PImageDouble angles, PImageInt used, Ring* cring, int x, int y,
                             int grad_dir, Point* buff, int* size_buff, int label,
                             double logNT_ell ) {
    int pts;
    double err;
    ...
    if ( size_buff == NULL ) error( "circ_ring_nfa: 'size_local_buff' must be non null." );
    if ( label <= 1 ) error( "circ_ring_nfa: forbidden label value." );

    /* count additive error */
    circ_count_error( angles, used, cring, x, y, grad_dir, buff, size_buff, label, &pts, &err );
 ...
 */
static double circ_ring_improve( PImageDouble angles, PImageInt used, Ring* cring, int x, int y,
                                 int grad_dir, Point** local_buff, int* size_local_buff,
    ...
    ( *label )++;
    *size_local_buff = 0;
    log_nfa = circ_ring_nfa( angles, used, cring, x, y, grad_dir, *local_buff, size_local_buff,
                             *label, logNT_ell );
    ...
    while ( r.wmax - delta > 0.0 ) {
        ...
        log_nfa_new = circ_ring_nfa( angles, used, &r, x, y, grad_dir, *tmp_buff, &size_tmp_buff,
                                     *label, logNT_ell );
        if ( log_nfa_new > log_nfa ) {
            cring->wmax -= delta;
            log_nfa = log_nfa_new;
...
/** Compute additive normalised per-pixel error inside an elliptical ring, and
    ...
    the most meaningful primitive.
 */
static void ell_count_error( PImageDouble angles, PImageInt used, Ring* ering, double* foci, int x,
                             int y, int grad_dir, Point* buff, int* size_buff, int label, int* pts,
                             double* err ) {
    int xx, yy, i, adr;
    double theta;
    ...
    if ( size_buff == NULL ) error( "ell_count_error: 'size_local_buff' must be non null." );
...
/** Iteration to improve NFA: count additive error and compute NFA.
 */
static double ell_ring_nfa( PImageDouble angles, PImageInt used, Ring* ering, double* foci, int x,
                            int y, int grad_dir, Point* buff, int* size_buff, int label,
                            double logNT_ell ) {
    int pts;
    double err;
    ...
    if ( size_buff == NULL ) error( "ell_ring_nfa: 'size_local_buff' must be non null." );
    if ( label <= 1 ) error( "ell_ring_nfa: forbidden label value." );

    /* count additive error */
    ell_count_error( angles, used, ering, foci, x, y, grad_dir, buff, size_buff, label, &pts,
                     &err );
 ...
 */
static double ell_ring_improve( PImageDouble angles, PImageInt used, Ring* ering, double* foci,
                                int x, int y, int grad_dir, Point** local_buff,
                                int* size_local_buff, Point** tmp_buff, int* label,
                                double logNT_ell ) {
    double log_nfa, log_nfa_new;
    double delta = 0.5;
    int size_tmp_buff;
    ...
    if ( size_local_buff == NULL ) error( "ell_ring_improve: 'size_local_buff' must be non null." );
    if ( *label <= 1 ) error( "ell_ring_improve: forbidden label value." );

    if ( !check_ell_ring( ering ) ) return -logNT_ell;
    ( *label )++;
    *size_local_buff = 0;
    log_nfa = ell_ring_nfa( angles, used, ering, foci, x, y, grad_dir, *local_buff, size_local_buff,
                            *label, logNT_ell );
    ...
    while ( r.wmax - delta > 0.0 ) {
        ...
        log_nfa_new = ell_ring_nfa( angles, used, &r, foci, x, y, grad_dir, *tmp_buff,
                                    &size_tmp_buff, *label, logNT_ell );
        if ( log_nfa_new > log_nfa ) {
            ering->wmax -= delta;
            log_nfa = log_nfa_new;
            /* switch addresses of the pointers to the local and tmp buffer */
 ...
 */
void add_poly_out( Polygon** poly_out, PolyRect* poly, int poly_count, int* poly_count_max,
                   int** labels, int lbl ) {
    if ( poly_count > *poly_count_max ) {
        *poly_out = (Polygon*) realloc( *poly_out, ( 2 * poly_count ) * sizeof( Polygon ) );
 ...
 */
void add_ell_out( Ring** ell_out, Ring* ell, int ell_count, int* ell_count_max, int** labels,
                  int lbl ) {
...
void ELSDc( PImageDouble in, int* ell_count, Ring** ell_out, int** ell_labels, int* poly_count,
    ...
    if ( ( out == NULL ) || ( out->data == NULL ) ) error( "ELSDc: invalid output label image." );
    ...
    /* Number of tests for ellipse arcs. We must count (3*xsize)*(3*ysize)
       ...
       In log, we get:   */
    logNT_ell    = 4.0 * ( log10( (double) xsize ) + log10( (double) ysize ) ) + LOG10_9 + LOG10_2;
    min_size_ell = (int) ( ( -logNT_ell - mlog10eps ) / log10( p ) );

    logNT_seg    = 2.5 * ( log10( (double) xsize ) + log10( (double) ysize ) ) + LOG10_2;
    ...
    img_gradient_sort( imgauss, rho, &list_p, &mem_p, n_bins, max_grad, &angles, &gradmag, &gradx,
                       &grady );
    ...
    for ( ; list_p; list_p = list_p->next ) {
        ...
        if ( !curve_grow( gradmag, angles, used, reg, &reg_size, density_th, prec, poly, &label,
                          pext2, pext1, &spir ) )
            continue;
        ...
        /* Compute number of tests for polygon; it must be done for each
           ...
           polygon */
        logNT_poly = ( poly->dim + 1.5 ) * ( log10( (double) xsize ) + log10( (double) ysize ) ) +
                     ( poly->dim + 1.0 ) * LOG10_2;
        /* polygon's best validation score */
        min_size_poly = (int) ( ( -logNT_poly - mlog10eps ) / log10( p ) );

        if ( reg_size > min_size_poly ) {
            best_nfa = poly_improve( angles0, used, poly, &best_buff, &size_best_buff, &tmp_buff,
                                     logNT_poly );
        ...
        log_nfa =
            poly_improve( angles0, used, seg, &new_buff, &size_new_buff, &tmp_buff, logNT_seg );
        ...
        if ( reg_size > min_size_ell )
            conic_fit( gradx, grady, reg, reg_size, &buff_fit, &size_buff_fit, cparam, eparam );

        /* Get parameters of the circle ring and compute validation score */
        ang0 = angles->data[reg[0].y * angles->xsize + reg[0].x];
        if ( get_ring( reg, reg_size, ang0, cparam, pext1, pext2, CIRCLE, spir, &cring, &grad_dirc,
                       foci, msize ) ) {
            log_nfa = circ_ring_improve( angles0, used, &cring, reg[0].x, reg[0].y, grad_dirc,
                                         &new_buff, &size_new_buff, &tmp_buff, &label, logNT_ell );
            /* if more meaningful than previous, update best feature */
        ...
        if ( get_ring( reg, reg_size, ang0, eparam, pext1, pext2, ELLIPSE, spir, &ering, &grad_dire,
                       foci, msize ) ) {
            log_nfa = ell_ring_improve( angles0, used, &ering, foci, reg[0].x, reg[0].y, grad_dire,
                                        &new_buff, &size_new_buff, &tmp_buff, &label, logNT_ell );
            /* if more meaningful than previous, update best feature */
            if ( log_nfa > best_nfa && size_new_buff > min_size_ell ) {
                best_nfa     = log_nfa;
        ...
        if ( best_type == 'c' ) {
            ( *ell_count )++;
            add_ell_out( ell_out, (Ring*) best_feature, *ell_count, &ell_count_max, ell_labels,
                         label_meaningful );
            /*Ring *rr = (Ring*)best_feature;*/
            /* free(rr); */
        } else if ( best_type == 'p' ) {
            ( *poly_count )++;
            add_poly_out( poly_out, (PolyRect*) best_feature, *poly_count, &poly_count_max,
                          poly_labels, label_meaningful );
        }
