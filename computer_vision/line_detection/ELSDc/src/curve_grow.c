 ...
 */
static void region_grow( PImageDouble angles, PImageInt used, Point* reg, int start, int idx_buff,
                         int* end, int label, int label_start, double prec, double* reg_angle ) {
 ...
 */
static double get_theta( PImageDouble gradmag, Point* reg, int start, int end, double cx, double cy,
 ...
 */
void region2rect( PImageDouble gradmag, Point* reg, int start, int end, double reg_angle,
                  Rectangle* rec, double prec ) {
 ...
 */
static int reduce_region_radius( PImageDouble gradmag, PImageDouble angles, PImageInt used,
                                 Point* reg, int start, int* end, double reg_angle, Rectangle* rec,
    ...
    density =
        (double) ( *end - start ) / ( dist( rec->x1, rec->y1, rec->x2, rec->y2 ) * rec->width );
 ...
 */
static int refine( PImageDouble gradmag, PImageDouble angles, PImageInt used, Point* reg, int start,
                   int idx_buff, int* end, int* label, int label_start, Rectangle* rec,
                   double density_th, double prec ) {
    ...
    for ( i = start; i < *end; i++ ) {
        used->data[reg[i].x + reg[i].y * used->xsize] = NOTUSED;
        if ( dist( (double) xc, (double) yc, (double) reg[i].x, (double) reg[i].y ) < rec->width ) {
    ...
    tau = max(
        2.0 * sqrt( ( s_sum - 2.0 * mean_angle * sum ) / (double) n + mean_angle * mean_angle ),
        0.2 );
    tau = min( tau, prec );
    ...
    *end = idx_buff;
    ( *label )++;
    region_grow( angles, used, reg, start, idx_buff, end, *label, label_start, tau, &reg_angle );
    ...
    if ( density < density_th )
        return reduce_region_radius( gradmag, angles, used, reg, start, end, reg_angle, rec, xc, yc,
                                     density_th, prec );
    return TRUE;
}
 ...
 */
static void px_seed( PImageDouble angles, PImageInt used, Rectangle* rec, Point* reg, int start,
                     int* idx_buff, int end, double* pext, double prec, int label,
                     int label_start ) {
    int xx, yy, i;
    int adr;
                ...
                for ( yy = reg[i].y - 1; yy <= reg[i].y + 1; yy++ ) {
                    ...
                    if ( in_image( xx, yy, used->xsize, used->ysize ) ) {
                        ...
                        if ( ( used->data[adr] != USED ) && ( used->data[adr] != label ) &&
                            ...
                            l2 = fabs( ( xx - rec->cx ) * rec->dx + ( yy - rec->cy ) * rec->dy );
                            /* If point's orientation is similar and the point is at
                               an acceptable distance  */
                            if ( ( angle_diff( angles->data[adr], rec->theta - M_1_2_PI ) <
                                   3 * prec ) &&
                                 ( l2 >= l1 - 1 ) ) {
                                reg[*idx_buff].x = xx;
                                reg[*idx_buff].y = yy;
 ...
 */
static void subcurve( PImageDouble angles, PImageDouble gradmag, PImageInt used, PolyRect* poly,
                      Point* reg, int start, int idx_buff, int* end, double* pext, double* spir,
                      double density_th, double prec, int* sgn, int* label, int label_start,
                      int w ) {
    int convex = 1;
    ...
    while ( convex ) {
        ...
        *end    = idx_buff;
        ...
        region_grow( angles, used, reg, start, idx_buff, end, *label, label_start, prec,
                     &reg_angle );
        if ( *end - start > buf_tmp ) {
            /* Estimate and refine rectangle */
            region2rect( gradmag, reg, start, *end, reg_angle, &rec, prec );
            ( *label )++;
            if ( !refine( gradmag, angles, used, reg, start, idx_buff, end, label, label_start,
                          &rec, density_th, prec ) ) {
            ...
            /* Convexity rule imposes that consecutive rectangles turn in the same
               ...
               then set sign. */
            if ( *sgn == 0 ) *sgn = sign( angle_diff_signed( rec.theta, ang_prev ) );
            ...
            if ( ( sign( difang ) == *sgn ) && ( fabs( difang ) < ( M_PI / 3.0 ) ) &&
                ...
                px_seed( angles, used, &rec, reg, start, &idx_buff, *end, pext, prec, *label,
                         label_start );
 ...
 */
int curve_grow( PImageDouble gradmag, PImageDouble angles, PImageInt used, Point* reg,
                int* reg_size, double density_th, double prec, PolyRect* poly, int* label,
                double* pext1, double* pext2, double* spir ) {
    int sgn = 0;
    Rectangle rec;
    ...
    region_grow( angles, used, reg, start, idx_buff, reg_size, *label, label_start, prec,
                 &reg_angle );
    ...
    /* Refine rectangle if density of aligned points is less than density
       threshold */
    if ( !refine( gradmag, angles, used, reg, start, idx_buff, reg_size, label, label_start, &rec,
                  density_th, prec ) ) {
    ...
    ( *label )++;
    idx_buff = *reg_size;
    px_seed( angles, used, &rec, reg, start, &idx_buff, *reg_size, pext1, prec, *label,
             label_start );

    /* Lauch scan in first direction */
    subcurve( angles, gradmag, used, poly, reg, start, idx_buff, reg_size, pext1, spir, density_th,
              prec, &sgn, label, label_start, 1 );
    ...
    if ( *spir < M_2__PI ) {
        ...
        idx_buff = *reg_size;
        ( *label )++;
        px_seed( angles, used, &rec, reg, 0, &idx_buff, size_first_reg, pext2, prec, *label,
                 label_start );
        /* When scanning the other direction, the sign must be inverted */
        sgn = -sgn;
        /* Lauch scan in second direction */
        subcurve( angles, gradmag, used, poly, reg, start, idx_buff, reg_size, pext2, spir,
                  density_th, prec, &sgn, label, label_start, 0 );
    }
    return 1;
}
