...
int main( int argc, char** argv ) {
    ...
    ELSDc( in, &ell_count, &ell_out, &ell_labels, &poly_count, &poly_out, &poly_labels, out );
    ...
    if ( ell_out != NULL ) {
        ...
        for ( i = 0; i < ell_count; i++ ) {
            ...
            fprintf( ell_ascii, "%lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf \n", ell_out[i].x1,
                     ell_out[i].y1, ell_out[i].x2, ell_out[i].y2, ell_out[i].cx, ell_out[i].cy,
                     ell_out[i].ax, ell_out[i].bx, ell_out[i].theta, ell_out[i].ang_start,
                     ell_out[i].ang_end );
        }
        fclose( ell_ascii );
    }
    ...
    if ( ( ell_out != NULL ) || ( poly_out != NULL ) ) {
        ...
        for ( i = 0; i < ell_count; i++ )
            ...
            if ( ( double_equal( ell_out[i].ax,
                                 ell_out[i].bx ) ) ) /* && (ell_out[i].theta == 0) ) */
                write_svg_circ_arc( fsvg, &ell_out[i] );
            else
                write_svg_ell_arc( fsvg, &ell_out[i] );

        /* write polygons */
        for ( i = 0; i < poly_count; i++ ) write_svg_poly( fsvg, &poly_out[i] );
        /* close svg file */
        fclose_svg( fsvg );
    }
    ...
    if ( poly_out != NULL ) {
        for ( i = 0; i < poly_count; i++ ) free( poly_out[i].pts );
        free( poly_out );
        free( poly_labels );
    }
