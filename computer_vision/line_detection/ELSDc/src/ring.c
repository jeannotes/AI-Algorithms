...
void copy_ring( Ring* in, Ring* out ) {
    /* check parameters */
    if ( ( in == NULL ) || ( out == NULL ) ) error( "copy_ring: invalid 'in' or 'out'." );
...
int is_in_circ_ring( Ring* r, int x, int y ) {
    ...
    } else if ( ( theta < r->ang_start ) && ( theta > r->ang_end ) )
        return FALSE;
...
static int dir_gradient_circ( int px, int py, double ang, double* param ) {
    ...
    if ( angle_diff( a, ang ) < M_1_2_PI ) dir = 1;
    return dir;
}
...
int get_ring( Point* reg, int reg_size, double ang, double* param, double* pext1, double* pext2,
    ...
    /* if the ends of the last rectangles got interwined, swap the ends
       of the ring  */
    if ( r->ang_start > r->ang_end )
        tang = r->ang_end + M_2__PI;
    else
        tang = r->ang_end;
    if ( ( spir > M_PI ) && ( tang - r->ang_start < M_1_2_PI / 2.0 ) ) swap_ring( r );
