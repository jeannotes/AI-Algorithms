...
PImageDouble read_pgm_image_double( char* name ) {
    ...
    /* open file */
    f = fopen( name, "rb" );
    if ( f == NULL ) error( "read_pgm_image_double: unable to open input image file." );
    ...
    if ( ( c = getc( f ) ) == '2' )
        ...
        error( "read_pgm_image_double: not a PGM file." );
    ...
    /* white before data */
    if ( !isspace( c = getc( f ) ) ) error( "read_pgm_image_double: corrupted PGM file." );
...
void write_pgm_image_int( int* image, int xsize, int ysize, char* name ) {
    ...
    if ( strcmp( name, "-" ) == 0 )
        ...
        f = fopen( name, "w" );
    if ( f == NULL ) error( "write_pgm_image_int: unable to open output image file." );
