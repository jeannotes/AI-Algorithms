...
void free_PImageChar( PImageChar i ) {
    /* check parameters */
    if ( ( i == NULL ) || ( i->data == NULL ) ) error( "free_PImageChar: invalid input image." );
    free( (void*) i->data );
    free( (void*) i );
}
...
PImageChar new_PImageChar( unsigned int xsize, unsigned int ysize ) {
    ...
    /* check parameters */
    if ( ( xsize == 0 ) || ( ysize == 0 ) ) error( "new_PImageChar: invalid image size." );
...
void free_PImageInt( PImageInt i ) {
    /* check parameters */
    if ( ( i == NULL ) || ( i->data == NULL ) ) error( "free_PImageInt: invalid input image." );
    free( (void*) i->data );
    free( (void*) i );
}
...
PImageInt new_PImageInt( unsigned int xsize, unsigned int ysize ) {
    ...
    /* check parameters */
    if ( ( xsize == 0 ) || ( ysize == 0 ) ) error( "new_PImageInt: invalid image size." );
...
void free_PImageDouble( PImageDouble i ) {
    /* check parameters */
    if ( ( i == NULL ) || ( i->data == NULL ) ) error( "free_PImageDouble: invalid input image." );
    free( (void*) i->data );
    free( (void*) i );
}
...
PImageDouble new_PImageDouble( unsigned int xsize, unsigned int ysize ) {
    ...
    /* check parameters */
    if ( ( xsize == 0 ) || ( ysize == 0 ) ) error( "new_PImageDouble: invalid image size." );
...
void img_gradient_sort( PImageDouble in, double threshold, CoordList** list_p, void** mem_p,
    ...
    if ( threshold < 0.0 ) error( "img_gradient_sort: 'threshold' must be positive." );
    ...
    if ( max_grad <= 0.0 ) error( "img_gradient_sort: 'max_grad' must be positive." );
...
void mark_img_pts( PImageInt im, Point* pts, int start, int end, int label ) {
    ...
    for ( i = start; i < end; i++ ) { im->data[pts[i].y * im->xsize + pts[i].x] = label; }
}
...
int in_image( int x, int y, unsigned int xsize, unsigned int ysize ) {
    /* check parameters */
    if ( ( xsize <= 0 ) || ( ysize <= 0 ) ) error( "in_image: invalid input image dimension." );
    if ( ( x >= 0 ) && ( x < (int) xsize ) && ( y >= 0 ) && ( y < (int) ysize ) ) return TRUE;
    return FALSE;
}
