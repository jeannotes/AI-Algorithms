...
%         lambda = eig(A);
        ...
        for iter = 1:MAXI
            ...
            % x =repmat([1 2 3 ..m], n, 1)
            % y =repmat([1 2 3 ..n)]' , 1,m)
            % n =size(l1, 1)
            % m =size(l1,2)
            xp = x+u;
            yp = y+v;
            % u, v is how fast it moves!
            win2 = interp2(x,y,I2,xp(top:bottom,left:right),yp(top:bottom,left:right));
            % u is vector, v is vector, every time, it iterate
            % to have a better u & v to xp & yp, and we get corresponding
            % win2, the question is first we have [1 2 3], if we plus u & v
            % we might have [3 4 5], it doesn't matter, because we interp them.
            it = win2-win1;
            ixt = it.*ix;
            iyt = it.*iy;
            B = -1.*[sum(ixt(:)); sum(iyt(:))];
