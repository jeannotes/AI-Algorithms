...
{
inline void getClosestCells(
    const perception_oru::NDTMap& map, const perception_oru::NDTCell& cell, int neig_size,
...
inline void getEigenVectors2D( const perception_oru::NDTCell& cell, Eigen::Vector3d& eigenval,
...
inline int getBiggestEigenVector2D( const perception_oru::NDTCell& cell, Eigen::Vector3d& eigenval,
                                    Eigen::Matrix3d& eigenvec ) {
    ...
    if ( eigenval( 1 ) > eigenval( 0 ) ) { return 1; }
    return 0;
}
...
inline double NDTCellAngle( const perception_oru::NDTCell& cell,
                            const Eigen::Vector2d& reference = Eigen::Vector2d( 0, 1 ) ) {
...
inline Eigen::Vector3d collisionNDTCells( const perception_oru::NDTCell& cell1,
                                          const perception_oru::NDTCell& cell2 ) {
    // 			Eigen::Vector3d eigenval;
    // 			Eigen::Matrix3d eigenvec;
    // 			int index = getBiggestEigenVector2D(cell1, eigenval, eigenvec);
...
inline void angleNDTCells( const perception_oru::NDTCell& cell1,
                           const perception_oru::NDTCell& cell2,
                           const Eigen::Vector3d& collision_point, double& angle,
    ...
    double angle_between =
        atan2( vector1( 1 ), vector1( 0 ) ) - atan2( vector2( 1 ), vector2( 0 ) );
    ...
    angle = angle_between;
    if ( angle >= 2 * M_PI ) {
        throw( std::runtime_error( "Making sure that this get never hits. It's in Utils.hpp "
                                   "ndt_feature_finder" ) );
    }
    if ( angle < 0 ) {
        throw( std::runtime_error( "Making sure that this get never hits. It's in Utils.hpp "
...
/**
 * @brief return the angle between two ndt cell and all possible direction found using initialized
 * but unoccupied cells.
 * @param[in] cell1 : first cell
 * @param[in] cell2 : second cell
 * @param[in] collision_point : collision point of both cells
 * @param[in] initialized_cells : initialized_cells around corner
 * @param[in] angle : actual angle
 * @param[out] angle_directions : all possible direction of the angle depending on empty cells
 * around it.
 */
inline void orientationNDTCells( const perception_oru::NDTMap& map,
                                 const perception_oru::NDTCell& cell1,
                                 const perception_oru::NDTCell& cell2,
                                 const Eigen::Vector3d& collision_point, double angle,
                                 double orientation, std::vector< double >& angles,
                                 std::vector< double >& angle_orientations ) {
    auto point_center  = cell1.getCenter();
    auto point_center2 = cell2.getCenter();

    // 			std::cout << "center point " << point_center.x << " " << point_center.y << " " <<
    ...
    // 			std::cout << "point mean1 " << mean_cell1(0) <<" " << mean_cell1(1) << " " <<
    // mean_cell1(2) <<  std::endl;
    // 			std::cout << "point mean2 " << mean_cell2(0) <<" " << mean_cell2(1) << " " <<
    // mean_cell2(2) <<  std::endl;

    auto vector1 = mean_cell1 - collision_point;
    auto vector2 = mean_cell2 - collision_point;

    // 			Might not work in some specific situations so we check that the direction is more or
    // less
    ...
    // 				std::cout << collision_point(0) << " + (" << x_cell_size << " * " <<
    // std::cos(orientation
    // + (i * M_PI) ) << " ) " << std::endl;
    // 				std::cout << collision_point(1) << " + (" << x_cell_size << " * " <<
    // std::sin(orientation
    // + (i * M_PI) ) << " ) " << std::endl;
    ...
    // 				Eigen::Vector3d forward_point_eigen; forward_point_eigen << forward_point.x,
    // forward_point.y, forward_point.z;
    // 				auto orientation_vector = forward_point_eigen - collision_point;

    // 				std::cout << "orienation vector " << orientation_vector(0) <<" " <<
    // orientation_vector(1)
    // << " " << orientation_vector(2) <<  std::endl;
    ...
    if ( cell_collision != NULL ) {
        ...
        for ( auto it = cells_initialized.begin(); it != cells_initialized.end(); ++it ) {
            ...
            std::cout << "Get closest cells center" << std::endl;
            Eigen::Vector3d init_center_eigen;
            init_center_eigen << init_center.x, init_center.y, init_center.z;
            auto vector_init = init_center_eigen - collision_point;

            // CHecking both possible orientation to see which one is aligned with the uninitialized
            // cell
            for ( int i = 0; i < 2; ++i ) {
                ...
                pcl::PointXYZ forward_point;
                forward_point.x = collision_point( 0 ) +
                                  ( 2 * x_cell_size * std::cos( orientation + ( i * M_PI ) ) );
                forward_point.y = collision_point( 1 ) +
                                  ( 2 * x_cell_size * std::sin( orientation + ( i * M_PI ) ) );
                forward_point.z = 0;

                std::cout << collision_point( 0 ) << " + (" << x_cell_size << " * "
                          << std::cos( orientation + ( i * M_PI ) ) << " ) " << std::endl;
                std::cout << collision_point( 1 ) << " + (" << x_cell_size << " * "
                          << std::sin( orientation + ( i * M_PI ) ) << " ) " << std::endl;

                std::cout << "point forward " << forward_point.x << " " << forward_point.y << " "
                          << forward_point.z << std::endl;
                std::cout << "point collision " << collision_point( 0 ) << " "
                          << collision_point( 1 ) << " " << collision_point( 2 ) << std::endl;

                Eigen::Vector3d forward_point_eigen;
                forward_point_eigen << forward_point.x, forward_point.y, forward_point.z;
                auto orientation_vector = forward_point_eigen - collision_point;

                // Calculating the angle between both vector
                Eigen::Vector2d vector_init_2d;
                vector_init_2d << vector_init( 0 ), vector_init( 1 );
                Eigen::Vector2d orientation_vector_2d;
                orientation_vector_2d << orientation_vector( 0 ), orientation_vector( 1 );
                // 							double angle_init_orientation =
                // getAngleDirected(vector_init_2d,
                // orientation_vector_2d);
                double angle_init_orientation = getAngle( vector_init, orientation_vector );

                // 							std::cout << angle_init_orientation << " == " <<
                ...
                // Angle between 0 and pi Because we don't care about orientation as in how close
                // they are
                if ( angle_init_orientation >= M_PI )
                    angle_init_orientation = ( 2 * M_PI ) - angle_init_orientation;

                // Are they pointing the same direction? Thus is the angle between the orientation
                // and the initialised cell less than 0.78rad (45deg)
                if ( angle_init_orientation < 0.78 ) {
                    // Check that the direction is in the middle of the gaussians
                    double angle_between =
                        atan2( vector1( 1 ), vector1( 0 ) ) -
                        atan2( orientation_vector( 1 ), orientation_vector( 0 ) );
                    double angle_between2 =
                        atan2( orientation_vector( 1 ), orientation_vector( 0 ) ) -
                    ...
                    while ( angle_final >= 2 * M_PI ) { angle_between -= 2 * M_PI; }

                    std::cout << " ANGLE "
                              << atan2( vector1( 1 ), vector1( 0 ) ) -
                                     atan2( orientation_vector( 1 ), orientation_vector( 0 ) )
                              << " and " << std::abs( atan2( vector2( 1 ), vector2( 0 ) ) -
                                                      atan2( orientation_vector( 1 ),
                                                             orientation_vector( 0 ) ) )
                              << std::endl;
                    std::cout << "FINAL ANGLES " << angle_between << " " << angle_between2
                              << std::endl;

                    // If the angles are roughly equal then it's in the middle just fine
                    if ( ( angle_between < ( angle_between2 + 0.1 ) ) &&
                         ( angle_between > ( angle_between2 - 0.1 ) ) ) {
                        std::cout << "Pushing back " << orientation + ( i * M_PI ) << " with "
                                  << angle_between << " " << angle_between2 << std::endl;
    ...
    // Change the check on cell initialized by checking all cells between the gaussians that gave
    // the corner and passing by the orientation are initialized.
    // 				if(cell != NULL){
    //
    //
    //
    ...
    //
    // 					if(center_forward.x == point_center.x && center_forward.y == point_center.y
    // &&
    ...
    // 						std::cout << "center forward " << collision_cell_pcl.x << " " <<
    // collision_cell_pcl.y
    // << " " << collision_cell_pcl.z << std::endl;
    // 						if(center_forward.x == collision_cell_pcl.x && center_forward.y ==
    // collision_cell_pcl.y
    // && center_forward.z == collision_cell_pcl.z){
    ...
    //
    // 							//Angle between 0 and pi
    // 							if (angle_between >= M_PI) angle_between = (2 * M_PI) -
    // angle_between;
    // 							if (angle_between2 >= M_PI) angle_between2 = (2 * M_PI) -
    // angle_between2;
    //
    //
    // 							std::cout << " ANGLE "  << atan2(vector1(1), vector1(0)) -
    // atan2(orientation_vector(1), orientation_vector(0)) << " and " << std::abs( atan2(vector2(1),
    // vector2(0)) - atan2(orientation_vector(1), orientation_vector(0)) ) << std::endl;
    ...
    //
    // // 			exit(0);
    // 			//CHeck that line between the mean does not contain any Gaussian unless it's the
    // corner
    // cell itself. Only work for the small angle
    // // 			double x_cell_size, y_cell_size, z_cell_size;
    // // 			map.getCellSizeInMeters(x_cell_size, y_cell_size, z_cell_size);
    // //
    // // 			auto mean_cell1 = cell1.getMean();
...
/**
 * @brief return the angle between two ndt cell and all possible orientations found using
 * initialized but unoccupied cells.
 * @param[in] cell1 : first cell
 * @param[in] cell2 : second cell
 * @param[in] collision_point : collision point of both cells
 * @param[in] initialized_cells : initialized_cells around corner
 ...
 * around it.
 */
inline void angleNDTCells( const perception_oru::NDTMap& map, const perception_oru::NDTCell& cell1,
                           const perception_oru::NDTCell& cell2,
                           const Eigen::Vector3d& collision_point, double& angle,
    ...
    orientationNDTCells( map, cell1, cell2, collision_point, angle, orientation, angles,
                         angle_orientations );
}

/**
* @brief return all the cells around cell that possess a gaussian and are maximum the resolution of
* the map away.
*/
void getClosestCells(
    ...
    if ( max < y_cell_size ) { max = y_cell_size; }
    if ( max < z_cell_size ) { max = z_cell_size; }
    // 			std::cout << "RADIUS " << radius << "*" << neig_size << std::endl;
    ...
    // Get all closest cell and then classify to keep the one with gaussians and the initialized
    // ones
