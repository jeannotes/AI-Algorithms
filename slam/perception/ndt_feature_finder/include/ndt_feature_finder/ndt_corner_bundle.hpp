...
class NDTCornerBundle {
  protected:
    struct EigenValVec {
        ...
        double eigenval;
        Eigen::Vector3d NormEigen() { return eigenvec * eigenval; }
    };
  ...
  public:
    ...
    NDTCornerBundle( const Eigen::Vector3d& m ) : _mean( m ), _mean_set( true ) {}
    ...
    void gaussian() {
        ...
        // Second collision line with only one of the two line moved. It doesn't matter which but it
  ...
  private:
    void getEigenVectors( const perception_oru::NDTCell& cell, EigenValVec& biggest,
                          EigenValVec& smallest ) const {
