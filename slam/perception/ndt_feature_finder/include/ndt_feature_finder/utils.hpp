...
inline double getAngleG( const Eigen::VectorXd& base, const Eigen::VectorXd& toward ) {
    ...
    // 			std::cout << " AC " << ac << " angle has to be " << std::acos(ac) << " " <<
    // std::acos(1)
    // <<std::endl;
    ac = std::acos( ac );
...
inline void EigenSort2D( Eigen::Vector3d& eigenvalues, Eigen::Matrix3d& eigenvectors ) {
    ...
    double xy_val;
    for ( int i = 0; i < 3; ++i ) {
        // 				std::cout << "Vect : " << eigenvectors.col(i) << std::endl << " dot " <<
        // fabs(eigenvectors.col(i).dot(ez)) << std::endl;
        if ( dist == -1 ) {
            z_id = i;
...
inline auto distancePoints( const cv::Point p1, const cv::Point p2 ) -> double {
    ...
    // 						exit(0);
    // 					}
    if ( dst > 0 ) { return std::sqrt( dst ); }
    return 0;
};

inline auto getSmallestDistance( const cv::Point2d& pt, const std::vector< cv::Point2d >& model,
                                 double& distance ) -> size_t {
    size_t i, index = -1;
    distance = -1;
    for ( i = 0; i < model.size(); ++i ) {
        ...
        if ( distance == -1 ) {
            ...
            // 				std::cout << distance << " > " << distancePoints(pt, model[i]) <<
            // std::endl;
...
inline Eigen::Vector3d collisionRay( const Eigen::Vector3d& ray_direction,
    ...
    A << -ray_direction( 0 ), ray_direction_second( 0 ), 0, -ray_direction( 1 ),
        ray_direction_second( 1 ), 0, -ray_direction( 2 ), ray_direction_second( 2 ), 0;
    b << ray_point( 0 ) - ray_point_second( 0 ), ray_point( 1 ) - ray_point_second( 1 ),
