...
std::vector< perception_oru::ndt_feature_finder::NDTCornerBundle >
perception_oru::ndt_feature_finder::NDTCorner::getAllCorners( const perception_oru::NDTMap& map,
...
bool perception_oru::ndt_feature_finder::NDTCorner::cellIsCorner(
    const perception_oru::NDTMap& map, const perception_oru::NDTCell& cell,
    const std::vector< boost::shared_ptr< perception_oru::NDTCell > >& allCells,
    NDTCornerBundle& corner ) {
    ...
    try {
        ...
        // For every cell get all other cells around and test if at least two are pointing toward
        // the center cell
        ...
        if ( neighbor.size() >= 8 ) {
            ...
            for ( size_t j = 0; j < neighbor.size(); ++j ) {
                ...
                for ( size_t jj = j + 1; jj < neighbor.size(); ++jj ) {
                    // 					std::cout << "Cells " << neighbor[jj]->getMean() <<
                    // std::endl;
                    if ( neighbor[j]->getMean() == neighbor[jj]->getMean() && j != jj ) {
                        // 						std::cout << "Already SEEN " <<j << " and " << jj <<
                        // std::endl;
        ...
        if ( neighbor.size() > 1 ) {
            ...
            if ( possible_neighbor.size() > 0 ) {
                // 				std::cout << "nieghbor size " << possible_neighbor.size() <<
                // std::endl;
...
// double perception_oru::ndt_feature_finder::NDTCorner::getCellOrientation(const
// perception_oru::NDTCell& cell) const
// {
//
...
// void perception_oru::ndt_feature_finder::NDTCorner::getClosestCells(const perception_oru::NDTMap&
// map, const perception_oru::NDTCell& cell, int neig_size, std::vector< boost::shared_ptr<
// perception_oru::NDTCell > >& cells_gaussian, std::vector< boost::shared_ptr<
// perception_oru::NDTCell > >& cells_initialized) const{
...
// std::vector< perception_oru::NDTCell* >
// perception_oru::ndt_feature_finder::NDTCorner::getClosestCells(const std::vector<
// perception_oru::NDTCell* >& allcells, const perception_oru::NDTCell& cell) const
// {
// 	double max = _x_cell_size;
// 	if(max < _y_cell_size){
// 		max = _y_cell_size;
...
// double perception_oru::ndt_feature_finder::NDTCorner::getAngle(const Eigen::Vector3d& vec, const
...
std::vector< perception_oru::NDTCell* >
perception_oru::ndt_feature_finder::NDTCorner::getCellsPointingToward(
    std::vector< perception_oru::NDTCell* >& neighbor, const perception_oru::NDTCell& cell ) const {
    // Check if the angle between the center and the neighbor is going toward the center == Angle of
    // the vector mean|mean to reference is the same as neighbor variance to reference
    ...
    for ( size_t j = 0; j < neighbor.size(); ++j ) {
        ...
        if ( angle_tmp > M_PI ) { angle_tmp = ( M_PI * 2 ) - angle_tmp; }
...
bool perception_oru::ndt_feature_finder::NDTCorner::gotAngledNDT(
    const perception_oru::NDTMap& map,
    const std::vector< boost::shared_ptr< perception_oru::NDTCell > >& neighbor,
    const std::vector< boost::shared_ptr< perception_oru::NDTCell > >& cells_initialized,
    NDTCornerBundle& corner ) {
    for ( size_t j = 0; j < neighbor.size(); ++j ) {
        // Get orientation of neighbor
        ...
        for ( size_t i = j + 1; i < neighbor.size(); ++i ) {
            ...
            Eigen::Vector3d collision_point_old = collisionRay(
                orientation_tmp, neighbor[i]->getMean(), orientation, neighbor[j]->getMean() );
            //
            // END OF TEST
            ...
            if ( angle_between_test > M_PI ) {
                // 					std::cout << (2*M_PI) - width << " == " << angle_tmp <<
                // std::endl;
                // 					assert((2*M_PI) - width <= angle_tmp + 0.1);
                // 					assert((2*M_PI) - width >= angle_tmp - 0.1);
                angle_between_test = ( M_PI * 2 ) - angle_between_test;
                direction_test - M_PI;
                if ( direction_test < 0 ) { direction_test = direction_test + ( 2 * M_PI ); }
            }
            //
            //
            ...
            // <<
            // std::endl << orientation_tmp << std::endl;
            ...
            if ( ( angle_tmp < ( ( M_PI / 2 ) + 0.1 ) ) && ( angle_tmp > ( ( M_PI / 2 ) - 0.1 ) ) ||
                 ( angle_tmp < ( ( 3 * M_PI / 2 ) + 0.1 ) ) &&
                     ( angle_tmp > ( ( 3 * M_PI / 2 ) - 0.1 ) ) ) {
                std::vector< double > orientations;
                std::vector< double > angles;
                // Calculate orientations. Slower function so we only use it if we found an angle
                // 				angleNDTCells(map, *neighbor[j], *neighbor[i], collision_point,
                // angle_tmp,
                // orientations);
                orientationNDTCells( map, *neighbor[j], *neighbor[i], collision_point, angle_tmp,
                                     orientation_t, angles, orientations );
                // 				assert(orientations.size() >= 1);
                // Need to correct angle orientation depending on where does the robot see the angle
                // from. Check on which direction with initialized cells with no gaussians. Add a
                // direction there.

                // TODO Parameter to check that a certain number of neighbor of the crossing ndt or
                // aligned.
                ...
                // 				auto cell = map.getCellAtID(collision_point(0), collision_point(1),
                // collision_point(2))->copy();
                // 				boost::shared_ptr<perception_oru::NDTCell> cell_ptr(cell);
                // 				corner.setNDTCell(cell_ptr);

                // Saving information about the corner: the mean and the two gaussian that defined
                // it.
...
void perception_oru::ndt_feature_finder::NDTCorner::clearAccurateCorner() {
    ...
    for ( size_t i = 0; i < _corners.size(); ++i ) {
        ...
        for ( size_t j = 0; j < tmp.size(); ++j ) {
            ...
            if ( vec.norm() < _x_cell_size ) { seen = true; }
        }
        if ( seen == false ) {
...
void perception_oru::ndt_feature_finder::NDTCorner::calculateAngles(
    const perception_oru::NDTMap& map ) {}

/**
 * return true if at least one ndt in the neighbor is aligned with the given cell
 */
bool perception_oru::ndt_feature_finder::NDTCorner::AlignedNDT(
    const std::vector< perception_oru::NDTCell* >& neighbor, const perception_oru::NDTCell& cell ) {
    Eigen::Vector3d eigenval;
    Eigen::Matrix3d eigenvec;
    ...
    for ( size_t j = 0; j < neighbor.size(); ++j ) {
        if ( neighbor[j] != &cell ) {
            ...
            double angle_between         = atan2( orientation( 1 ), orientation( 0 ) ) -
                                   atan2( orientation2( 1 ), orientation2( 0 ) );
            if ( angle_between < 0 ) angle_between += 2 * M_PI;

            // Around 0
            if ( angle_between <= 0.349 || angle_between >= 5.934 ) {
                // Around 180
                if ( angle_between >= 2.793 && angle_between <= 3.491 ) { return true; }
            }
        }
    }
    return false;
