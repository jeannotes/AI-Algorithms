...
BOOST_AUTO_TEST_CASE( trying ) {
    ...
    Eigen::Vector3d out = perception_oru::ndt_feature_finder::collisionRay(
        ray_direction, ray_point, ray_direction_second, ray_point_second );
    Eigen::Vector3d test;
    test << 0, 0, 0;
    BOOST_CHECK_EQUAL( out, test );
    // 	ray_point << 0, 0, 0;
    ray_point_second << -1, 0, 0;
    out = perception_oru::ndt_feature_finder::collisionRay(
        ray_direction, ray_point, ray_direction_second, ray_point_second );
    test << -1, 0, 0;
    ...
    if ( rotation_degree > M_PI ) { rotation_degree = ( M_PI * 2 ) - rotation_degree; }
    double rotation_degree2 =
        perception_oru::ndt_feature_finder::getAngle( horizon, ray_direction );
    if ( rotation_degree2 > M_PI ) { rotation_degree2 = ( M_PI * 2 ) - rotation_degree2; }

    // 	BOOST_CHECK_EQUAL(angle_to, rotation_degree);
    // 	BOOST_CHECK_EQUAL(rotation_degree2, rotation_degree);

    double rotation_degree4 =
        perception_oru::ndt_feature_finder::getAngle( ray_direction, ray_direction_second );
    if ( rotation_degree4 > M_PI ) { rotation_degree4 = ( M_PI * 2 ) - rotation_degree4; }
    ...
    if ( a_b > M_PI ) { a_b = ( M_PI * 2 ) - a_b; }
    double width = std::abs( angle_to - angle_to2 );
    if ( width > M_PI ) { width = ( M_PI * 2 ) - width; }
