...
int main( int argc, char** argv ) {
    ...
    if ( map.loadFromJFF( file.c_str() ) < 0 ) std::cout << "File didn't load" << std::endl;
    ...
    // 	std::cout << "Found " << ret_export.size() << " Corners. yes same as " <<
    // ret_opencv_export.size() << std::endl;
    //
    ...
    // 	ros::Publisher marker_pub_graph =
    // nh.advertise<visualization_msgs::Marker>("visualization_marker_graph", 10);
    // 	ros::Publisher marker_pub_graph2 =
    // nh.advertise<visualization_msgs::Marker>("visualization_marker_graph2", 10);
    ros::Publisher marker_pub_graph_accurate =
        nh.advertise< visualization_msgs::Marker >( "accurate_markers", 10 );
