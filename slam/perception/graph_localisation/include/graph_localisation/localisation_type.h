...
class LocalisationType {
  public:
    ...
    virtual void InitializeLocalization( const Eigen::Affine3d& pose,
                                         const Vector6d& variance ) = 0;   // Vector3d variance={Vx,
                                                                           // Vy, Vz Vp Vr Vy}
    virtual void UpdateAndPredict( pcl::PointCloud< pcl::PointXYZ >& cloud,
                                   const Eigen::Affine3d& Tmotion ) = 0;
    virtual void UpdateAndPredict( pcl::PointCloud< velodyne_pointcloud::PointXYZIR >& cloud,
                                   const Eigen::Affine3d& Tmotion ) = 0;
  ...
  protected:
    ...
    LocalisationParamPtr param_;   // holds the parameters for localisation, can be used to
