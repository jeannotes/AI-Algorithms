...
MCLNDTType::MCLNDTType( LocalisationParamPtr param ) : LocalisationType( param ) {
    if ( MCLNDTParamPtr mclParam = boost::dynamic_pointer_cast< MCLNDTParam >( param ) ) {
        NDTMapPtr current_map =
            boost::dynamic_pointer_cast< NDTMapType >( graph_map_->GetCurrentNode()->GetMap() );
        if ( current_map != NULL ) {
            resolution =
                boost::dynamic_pointer_cast< NDTMapType >( graph_map_->GetCurrentNode()->GetMap() )
                    ->GetResolution();
...
NDTMap* MCLNDTType::GetCurrentNodeNDTMap() {
    ...
    // and return it...
    {
        NDTMapPtr p =
            boost::dynamic_pointer_cast< NDTMapType >( graph_map_->GetCurrentNode()->GetMap() );
        if ( p != NULL ) { return p->GetNDTMap(); }
    }

    {
        NDTDLMapPtr p =
            boost::dynamic_pointer_cast< NDTDLMapType >( graph_map_->GetCurrentNode()->GetMap() );
        return p->GetNDTMapFlat();
    }
    return NULL;
}

void MCLNDTType::UpdateAndPredict( pcl::PointCloud< pcl::PointXYZ >& cloud,
    ...
    incr << fabs( tr[0] ), fabs( tr[1] ), fabs( tr[2] ), fabs( rot[0] ), fabs( rot[1] ),
        fabs( rot[2] );
    ...
    for ( size_t i = 0; i < motion_model_offset.size(); i++ ) { m[i] += motion_model_offset[i]; }
    pose_          = graph_map_->GetCurrentNodePose() * pf.getMean();
    // std::cerr << "motion var(2) : " << m.transpose() << std::endl;
    Eigen::Affine3d remap_prev_to_new_frame;
    if ( new_ref_frame ) {
        remap_prev_to_new_frame =
            ( graph_map_->GetPreviousNodePose().inverse() * graph_map_->GetCurrentNodePose() )
                .inverse();
        cout << "change frame, transf=" << remap_prev_to_new_frame.translation().transpose()
             << endl;
        pf.predict( Tmotion, m[0], m[1], m[2], m[3], m[4], m[5], remap_prev_to_new_frame );
        // map_= boost::dynamic_pointer_cast< NDTMapType
        // >(graph_map_->GetCurrentNode()->GetMap())->GetNDTMap();
        map_ = this->GetCurrentNodeNDTMap();
    } else
        pf.predict( Tmotion, m[0], m[1], m[2], m[3], m[4], m[5] );
    pose_ = graph_map_->GetCurrentNodePose() * pf.getMean();

    if ( ( pose_.translation() - pose_last_update_.translation() ).norm() < 0.01 &&
         ( pose_last_update_.inverse() * pose_ ).rotation().eulerAngles( 0, 1, 2 ).norm() < 0.005 )
        return;

    // if(rot[2]<(0.5 * M_PI/180.0) && tr[0]>=0){
    //     pf.predict(Tmotion, tr[0]*pos_factor[0] + pos_offset, tr[1]*pos_scale[1] + pos_offset[1],
    //     tr[2]*pos_factor[2]/2.+pos_offset[2]
    //     ,rot[0]*rot_factor[0]+rot_offset[0],rot[1]*rot_factor[1]+rot_offset[1],
    //     rot[2]*rot_factor[2]+rot_offset[2]);
    // }else if(tr[0]>=0){
    //   pf.predict(Tmotion,tr[0]*tr_scale*2.5 + tr_offset, tr[1]*tr_scale/2.+ tr_offset,
    //   tr[2]*tr_scale/2.+tr_offset,rot[0]*rot_scale+rot_offset,rot[1]*rot_scale+rot_offset,
    //   rot[2]*rot_scale*2+rot_offset);
    // }else{
    //     pf.predict(Tmotion, tr[0]*tr_scale + tr_offset, tr[1]*tr_scale / 2.+ tr_offset,
    //     tr[2]*tr_scale+tr_offset ,rot[0]*rot_scale+rot_offset,rot[1]*rot_scale+rot_offset,
    //     rot[2]*rot_scale+rot_offset);
    // }

    double t_pred = getDoubleTime() - time_start;

    // std::cerr<<"cloud points "<<cloud.points.size()<<" res :"<<resolution<<" sres:
    // "<<resolution_sensor<<std::endl;
...
#pragma omp for
                ...
                if ( map_->getCellAtPoint( p, cell ) ) {
                    ...
                    if ( cell->hasGaussian_ ) {
                        ...
                        score += score_cell_weight +
                                 ( 1.0 - score_cell_weight ) * exp( -0.05 * l / 2.0 );
                    } else {
                    }
                }
            }
    ...
    if ( forceSIR ) {
        ...
        if ( varP > /*0.006*/ SIR_varP_threshold ||
             sinceSIR_ > /*25*/ SIR_max_iters_wo_resampling_ ) {
            // fprintf(stderr,"-SIR- ");
            sinceSIR_ = 0;
            pf.SIRUpdate();
        } else {
...
void MCLNDTType::UpdateAndPredict( pcl::PointCloud< velodyne_pointcloud::PointXYZIR >& cloud,
                                   const Eigen::Affine3d& Tmotion ) {
    ...
    // segmentPointCurvature(cloud, cornerPointsSharp, cornerPointsLessSharp, surfPointsFlat,
    // surfPointsLessFlat);
...
void MCLNDTType::InitializeLocalization( const Eigen::Affine3d& pose,
                                         const Vector6d& variance ) {   // Vector3d variance={Vx,
                                                                        // Vy, Vz Vp Vr Vy} pose
                                                                        // specified in global frame
    pose_             = pose;
    pose_last_update_ = pose_;
    ...
    pf.initializeNormalRandom( n_particles_, pos( 0 ), pos( 1 ), pos( 2 ), euler( 0 ), euler( 1 ),
                               euler( 2 ), variance( 0 ), variance( 1 ), variance( 2 ),
