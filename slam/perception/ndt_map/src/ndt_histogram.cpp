...
{
NDTHistogram::NDTHistogram( int linear_classes, int flat_classes, int spherical_classes, double _D1,
                            double _D2 ) {
    ...
    averageDirections = std::vector< Eigen::Vector3d, Eigen::aligned_allocator< Eigen::Vector3d > >(
        N_FLAT_BINS, Eigen::Vector3d( 0, 0, 0 ) );
    computeDirections();
...
NDTHistogram::NDTHistogram( NDTMap& map, int linear_classes, int flat_classes,
                            int spherical_classes, double _D1, double _D2 ) {
    N_LINE_BINS   = linear_classes;
    N_FLAT_BINS   = flat_classes;
    N_SPHERE_BINS = spherical_classes;
...
void NDTHistogram::constructHistogram( NDTMap& map, double linear_factor, double flat_factor ) {
    ...
    for ( int i = 0; i < averageDirections.size(); i++ ) { averageDirections[i].normalize(); }
}
...
void NDTHistogram::incrementFlatBin( Eigen::Vector3d& normal, double d ) {
    ...
    if ( idmin >= 0 && idmin < histogramBinsFlat.size() ) {
        ...
        if ( d < D1 )
            dist_histogramBinsFlat[0][idmin]++;
        else if ( d > D2 )
            dist_histogramBinsFlat[2][idmin]++;
        else
            dist_histogramBinsFlat[1][idmin]++;
    }
}
...
pcl::PointCloud< pcl::PointXYZ > NDTHistogram::getDominantDirections( int nDirections ) {
    ...
    for ( int i = 0; i < nDirections; i++ ) {
        ...
        while ( !found ) {
            ...
            // suppress this max and neighbours --- independent of the value of found, this is
            // necessarry
...
void NDTHistogram::bestFitToHistogram(
    NDTHistogram& target, Eigen::Transform< double, 3, Eigen::Affine, Eigen::ColMajor >& T,
    bool bound_transform ) {
            ...
            for ( int a2 = 0; a2 < dominantBinsTarget.points.size(); a2++ ) {
                for ( int b2 = 0; b2 < dominantBinsTarget.points.size(); b2++ ) {
                    ...
                    /*			    double good2 = 0, scale2 = 0;
                                    ...
                                    other_tr = target.directions[idmin]; //fall back to bin
                       direction
                                    }

                                    //compute norm of difference, scale by cardinality
                                    //double factor = fabsf((double)(this->histogramBinsFlat[q] -
                       target.histogramBinsFlat[idmin]+1)/
                                    //
                       (double)(this->histogramBinsFlat[q]+target.histogramBinsFlat[idmin]+1));
                                    //double factor =
                       this->histogramBinsFlat[q]+target.histogramBinsFlat[idmin];
                                    //scale2 += factor;
                                    //good2 += factor*(tr-other_tr).norm();
                                    good2 += pow((double)this->histogramBinsFlat[q]/N_THIS -
                       (double)target.histogramBinsFlat[idmin]/N_OTHER,2);
                    ...
                    */
                    ...
                    //				double factor =
                    (mineNew.points[i].intensity+useTarget.points[i].intensity);
                    //				good += factor*sqrt(
                    pow(mineNew.points[i].x-useTarget.points[i].x,2) +
...
void NDTHistogram::closedFormSolution(
    pcl::PointCloud< pcl::PointXYZ >& src, pcl::PointCloud< pcl::PointXYZ >& tgt,
    Eigen::Transform< double, 3, Eigen::Affine, Eigen::ColMajor >& T ) {
    T.setIdentity();
    Eigen::Matrix3d H;   // = (cloud_src_demean * cloud_tgt_demean.transpose ()).topLeftCorner<3,
                         // 3>();
...
double NDTHistogram::getSimilarity(
    NDTHistogram& other, Eigen::Transform< double, 3, Eigen::Affine, Eigen::ColMajor >& T ) {
    double score[3];
    ...
    for ( int q = 0; q < averageDirections.size(); q++ ) {
        ...
        // std::cout<<idmin<<std::endl;
        if ( !( idmin >= 0 && idmin < histogramBinsFlat.size() ) ) { continue; }

        for ( unsigned int r = 0; r < 3; r++ ) {
            score[r] += pow( (double) this->dist_histogramBinsFlat[r][q] / N_THIS[r] -
                                 (double) other.dist_histogramBinsFlat[r][idmin] / N_OTHER[r],
                             2 );
        }
    }
    for ( unsigned int r = 0; r < 3; r++ ) {
        for ( int i = 0; i < histogramBinsSphere.size(); i++ ) {
            score[r] += pow( (double) this->dist_histogramBinsSphere[r][i] / N_THIS[r] -
                                 (double) other.dist_histogramBinsSphere[r][i] / N_OTHER[r],
                             2 );
        }

        score[r] += pow( (double) this->dist_histogramBinsLine[r][0] / N_THIS[r] -
                             (double) other.dist_histogramBinsLine[r][0] / N_OTHER[r],
                         2 );
        double maxN, minN;
