...
LazyGrid::LazyGrid( double _sizeXmeters, double _sizeYmeters, double _sizeZmeters,
                    double _cellSizeX, double _cellSizeY, double _cellSizeZ, double _centerX,
                    double _centerY, double _centerZ, NDTCell* cellPrototype ) {
...
LazyGrid::~LazyGrid() {
    if ( initialized ) {
        ...
        for ( int i = 0; i < sizeX; i++ ) {
            for ( int j = 0; j < sizeY; j++ ) { delete[] dataArray[i][j]; }
            delete[] dataArray[i];
        }
        delete[] dataArray;
...
void LazyGrid::getCellAtAllocate( const pcl::PointXYZ& pt, NDTCell*& cell ) {
    ...
    if ( std::isnan( point.x ) || std::isnan( point.y ) || std::isnan( point.z ) ) { return; }
    int indX, indY, indZ;
    this->getIndexForPoint( point, indX, indY, indZ );
    pcl::PointXYZ centerCell;
...
NDTCell* LazyGrid::addPoint( const pcl::PointXYZ& point_c ) {
    ...
    if ( cell != NULL ) cell->addPoint( point_c );
    return cell;
}
...
SpatialIndex* LazyGrid::copy() const {
    ...
    while ( it != activeCells.end() ) {
        NDTCell* r = ( *it );
        if ( r == NULL ) continue;
        for ( unsigned int i = 0; i < r->points_.size(); i++ ) { ret->addPoint( r->points_[i] ); }
        it++;
    }
    return ret;
}

void LazyGrid::getNeighbors( const pcl::PointXYZ& point, const double& radius,
...
void LazyGrid::getNeighborsShared( const pcl::PointXYZ& point, const double& radius,
...
std::vector< NDTCell* > LazyGrid::getClosestNDTCells( const pcl::PointXYZ& point, int& n_neigh,
...
std::vector< boost::shared_ptr< NDTCell > > LazyGrid::getClosestNDTCellsShared(
    const pcl::PointXYZ& point, int& n_neigh, bool checkForGaussian ) {
    int indXn, indYn, indZn;
    int indX, indY, indZ;
    this->getIndexForPoint( point, indX, indY, indZ );
    std::vector< boost::shared_ptr< NDTCell > > cells;
...
bool LazyGrid::traceLine( const Eigen::Vector3d& origin, const pcl::PointXYZ& endpoint,
                          const Eigen::Vector3d& diff_, const double& maxz,
                          std::vector< NDTCell* >& cells ) {
...
bool LazyGrid::traceLineWithEndpoint( const Eigen::Vector3d& origin, const pcl::PointXYZ& endpoint,
                                      const Eigen::Vector3d& diff_, const double& maxz,
                                      std::vector< NDTCell* >& cells,
    ...
    if ( N == 0 ) {
        ...
        if ( ptCell != NULL ) { cells.push_back( ptCell ); }
        return true;
    }
