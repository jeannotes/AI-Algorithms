...
void NDTMapHMT::initializeGrids() {
    ...
    std::cout << "inti grids: res=" << resolution << " cen " << centerX << " " << centerY << " "
              << centerZ << " size " << sizeX << " " << sizeY << " " << sizeZ << std::endl;
...
bool NDTMapHMT::tryLoadPosition( const Eigen::Vector3d& newPos ) {
    ...
    if ( getline( &line, &len, meta_f ) > 0 ) {
        ...
        if ( strncmp( tk, "VERSION", 7 ) == 0 ) {
            ...
            if ( strncmp( tk, "2.0", 3 ) == 0 ) {
                if ( !getline( &line, &len, meta_f ) > 0 ) { return false; }
                tk = strtok( line, " " );
                if ( tk == NULL ) return false;
                ...
                if ( fabsf( sizeMeta - sizeX ) > 0.01 ) {
                    std::cerr << "cannot load map, different grid size used... reverting to empty "
                                 "map\n";
                    return false;
                }
            }
    ...
    while ( getline( &line, &len, meta_f ) > 0 ) {
        ...
        if ( fabsf( newPos( 0 ) - cen.x ) < sizeX / 2 && fabsf( newPos( 1 ) - cen.y ) < sizeY / 2 &&
             fabsf( newPos( 2 ) - cen.z ) < sizeZ / 2 ) {
            found   = true;
            centerX = cen.x;
            centerY = cen.y;
            centerZ = cen.z;
...
void NDTMapHMT::setInsertPosition( const Eigen::Vector3d& newPos ) {
    ...
    for ( int i = 0; i < 3; i++ ) {
        for ( int j = 0; j < 3; j++ ) { copy[i][j] = false; }
    }
    int oi, oj;
    for ( int i = 0; i < 3; i++ ) {
        for ( int j = 0; j < 3; j++ ) {
 ...
 */
void NDTMapHMT::addPointCloud( const Eigen::Vector3d& origin,
    ...
    while ( it != pc.points.end() ) {
        ...
        if ( ( diff2 ).norm() > eps ) {
            ...
            int i = 0, j = 0;
            if ( it->x > centerX + sizeXmeters / 2 - resolution / 2 ) { i = 1; }
            if ( it->x < centerX - sizeXmeters / 2 - resolution / 2 ) { i = -1; }
            if ( it->y > centerY + sizeYmeters / 2 - resolution / 2 ) { j = 1; }
            if ( it->y < centerY - sizeYmeters / 2 - resolution / 2 ) { j = -1; }
            std::vector< NDTCell* > cells2;
            if ( !grid_[i + 1][j + 1]->traceLineWithEndpoint( pf, *it, diff2, maxz, cells, pf ) ) {
                std::cerr << "ray outside central map: at " << it->x << " " << it->y << " " << it->z
                          << " with diff " << diff.transpose() << " starts at "
                          << origin.transpose() << " stops at " << pf.transpose() << " diff2 is "
                          << diff2.transpose() << " check grid " << i << " " << j << std::endl;
                it++;
                continue;
            }
...
void NDTMapHMT::addPointCloudMeanUpdate( const Eigen::Vector3d& origin,
                                         ...
                                         unsigned int maxnumpoints, float occupancy_limit,
                                         double maxz, double sensor_noise ) {
    // std::cout<<localmapsize.transpose()<<" "<<maxnumpoints<<" "<<occupancy_limit<<" "<<maxz<<"
    // "<<sensor_noise<<std::endl;
    ...
    for ( unsigned int it = 0; it < ndts.size(); it++ ) {
        ...
        if ( ( diff2 ).norm() > eps ) {
            // line continues outside center map, need to trace more!
            int i = 0, j = 0;
            if ( mpoint.x > centerX + sizeXmeters / 2 - resolution / 2 ) { i = 1; }
            if ( mpoint.x < centerX - sizeXmeters / 2 - resolution / 2 ) { i = -1; }
            if ( mpoint.y > centerY + sizeYmeters / 2 - resolution / 2 ) { j = 1; }
            if ( mpoint.y < centerY - sizeYmeters / 2 - resolution / 2 ) { j = -1; }
            std::vector< NDTCell* > cells2;
            if ( !grid_[i + 1][j + 1]->traceLineWithEndpoint( pf, mpoint, diff2, maxz, cells,
                                                              pf ) ) {
                std::cerr << "ray outside central map: at " << mpoint.x << " " << mpoint.y << " "
                          << mpoint.z << " with diff " << diff.transpose() << " starts at "
                          << origin.transpose() << " stops at " << pf.transpose() << " diff2 is "
                          << diff2.transpose() << " check grid " << i << " " << j << std::endl;
                it++;
                continue;
 ...
 */
void NDTMapHMT::addDistributionToCell( const Eigen::Matrix3d& ucov, const Eigen::Vector3d& umean,
                                       unsigned int numpointsindistribution, float r, float g,
                                       float b, unsigned int maxnumpoints, float max_occupancy ) {
    pcl::PointXYZ pt;
    pt.x = umean[0];
    pt.y = umean[1];
    pt.z = umean[2];
    ...
    int i = 0, j = 0;
    if ( pt.x > centerX + sizeXmeters / 2 - resolution / 2 ) { i = 1; }
    if ( pt.x < centerX - sizeXmeters / 2 - resolution / 2 ) { i = -1; }
    if ( pt.y > centerY + sizeYmeters / 2 - resolution / 2 ) { j = 1; }
    if ( pt.y < centerY - sizeYmeters / 2 - resolution / 2 ) { j = -1; }
 ...
 */
void NDTMapHMT::computeNDTCells( int cellupdatemode, unsigned int maxnumpoints,
                                 float occupancy_limit, Eigen::Vector3d origin,
                                 double sensor_noise ) {
    ...
    while ( getline( &line, &len, meta_f ) > 0 ) {
        ...
        for ( int i = 0; i < 3; i++ ) {
            for ( int j = 0; j < 3; j++ ) {
                if ( grid_[i][j] == NULL ) return -1;
                grid_[i][j]->getCenter( centx, centy, centz );
                if ( fabsf( cen.x - centx ) < epsilon && fabsf( cen.y - centy ) < epsilon &&
                     fabsf( cen.z - centz ) < epsilon ) {
                    already[i][j]              = true;
...
bool NDTMapHMT::tryLoad( const double& cx, const double& cy, const double& cz, LazyGrid*& grid ) {
    ...
    while ( getline( &line, &len, meta_f ) > 0 ) {
        ...
        if ( fabsf( cen.x - cx ) < epsilon && fabsf( cen.y - cy ) < epsilon &&
             fabsf( cen.z - cz ) < epsilon ) {
            token[strlen( token ) - 1] = '\0';
            jffname                    = token;
            found                      = true;
            // std::cerr<<"file already for "<<jffname<<std::endl;
...
std::vector< NDTCell* > NDTMapHMT::getCellsForPoint( const pcl::PointXYZ pt, int n_neigh,
...
bool NDTMapHMT::getCellForPoint( const pcl::PointXYZ& pt, NDTCell*& out_cell,
                                 bool checkForGaussian ) const {
    out_cell = NULL;
...
std::vector< NDTCell* >
NDTMapHMT::pseudoTransformNDT( Eigen::Transform< double, 3, Eigen::Affine, Eigen::ColMajor > T ) {
