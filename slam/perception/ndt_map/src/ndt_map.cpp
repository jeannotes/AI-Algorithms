...
* \note every subsequent call will destroy the previous map!
*/
void NDTMap::loadPointCloudCentroid( const pcl::PointCloud< pcl::PointXYZ >& pc,
                                     const Eigen::Vector3d& origin,
                                     const Eigen::Vector3d& old_centroid,
                                     const Eigen::Vector3d& map_size, double range_limit ) {
    if ( index_ != NULL ) {
        SpatialIndex* si = index_->clone();
    ...
    while ( it != pc.points.end() ) {
        ...
        // }
        {
            ...
            // //          std::cout << "gridsize : " << indX << "," << indY << "," << indZ <<
            // std::endl;
        }
#endif
        it++;
...
*/
void NDTMap::addDistributionToCell( const Eigen::Matrix3d& ucov, const Eigen::Vector3d& umean,
                                    unsigned int numpointsindistribution, float r, float g, float b,
                                    unsigned int maxnumpoints, float max_occupancy ) {
    pcl::PointXYZ pt;
    pt.x         = umean[0];
    pt.y         = umean[1];
    pt.z         = umean[2];
    ...
    if ( ptCell != NULL ) {
        ...
        ptCell->updateSampleVariance( ucov, umean, numpointsindistribution, true, max_occupancy,
                                      maxnumpoints );
        ptCell->setRGB( r, g, b );
        //	std::cout<<"AFTER\n";
        //	std::cout<<ptCell->getMean().transpose()<<std::endl;
        ...
        //      std::cerr << "addDistributionToCell: failed to get a cell to add the distribution
        //      to" << std::endl;
    }
}
...
void NDTMap::addPointCloud( const Eigen::Vector3d& origin,
                            const pcl::PointCloud< pcl::PointXYZ >& pc, double classifierTh,
                            double maxz, double sensor_noise, double occupancy_limit ) {
        ...
        for ( unsigned int i = 0; i < cells.size(); i++ ) {
            ...
            if ( ptCell != NULL ) {
                ...
                if ( ptCell->hasGaussian_ ) {
                    ...
                    double thr    = exp( -0.5 * ( l2target * l2target ) /
                                      ( snoise * snoise ) );   /// This is the probability of max
                                                               /// lik point being endpoint
                    lik *= ( 1.0 - thr );
                    if ( lik < 0.3 ) continue;
        ...
        for ( int i = 0; i < N - 1; i++ ) {
            ...
            if ( ptCell != NULL ) {
                ...
                if ( ptCell->hasGaussian_ ) {
                    ...
                    ptCell->updateEmpty( -0.2, l2target );   /// The cell does not have gaussian, so
                                                             /// we mark that we saw it empty...
                }
            } else {
...
void NDTMap::addPointCloudMeanUpdate( const Eigen::Vector3d& origin,
                                      ...
                                      unsigned int maxnumpoints, float occupancy_limit, double maxz,
                                      double sensor_noise ) {
    if ( isFirstLoad_ ) {
    ...
    ndlocal.loadPointCloudCentroid( pc, origin, old_centroid, localmapsize,
                                    /*70.0*/ -1. );   // No range limit used here, assume that the
                                                      // point cloud is already filtered.
    ...
    for ( unsigned int it = 0; it < ndts.size(); it++ ) {
        if ( ndts[it] == NULL ) {
            fprintf( stderr, "NDTMap::addPointCloudMeanUpdate::GOT NULL FROM MAP -- SHOULD NEVER "
                             "HAPPEN!!!\n" );
            continue;
        }

        if ( !ndts[it]->hasGaussian_ ) {
            fprintf( stderr, "NDTMap::addPointCloudMeanUpdate::NO GAUSSIAN!!!! -- SHOULD NEVER "
                             "HAPPEN!!!\n" );
            continue;
        }
        ...
        if ( m( 2 ) > maxz ) {
            num_high++;
            updatePositive =
                false;   /// Lets update negative even though the measurement was too high
        }
        ...
        for ( int i = 0; i < NN - 2; i++ ) {
            ...
            if ( ptCell != NULL ) {
                ...
                if ( ptCell->hasGaussian_ ) {
                    ...
                    ptCell->updateOccupancy( -0.85 * numpoints,
                                             occupancy_limit );   /// The cell does not have
                                                                  /// gaussian, so we mark that we
                                                                  /// saw it empty...
                    // ptCell->updateEmpty(-0.2*numpoints,l2target); ///The cell does not have
                    // gaussian, so we mark that we saw it empty...
                }
            } else {
                ptCell = dynamic_cast< NDTCell* >(
                    index_->addPoint( pt ) );   /// Add fake point to initialize!
            }
        }
        if ( updatePositive ) {
            ...
            addDistributionToCell(
                ucov, m, numpoints, r, g, b, maxnumpoints,
                occupancy_limit );   ///<FIXME:: local implementation can be faster?
        }
    }
 ...
 */
bool NDTMap::addMeasurement( const Eigen::Vector3d& origin, pcl::PointXYZ endpoint,
                             double classifierTh, double maxz, double sensor_noise ) {
    ...
    bool updatePositive = true;
    if ( endpoint.z > maxz ) { return false; }
    ...
    for ( int i = 0; i < NN - 2; i++ ) {
        ...
        if ( ptCell != NULL ) {
            ...
            if ( ptCell->hasGaussian_ ) {
                ...
                double snoise = sigma_dist + sensor_noise;
                double thr    = exp( -0.5 * ( l2target * l2target ) /
                                  ( snoise * snoise ) );   /// This is the probability of max lik
                                                           /// point being endpoint
                lik *= ( 1.0 - thr );
                lik              = 0.2 * lik + 0.5;   /// Evidence value for empty - alpha * p(x);
                double logoddlik = log( ( 1.0 - lik ) / ( lik ) );
                ptCell->updateEmpty( logoddlik, l2target );
                ...
                ptCell->updateEmpty( -0.1, l2target );   /// The cell does not have gaussian, so we
                                                         /// mark that we saw it empty...
            }
        } else {
            index_->addPoint( pt );   /// Add fake point to initialize!
...
double NDTMap::getDepth( Eigen::Vector3d origin, Eigen::Vector3d dir, double maxDepth ) {
    ...
    if ( !lz->traceLine( origin, endP, diff, 1000.0, cells ) ) { return maxDepth + 1.0; }
...
double NDTMap::getDepthSmooth( Eigen::Vector3d origin, Eigen::Vector3d dir, double maxDepth,
                               int n_neigh, double weight, double threshold,
                               Eigen::Vector3d* hit ) {
    Eigen::Vector3d ray_endpos = origin + dir * maxDepth;
...
void NDTMap::loadPointCloud( const pcl::PointCloud< pcl::PointXYZ >& pc,
                             const std::vector< std::vector< size_t > >& indices ) {
    //    loadPointCloud(pc);
    // Specific function related to CellVector
    CellVector* cl = dynamic_cast< CellVector* >( index_ );
    if ( cl != NULL ) {
        for ( size_t i = 0; i < indices.size(); i++ ) { cl->addCellPoints( pc, indices[i] ); }
...
void NDTMap::loadDepthImage( const cv::Mat& depthImage,
...
pcl::PointCloud< pcl::PointXYZ > NDTMap::loadDepthImageFeatures(
    const cv::Mat& depthImage, std::vector< cv::KeyPoint >& keypoints, size_t& supportSize,
    double maxVar, DepthCamera< pcl::PointXYZ >& cameraParams, bool estimateParamsDI,
    bool nonMean ) {
...
void NDTMap::computeMaximumLikelihoodPointCloudWithRangePairs(
    const Eigen::Vector3d& origin, const pcl::PointCloud< pcl::PointXYZ >& pc,
    const Eigen::Vector3d& virtualOrigin, pcl::PointCloud< pcl::PointXYZ >& pc_out,
    std::vector< std::pair< double, double > >& ranges, double max_range = 130. ) const {
    pc_out.clear();
    if ( isFirstLoad_ || index_ == NULL ) { return; }
    pcl::PointCloud< pcl::PointXYZ >::const_iterator it = pc.points.begin();
        ...
        for ( unsigned int i = 0; i < cells.size(); i++ ) {
            ...
            if ( ptCell != NULL ) {
                ...
                if ( ptCell->hasGaussian_ ) {
                    ...
                    if ( lik > 0. ) {
                        ranges.push_back(
                            std::pair< double, double >( l, ( out - origin ).norm() ) );
...
void NDTMap::computeMaximumLikelihoodRanges( const Eigen::Vector3d& origin,
    ...
    for ( unsigned int i = 0; i < dirs.size(); i++ ) {
        ...
        // Eigen::Vector3d p2 = origin+(rawRanges[i]+resolution)*dirs[i];
        if ( !lz->traceLine( p1, p2, dirs[i] * 100., 1000.0, cells ) ) { continue; }
...
*/
void NDTMap::computeNDTCells( int cellupdatemode, unsigned int maxnumpoints, float occupancy_limit,
                              Eigen::Vector3d origin, double sensor_noise ) {
    ...
    while ( it != update_set.end() ) {
        NDTCell* cell = *it;
        if ( cell != NULL ) {
            cell->computeGaussian( cellupdatemode, maxnumpoints, occupancy_limit, origin,
                                   sensor_noise );
...
void NDTMap::computeNDTCellsSimple( bool keepPoints ) {
    ...
    while ( it != index_->end() ) {
        ...
        if ( cell != NULL ) {
            ...
            if ( !keepPoints ) { cell->points_.clear(); }
        } else {
        }
        it++;
    }
...
int NDTMap::writeToJFF( const char* filename ) {
    ...
    switch ( this->getMyIndexInt() ) {
        case 1: writeCellVectorJFF( jffout ); break;
        case 2:
            // writeOctTreeJFF(jffout);
            break;
        case 3: writeLazyGridJFF( jffout ); break;
        default:
...
int NDTMap::loadFromJFF( const char* filename ) {
    ...
    if ( filename == NULL ) { JFFERR( "problem outputing to jff" ); }
    ...
    int indexType;
    if ( fread( &indexType, sizeof( int ), 1, jffin ) <= 0 ) { JFFERR( "reading version failed" ); }
    ...
    switch ( indexType ) {
        case 1: {
            CellVector* cv = dynamic_cast< CellVector* >( index_ );
            if ( cv->loadFromJFF( jffin ) < 0 ) { JFFERR( "Error loading CellVector" ); }
            break;
        }
#if 0
    case 2: {
        ...
        case 3: {
            ...
            if ( gr->loadFromJFF( jffin ) < 0 ) { JFFERR( "Error loading LazyGrid" ); }
            break;
        }
        default: JFFERR( "error casting index" );
    }
...
std::string NDTMap::getMyIndexStr() const {
    CellVector* cl = dynamic_cast< CellVector* >( index_ );
    if ( cl != NULL ) { return std::string( "CellVector" ); }
#if 0
...
#endif
    LazyGrid* gr = dynamic_cast< LazyGrid* >( index_ );
    if ( gr != NULL ) { return std::string( "LazyGrid<PointT>" ); }
...
std::vector< NDTCell* > NDTMap::getCellsForPoint( const pcl::PointXYZ pt, int n_neigh,
...
NDTCell* NDTMap::getCellIdx( unsigned int idx ) const {
    CellVector* cl = dynamic_cast< CellVector* >( index_ );
    if ( cl != NULL ) { return cl->getCellIdx( idx ); }
    return NULL;
}
...
NDTMap::getAllInitializedCellsShared() const {
    std::vector< boost::shared_ptr< NDTCell > > ret;
    typename SpatialIndex::CellVectorItr it = index_->begin();
    while ( it != index_->end() ) {
        NDTCell* nd = ( *it )->copy();
...
std::string NDTMap::ToString() {
    std::stringstream ss;
    if ( index_ != NULL ) ss << "NDTMap: index= \n" << index_->ToString() << std::endl;
    return ss.str();
}
}
