...
CellVector::~CellVector() {
    ...
    for ( unsigned int i = 0; i < activeCells.size(); ++i ) {
        if ( activeCells[i] != NULL ) { delete activeCells[i]; }
    }
...
void CellVector::addCellPoints( pcl::PointCloud< pcl::PointXYZ > pc,
...
void CellVector::getNeighbors( const pcl::PointXYZ& point, const double& radius,
    ...
    if ( treeUpdated ) {
        ...
        for ( int i = 0; i < NCELLS; i++ ) {
            ...
            if ( tmp != NULL ) cells.push_back( tmp );
        }
    } else {
        ...
        while ( it != this->end() ) {
            float tmp = pcl::squaredEuclideanDistance( ( *it )->getCenter(), point );
            if ( tmp < radius_sqr ) { cells.push_back( *it ); }
        }
    }
}
...
void CellVector::setCellType( NDTCell* type ) {
    if ( type != NULL ) { protoType = type->clone(); }
}
...
std::vector< NDTCell* > CellVector::getClosestNDTCells( const pcl::PointXYZ& point,
                                                        double& radius ) {
...
NDTCell* CellVector::getCellIdx( unsigned int idx ) const {
    if ( idx >= activeCells.size() ) return NULL;
    ...
    while ( it != this->end() ) {
        ...
        if ( ndcell != NULL ) {
            if ( ndcell->hasGaussian_ ) {
                ...
                // std::cout<<"rem cell at "<<ndcell->getMean().transpose()<<" evals are
