...
class NDTCalibScan {
  public:
    ...
    Eigen::Affine3d pose;   // estimated pose in global frame (same as originalPose) but with a
                            // corrected pose based on a correction time_offset factor, this
                            // variable is special since this could be changed in the optimization
                            // wheras the others are constants

    Eigen::Affine3d originalPose;    // estimated pose in global frame (preferably from a GT
                                     // system)...
    Eigen::Affine3d estSensorPose;   // estimated sensor pose in global frame (typically from a SLAM
                                     // / registration system)
    ...
    NDTCalibScan( const Eigen::Affine3d& _pose, const Eigen::Affine3d& _estSensorPose,
                  double _stamp )
        : pose( _pose ),
          originalPose( _pose ),
          estSensorPose( _estSensorPose ),
          stamp( _stamp ),
          ndtmap( NULL ) {}

    NDTCalibScan( const pcl::PointCloud< pcl::PointXYZ >& _cloud, const Eigen::Affine3d& _pose,
                  double _stamp )
        : cloud( _cloud ), pose( _pose ), originalPose( _pose ), stamp( _stamp ), ndtmap( NULL ) {}

    NDTCalibScan( const pcl::PointCloud< pcl::PointXYZ >& _cloud, const Eigen::Affine3d& _pose,
                  const Eigen::Affine3d& _estSensorPose, double _stamp )
        : cloud( _cloud ),
          pose( _pose ),
          originalPose( _pose ),
          estSensorPose( _estSensorPose ),
    ...
    void computeNDTMap( double resolution ) {
        lazygrid = boost::shared_ptr< perception_oru::LazyGrid >(
            new perception_oru::LazyGrid( resolution ) );
        ndtmap = boost::shared_ptr< perception_oru::NDTMap >(
            new perception_oru::NDTMap( lazygrid.get(), false ) );
        ndtmap->loadPointCloud( this->cloud );
        ndtmap->computeNDTCellsSimple();
    }

    void appendPointCloudGlobal( const Eigen::Affine3d& Ts,
                                 pcl::PointCloud< pcl::PointXYZ >& p ) const {
    ...
    // Return Ts from the global estimate sensor pose and the vehicle pose (both in the same global
    // reference frame).
    Eigen::Affine3d getTs() const { return ( pose.inverse() * estSensorPose ); }
    ...
    Eigen::Affine3d getSensorPoseFromPose( const Eigen::Affine3d& Ts ) const { return pose * Ts; }
};
...
class NDTCalibScanPair {
  public:
    ...
    // Compute the score for a relative sensor pose offset - that is the pose and the est sensor
    // pose don't have to be in the same coordinate frame.
    ...
    double scoreICP( const Eigen::Affine3d& Ts ) const {
        ...
        pcl::KdTreeFLANN< pcl::PointXYZ > kdtree;   // TODO, compute the KD-tree once to gain some
                                                    // more speed.
        typename pcl::KdTree< pcl::PointXYZ >::PointCloudPtr mp(
            new pcl::PointCloud< pcl::PointXYZ > );
        if ( c1.size() == 0 ) { std::cerr << " Check -> num points = " << c1.size() << std::endl; }
        ( *mp ) = c1;
        ...
        for ( unsigned int i = 0; i < c2.size(); i++ ) {
            if ( kdtree.nearestKSearch( c2[i], K, pointIdxNKNSearch, pointNKNSquaredDistance ) >
                 0 ) {
    ...
    void computeNDTMap( double resolution ) {
        if ( first.ndtmap == NULL ) { first.computeNDTMap( resolution ); }
        if ( second.ndtmap == NULL ) { second.computeNDTMap( resolution ); }
    }
    ...
    double scoreNDT( const Eigen::Affine3d& Ts ) const {
        ...
        for ( size_t i = 0; i < m2.size(); i++ ) { delete m2[i]; }
        return score;
    }
};
...
class NDTCalibScanPairs : public std::vector< NDTCalibScanPair > {
  public:
    ...
    void computeNDTMap( double resolution ) {
        for ( size_t i = 0; i < this->size(); i++ ) { ( *this )[i].computeNDTMap( resolution ); }
    }
};

class NDTCalibOptimize {
  public:
    enum ScoreType { SCORE_ICP = 0, SCORE_EST_SENSOR_POSE, SCORE_EST_SENSOR_POSE_REL, SCORE_NDT };
    ...
    class ObjectiveType : public std::vector< bool > {
      public:
        ...
        bool optimizeTime() const { return ( *this )[6]; }
        ...
        void getInitialActiveParameters( const Eigen::VectorXd& initParams,
        ...
        // Given the initial parameters, the active parameters, return a merged version where all
        // active perameters has overwritten the initial ones.
        void getAllParameters( const Eigen::VectorXd& initParams,
                               const Eigen::VectorXd& activeParams, Eigen::VectorXd& params ) {
    ...
    NDTCalibOptimize( NDTCalibScanPairs& pairs, int scoreType, ObjectiveType objectiveType,
                      PoseInterpolationInteface /*PoseInterpolationNavMsgsOdo*/& poseInterp,
                      const std::string& poseFrameId )
        : _pairs( pairs ),
          _scoreType( scoreType ),
          _objectiveType( objectiveType ),
          _poseInterp( poseInterp ),
          _poseFrameId( poseFrameId ) {}

    // Main function to call. Provide the initial sensor pose and time offset. These will be updated
    // with the calibrated results.
    ...
    // Score value given a sensor pose (in order to get the score with another time offset use
    // interpPairPoses first).
...
void loadNDTCalibScanPairs( const std::string& gt_file, const std::string& est_sensorpose_file,
                            const std::string& base_name_pcd, NDTCalibScanPairs& scans,
                            double max_translation, double min_rotation, int index_offset = 0 ) {
    scans.resize( 0 );
    ...
    std::vector< Eigen::Affine3d > Test_sensorpose =
        ndt_generic::loadAffineFromEvalFile( est_sensorpose_file );
    ...
    // Need to find a pair of scans which is useful in the optimization scheme. Pair of scans with
    // limited translation but with some rotation...
    size_t j = 0;
    for ( size_t i = 1; i < Tgt.size(); i++ ) {
        ...
        if ( Tmotion.translation().norm() > max_translation ) {
            j = i;   // reset -> make this better?, could be that many potential pairs are left
                     // out..., add a while loop instead.
            continue;
        }
        Eigen::Vector3d rot = Tmotion.rotation().eulerAngles( 0, 1, 2 );
        normalizeEulerAngles( rot );
        if ( rot.norm() < min_rotation ) { continue; }
        ...
        ndt_generic::loadCloud(
            base_name_pcd, j + index_offset,
            scans.back().first.cloud );   // cloud1.pcd -> first line in the eval files...
