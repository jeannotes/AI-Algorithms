...
class NDTCalibScan {
  public:
    ...
    NDTCalibScan( const Eigen::Affine3d& _pose, const Eigen::Affine3d& _estSensorPose,
                  double _stamp )
        : pose( _pose ), estSensorPose( _estSensorPose ), stamp( _stamp ) {}

    NDTCalibScan( const pcl::PointCloud< pcl::PointXYZ >& _cloud, const Eigen::Affine3d& _pose,
                  double _stamp )
        : cloud( _cloud ), pose( _pose ), stamp( _stamp ) {}

    NDTCalibScan( const pcl::PointCloud< pcl::PointXYZ >& _cloud, const Eigen::Affine3d& _pose,
                  const Eigen::Affine3d& _estSensorPose, double _stamp )
        : cloud( _cloud ), pose( _pose ), estSensorPose( _estSensorPose ), stamp( _stamp ) {}
...
class NDTCalibScanPair {
  public:
    ...
    double scoreICP( const Eigen::Affine3d& Ts ) const {
        ...
        if ( c1.size() == 0 ) { fprintf( stderr, "Check -> num points = %d\n", (int) c1.size() ); }
...
void loadNDTCalibScanPairs( const std::string& gt_file, const std::string& est_sensorpose_file,
                            ...
                            std::vector< NDTCalibScanPair >& scans ) {
    scans.resize( 0 );
    ...
    for ( size_t i = 1; i < Tgt.size(); i++ ) {
        ...
        if ( Tmotion.rotation().eulerAngles( 0, 1, 2 ).norm() < 5.0 * M_PI / 180.0 ) { continue; }
...
visualization_msgs::Marker getMarkerPoseFromNDTCalibScan( const NDTCalibScan& scan, int id,
                                                          int color ) {
    return getMarkerArrowAffine3d( scan.pose, id, color, "ndt_calib_pose" );
}

visualization_msgs::Marker getMarkerEstSensorPoseFromNDTCalibScan( const NDTCalibScan& scan, int id,
                                                                   int color ) {
    return getMarkerArrowAffine3d( scan.estSensorPose, id, color, "ndt_calib_est_sensor_pose" );
}

visualization_msgs::Marker getCorrMarkerPoseToEstSensorPose( const NDTCalibScan& scan, int id,
                                                             int color ) {
    visualization_msgs::Marker m;
    assignDefault( m );
    assignColor( m, color );
...
getMarkerArrayFromNDTCalibScanPairs( const std::vector< NDTCalibScanPair >& pairs ) {
    visualization_msgs::MarkerArray m;
    ...
    for ( size_t i = 0; i < pairs.size(); i++ ) {
        ...
        m.markers.push_back(
            getMarkerEstSensorPoseFromNDTCalibScan( pairs[i].second, 2 * i + 1, 2 ) );
        m.markers.push_back( getCorrMarkerPoseToEstSensorPose( pairs[i].first, i, 0 ) );
    }
    return m;
}

visualization_msgs::MarkerArray getMarkerArrayRelFromNDTCalibScanPair( const NDTCalibScanPair& pair,
    ...
    appendMarkerArray( m, getMarkerFrameAffine3d( pair.getRelativeEstSensorPose(),
                                                  std::string( "ndt_calib_rel_est_sensor_pose" ),
                                                  1.0, 0.1 ) );
    appendMarkerArray( m, getMarkerFrameAffine3d(
                              pair.getPredictedRelativeEstSensorPose( Ts ),
                              std::string( "ndt_calib_pred_rel_est_sensor_pose" ), 1.0, 0.1 ) );

    appendMarkerArray( m, getMarkerFrameAffine3d(
                              pair.first.getSensorPoseFromPose( Ts ),
                              std::string( "ndt_calib_sensor_pose_from_first_pose" ), 1., 0.1 ) );
    appendMarkerArray( m, getMarkerFrameAffine3d(
                              pair.first.getSensorPoseFromPose( Ts ),
                              std::string( "ndt_calib_sensor_pose_from_second_pose" ), 1., 0.1 ) );
