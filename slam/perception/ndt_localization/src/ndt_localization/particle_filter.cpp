...
perception_oru::particle_filter::particle_filter(
    perception_oru::NDTMap* ndtMap_, int particleCount_ /*, init_type initializationType_*/,
    bool be2D_, bool forceSIR_, double varLimit_, int sirCount_ ) {
    be2D   = be2D_;
    ndtMap = ndtMap_;
...
void perception_oru::particle_filter::InitializeNormal( double x, double y, double th,
                                                        double var ) {
    ...
    for ( int parNo = 0; parNo < particleCount; parNo++ ) {
        ...
        particleCloud.emplace_back( 0.0, 0.0, distribution_t( generator ),
                                    distribution_x( generator ), distribution_y( generator ), 0.0 );
    }
}
...
void perception_oru::particle_filter::GetPoseMeanAndVariance2D( Eigen::Vector3d& mean,
                                                                Eigen::Matrix3d& cov ) {
    double sumX = 0, sumY = 0;
    double sumW = 0;
    double ax = 0, ay = 0;
    ...
    cov << wc * xx, wc * xy, 0, wc * xy, wc * yy, 0, 0, 0, atan2( wc * aay, wc * aax );
}
...
void perception_oru::particle_filter::InitializeUniformMap() {
    ...
    for ( int cInd = 0; cInd < allCells.size(); cInd++ )
        if ( allCells[cInd]->getOccupancy() < 0.0 ) cells.push_back( allCells[cInd] );
...
void perception_oru::particle_filter::UpdateAndPredict( Eigen::Affine3d tMotion,
                                                        perception_oru::NDTMap ndtLocalMap_ ) {
    Eigen::Vector3d tr  = tMotion.translation();
    Eigen::Vector3d rot = tMotion.rotation().eulerAngles( 0, 1, 2 );

    if ( tr[0] != 0.0 && tr[1] != 0.0 && rot[2] != 0 ) {
        Predict2D( tr[0], tr[1], rot[2], tr[0] * 0.03 + 0.005, tr[1] * 0.03 + 0.005,
                   rot[2] * 0.08 + 0.01 );
#pragma omp parallel for
        for ( int i = 0; i < particleCloud.size(); i++ ) {
            ...
            for ( unsigned int j = 0; j < ndts.size(); j++ ) delete ndts[j];
        }
        Normalize();
        if ( forceSIR )
            SIRUpdate();
...
void perception_oru::particle_filter::UpdateAndPredict( Eigen::Affine3d tMotion,
    ...
    if ( tr[0] != 0.0 && tr[1] != 0.0 && rot[2] != 0 ) {
        Predict2D( tr[0], tr[1], rot[2], tr[0] * 0.1 + 0.005, tr[1] * 0.1 + 0.005,
                   rot[2] * 0.1 + 0.001 );
#pragma omp parallel for
        for ( int i = 0; i < particleCloud.size(); i++ ) {
            Eigen::Affine3d T = particleCloud[i].GetAsAffine();
...
void perception_oru::particle_filter::UpdateAndPredictEff(
    Eigen::Affine3d tMotion, perception_oru::NDTMap* ndtLocalMap_, double subsample_level,
    double z_cut ) {   // you may add z cut here if necessarry
    ...
    if ( tr[0] != 0.0 && tr[1] != 0.0 && rot[2] != 0 ) {
        ...
        int subsample_cells;
        int used_cells = 0;
        Predict2D( tr[0], tr[1], rot[2], fabs( tr[0] ) * 0.50 + 0.001, fabs( tr[1] ) * 0.70 + 0.001,
                   fabs( rot[2] ) * 0.15 + 0.001 );   // this line!!!!!!!!!!!!!!!!!!!!!!!!!
        // Predict2D(tr[0], tr[1], rot[2], fabs(tr[0]) * 0.25 + 0.001, fabs(tr[1]) * 0.25 + 0.001,
        // fabs(rot[2]) * 0.15 + 0.001); //this line!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        if ( subsample_level != 1 ) {
            ...
            for ( int i = 0; i < ndts0.size(); ++i ) {
                ...
                if ( p < subsample_level && ndts0[i]->getMean()[2] > z_cut &&
                     ndts0[i]->getClass() != perception_oru::NDTCell::HORIZONTAL )
                ...
                if ( ndtMap->getCellAtPoint( p, cell ) ) {
                    ...
                    if ( cell->hasGaussian_ ) {
                        ...
                        Eigen::Matrix3d covCombined =
                            map_cov + T.rotation() * ndts[n]->getCov() * T.rotation().transpose();
                        Eigen::Matrix3d icov;
                        bool exists;
                        double det = 0;
                        covCombined.computeInverseAndDetWithCheck( icov, det, exists );
...
void perception_oru::particle_filter::UpdateAndPredictEffRe(
    Eigen::Affine3d tMotion, perception_oru::NDTMap* ndtLocalMap_, double subsample_level,
    double z_cut, double x_var, double y_var, double th_var, double r_x_var, double r_y_var,
    double r_th_var, int tres ) {
    if ( subsample_level < 0 || subsample_level > 1 ) subsample_level = 1;
    ...
    if ( tr[0] != 0.0 || tr[1] != 0.0 || rot[2] != 0 ) {
        ...
        int subsample_cells;
        int used_cells = 0;
        Predict2D( tr[0], tr[1], rot[2], fabs( tr[0] ) * x_var, fabs( tr[1] ) * y_var,
                   fabs( rot[2] ) * th_var );   // this line!!!!!!!!!!!!!!!!!!!!!!!!!
        std::vector< perception_oru::NDTCell* > ndts0 = ndtLocalMap_->getAllCells();
        std::vector< perception_oru::NDTCell* > ndts;
        if ( subsample_level != 1 ) {
            srand( time( NULL ) );
        ...
        for ( int i = 0; i < particleCloud.size(); i++ ) {
            ...
            if ( particleCloud[i].GetProbability() > tre ) {
                ...
                particleCloud[i].Set( 0.0, 0.0, distribution_t( generator ),
                                      distribution_x( generator ), distribution_y( generator ),
                                      0.0 );
...
void perception_oru::particle_filter::Predict2D( double x, double y, double th, double sx,
                                                 double sy, double sth ) {
...
void perception_oru::particle_filter::Normalize() {
    ...
    for ( i = 0; i < particleCloud.size(); i++ ) {
        particleCloud[i].SetProbability( particleCloud[i].GetProbability() *
                                         particleCloud[i].GetLikelihood() );
...
void perception_oru::particle_filter::GetRandomPoint( perception_oru::NDTCell* cell, double& x,
                                                      double& y, double& th ) {
...
void perception_oru::particle_filter::EigenSort( Eigen::Vector3d& eigenvalues,
    ...
    for ( i = 0; i < 2; i++ ) {
        p = eigenvalues( k = i );
        for ( j                                            = i + 1; j < 3; j++ )
            if ( fabs( eigenvalues( j ) ) >= fabs( p ) ) p = eigenvalues( k = j );
        if ( k != i ) {
