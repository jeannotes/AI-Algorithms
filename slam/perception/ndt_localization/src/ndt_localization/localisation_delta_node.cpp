...
class delta {
  ...
  public:
    delta( ros::NodeHandle parameters ) {
        ...
        while ( true ) {
            tf_listener_mcl.waitForTransform( "/map", "/base_link", ros::Time( 0 ),
                                              ros::Duration( 0.1 ) );
            try {
                tf_listener_mcl.lookupTransform( "/map", "/base_link", ros::Time( 0 ),
                                                 transform_mcl );
                mcl_th = tf::getYaw( transform.getRotation() );
                mcl_x  = transform.getOrigin().x();
                mcl_y  = transform.getOrigin().y();
                tf_listener_odom.waitForTransform( "/odom", "/base_link", ros::Time( 0 ),
                                                   ros::Duration( 0.1 ) );
                try {
                    tf_listener_mcl.lookupTransform( "/map", "/base_link", ros::Time( 0 ),
                                                     transform_mcl );
