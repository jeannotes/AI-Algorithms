...
typedef message_filters::sync_policies::ApproximateTime< sensor_msgs::LaserScan,
                                                         nav_msgs::Odometry >
    LaserOdomSync;
typedef message_filters::sync_policies::ApproximateTime< velodyne_msgs::VelodyneScan,
                                                         nav_msgs::Odometry >
    VeloOdomSync;
...
class particle_filter_wrap {
    ...
    nav_msgs::Odometry Pose2DToMsg( Eigen::Vector3d mean, Eigen::Matrix3d cov, ros::Time ts,
                                    Eigen::Affine3d Todometry ) {
        nav_msgs::Odometry O;
        static int seq = 0;
        ...
        tf_map_odo.setOrigin(
            tf::Vector3( Tmap_odo.translation()( 0 ), Tmap_odo.translation()( 1 ), 0.0 ) );
    ...
    int LoadMap() {
        ...
        if ( ndtMap->loadFromJFF( mapFile.c_str() ) < 0 ) return -1;
        return 0;
    }
    ...
    void VelodyneOdomCallback( const velodyne_msgs::VelodyneScan::ConstPtr& msg,
    ...
    void PCOdomCallback( const sensor_msgs::PointCloud2::ConstPtr& msg,
                         const nav_msgs::Odometry::ConstPtr& odo_in ) {
    ...
    void LaserOdomCallback( const sensor_msgs::LaserScan::ConstPtr& msg,
                            const nav_msgs::Odometry::ConstPtr& odo_in ) {
    ...
    void processFrame( pcl::PointCloud< pcl::PointXYZ >& cloud,
                       const nav_msgs::Odometry::ConstPtr& odo_in, ros::Time ts ) {
        if ( !initialized ) {
            initialized = true;
            MCL->InitializeNormal( initial_pose.position.x, initial_pose.position.y,
                                   tf::getYaw( initial_pose.orientation ), initVar );
        ...
        transform_2.rotate(
            Eigen::AngleAxisf( sensorOffsetR, Eigen::Vector3f::UnitX() ) *
        ...
        perception_oru::NDTMap* localMap =
            new perception_oru::NDTMap( new perception_oru::LazyGrid( resolution ) );
        ...
        delete localMap;
        if ( showParticles ) particlePub.publish( ParticlesToMsg( MCL->particleCloud ) );
        Eigen::Matrix3d cov;
        Eigen::Vector3d mean;
  ...
  public:
    particle_filter_wrap( ros::NodeHandle param ) {
        ...
        if ( beVelodyne ) {
            vel_sub_ = new message_filters::Subscriber< velodyne_msgs::VelodyneScan >(
                nh, inputTopicName, 1 );
            odom_sub_ =
                new message_filters::Subscriber< nav_msgs::Odometry >( nh, odomTopicName, 10 );
            sync_vo_ = new message_filters::Synchronizer< VeloOdomSync >( VeloOdomSync( 20 ),
                                                                          *vel_sub_, *odom_sub_ );
            sync_vo_->registerCallback(
                boost::bind( &particle_filter_wrap::VelodyneOdomCallback, this, _1, _2 ) );
        }
        ...
        if ( bePC ) {
            ...
            sync_po_ = new message_filters::Synchronizer< PointsOdomSync >(
                PointsOdomSync( 20 ), *points2_sub_, *odom_sub_ );
            sync_po_->registerCallback(
                boost::bind( &particle_filter_wrap::PCOdomCallback, this, _1, _2 ) );
        }
        // PCSub = nh.subscribe(inputTopicName, 1, &particle_filter_wrap::PCCallback, this);
        if ( beLaser ) {
            laser_sub_ =
                new message_filters::Subscriber< sensor_msgs::LaserScan >( nh, inputTopicName, 1 );
            odom_sub_ =
                new message_filters::Subscriber< nav_msgs::Odometry >( nh, odomTopicName, 10 );
            sync_lo_ = new message_filters::Synchronizer< LaserOdomSync >(
                LaserOdomSync( 20 ), *laser_sub_, *odom_sub_ );
            sync_lo_->registerCallback(
                boost::bind( &particle_filter_wrap::LaserOdomCallback, this, _1, _2 ) );
        }
        //   PCSub = nh.subscribe(inputTopicName, 1, &particle_filter_wrap::LaserCallback, this);

        initPoseSub =
            nh.subscribe( "/initialpose", 1000, &particle_filter_wrap::initialposeCallback, this );

        if ( showParticles )
            particlePub            = nh.advertise< geometry_msgs::PoseArray >( "particles", 1 );
        if ( showPose ) mclPosePub = nh.advertise< nav_msgs::Odometry >( "ndt_mcl_pose", 1 );
