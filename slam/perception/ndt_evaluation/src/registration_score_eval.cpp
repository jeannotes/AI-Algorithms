...
RegistrationScoreEval::RegistrationScoreEval(
    const std::string& gt_file, const std::string& odom_file, const std::string& base_name_pcd,
    const Eigen::Affine3d& sensor_pose, const perception_oru::MotionModel3d::Params& motion_params )
    : base_name_pcd_( base_name_pcd ), sensor_pose_( sensor_pose ) {
...
bool RegistrationScoreEval::setGlobalMap( const std::string& file_name, const Eigen::Affine3d& T ) {
    try {
...
bool RegistrationScoreEval::setGlobalGraphMap( const std::string& file_name, int node_idx ) {
    try {
        std::ifstream ifs( file_name.c_str() );
        boost::archive::text_iarchive ia( ifs );
...
std::vector< Eigen::Affine3d > RegistrationScoreEval::generateOffset2DSet( int dimidx1,
                                                                           int dimidx2 ) {
    std::vector< Eigen::Affine3d > ret;
    std::vector< double > incr( 6 );
    incr.resize( 6 );
...
int RegistrationScoreEval::nbPoses() const {
    if ( Todom.size() < Tgt.size() ) return Todom.size();
    return Tgt.size();
}
...
void RegistrationScoreEval::computeScoreSetsGlobalMap( int idx2, int dimidx1, int dimidx2 ) {
    ...
    for ( int i = 0; i < Ts.size(); i++ ) {
        ...
        for ( unsigned int j = 0; j < nextNDT.size(); j++ ) {
            if ( nextNDT[j] != NULL ) delete nextNDT[j];
        }
...
void RegistrationScoreEval::computeScoreSets( int idx1, int idx2, int dimidx1, int dimidx2 ) {
    ...
    T_rel_ = T_rel_odom_;
    if ( use_d2d_in_grid ) {
        T_rel_ = T_d2d_;   // Will only work with the d2d not d2d SC since the d2d SC has an impact
                           // on the offset used when computing the score. To get SC to work the
                           // offset's must be adjusted accordingly.
    ...
    for ( int i = 0; i < Ts.size(); i++ ) {
        ...
        std::cout << "." << std::flush;
        if ( use_d2d_in_grid ) {
            matcher_d2d_sc.scoreComparision( nd1, nd2, T_d2d_, Tcov, score_d2d, score_d2d_sc, Ts[i],
                                             Ts[i], alpha_ );
        } else if ( use_d2d_sc_in_grid ) {
            // The provided offset needs to be relative to the proided
            matcher_d2d_sc.scoreComparision( nd1, nd2, T_d2d_sc_, Tcov, score_d2d, score_d2d_sc,
                                             Ts[i], T_rel_odom_.inverse() * T_d2d_sc_ * Ts[i],
                                             alpha_ );
        } else {
            matcher_d2d_sc.scoreComparision( nd1, nd2, T_rel_odom_, Tcov, score_d2d, score_d2d_sc,
                                             Ts[i], Ts[i], alpha_ );
        }
        results[i].offset       = Ts[i];
        results[i].score_d2d    = score_d2d;
    ...
    if ( matcher_d2d.covariance( nd1, nd2, T_d2d_, T_d2d_cov ) ) {
        ...
        std::cout << "T_d2d_cov condition : " << ndt_generic::getCondition( T_d2d_cov )
                  << std::endl;
    ...
    if ( matcher_icp.covariance( pc1, pc2, T_icp_, T_icp_cov ) ) {
        ...
        std::cout << "T_icp_cov condition : " << ndt_generic::getCondition( T_icp_cov )
                  << std::endl;
...
void RegistrationScoreEval::save( const std::string& filename, int grididx ) const {
    ...
    for ( int i = 0; i < results.size(); i++ ) {
        ofs << ndt_generic::affine3dToStringRPY( results[i].offset ) << " " << results[i].score_d2d
            << " " << results[i].score_d2d_sc << std::endl;
...
void RegistrationScoreEval::save2D( const std::string& filename, int dimidx1, int dimidx2 ) const {
    ...
    for ( int i = 0; i < results.size(); i++ ) {
        ...
        ofs << x[dimidx1] << " " << x[dimidx2] << " " << results[i].score_d2d << " "
            << results[i].score_d2d_sc << std::endl;
...
void RegistrationScoreEval::savePoseEstInOdomFrame( const std::string& filename ) const {
    ndt_generic::saveAffine3dRPY( filename + std::string( ".gt" ),
                                  T_rel_odom_.inverse() * T_rel_gt_ );
    ndt_generic::saveAffine3dRPY( filename + std::string( ".d2d" ),
                                  T_rel_odom_.inverse() * T_d2d_ );
    ndt_generic::saveAffine3dRPY( filename + std::string( ".d2d_sc" ),
                                  T_rel_odom_.inverse() * T_d2d_sc_ );
    ndt_generic::saveAffine3dRPY( filename + std::string( ".icp" ),
                                  T_rel_odom_.inverse() * T_icp_ );
    ndt_generic::saveAffine3dRPY( filename + std::string( ".filter" ),
                                  T_rel_odom_.inverse() * T_filter_ );
    ndt_generic::saveAffine3dRPY( filename + std::string( ".odom" ),
                                  T_rel_odom_.inverse() * T_rel_odom_ );
}
...
RegistrationScoreEval::getScoreSegments( int dimidx1, int dimidx2, bool d2d_sc_score ) const {
    ...
    for ( int i = 0; i < results.size(); i++ ) {
        ...
        if ( d2d_sc_score ) { score = results[i].score_d2d_sc; }
