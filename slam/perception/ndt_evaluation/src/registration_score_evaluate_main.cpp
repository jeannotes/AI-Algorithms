...
int main( int argc, char** argv ) {
    ...
    desc.add_options()( "help", "produce help message" )(
        ...
        "odom_file", po::value< std::string >( &odom_file )->default_value( std::string( "" ) ),
        "estimated sensor poses (from egomotion) to be used in SC in world frame" )(
        "base_name_pcd", po::value< string >( &base_name_pcd )->default_value( std::string( "" ) ),
        "prefix for the .pcd files" )(
        "out_file", po::value< string >( &out_file )->default_value( std::string( "scores" ) ),
        "prefix for the output files" )( "idx1", po::value< int >( &idx1 )->default_value( 0 ),
                                         "'fixed' index of pose/pointcloud to be used" )(
        "idx2", po::value< int >( &idx2 )->default_value( 1 ), "'moving' index of pose/pointcloud "
                                                               "to be used" )(
        "dimidx1", po::value< int >( &dimidx1 )->default_value( 0 ),
        "dimension index_x (what to plot on the X axis in gnuplot - 0,1,2,3,4,5 -> "
        "x,y,z,roll,pitch,yaw)" )( "dimidx2", po::value< int >( &dimidx2 )->default_value( 1 ),
                                   "dimension index_y (what to plot on the Y axis in gnuplot - "
                                   "0,1,2,3,4,5 -> x,y,z,roll,pitch,yaw)" )

        ( "x", po::value< double >( &transl[0] )->default_value( 0. ), "sensor pose - translation "
                                                                       "vector x" )(
            ...
            "resolution of the map" )( "offset_size",
                                       po::value< int >( &offset_size )->default_value( 100 ),
                                       "2*number of evaluated points per dimension" )(
            "incr_dist", po::value< double >( &incr_dist )->default_value( 0.01 ),
            "incremental steps for distance directions (x,y,z)" )(
            "incr_ang", po::value< double >( &incr_ang )->default_value( 0.002 ),
            "incremental steps for angular directions (R,P,Y)" )
            ...
            ( "Dd", po::value< double >( &motion_params.Dd )->default_value( 1. ),
              "forward uncertainty on distance traveled" )(
                ...
                "save_global_Ts", "if global transforms on registrations odom, etc. should be "
                                  "generate, for evaluation with ate.py, rpe.py" )(
                "iter_all_poses", "iterate over all available poses" )(
                "iter_step", po::value< int >( &iter_step )->default_value( 1 ),
                "iter step size" )( "iters", po::value< int >( &iters )->default_value( 1 ),
                                    "if additional iters should be evaluated (idx1 + iter, idx2 + "
                                    "iter)" )( "gnuplot", "if gnuplot should be invoked" )(
                ...
                "global_ndtmap_file",
                po::value< std::string >( &global_ndtmap_file )->default_value( "" ),
                "global map name to be registred towards" )(
                "node_idx", po::value< int >( &node_idx )->default_value( 0 ),
                "node index to be used in a graph map" )(
                "mx", po::value< double >( &transl[0] )->default_value( 0. ),
                "global map offset in x" )( "my",
                                            po::value< double >( &transl[1] )->default_value( 0. ),
                                            "global map offset in y" )
    ...
    if ( use_d2d_sc_in_grid ) { use_score_d2d_sc = true; }

    Eigen::Affine3d sensor_pose = ndt_generic::vectorsToAffine3d( transl, euler );
    std::cout << "Sensor pose used : " << ndt_generic::affine3dToStringRPY( sensor_pose )
              << std::endl;
    ...
    if ( iter_all_poses ) { iters = se.nbPoses(); }
    ...
    out_file += ".res" + ndt_generic::toString( resolution ) + "Dd" +
                ndt_generic::toString( motion_params.Dd ) + "Dt" +
                ndt_generic::toString( motion_params.Dt );
    ...
    while ( i < iters ) {
        ...
        if ( save_pcd ) {
            std::string out_file_pcd =
                out_file + ndt_generic::toString( _idx1 ) + "_" + ndt_generic::toString( _idx2 );
        ...
        if ( use_gnuplot ) {
            ...
            if ( save_eps ) {
                std::string out_file_eps = out_file + ndt_generic::toString( _idx1 ) + "_" +
                                           ndt_generic::toString( _idx2 ) + "x" +
                                           ndt_generic::toString( dimidx1 ) + "y" +
                                           ndt_generic::toString( dimidx2 ) + "sc" +
                                           ndt_generic::toString( use_score_d2d_sc ) + ".eps";
                std::cout << "saving : " << out_file_eps << std::endl;
                gp << "set terminal postscript eps size 3.5,2.62 enhanced color font "
                      "'Helvetica,12' lw 1\n";
            ...
            title += " [" + ndt_generic::toString( dimidx1 ) + "," +
                     ndt_generic::toString( dimidx2 ) + "]";
            ...
            if ( !draw_icp ) {
                gp << "splot '-' with pm3d notitle, '-' with points pt 1 title 'GT',  '-' with "
                      "points pt 4 title 'odom', '-' with points pt 2 title 'd2d', '-' with points "
                      "pt 3 title 'd2d sc', '-' with points pt 2 title 'filter'\n";
                ...
                gp << "splot '-' with pm3d notitle, '-' with points pt 1 title 'GT', '-' with "
                      "points pt 2 title 'd2d', '-' with points pt 3 title 'd2d sc', '-' with "
                      "points pt 4 title 'icp', '-' with points pt 2 title 'filter', '-' with "
                      "points pt 4 title 'icp filter'\n";
                gp.send2d( se.getScoreSegments( dimidx1, dimidx2, use_score_d2d_sc ) );
                gp.send1d( se.getRelPoseGT( dimidx1, dimidx2 ) );
                gp.send1d( se.getRelPoseD2D( dimidx1, dimidx2 ) );
                gp.send1d( se.getRelPoseD2D_SC( dimidx1, dimidx2 ) );
