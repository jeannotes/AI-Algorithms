...
void ExpNDTFrameProc< PointT >::addFrameIncremental( NDTFrame< PointT >* f, bool skipMatching,
    ...
    if ( !( match_full || match_no_association ) ) {
        ...
        if ( !skipMatching ) {
            ...
            matcher_feat.match( frames[0]->ndt_map, frames[i]->ndt_map, tr,
...
void ExpNDTFrameProc< PointT >::trimNbFrames( size_t maxNbFrames ) {
    if ( frames.size() > maxNbFrames ) {
        ...
        transformVector.erase(
            transformVector.begin() + 1,
            transformVector.end() );   // Just to keep a 1-1 coorespondance with frames.
    }
}

template < typename PointT >
void ExpNDTFrameProc< PointT >::detectKeypoints( NDTFrame< PointT >* f ) const {
    if ( img_scale == 1. ) {
...
template < typename PointT >
void ExpNDTFrameProc< PointT >::calcDescriptors( NDTFrame< PointT >* f ) const {
    if ( img_scale == 1. ) {
        ...
        for ( unsigned int q = 0; q < f->kpts.size(); ++q ) {
            support_size_max =
                ( f->kpts[q].size > support_size_max ) ? f->kpts[q].size : support_size_max;
        }
        ...
        cv::Mat projected_drawn( support_size_max * N_DIMS, support_size_max * N_DIMS,
                                 f->img.type() );
        ...
        for ( unsigned int q = 0; q < f->kpts.size(); ++q ) {
            support_size =
                ( f->kpts[q].size > support_size_max ) ? support_size_max : f->kpts[q].size;
