...
PoseEstimator< PointSource, PointTarget >::PoseEstimator( int NRansac, double maxidx,
                                                          double maxidd ) {
    numRansac       = NRansac;
...
template < typename PointSource, typename PointTarget >
void PoseEstimator< PointSource, PointTarget >::matchFrames(
    const NDTFrame< PointSource >& f0, const NDTFrame< PointTarget >& f1,
    std::vector< cv::DMatch >& fwd_matches ) {
    cv::Mat mask;
    if ( windowed ) mask = cv::windowedMatchingMask( f0.kpts, f1.kpts, wx, wy );
...
template < typename PointSource, typename PointTarget >
size_t PoseEstimator< PointSource, PointTarget >::estimate( const NDTFrame< PointSource >& f0,
    ...
    for ( int i = 0; i < (int) fwd_matches.size(); ++i ) {
        ...
        // matches.push_back( cv::DMatch(fwd_matches[i].queryIdx, fwd_matches[i].trainIdx,
        // fwd_matches[i].distance) );
    }
    for ( int i = 0; i < (int) rev_matches.size(); ++i ) {
        ...
        // matches.push_back( cv::DMatch(rev_matches[i].trainIdx, rev_matches[i].queryIdx,
        // rev_matches[i].distance) );
    }
...
template < typename PointSource, typename PointTarget >
size_t
PoseEstimator< PointSource, PointTarget >::estimate( const NDTFrame< PointSource >& f0,
                                                     const NDTFrame< PointTarget >& f1,
                                                     const std::vector< cv::DMatch >& matches ) {
    // convert keypoints in match to 3d points
    std::vector< Eigen::Vector4d, Eigen::aligned_allocator< Eigen::Vector4d > > p0;   // homogeneous
                                                                                      // coordinates
    ...
    for ( int i = 0; i < numRansac; i++ ) {
        ...
        int a                        = rand() % nmatch;
        int b                        = a;
        while ( a == b ) b           = rand() % nmatch;
        int c                        = a;
        while ( a == c || b == c ) c = rand() % nmatch;
        ...
        if ( i0a == i0b || i0a == i0c || i0b == i0c || i1a == i1b || i1a == i1c || i1b == i1c )
            continue;
        ...
        Eigen::Matrix3d H = p1a * p0a.transpose() + p1b * p0b.transpose() + p1c * p0c.transpose();
...
//		 inl++;
        ...
        for ( int i = 0; i < nmatch; i++ ) {
            ...
            if ( projectMatches ) {
                // The central idea here is to divide by the distance (this is essentially what
                // cam2pix does).
                dx = dx / z;
                dy = dy / z;
            }
...
// Vector3d pt1 = pt1_unchanged;
        ...
        if ( projectMatches ) {
            // The central idea here is to divide by the distance (this is essentially what cam2pix
            // does).
            dx = dx / z;
            dy = dy / z;
        }
        ...
        if ( dx * dx < maxInlierXDist2 && dy * dy < maxInlierXDist2 && dd * dd < maxInlierDDist2 ) {
            ...
            //	       if (fabs(f0.kpts[matches[i].queryIdx].pt.y -
            // f1.kpts[matches[i].trainIdx].pt.y) > 300)
            {
                //		   std::cout << " ---------- " << dx << "," << dy << "," << dd << ",\npt0 "
                //<<
                ...
                // pt1_unchanged.transpose() << std::endl;
