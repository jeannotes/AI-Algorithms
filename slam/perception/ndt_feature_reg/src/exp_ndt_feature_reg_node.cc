...
void subsamplePointCloud( const pcl::PointCloud< pcl::PointXYZ >& orig,
                          pcl::PointCloud< pcl::PointXYZ >& sub, int step ) {
    uint32_t x = 0;
    while ( x < orig.width ) {
        uint32_t y = 0;
...
class NDTFeatureRegNode {
  protected:
    ...
    void TransformEigenToTF( const Eigen::Transform< double, 3, Eigen::Affine, Eigen::ColMajor >& k,
                             tf::Transform& t ) {
        t.setOrigin( tf::Vector3( k.matrix()( 0, 3 ), k.matrix()( 1, 3 ), k.matrix()( 2, 3 ) ) );
        t.setBasis( btMatrix3x3( k.matrix()( 0, 0 ), k.matrix()( 0, 1 ), k.matrix()( 0, 2 ),
                                 k.matrix()( 1, 0 ), k.matrix()( 1, 1 ), k.matrix()( 1, 2 ),
                                 k.matrix()( 2, 0 ), k.matrix()( 2, 1 ), k.matrix()( 2, 2 ) ) );
    };
    ...
    void setupCamera( const sensor_msgs::CameraInfoConstPtr& camera_info ) {
        ...
        if ( !camerasetup_ ) {
            ...
            std::cout << "Params: " << fx << " " << fy << " " << cx << " " << cy << " "
                      << dist.size() << std::endl;
            cameraparams_ =
                lslgeneric::DepthCamera< pcl::PointXYZ >( fx, fy, cx, cy, dist, ds, true );
  ...
  public:
    ...
    NDTFeatureRegNode( ros::NodeHandle param_nh ) : sync_( 2 ), sync2_( 2 ) {
        ...
        if ( !xyzi_data ) {
            ...
            depth_param_sub_ =
                nh_.subscribe( "depth_param", 1, &NDTFeatureRegNode::setupCamera, this );
            ...
            sync2_.registerCallback(
                boost::bind( &NDTFeatureRegNode::intensityPoints2Cb, this, _1, _2 ) );
        }
        ...
        proc = new ExpNDTFrameProc< pcl::PointXYZ >( nb_ransac, max_inldist_xy, max_inldist_z );
    ...
    void process( cv::Mat& rgb_img, cv::Mat& depth_img, const ros::Time& current_timestamp ) {
        ...
        proc->addFrameIncremental( frame, skip_matching_, estimate_di_, match_full_,
                                   match_no_association_ );

        if ( visualize_ ) { viewKeypointMatchesFirst( proc, 10 ); }

        proc->trimNbFrames( max_nb_frames_ );

        global_transform_ = proc->transformVector.back();
        ...
        tf_.sendTransform(
            tf::StampedTransform( transform, current_timestamp, "world", "ndt_feature_reg" ) );
        tf_.sendTransform( tf::StampedTransform( global_transform, current_timestamp, "world",
                                                 "global_ndt_feature_reg" ) );

        std::string gt_frame = "/camera_rgb_optical_frame";
        if ( publish_cloud_ ) {
        ...
        try {
            ...
            tf_listener_.waitForTransform( gt_frame, prev_timestamp_, gt_frame, current_timestamp,
                                           "/world", ros::Duration( 2.0 ) );

            tf_listener_.lookupTransform( gt_frame, prev_timestamp_, gt_frame, current_timestamp,
                                          "/world", gt_transform );

            tf_.sendTransform(
                tf::StampedTransform( gt_transform, current_timestamp, "world", "gt_reg" ) );

            if ( set_initial_pose_ ) {
                tf::StampedTransform glb_gt_transform;
                tf_listener_.lookupTransform( "/world", gt_frame, current_timestamp,
                                              glb_gt_transform );
    ...
    void intensityPoints2Cb( const sensor_msgs::ImageConstPtr& msg_img,
        ...
        // Currently lacking support to handle pointcloud directly convert it to a depth image...
        // also we don't have any calibration data from the Fotonic camera in the same format but
        // can derrive this from the 3D depth data.
    ...
    void rgbDepthCb( const sensor_msgs::ImageConstPtr& msg_rgb,
        ...
        if ( camerasetup_ ) { process( rgb_img, depth_img, msg_rgb->header.stamp ); }
        m_.unlock();
    }
