...
inline std::vector< std::string > split_string( const std::string& str,
    ...
    while ( !stop ) {
        ...
        if ( it ==
             std::string::npos ) {   // didn't find any new comma, return the remaining string.
...
vector< pair< string, string > > load_associations( const string& association_file,
                                                    vector< string >& timestamps, int skip_nb ) {
...
int main( int argc, char** argv ) {
    ...
    desc.add_options()( "help", "produce help message" )( "debug", "print debug output" )(
        "visualize",
        "visualize the output" )( "association", po::value< string >( &association_name ),
                                  "association file name (generated by association.py script)" )(
        "output", po::value< string >( &output_name ), "file name of the estimation output (to be "
                                                       "used with the evaluation python script)" )(
        "time_output", po::value< string >( &times_name ),
        "file name for saving the per-frame processing time (to be used with the evaluation python "
        "script)" )( "scale", po::value< double >( &scale )->default_value( 0.0002 ),
                     "depth scale (depth = scale * pixel value)" )(
        "max_inldist_xy", po::value< double >( &max_inldist_xy )->default_value( 0.02 ),
        "max inlier distance in xy related to the camera plane (in meters)" )(
        "max_inldist_z", po::value< double >( &max_inldist_z )->default_value( 0.05 ),
        "max inlier distance in z - depth (in meters)" )(
        "nb_ransac", po::value< int >( &nb_ransac )->default_value( 1000 ),
        "max number of RANSAC "
        "iterations" )( "nb_points", po::value< int >( &nb_points )->default_value( 21 ),
                        "number of surrounding points" )(
        "support_size", po::value< size_t >( &support_size )->default_value( 10 ),
        "width of patch around each feature in pixels" )(
        "max_var", po::value< double >( &max_var )->default_value( 0.3 ), "max variance of ndt "
                                                                          "cells" )(
        "current_res", po::value< double >( &current_res )->default_value( 0.2 ),
        "resolution of ndt cells if doing full matching" )( "skip_matching", "skip the ndt "
                                                                             "matching step" )(
        ...
        "max_nb_frames", po::value< size_t >( &max_nb_frames )->default_value( 10 ),
        "max number of frames to be keept in the processing step" )(
        "delay_vis", po::value< int >( &delay_vis )->default_value( 10 ),
        "param to opencv function waitKey (if visualize is used)" )(
        "img_scale", po::value< double >( &img_scale )->default_value( 1. ),
        "if the rgb image should be scaled when feature are detected and extracted - mainly to "
        "boost the speed" )( "detector_thresh",
                             po::value< double >( &detector_thresh )->default_value( 400. ),
                             "playaround parameter, used to test different detectorvalues" )(
        "skip_nb", po::value< int >( &skip_nb )->default_value( 0 ),
        "how many intermediate frames should be skipped between the registration is runned" )(
        "old_reg_code", "use the old registration code - for debugging only" )(
        "freiburg_camera_nb", po::value< int >( &freiburg_camera_nb )->default_value( 1 ),
        "the camera used for the evaluation 1-> freiburg camera #1, 2-> freiburg camera #2" )(
        "max_kp_dist", po::value< double >( &max_kp_dist )->default_value( 10. ),
        "max distance of keypoints used" )(
        "min_kp_dist", po::value< double >( &min_kp_dist )->default_value( 0. ), "min distance of "
                                                                                 "keypoints used" )(
        "windowed_matching", "limit the search of the keypoint matches to a local region (will not "
                             "work that good if the skip_nb is large)" )(
        "trim_factor", po::value< double >( &trim_factor )->default_value( 1. ),
        "only utilize the trimfactor number of keypoints with the closest NDT distance - similar "
        "idea as in trimmed ICP" )( "non_mean", "if the mean used in NDT matching (including the "
                                                "RANSAC step) are computed direclty from the "
                                                "closest depth image pixel" );
    ...
    if ( times_name == "" ) { times_name = output_name + ".times"; }

    vector< string > timestamps;
    vector< pair< string, string > > associations =
        load_associations( association_name, timestamps, skip_nb );
    if ( debug ) {
        for ( size_t i = 0; i < associations.size(); i++ ) {
            cout << "associations : " << associations[i].first << " <-> " << associations[i].second
                 << endl;
        }
    ...
    switch ( freiburg_camera_nb ) {
        ...
        case 2:
            ...
            ds      = 1.031;
            break;
        default: cerr << "unknown camera number : " << freiburg_camera_nb << endl; return 1;
    }

    double t1, t2;

    lslgeneric::DepthCamera< pcl::PointXYZ > cameraparams( fx, fy, cx, cy, dist, ds * scale,
                                                           false );
    ...
    if ( old_reg_code ) {
        for ( size_t i = 1; i < associations.size(); i++ ) {
            ...
            frame1->depth_img = cv::imread( associations[i - 1].second,
                                            CV_LOAD_IMAGE_ANYDEPTH );   // CV_LOAD_IMAGE_ANYDEPTH is
                                                                        // important to load the
                                                                        // 16bits image
            if ( i == 1 ) cameraparams.setupDepthPointCloudLookUpTable( frame1->depth_img.size() );
            ...
            frame2->depth_img = cv::imread( associations[i].second,
                                            CV_LOAD_IMAGE_ANYDEPTH );   // CV_LOAD_IMAGE_ANYDEPTH is
                                                                        // important to load the
                                                                        // 16bits image
            ...
            if ( visualize ) viewKeypointMatches( &proc, delay_vis );

            if ( i == 1 ) transformVector.push_back( proc.transformVector[0] );
        ...
        for ( size_t i = 0; i < associations.size(); i++ ) {
            ...
            frame->depth_img = cv::imread( associations[i].second,
                                           CV_LOAD_IMAGE_ANYDEPTH );   // CV_LOAD_IMAGE_ANYDEPTH is
                                                                       // important to load the
                                                                       // 16bits image
            ...
            if ( i == 0 ) cameraparams.setupDepthPointCloudLookUpTable( frame->depth_img.size() );
            frame->cameraParams = cameraparams;
            // frame->supportSize = 10;

            double t1 = getDoubleTime();
            proc.addFrameIncremental( frame, skip_matching, estimate_di, match_full,
                                      match_no_association );
            double t2 = getDoubleTime();
    ...
    for ( size_t i = 0; i < transformVector.size(); i++ ) {
        ...
        cout << timestamps[i] << " " << global_transform.translation().transpose() << " " << tmp.x()
             << " " << tmp.y() << " " << tmp.z() << " " << tmp.w() << endl;
        file << timestamps[i] << " " << global_transform.translation().transpose() << " " << tmp.x()
             << " " << tmp.y() << " " << tmp.z() << " " << tmp.w() << endl;
    }
    file.close();
