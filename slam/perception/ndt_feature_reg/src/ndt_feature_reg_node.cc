...
class NDTFeatureRegNode {
  protected:
    ...
    //     t.setBasis(tf::Matrix3x3(k.matrix()(0,0),
    //     k.matrix()(0,1),k.matrix()(0,2),k.matrix()(1,0),
    //     k.matrix()(1,1),k.matrix()(1,2),k.matrix()(2,0), k.matrix()(2,1),k.matrix()(2,2)));
    // };
  ...
  public:
    ...
    void process( const cv::Mat& rgb_img, const cv::Mat& depth_img,
        ...
        if ( visualize_ ) viewKeypointMatches( proc, 10 );
        ...
        // tf_.sendTransform(tf::StampedTransform(transform, current_timestamp, world_str,
        // "ndt_feature_reg"));

        tf_.sendTransform( tf::StampedTransform( global_transform, current_timestamp, world_str,
        ...
        try {
            /*
                ...
                tf_.sendTransform(tf::StampedTransform(gt_transform, current_timestamp, world_str,
               "gt_reg"));
            */

            if ( set_initial_pose_ ) {
                tf::StampedTransform glb_gt_transform;
                tf_listener_.lookupTransform( world_str, gt_frame_, current_timestamp,
                                              glb_gt_transform );
    ...
    void rgbDepthCb( const sensor_msgs::ImageConstPtr& msg_rgb,
        ...
        try {
            ...
            cv_ptr_depth =
                cv_bridge::toCvShare( msg_depth /*, sensor_msgs::image_encodings::TYPE_32FC1*/ );
        } catch ( cv_bridge::Exception& e ) {
            ROS_ERROR( "cv_bridge exception: %s", e.what() );
            return;
        }
