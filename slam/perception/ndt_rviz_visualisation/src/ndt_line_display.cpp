...
NDTLineDisplay::NDTLineDisplay() {
    ...
    history_length_property_ =
        new rviz::IntProperty( "History Length", 1, "Number of prior measurements to display.",
                               this, SLOT( updateHistoryLength() ) );
    history_length_property_->setMin( 1 );
    history_length_property_->setMax( 100000 );

    draw_lines_ = new rviz::BoolProperty( "Draw Lines", false,
                                          "If lines should be drawn instead of ellipsoids.", this,
                                          SLOT( updateDrawLines() ) );
}
    ...
    for ( int itr = 0; itr < msg->cells.size(); itr++ ) {
        ...
        if ( msg->cells[itr].hasGaussian_ == true ) {
            ...
            if ( !( msg->x_cell_size == msg->y_cell_size &&
                    msg->y_cell_size == msg->z_cell_size ) ) {
