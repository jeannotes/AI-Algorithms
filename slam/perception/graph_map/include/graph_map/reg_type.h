...
public:
    ...
    virtual bool Register(MapTypePtr maptype, Eigen::Affine3d &Tnow, pcl::PointCloud<pcl::PointXYZ> &cloud, Matrix6d covar = unit_covar) {
        cerr << "No registration for pcl::PointXYZ implemented" << endl;   //This methods attempts to register the point cloud versus the map using the affine transformation guess "Tm"
    }
    virtual bool Register(MapTypePtr maptype, Eigen::Affine3d &Tnow, pcl::PointCloud<velodyne_pointcloud::PointXYZIR> &cloud, Matrix6d covar = unit_covar) {
        cerr << "No registration for velodyne_pointcloud::PointXYZIR implemented" << endl;
    }
