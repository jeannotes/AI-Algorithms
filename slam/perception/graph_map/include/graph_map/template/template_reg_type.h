...
class TemplateRegType : public registrationType {
  public:
    ~TemplateRegType();
    bool Register( MapTypePtr maptype, Eigen::Affine3d& Tnow,
                   pcl::PointCloud< pcl::PointXYZ >& cloud,
                   Matrix6d covar = unit_covar );   // This methods attempts to register the point
                                                    // cloud versus the map using Tmotion as a first
                                                    // guess
  protected:
...
class TemplateRegTypeParam : public registrationParameters {
  ...
  protected:
    TemplateRegTypeParam();   // Constructor is protected to allow only graphcatory to instanciate
                              // or derived classes create this type
  private:
    friend class GraphFactory;
};
}
