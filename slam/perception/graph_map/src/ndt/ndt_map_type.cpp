...
NDTMapType::NDTMapType( MapParamPtr paramptr ) : MapType( paramptr ) {
    NDTMapParamPtr param =
        boost::dynamic_pointer_cast< NDTMapParam >( paramptr );   // Should not be NULL
    if ( param != NULL ) {
...
void NDTMapType::update( const Eigen::Affine3d& Tsensor, pcl::PointCloud< pcl::PointXYZ >& cloud,
    ...
    if ( initialized_ && enable_mapping_ ) {
        ...
        if ( !simple ) {
            map_->addPointCloudMeanUpdate( Tsensor.translation(), cloud, localMapSize, 1e5, 25,
                                           sizez_, 0.06 );
        } else {
...
void NDTMapType::InitializeMap( const Eigen::Affine3d& Tsensor,
                                pcl::PointCloud< pcl::PointXYZ >& cloud, bool simple ) {
...
bool NDTMapType::CompoundMapsByRadius( MapTypePtr target, const Affine3d& T_source,
                                       const Affine3d& T_target, double radius ) {
    Affine3d Tdiff = Affine3d::Identity();
    Tdiff          = T_source.inverse() * T_target;
    pcl::PointXYZ center_pcl( Tdiff.translation()( 0 ), Tdiff.translation()( 1 ),
                              Tdiff.translation()( 2 ) );
