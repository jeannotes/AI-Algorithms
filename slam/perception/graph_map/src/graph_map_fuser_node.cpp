...
class GraphMapFuserNode {
  protected:
    ...
    std::string world_link_id, odometry_link_id, fuser_base_link_id, laser_link_id, init_pose_frame,
        gt_topic, bag_name, state_base_link_id;
    double size_x, size_y, size_z, resolution, sensor_range, min_laser_range_;
    bool visualize, match2D, matchLaser, beHMT, useOdometry, initPoseFromGT, initPoseFromTF,
        initPoseSet, gt_mapping;
    ...
    ros::Publisher map_publisher_, laser_publisher_, point2_publisher_, odom_publisher_,
        adjusted_odom_publisher_, fuser_odom_publisher_;
  ...
  public:
    ...
    GraphMapFuserNode( ros::NodeHandle param_nh ) : frame_nr_( 0 ) {
        ...
        param_nh.param< std::string >( "odometry_adjusted", odometry_adjusted_topic, "odometry_"
                                                                                     "adjusted" );
        // get it from TF?
        ...
        param_nh.param< std::string >( "state_base_link_id", state_base_link_id, "/state_base_"
                                                                                 "link" );
        ...
        param_nh.param< std::string >( "odometry_frame_id", odometry_link_id,
                                       std::string( "/odom_base_link" ) );
        ...
        laser_publisher_ =
            param_nh.advertise< sensor_msgs::LaserScan >( "laserscan_in_fuser_frame", 50 );
        ...
        if ( gt_mapping )
            use_tf_listener_ =
                use_tf_listener_ &&
                state_base_link_id != std::string( "" );   // check if odometry topic exists
        else
            use_tf_listener_ =
                use_tf_listener_ &&
                odometry_link_id != std::string( "" );   // check if odometry topic exists
        ...
        if ( !matchLaser ) {
            points2_sub_ =
                new message_filters::Subscriber< sensor_msgs::PointCloud2 >( nh_, points_topic, 2 );
            if ( useOdometry ) {
                if ( gt_mapping ) {
                    if ( !use_tf_listener_ ) {
                        gt_fuser_sub_ = new message_filters::Subscriber< nav_msgs::Odometry >(
                            nh_, gt_topic, 10 );
                        sync_GTodom_ = new message_filters::Synchronizer< PointsGTOdomSync >(
                            PointsGTOdomSync( SYNC_FRAMES ), *points2_sub_, *gt_fuser_sub_ );
                        sync_GTodom_->registerCallback( boost::bind(
                            &GraphMapFuserNode::GTLaserPointsOdomCallback, this, _1, _2 ) );
                    } else
                        points2OdomTfSub = nh_.subscribe< sensor_msgs::PointCloud2 >(
                            points_topic, 10, &GraphMapFuserNode::GTLaserPointsOdomCallbackTF,
                            this );
                    ...
                    if ( !use_tf_listener_ ) {
                        ...
                        sync_po_->registerCallback(
                            boost::bind( &GraphMapFuserNode::points2OdomCallback, this, _1, _2 ) );
                    } else
                        points2OdomTfSub = nh_.subscribe< sensor_msgs::PointCloud2 >(
                            points_topic, 10, &GraphMapFuserNode::points2OdomCallbackTF, this );
                }
            }
        } else {
            laser_sub_ =
                new message_filters::Subscriber< sensor_msgs::LaserScan >( nh_, laser_topic, 2 );
            if ( useOdometry ) {
                odom_sub_ = new message_filters::Subscriber< nav_msgs::Odometry >(
                    nh_, odometry_topic, 10 );
                sync_lo_ = new message_filters::Synchronizer< LaserOdomSync >(
                    LaserOdomSync( SYNC_FRAMES ), *laser_sub_, *odom_sub_ );
                sync_lo_->registerCallback(
                    boost::bind( &GraphMapFuserNode::laserOdomCallback, this, _1, _2 ) );
            } else {
                ( (void) 0 );   // Do nothing, seriously consider using a laser only callback (no
                                // odometry sync)
            }
        }
        if ( initPoseFromGT ) {
    ...
    void processFrame( pcl::PointCloud< pcl::PointXYZ >& cloud, Eigen::Affine3d Tmotion ) {
        if ( !initPoseSet ) return;

        frame_nr_++;
        cout << "frame nr=" << frame_nr_ << endl;
        if ( ( Tmotion.translation().norm() < 0.005 &&
               Tmotion.rotation().eulerAngles( 0, 1, 2 ).norm() < 0.005 ) &&
             useOdometry ) {   // sanity check for odometry
            return;
        }
        (void) ResetInvalidMotion( Tmotion );

        cout << "frame=" << frame_nr_
             << "movement=" << ( fuser_->GetPoseLastFuse().inverse() * pose_ ).translation().norm()
             << endl;
        ...
        tf_.sendTransform(
            tf::StampedTransform( Transform, tplot, world_link_id, fuser_base_link_id ) );
        tf_.sendTransform(
            tf::StampedTransform( tf_sensor_pose_, tplot, fuser_base_link_id, laser_link_id ) );
    ...
    inline bool getAffine3dTransformFromTF( const ros::Time& time, const std::string& link_id,
                                            Eigen::Affine3d& ret, const ros::Duration& wait ) {
    ...
    void plotPointcloud2( pcl::PointCloud< pcl::PointXYZ >& cloud,
    ...
    void points2OdomCallback( const sensor_msgs::PointCloud2::ConstPtr& msg_in,
                              const nav_msgs::Odometry::ConstPtr& odo_in ) {   // callback is used
                                                                               // in conjunction
                                                                               // with odometry time
                                                                               // filter.
    ...
    void points2OdomCallbackTF(
        const sensor_msgs::PointCloud2::ConstPtr& msg_in ) {   // this callback is used to look up
                                                               // tf transformation for scan data
        Eigen::Affine3d Tm;
        static bool last_odom_found = false;
        pcl::PointCloud< pcl::PointXYZ > cloud;
        pcl::fromROSMsg( *msg_in, cloud );
        bool found_odom = getAffine3dTransformFromTF(
            ( msg_in->header.stamp - ros::Duration( sensor_offset_t_ ) ), odometry_link_id,
            this_odom, ros::Duration( 0.1 ) );
    ...
    void GTLaserPointsOdomCallback( const sensor_msgs::PointCloud2::ConstPtr& msg_in,
                                    const nav_msgs::Odometry::ConstPtr&
                                        odo_in ) {   // this callback is used for GT based mapping
        cout << "GT diff:" << ( msg_in->header.stamp - odo_in->header.stamp ).toSec() << endl;
        Eigen::Affine3d Tmotion;
        if ( frame_nr_ == 0 ) { Tmotion = Eigen::Affine3d::Identity(); }
        ...
        tf_.sendTransform( tf::StampedTransform( gt_base, t_stamp, world_link_id,
                                                 std::string( "online_" ) + state_base_link_id ) );
        tf_.sendTransform( tf::StampedTransform( tf_sensor_pose_, t_stamp,
                                                 std::string( "online_" ) + state_base_link_id,
                                                 laser_link_id ) );
    ...
    void GTLaserPointsOdomCallbackTF(
        const sensor_msgs::PointCloud2::ConstPtr& msg_in ) {   // this callback is used for GT based
                                                               // mapping with TF lookup
        cout << "GT-TF toffset:" << sensor_offset_t_ << endl;
        Eigen::Affine3d tmp_pose;
        Eigen::Affine3d Tmotion = Eigen::Affine3d::Identity();
        bool found_odom         = getAffine3dTransformFromTF(
            ( msg_in->header.stamp - ros::Duration( sensor_offset_t_ ) ), state_base_link_id,
            tmp_pose, ros::Duration( 0.1 ) );
        ...
        if ( found_odom ) {
            ...
            tf_.sendTransform(
                tf::StampedTransform( tf_gt_base, t_stamp, world_link_id,
                                      std::string( "online_" ) + state_base_link_id ) );
            tf_.sendTransform( tf::StampedTransform( tf_sensor_pose_, t_stamp,
                                                     std::string( "online_" ) + state_base_link_id,
                                                     laser_link_id ) );
    ...
    // Callback
    void gt_callback( const nav_msgs::Odometry::ConstPtr& msg_in ) {   // This callback is used to
                                                                       // set initial pose from GT
                                                                       // data.
