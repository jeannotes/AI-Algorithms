...
{
using namespace std;
GraphMapFuser::GraphMapFuser( string maptype, string registratorType,
...
GraphMapFuser::GraphMapFuser( RegParamPtr regParam, MapParamPtr mapParam, GraphParamPtr graph_param,
                              const Eigen::Affine3d& init_pose,
                              const Eigen::Affine3d& sensorPose ) {
    mapParam_          = mapParam;
    regParam_          = regParam;
...
//! \return true if base has moved outside bounds and can fuse the current frame, otherwise return
//! false. Will always return true the first frame
//!
bool GraphMapFuser::KeyFrameBasedFuse( const Affine3d& Tnow ) {
    ...
    frame_idx_++;
    if ( ret == true ) {
        if ( frame_idx_ % 4 == 0 ) {   // Avoid that the scanner covers the same region (one scan
                                       // covers ~270 degrees angle and the same area is repeaded
                                       // every 4rd frame)
            ret = false;
        } else {
            frame_idx_ = 0;
...
void GraphMapFuser::PlotMapType() {
    /*  NDTMapPtr curr_node = boost::dynamic_pointer_cast< NDTMapType
      >(graph_map_->GetCurrentNode()->GetMap());
      GraphPlot::SendGlobalMapToRviz(curr_node->GetNDTMap(),1,graph_map_->GetCurrentNodePose());*/
    GraphPlot::PlotMap( graph_map_->GetCurrentNode()->GetMap(), -1,
                        graph_map_->GetCurrentNodePose(), marker_ );
