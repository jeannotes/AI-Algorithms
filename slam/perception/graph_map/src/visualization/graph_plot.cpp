...
void GraphPlot::PlotMap( MapTypePtr map, int color, const Affine3d& offset, PlotMarker marker,
    ...
    if ( NDTDLMapPtr ptr = boost::dynamic_pointer_cast< NDTDLMapType >( map ) ) {
        if ( marker == plotmarker::sphere ) {
            ...
            PublishMapAsPoints( mean, color, 0.4 * ptr->GetResolution(), offset,
                                ns + std::string( "flat" ) );
            GetAllCellsMeanCov( ptr->GetNDTMapEdge(), cov, mean );
            PublishMapAsPoints( mean, color + 2, /*0.2**/ ptr->GetResolution(), offset,
                                ns + std::string( "edge" ) );
        }
    }
}

void GraphPlot::PlotMap( NDTMap& map, int color, const Affine3d& offset, PlotMarker marker,
                         std::string ns, double point_size ) {
    if ( marker == plotmarker::sphere ) {
...
void GraphPlot::Initialize() {
    ...
    *localMapPublisher_ =
        nh_->advertise< visualization_msgs::MarkerArray >( NDT_LOCAL_MAP_TOPIC, 10 );
    *globalMapPublisher_ =
    ...
    *graphInfoPublisher_ =
        nh_->advertise< visualization_msgs::MarkerArray >( GRAPH_INFO_TOPIC, 10 );
...
void GraphPlot::CovarToMarker( const Eigen::Matrix3d& cov, const Eigen::Vector3d& mean,
                               visualization_msgs::Marker& marker ) {
    if ( !initialized_ ) Initialize();
    ...
    if ( marker.scale.x == 0 ) marker.scale.x == 1.0;
    if ( marker.scale.y == 0 ) marker.scale.y == 1.0;
    if ( marker.scale.z == 0 ) marker.scale.z == 1.0;
...
void GraphPlot::PublishMapAsPoints( mean_vector& mean, int color, double scale,
                                    const Eigen::Affine3d& offset, std::string ns, int id ) {
    if ( !initialized_ ) Initialize();
...
void GraphPlot::sendMapToRviz( mean_vector& mean, cov_vector& cov, ros::Publisher* mapPublisher,
                               string frame, int color, const Affine3d& offset, string ns,
                               int markerType ) {
    static unsigned int max_size = 0;
    if ( !initialized_ ) Initialize();

    if ( max_size < mean.size() ) max_size = mean.size();
...
void GraphPlot::SendLocalMapToRviz( perception_oru::NDTMap* mapPtr, int color,
...
void GraphPlot::GetAllCellsMeanCov( const perception_oru::NDTMap* mapPtr, cov_vector& cov,
                                    mean_vector& mean ) {
    cov.clear();
    mean.clear();
...
void GraphPlot::GetAllCellsMeanCov( std::vector< perception_oru::NDTCell* > cells, cov_vector& cov,
                                    mean_vector& mean ) {
    cov.clear();
    mean.clear();
    for ( int i = 0; i < cells.size(); i++ ) {
...
void GraphPlot::SendGlobalMapToRviz( perception_oru::NDTMap* mapPtr, int color,
                                     const Affine3d& offset ) {
    cov_vector cov;
    mean_vector mean;
    if ( !initialized_ ) Initialize();
...
void GraphPlot::SendGlobal2MapToRviz( perception_oru::NDTMap* mapPtr, int color,
                                      const Affine3d& offset ) {
    cov_vector cov;   // end namespacev;
    mean_vector mean;
    if ( !initialized_ ) Initialize();
    GetAllCellsMeanCov( mapPtr, cov, mean );
    sendMapToRviz( mean, cov, globalMapPublisher_, "world", color, offset, "prev",
                   visualization_msgs::Marker::CUBE );
}
void GraphPlot::SendGlobal2MapToRviz( std::vector< perception_oru::NDTCell* > cells, int color,
                                      const Affine3d& offset ) {
    cov_vector cov;
    mean_vector mean;
