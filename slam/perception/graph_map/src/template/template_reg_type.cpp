...
{
TemplateRegType::TemplateRegType( const Affine3d& sensor_pose, RegParamPtr paramptr )
    : registrationType( sensor_pose, paramptr ) {
    TemplateRegTypeParamPtr param =
        boost::dynamic_pointer_cast< TemplateRegTypeParam >( paramptr );   // Should not be NULL
    if ( param != NULL ) {
...
bool TemplateRegType::Register( MapTypePtr maptype, Eigen::Affine3d& Tnow,
    ...
    if ( !enableRegistration_ || !maptype->Initialized() ) {
        ...
        cout << "please fill in code for registration- uintil then, registration is disabled"
             << endl;
    }
...
void TemplateRegTypeParam::GetParametersFromRos() {
    ...
    nh.param< std::string >( "super_important_parameter", super_important_parameter_, "default "
                                                                                      "string" );
}
}
