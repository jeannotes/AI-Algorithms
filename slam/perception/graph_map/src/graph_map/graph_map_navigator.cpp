...
{
GraphMapNavigator::GraphMapNavigator( const Eigen::Affine3d& nodepose, const MapParamPtr& mapparam,
                                      const GraphParamPtr graphparam )
    : GraphMap( nodepose, mapparam, graphparam ) {}

bool GraphMapNavigator::SwitchToClosestMapNode( Affine3d& Tnow, const Matrix6d& cov,
                                                Affine3d& T_world_to_local_map,
                                                const double radius ) {
    ...
    for ( std::vector< MapNodePtr >::iterator itr_node = map_nodes_.begin();
          itr_node != map_nodes_.end(); ++itr_node ) {   // loop thorugh all existing nodes
        if ( radius == 0.0 ||
             ( *itr_node )->WithinRadius( Tnow, radius ) ) {   // if a node is within radius of pose
                                                               // and the node is of type "map type"
            double found_distance =
                Eigen::Vector3d( Tnow.translation() - ( *itr_node )->GetPose().translation() )
                    .norm();   // Euclidian distance between robot pose and map node pose;
            cout << "Node is within range of previously created map, distance=" << found_distance
                 << endl;
    ...
    if ( node_found ) {   // if any node at all was found, switch to the closest and update Tnow &
                          // T_world_to_local_map (transformation to current node)
...
bool GraphMapNavigator::SwitchToClosestMapNode( Affine3d& Tnow ) {
    ...
    for ( std::vector< MapNodePtr >::iterator itr_node = map_nodes_.begin();
          itr_node != map_nodes_.end(); ++itr_node ) {   // loop thorugh all existing nodes
        double distance =
            Eigen::Vector3d( Tnow.translation() - ( *itr_node )->GetPose().translation() ).norm();
    ...
    if ( closest_map_node != NULL ) {
        if ( closest_map_node != currentNode_ ) {
            ...
            cout << "Switched to node: " << currentNode_->GetPose().translation().transpose()
                 << endl;
            return true;
        } else
...
bool GraphMapNavigator::AutomaticMapInterchange( Affine3d& Tnow, const Matrix6d& cov_incr,
                                                 Affine3d& T_world_to_local_map,
                                                 bool& changed_map_node, bool& created_map_node ) {
    ...
    if ( use_submap_ != false ) {
        ...
        if ( !currentNode_->WithinRadius(
                 Tnow, interchange_radius_ ) ) {   // No longer within radius of node
            cout << "Left boundries of previous map, will  search through " << map_nodes_.size()
                 << " node(s) to find a map node within range of" << interchange_radius_ << "m"
                 << endl;
            if ( changed_map_node = SwitchToClosestMapNode( Tnow, cov_incr, T_world_to_local_map,
                                                            interchange_radius_ ) ) {
                ...
                prevNode_ = currentNode_;
                // NDT2DMapPtr prev_ndt_map = boost::dynamic_pointer_cast< NDTMapType
                // >(prevNode_->GetMap());
                AddMapNode( mapparam_, T_world_to_local_map * Tnow,
                            cov_incr );   // if no node already exists, create a new node
                prevNode_->GetMap()->CompoundMapsByRadius(
                    currentNode_->GetMap(), prevNode_->GetPose(), currentNode_->GetPose(),
                    compound_radius_ );
