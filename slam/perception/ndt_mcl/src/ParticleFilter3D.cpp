 ...
 */
void ParticleFilter3D::initializeNormalRandom( unsigned int NumParticles, double mx, double my,
                                               double mz, double mroll, double mpitch, double myaw,
                                               double vx, double vy, double vz, double vroll,
...
void ParticleFilter3D::normalize() {
    ...
    if ( summ > 0 ) {
        for ( i = 0; i < pcloud.size(); i++ ) { pcloud[i].p = pcloud[i].p / summ; }
    } else {
        for ( i = 0; i < pcloud.size(); i++ ) { pcloud[i].p = 1.0 / (double) pcloud.size(); }
    }
}

void ParticleFilter3D::predict( Eigen::Affine3d Tmotion, double vx, double vy, double vz,
    ...
    for ( unsigned int i = 0; i < pcloud.size(); i++ ) {
        ...
        if ( transform_cloud ) pcloud[i].T = offset * pcloud[i].T;
        ///}
    }
}
...
Eigen::Affine3d ParticleFilter3D::getMean() {
    ...
    //	return xyzrpy2affine(mx,my,mz, atan2(roll_y,roll_x), atan2(pitch_y,pitch_x),
    // atan2(yaw_y,yaw_x));
