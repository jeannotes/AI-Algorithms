...
*
* This is based now on our sample file (see data folder). The sample file also has ground truth pose
* information,
 ...
 */
void callback( const sensor_msgs::LaserScan& scan, tf::Transform odo_pose,
    ...
    fprintf( stderr, "Lt( %.1lfms %.1lfHz seq:%d) -", looptime * 1000, 1.0 / looptime,
             scan.header.seq );
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ndtmcl->updateAndPredict(
        Tmotion, *cloud );   ///< does the prediction, update and resampling steps (see ndt_mcl.hpp)
...
// If visualization desired
    ...
    for ( int i = 0; i < ndtmcl->pf.NumOfParticles; i++ ) {
        ndt_viz.addParticle( ndtmcl->pf.Particles[i].x, ndtmcl->pf.Particles[i].y, 0.5, 1.0, 1.0,
                             1.0 );
    }

    ndt_viz.addTrajectoryPoint( dm[0], dm[1], 0.5, 1.0, 0, 0 );
    ndt_viz.addTrajectoryPoint( Tgt.translation()( 0 ), Tgt.translation()( 1 ), 0.5, 1.0, 1.0,
                                1.0 );
    ndt_viz.addTrajectoryPoint( Todo.translation()( 0 ), Todo.translation()( 1 ), 0.5, 0.0, 1.0,
                                0.0 );
...
int main( int argc, char** argv ) {
    ...
    bool makeMapVeryStatic =
        false;   ///< indicates if we should make the map over confident (stationary that is)
    ...
    paramHandle.param< std::string >( "input_laser_topic", input_laser_topic,
                                      std::string( "/base_scan" ) );

    paramHandle.param< std::string >( "tf_base_link", tf_state_topic,
                                      std::string( "/state_base_link" ) );
    paramHandle.param< std::string >( "tf_laser_link", tf_laser_link,
                                      std::string( "/hokuyo1_link" ) );
    ...
    paramHandle.param< std::string >( "output_map_file_name", output_map_name,
    ...
    paramHandle.param< std::string >( "bagfile_name", bagfilename,
                                      std::string( "bagfile_unset.bag" ) );
    ...
    tfMessageReader< sensor_msgs::LaserScan > reader( bagfilename, input_laser_topic,
                                                      std::string( "/world" ), tf_odo_topic );
    ...
    while ( !reader.bagEnd() && ros::ok() ) {
        ...
        if ( reader.getNextMessage( s, odo_pose ) ) { hasOdo = true; }

        tf::Transform state_pose;

        if ( reader.getTf( tf_state_topic, s.header.stamp, state_pose ) ) { hasState = true; }

        /// If we have the data then lets run the localization
        if ( hasState && hasOdo ) { callback( s, odo_pose, state_pose ); }
#ifdef USE_VISUALIZATION_DEBUG
//                ndt_viz.win3D->process_events();
#endif
    }
