...
void CParticleFilter::allocate( int num_particles ) {
    ...
    // fprintf(stderr,"CParticleFilter::allocate():: Called and reserved %d bytes memory = %d
...
mcl::pose CParticleFilter::getDistributionMean( bool doWeighting ) {
    ...
    if ( doWeighting ) {
        ...
        if ( NumOfParticles == 0 )
            fprintf( stderr, "CParticleFilter::getDistributionMean():: WTF!!!!\n" );
...
void CParticleFilter::initializeNormalRandom( mcl::pose p0, mcl::pose variance, int size ) {
    ...
    if ( this->size != size ) { this->allocate( size ); }
    for ( i = 0; i < size; i++ ) {
...
void CParticleFilter::normalize() {
    ...
    if ( summ != 0 ) {
        for ( i = 0; i < NumOfParticles; i++ ) { Particles[i].p = Particles[i].p / summ; }
    } else {
        for ( i = 0; i < NumOfParticles; i++ ) { Particles[i].p = 1.0 / NumOfParticles; }
    }
}
...
void CParticleFilter::updateLikelihood( float* lik ) {
    int i;
    for ( i = 0; i < NumOfParticles; i++ ) { Particles[i].lik = lik[i]; }
    isAvgSet = false;
}
...
void CParticleFilter::print() {
    ...
    fprintf( stderr, "Filter:: size=%d, AVG:(%.1f,%.1f,%.1f), VAR:(%.1f,%.1f,%.1f)\n", size,
             average.x, average.y, average.a, variance.x, variance.y, variance.a );
}
...
void CParticleFilter::resize( int n ) {
    ...
    fprintf( stderr, "The best p=%f and worst %f \n", Particles[ind[0]].p,
             Particles[ind[NumOfParticles - 1]].p );

    for ( i = 0; i < n; i++ ) { tmp[i] = Particles[ind[NumOfParticles - 1 - i]]; }
    ...
    fprintf( stderr, "Filter resized. New size=%d. The Pb=%f Pw=%f\n", n, Particles[0].p,
             Particles[NumOfParticles - 1].p );
}

mcl::pose CParticleFilter::averageOverNBestAndRandomize( int N, int M, float vx, float vy,
                                                         float va ) {
    if ( N >= NumOfParticles ) { return getDistributionMean( true ); }
    mcl::pose avg;
    if ( N <= 0 ) return avg;
    int* ind = (int*) malloc( NumOfParticles * sizeof( int ) );
