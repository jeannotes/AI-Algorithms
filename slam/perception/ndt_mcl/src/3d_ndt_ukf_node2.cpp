...
class NDTUKF3DNode {
  ...
  public:
    NDTUKF3DNode( ros::NodeHandle param_nh )
        ...
        param_nh.param< double >( "ukf_range_filter_max_dist", ukf_params.range_filter_max_dist,
                                  1. );
        ...
        if ( scan_topic != std::string( "" ) ) {
            ...
            scan_sub_ = nh_.subscribe< velodyne_msgs::VelodyneScan >(
                scan_topic, 10, &NDTUKF3DNode::scan_callback, this );
            if ( scan2_topic != std::string( "" ) ) {
                scan2_sub_ = nh_.subscribe< velodyne_msgs::VelodyneScan >(
                    scan2_topic, 10, &NDTUKF3DNode::scan2_callback, this );
                ROS_INFO_STREAM(
        ...
        markerarray_pub_ =
            nh_.advertise< visualization_msgs::MarkerArray >( "visualization_marker_array", 10 );
        ...
        heartbeat_slow_visualization_ = nh_.createTimer(
            ros::Duration( 1.0 ), &NDTUKF3DNode::publish_visualization_slow, this );
        heartbeat_fast_visualization_ = nh_.createTimer(
            ros::Duration( 0.1 ), &NDTUKF3DNode::publish_visualization_fast, this );
        ...
        if ( !gt_file_.is_open() || !gt2d_file_.is_open() || !est_file_.is_open() ||
            ...
            ROS_ERROR_STREAM( "Failed to open : "
                              << est_file_.rdbuf() << " | " << est2d_file_.rdbuf() << " | "
                              << gt_file_.rdbuf() << " | " << gt2d_file_.rdbuf() );
        }

        if ( gt_topic != std::string( "" ) ) {
            ROS_INFO_STREAM( "Subscribing to : " << gt_topic );
            gt_sub = nh_.subscribe< nav_msgs::Odometry >( gt_topic, 10, &NDTUKF3DNode::gt_callback,
                                                          this );
        }
    }
    ~NDTUKF3DNode() {
    ...
    void publish_visualization_fast( const ros::TimerEvent& event ) {
        if ( do_pub_sigmapoints_markers_ ) {
            ...
            for ( int i = 0; i < sigmas.size(); i++ ) {
                markerarray_pub_.publish( ndt_visualisation::getMarkerFrameAffine3d(
                    sigmas[i], "sigmapoints" + ndt_generic::toString( i ), 1., 0.1 ) );
                std::cout << "sigmas[i] : " << ndt_generic::affine3dToStringRPY( sigmas[i] )
                          << std::endl;
            }
            ukf_m.unlock();
        }
    ...
    void initialize() {
        ...
        // ndtukf->initializeFilter(initPoseT, 50, 50, 10, 100.0*M_PI/180.0, 100.0*M_PI/180.0
        // ,100.0*M_PI/180.0);
        // ndtukf->initializeFilter(initPoseT, 0.5, 0.5, 0.5, 2.0*M_PI/180.0, 2.0*M_PI/180.0
        // ,2.0*M_PI/180.0);
        //        ndtukf->initializeFilter(initPoseT, 2, 2, 2, 5.0*M_PI/180.0, 5.0*M_PI/180.0
        //        ,5.0*M_PI/180.0);
    ...
    void scan_callback( const velodyne_msgs::VelodyneScan::ConstPtr& scanMsg ) {
        ...
        for ( size_t next = 0; next < scanMsg->packets.size(); ++next ) {
            ...
            for ( size_t i = 0; i < conv_points.size(); i++ ) {
                ...
                } else if ( conv_points.points[i].ring % ( skip_nb_rings_ + 1 ) ==
                            skip_start_ % ( skip_nb_rings_ + 1 ) ) {
                    cloud.push_back( pcl::PointXYZ( conv_points.points[i].x,
                                                    conv_points.points[i].y,
                                                    conv_points.points[i].z ) );
                } else {
...

        {
            ...
            if ( draw_ml_lines ) {
                marker_pub_.publish( ndt_visualisation::getMarkerLineListFromTwoPointClouds(
                    ndtukf->getFilterRaw(), ndtukf->getFilterPred(), 0, "ml_lines", "interppoints",
                    0.05 ) );
            }
        }
