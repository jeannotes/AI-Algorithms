...
void sendMapToRviz( perception_oru::NDTMap& map ) {
    ...
    for ( unsigned int i = 0; i < ndts.size(); i++ ) {
        ...
        if ( d <= 0 ||
             d != d ) {   // Note: "d!=d" is a great test for invalid numbers, don't remove!
            continue;
            // All zeros:
...
void callback( const sensor_msgs::LaserScan::ConstPtr& scan ) {
    ...
    tf_listener.waitForTransform( "world", tf_state_topic, scan->header.stamp,
                                  ros::Duration( 1.0 ) );
    ...
    if ( isFirstLoad == false ) {
        if ( ( Tmotion.translation().norm() < 0.005 &&
               fabs( Tmotion.rotation().eulerAngles( 0, 1, 2 )[2] ) < ( 0.2 * M_PI / 180.0 ) ) ) {
    ...
    ndtmcl->updateAndPredict(
        Tmotion, *cloud );   ///<predicts, updates and resamples if necessary (ndt_mcl.hpp)
    ...
    if ( counter % 50 == 0 ) { sendMapToRviz( ndtmcl->map ); }
