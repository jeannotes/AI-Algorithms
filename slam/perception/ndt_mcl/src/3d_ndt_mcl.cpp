...
void NDTMCL3D::updateAndPredict( Eigen::Affine3d Tmotion,
    ...
    pf.predict( Tmotion, tr[0] * 0.1, tr[1] * 0.1, tr[2] * 0.01, rot[0] * 0.01, rot[1] * 0.01,
                rot[2] * 0.1 );
    double t_pred = getDoubleTime() - t_start;

    // pf.predict(mcl::pose(tr[0],tr[1],rot[2]), mcl::pose(tr[0]*0.1 + 0.005,tr[1]*0.1+
    // 0.005,rot[2]*0.1+0.001));
    ...
    if ( forceSIR ) {
        ...
        varP /= pf.size();
        varP = sqrt( varP );
        fprintf( stderr, "Var P=%lf (Npf=%d, Nm=%d) (t_pred = %.3lf t_pseudo=%.3lf)", varP,
                 pf.size(), Nn, t_pred, t_pseudo );
        if ( varP > /*0.006*/ SIR_varP_threshold ||
             sinceSIR > /*25*/ SIR_max_iters_wo_resampling ) {
            fprintf( stderr, "-SIR- " );
            sinceSIR = 0;
            pf.SIRUpdate();
        } else {
...
void NDTMCL3D::updateAndPredictEff( Eigen::Affine3d Tmotion,
    ...
    std::cerr << "cloud points " << cloud.points.size() << " res :" << resolution
              << " sres: " << resolution_sensor << std::endl;
    perception_oru::NDTMap local_map( new perception_oru::LazyGrid( resolution_sensor ) );
