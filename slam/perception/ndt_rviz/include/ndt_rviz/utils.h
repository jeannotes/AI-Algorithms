...
inline double getRobustYawFromAffine3d( const Eigen::Affine3d& a ) {
    // To simply get the yaw from the euler angles is super sensitive to numerical errors which will
    // cause roll and pitch to have angles very close to PI...
    Eigen::Vector3d v1( 1, 0, 0 );
    Eigen::Vector3d v2 = a.rotation() * v1;
    double dot =
        v1( 0 ) * v2( 0 ) + v1( 1 ) * v2( 1 );   // Only compute the rotation in xy plane...
    double angle = acos( dot );
    // Need to find the sign
    if ( v1( 0 ) * v2( 1 ) - v1( 1 ) * v2( 0 ) > 0 ) return angle;
    return -angle;
}

inline void distanceBetweenAffine3d( const Eigen::Affine3d& p1, const Eigen::Affine3d& p2,
                                     double& dist, double& angularDist ) {
...
inline Eigen::Affine2d eigenAffine3dTo2d( const Eigen::Affine3d& a3d ) {
    return Eigen::Translation2d( a3d.translation().topRows< 2 >() ) *
           Eigen::Rotation2D< double >(
               getRobustYawFromAffine3d( a3d ) );   // a3d.linear().topLeftCorner<2,2>();
}

inline Eigen::Affine3d eigenAffine2dTo3d( const Eigen::Affine2d& a2d ) {
    //  Eigen::Rotation2D<double> rot =
    //  Eigen::Rotation2D<double>::fromRotationMatrix(a2d.rotation());//Eigen::fromRotationMatrix(a2d.translation());
    double angle = atan2(
        a2d.rotation()( 1, 0 ),
        a2d.rotation()( 0, 0 ) );   // rot.angle();//acosa2d.rotation()(0,1)/a2d.rotation()(0,0);
...
inline Eigen::Affine2d getAffine2d( double x, double y, double th ) {
    return Eigen::Translation2d( x, y ) * Eigen::Rotation2D< double >( th );
}

Eigen::Transform< double, 3, Eigen::Affine, Eigen::ColMajor >
ICPwithCorrMatch( perception_oru::NDTMap& targetNDT, perception_oru::NDTMap& sourceNDT,
...
void printTransf( const Eigen::Transform< double, 3, Eigen::Affine, Eigen::ColMajor >& T ) {
    std::cout << "translation " << T.translation().transpose() << " (norm) "
              << T.translation().norm() << std::endl;
    std::cout << "rotation " << T.rotation().eulerAngles( 0, 1, 2 ).transpose() << " (norm) "
              << T.rotation().eulerAngles( 0, 1, 2 ).norm() << std::endl;
}

void printTransf2d( const Eigen::Transform< double, 3, Eigen::Affine, Eigen::ColMajor >& T ) {
    std::cout << "[" << T.translation()[0] << "," << T.translation()[1] << "]("
...
void setNDTCellToMap( NDTMap* map, NDTCell* cell ) {
    ...
    if ( lz != NULL ) {
        ...
        if ( c == NULL ) {
            // Needs some means to create it... do this by adding a point the returned cell will be
            // an nice an allocated one.
...
Eigen::Vector3d computeLocalCentroid( const Eigen::Vector3d& map_centroid,
                                      const Eigen::Vector3d& local_pos, double resolution ) {
...
std::string
transformToEval2dString( const Eigen::Transform< double, 3, Eigen::Affine, Eigen::ColMajor >& T ) {
    std::ostringstream stream;
    stream << std::setprecision( std::numeric_limits< double >::digits10 );
    double yaw = getRobustYawFromAffine3d( T );
    Eigen::Affine3d T2 =
        Eigen::Translation< double, 3 >( T.translation()[0], T.translation()[1], 0. ) *
        Eigen::AngleAxis< double >( yaw, Eigen::Vector3d::UnitZ() );
    Eigen::Quaternion< double > tmp( T2.rotation() );
    stream << T2.translation()[0] << " " << T2.translation()[1] << " 0. "
           << " " << tmp.x() << " " << tmp.y() << " " << tmp.z() << " " << tmp.w() << std::endl;
    return stream.str();
}
