...
* This class reads the _whole_ tf to cache and uses this to sync the velodyne messages with the
...
* an extra link as a parameter (note that then give id to this extra link in constructor in this
case).
*
...
class VelodyneBagReader {
  public:
     ...
     * @param dur The buffer size (must be larger than the length of the bag file gives) default =
     * 3600s
     * @param sensor_link An optional static link that takes e.g. your /odom to the sensor frame
     */
    VelodyneBagReader( std::string calibration_file, std::string bagfilename,
                       std::string velodynetopic, const std::string& tf_pose_id,
                       const std::string& fixed_frame_id = "/world",
                       const std::string& tftopic        = "/tf",
                       ros::Duration dur                 = ros::Duration( 3600 ),
                       tf::StampedTransform* sensor_link = NULL, double velodyne_max_range = 130.0,
                       double velodyne_min_range = 2.0, double sensor_time_offset = 0.0 )

    {
        // The view_direction / view_width is important to have set. The min/max range is
        // overwritten in the setupOffline
        double view_direction = 0;
        double view_width     = 2 * M_PI;
        dataParser.setParameters( velodyne_min_range, velodyne_max_range, view_direction,
                                  view_width );
        std::cout << "Velodyne PARAMETERS FILTER: max=" << velodyne_max_range << ","
                  << velodyne_min_range << std::endl;
        if ( dataParser.setupOffline( calibration_file, velodyne_max_range, velodyne_min_range ) !=
             0 )
            exit( 0 );
        ...
        odosync =
            new PoseInterpolationNavMsgsOdo( view, tftopic, fixed_frame_id, dur, sensor_link );
#ifdef READ_RMLD_MESSAGES
     ...
     * @param sensor_pose [out] The pose of the sensor origin. (Utilizes the tf_pose_id and
     *sensor_link from the constructor).
     **/
    ...
    * @param[out] cloud<PointT> : point cloud out at the end. pointT is the type of the cloud but it
    * needs to be x,y,z even though it's templated.
    ...
    */
     ...
     */
    // bool readMultipleMeasurements(unsigned int Nmeas, pcl::PointCloud<PointT> &cloud,
    // tf::Transform &sensor_pose, tf::Transform &base_pose, std::string base_pose_id){
    bool readMultipleMeasurements( unsigned int Nmeas, pcl::PointCloud< PointT >& cloud,
                                   tf::Transform& sensor_pose, tf::Transform& base_pose,
                                   const std::string& base_pose_id ) {
        if ( readMultipleMeasurements( Nmeas, cloud, sensor_pose ) ) {
            odosync->getTransformationForTime( timestamp_of_last_sensor_message, base_pose_id,
                                               base_pose );
        } else {
            return false;
        }
    ...
    bool getPoseFor( tf::Transform& pose, const std::string& pose_id ) {
        if ( odosync->getTransformationForTime( timestamp_of_last_sensor_message, pose_id,
                                                pose ) ) {
            return true;
        }
        return false;
    ...
    ros::Time getTimeStampOfLastSensorMsg() const { return timestamp_of_last_sensor_message; }
  ...
  public:
    bool getSynchronizedRMLD( std::string rmld_tf_name, tf::Transform& rmld_pose, double& rmld_val,
                              double& amtec_pos, int& tilt_state ) {
        return rmldsync->getDataForTime( timestamp_of_last_sensor_message, rmld_tf_name, rmld_pose,
                                         rmld_val, amtec_pos, tilt_state );
    }
#endif
};
