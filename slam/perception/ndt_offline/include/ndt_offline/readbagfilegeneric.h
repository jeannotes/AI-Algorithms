...
class ReadBagFileGeneric {
  public:
    ReadBagFileGeneric( const std::string& type, const std::string velodyne_tf_interpolation_string,
                        std::string calibration_file, std::string bagfilename,
        ...
        if ( type.compare( "pcl_reader" ) == 0 ) {
            preader_ = new PointCloudBagReader< PointT >(
                calibration_file, bagfilename, point_cloud_topic, tf_pose_id, fixed_frame_id,
                tftopic, dur, sensor_link, velodyne_max_range, velodyne_min_range,
                sensor_time_offset );
        } else {
            vreader_ = new VelodyneBagReader< PointT >(
                calibration_file, bagfilename, point_cloud_topic, tf_pose_id, fixed_frame_id,
                tftopic, dur, sensor_link, velodyne_max_range, velodyne_min_range,
                sensor_time_offset );
        }
    ...
    bool ReadNextMeasurement( pcl::PointCloud< PointT >& cloud ) {
        if ( vreader_ != NULL ) {
            ...
            return vreader_->readMultipleMeasurements(
                nb_measurements_, cloud, tf_scan_source, tf_gt_base,
                velodyne_tf_interpolation_string_ );   // if velodyne reader is used, the cloud
                                                       // needs to be compensated for movement
                                                       // during scan
        } else if ( preader_ != NULL )
            return preader_->readNextMeasurement( cloud );   // if preader reader is used, the cloud
                                                             // was already adjusted for movement in
                                                             // theconversion.
    }
