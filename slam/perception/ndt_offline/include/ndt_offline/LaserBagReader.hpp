...
class LaserBagReader
    : public BagRangeReaderInterface< sensor_msgs::LaserScan, sensor_msgs::LaserScan::ConstPtr > {
  private:
  ...
  public:
    ...
    */
    LaserBagReader( std::string calibration_file, std::string bagfilename,
                    std::string velodynetopic, std::string tf_base_link, std::string tf_sensor_link,
                    std::string fixed_frame_id = "/world", std::string tftopic = "/tf",
                    ...
                    double sensor_time_offset = 0.0, double lvariance = 0.02,
                    double min_laser_range = 0.5, bool use_odo = true, bool use_sensor_pose = true )
        : BagRangeReaderInterface< sensor_msgs::LaserScan, sensor_msgs::LaserScan::ConstPtr >(
              calibration_file, bagfilename, velodynetopic, tf_base_link, tf_sensor_link,
              fixed_frame_id, tftopic, dur, sensor_link, sensor_time_offset, use_odo,
              use_sensor_pose ),
          _laser_variance( lvariance ),
          _min_laser_range( min_laser_range ) {}

    void setLaserVariance( double var ) { _laser_variance = var; }
    double getLaserVariance() { return _laser_variance; }

    void convertLaser( const sensor_msgs::LaserScan::ConstPtr laser,
    ...
    // 			* @param[out] cloud<PointT> : point cloud out at the end. pointT is the type of the
    // cloud
    ...
    */
    bool readMultipleMeasurements( unsigned int Nmeas, pcl::PointCloud< PointT >& cloud ) {
        ...
        // 				//Should be this but here is an error a certain point cloud
        // laser_scan_with_ransac
        // :
        ...
        if ( cloud.points.size() != cloud.width * cloud.height ) {
            std::cout << "Weird cloud sizes. I don't know what's going on:" << cloud.points.size()
                      << " != " << cloud.width << " * " << cloud.height << " laser "
                      << last_pointcloud.height << " " << last_pointcloud.width << std::endl;
            exit( 0 );
        } else {
