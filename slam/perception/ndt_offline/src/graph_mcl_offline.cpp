...
void filter_ring_nb_fun( pcl::PointCloud< PointT >& cloud,
                         pcl::PointCloud< PointT >& cloud_nofilter, const std::set< int >& rings ) {
    std::cerr << "Can only filter_ring_nb if they are of type velodyne_pointcloud::PointXYZIR"
              << std::endl;
    cloud = cloud_nofilter;
}

template <>
void filter_ring_nb_fun< velodyne_pointcloud::PointXYZIR >(
...
void ReadAllParameters( po::options_description& desc, int& argc, char*** argv ) {
    ...
    desc.add_options()( "help", "produce help message" )(
        "map-file-path",
        po::value< std::string >( &map_file_name )->default_value( std::string( "" ) ),
        ...
        "bag-file-path", po::value< string >( &bagfilename )->default_value( "" ),
        "File path to rosbag to play with maps" )( "visualize", "visualize the rosbag and fuser "
                                                                "estimate/gt" )(
        ...
        "output-dir-name", po::value< string >( &output_dir_name )->default_value( "" ),
        ...
        po::value< string >( &localisation_type )->default_value( "mcl_ndt" ),
        "name of localisation algorihm e.g. mcl_ndt" )( "filter-fov", "cutoff part of the field of "
                                                                      "view" )(
        ...
        "the minimum field of view angle horizontal" )( "filter-ring-nb", "if the number of rings "
                                                                          "should be reduced" )(
        "z-filter-height", po::value< double >( &z_filter_min_height )->default_value( -10000.0 ),
        ...
        "frame_id of the velodyne" )( "min_range",
                                      po::value< double >( &min_range )->default_value( 0.6 ),
                                      "minimum range used from scanner" )(
        "max_range", po::value< double >( &max_range )->default_value( 30 ),
        "minimum range used from scanner" )( "save-map", "saves the graph map at the end of "
                                                         "execution" )(
        "skip-frame", po::value< unsigned int >( &skip_frame )->default_value( 20 ),
        ...
        "ey", po::value< double >( &euler[1] )->default_value( 0. ), "sensor pose - euler angle "
                                                                     "vector y" )(
        "ez", po::value< double >( &euler[2] )->default_value( 0. ), "sensor pose - euler angle "
                                                                     "vector z" )(
        "sensor_time_offset", po::value< double >( &sensor_time_offset )->default_value( 0. ),
        "timeoffset of the scanner data" )( "registration2d", "registration2d" )(
        "resolution", po::value< double >( &resolution )->default_value( 0.4 ),
        "resolution of the map" )( "n-particles",
                                   po::value< unsigned int >( &n_particles )->default_value( 270 ),
        ...
        "use_pointtype_xyzir", "If the points to be processed should contain ring and intensity "
                               "information (velodyne_pointcloud::PointXYZIR)" )(
    ...
    if ( MCLNDTParamPtr parPtr =
             boost::dynamic_pointer_cast< MCLNDTParam >( localisation_param_ptr ) ) {
...
void printParameters() {
    ...
    cout << "Lidar topic: " << velodyne_packets_topic << ", lidar frame id: " << velodyne_frame_id
         << endl;
...
void processData() {
    ...
    if ( map_file_name.length() > 0 ) {
        ...
        if ( LocateMapFilePath( map_dir_name, map_file_path ) ) {
            ...
            for ( std::vector< std::string >::iterator it = map_file_path.begin();
                  it != map_file_path.end(); ++it )
                cout << *it << endl;
        } else {
    ...
    for ( std::vector< string >::iterator it = map_file_path.begin(); it != map_file_path.end();
          ++it ) {
        string map_file = *it;
        cout << "Opening map number :" << ( it - map_file_path.begin() + 1 ) << " out of "
             << ( ( map_file_path.end() - map_file_path.begin() ) ) << endl;
        std::ifstream ifs( map_file );
        boost::archive::text_iarchive ia( ifs );
        ia >> graph_map;

        localisation_param_ptr->graph_map_ = graph_map;
        localisation_type_ptr =
            LocalisationFactory::CreateLocalisationType( localisation_param_ptr );
        ...
        std::string output_file_name = ndt_generic::removeExtension( map_file ) + "_npart=" +
                                       toString( n_particles ) + "_mpsu=" +
                                       toString( min_nb_points_set_uniform ) + "_mnpfg=" +
                                       toString( min_nb_points_for_gaussian );
        ndt_generic::CreateEvalFiles eval_files( output_dir_name, output_file_name,
                                                 save_eval_results );
        int counter = 0;
        ReadBagFileGeneric< PointT > reader(
            reader_type, base_link_id, velodyne_config_file, bagfilename, velodyne_packets_topic,
            velodyne_frame_id, tf_world_frame, tf_topic, ros::Duration( 3600 ), &sensor_link,
            max_range, min_range, sensor_time_offset );
        printParameters();

        pcl::PointCloud< PointT > cloud, cloud_nofilter;
        tf::Transform tf_scan_source;
        tf::Transform tf_gt_base;
        Eigen::Affine3d Todom_base, odom_pose, Todom_base_prev,
            Todom_init;   // Todom_base =current odometry pose, odom_pose=current aligned with gt,
                          // Todom_base_prev=previous pose, Todom_init= first odometry pose in
                          // dataset.
        Eigen::Affine3d Tgt_base, Tgt_base_prev, Tgt_init;   // Tgt_base=current GT
                                                             // pose,Tgt_base_prev=previous GT pose,
                                                             // Tgt_init=first gt pose in dataset;
        ...
        while ( reader.ReadNextMeasurement( cloud_nofilter ) ) {
            ...
            if ( ( counter == 1 ) ) {
                ...
                localisation_type_ptr->InitializeLocalization( Tgt_base, variances );
                counter++;
                cloud.clear();
                cloud_nofilter.clear();
                continue;
            ...
            odom_pose =
                Tgt_init * Todom_init.inverse() * Todom_base;   // Correct for initial odometry
            ...
            if ( visualize && counter % skip_frame == 0 ) {   // This is relatively cheap to plot,
                                                              // note also that this is given in the
                                                              // vehicle frame...
                br.sendTransform(
                    tf::StampedTransform( sensor_link, tplot, tf_fuser_frame, velodyne_frame_id ) );
            ...
            if ( visualize && counter % skip_frame == 0 ) {
                GraphPlot::PlotMap( graph_map->GetCurrentNode()->GetMap(), -1,
                                    graph_map->GetCurrentNodePose(), marker_ );
            ...
            cloud.clear();
            cloud_nofilter.clear();
            eval_files.Write( reader.getTimeStampOfLastSensorMsg(), Tgt_base, odom_pose, fuser_pose,
                              sensor_offset );

            t4 = ros::Time::now();
            cout << "iteration: " << t5 - t4 << ", update: " << t2 - t1 << ", plot: " << t3 - t2
                 << endl;
            t5 = ros::Time::now();
            counter++;
            if ( step_control ) char c = getchar();
        }
        eval_files.Close();
    }
}
