...
bool ReadAllParameters( po::options_description& desc, int& argc, char*** argv ) {
    ...
    desc.add_options()( "help", "produce help message" )(
        "map-file-path",
        po::value< std::string >( &map_file_name )->default_value( std::string( "graph_map.MAP" ) ),
        "name of file to load containing graphMap" )( "visualize", "visualize the output" )(
        ...
        "use-submap", "Adopt the sub-mapping technique which represent the global map as a set of "
                      "local submaps" );
    ...
    if ( !GetSensorPose( dataset, transl, euler, tf_sensor_pose ) ) exit( 0 );
...
int main( int argc, char** argv ) {
    ...
    base_name += motion_params.getDescString() + std::string( "_res" ) + toString( resolution ) +
                 std::string( "_SC" ) + toString( do_soft_constraints ) +
                 std::string( "_mindist" ) + toString( min_dist ) + std::string( "_sensorcutoff" ) +
                 toString( max_range ) + std::string( "_stepcontrol" ) + toString( step_control ) +
                 std::string( "_neighbours" ) + toString( nb_neighbours ) + std::string( "_rlf" ) +
                 toString( resolution_local_factor );
    ...
    for ( int i = 0; i < ros_bag_paths.size(); i++ ) {
        ...
        cout << velodyne_config_file << "," << bagfilename << "," << velodyne_packets_topic << ","
             << velodyne_frame_id << "," << tf_world_frame << "," << tf_topic << endl;
        ...
        while ( vreader.readMultipleMeasurements( nb_scan_msgs, cloud_nofilter, tf_scan_source,
                                                  tf_gt_base, base_link_id ) ) {
            if ( !n_->ok() ) exit( 0 );
            ...
            if ( counter == 1 ) {
                ...
                NDTMapPtr curr_node       = boost::dynamic_pointer_cast< NDTMapType >(
                    graph_map->GetCurrentNode()->GetMap() );
                mcl = new NDTMCL3D( resolution, *curr_node->GetNDTMap(), 0 );
                ...
                mcl->initializeFilter( tgt_pos( 0 ), tgt_pos( 1 ), tgt_pos( 2 ), tgt_euler( 0 ),
                                       tgt_euler( 1 ), tgt_euler( 2 ), 0.1, 0.1, 0.0001, 0.00001,
                                       0.00001, 0.001, 600 );
                counter++;
                cloud.clear();
                cloud_nofilter.clear();
            ...
            if ( visualize ) {
                br.sendTransform( tf::StampedTransform( tf_gt_base, ros::Time::now(), "/world",
                                                        "/state_base_link" ) );
                if ( counter % 10 == 0 ) {
            ...
            if ( visualize && counter % 10 == 0 ) {
                ...
                NDTMapPtr curr_node = boost::dynamic_pointer_cast< NDTMapType >(
                    graph_map->GetCurrentNode()->GetMap() );
                GraphPlot::SendGlobalMapToRviz( curr_node->GetNDTMap(), 1,
                                                graph_map->GetCurrentNodePose() );
            }
            ...
            eval_files.Write( vreader.getTimeStampOfLastSensorMsg(), Tgt_base, Todom_base,
