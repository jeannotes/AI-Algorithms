...
double getYawFromXY( const geometry_msgs::Pose& prev, const geometry_msgs::Pose& curr ) {
    double dx = curr.position.x - prev.position.x;
    double dy = curr.position.y - prev.position.y;
    return atan2( dy, dx );
}

double getDistBetween( const geometry_msgs::Pose& p1, const geometry_msgs::Pose& p2 ) {
...
double getYawUsingPoses( const boost::circular_buffer< geometry_msgs::Pose >& poses,
                         const geometry_msgs::Pose& pose, double min_dist, bool& valid_yaw ) {
...
void get_euler_angles( Eigen::Vector3d& euler, const geometry_msgs::PoseStamped& p ) {
    // Set yaw from the current odometry... the current EKF pose utilize the GPS heading for the
    // yaw, the roll and pitch are directly taken from the IMU.
    Eigen::Quaterniond q;
...
void GetSensorPose( const std::string& dataset, Eigen::Vector3d& transl, Eigen::Vector3d& euler,
    ...
    cout << "Sensor offset (x,y,z)=(" << transl( 0 ) << "," << transl( 1 ) << "," << transl( 2 )
         << ")" << endl
         << "Sensor angles (r,p,y)=(" << euler( 0 ) << "," << euler( 1 ) << "," << euler( 2 ) << ")"
         << endl;
    if ( interactive ) {
...
int main( int argc, char** argv ) {
    ...
    std::string velodyne_config_file, velodyne_packets_topic, velodyne_frame_id, base_link_id,
        gt_base_link_id, tf_topic, tf_world_frame, interpolation_link_id;
    double min_dist;
    double min_range = 0.6, max_range = 30;
    Eigen::Vector3d imu_offset_vec;
    ;
    po::options_description desc( "Allowed options" );
    desc.add_options()( "help", "produce help message" )( "debug", "additional output" )(
        ...
        "velodyne_config_file", po::value< std::string >( &velodyne_config_file )
                                    ->default_value( ros::package::getPath( "ndt_offline" ) +
                                                     std::string( "/config/velo32.yaml" ) ),
        "configuration file for the scanner" )(
        "velodyne_packets_topic", po::value< std::string >( &velodyne_packets_topic )
        ...
        "tf topic to listen to" )( "min_range",
                                   po::value< double >( &min_range )->default_value( 0.7 ),
                                   "minimum range used from scanner" )(
        "max_range", po::value< double >( &max_range )->default_value( 30 ), "minimum range used "
                                                                             "from scanner" )(
        "interpolation_link",
        po::value< std::string >( &interpolation_link_id )->default_value( "/state_base_link" ),
        "id in which lidar is interpolated" )(
        "x", po::value< double >( &transl[0] )->default_value( 0. ),
        "sensor pose - translation "
        "vector x" )( "y", po::value< double >( &transl[1] )->default_value( 0. ),
                      "sensor pose - translation "
        ...
        "vector x" )( "ey", po::value< double >( &euler[1] )->default_value( 0. ),
                      "sensor pose - euler angle "
                      "vector y" )( "ez", po::value< double >( &euler[2] )->default_value( 0. ),
                                    "sensor pose - euler angle "
                                    "vector z" )(
        "data-set", po::value< string >( &dataset )->default_value( std::string( "arla-2012" ) ),
        "choose which dataset that is currently used, this option will assist with assigning the "
        "sensor pose" )( "create_images", "create depth and intensity images instead of point "
                                          "clouds" )( "interactive", "interactive mode" );
    ...
    sensor_link.frame_id_ =
        interpolation_link_id;   // base_link_id;//tf_base_link; //"/odom_base_link";
    sensor_link.setData( tf_sensor_pose );
    // vmc_navserver::VMCEncodersStamped prev_enc_msg;
    ConvertVelodyneBagsToPcl reader( outbag_name, velodyne_config_file, inbag_name,
                                     velodyne_packets_topic,   //"/velodyne_packets"
                                     velodyne_frame_id, tf_world_frame, tf_topic,
                                     ros::Duration( 3600 ), &sensor_link, max_range, min_range, 0 );
