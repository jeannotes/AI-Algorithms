...
std::string toString( const T& x ) {
    ...
    if ( !( o << x ) ) throw std::runtime_error( "::toString()" );

    return o.str();
}

std::string
transformToEvalString( const Eigen::Transform< double, 3, Eigen::Affine, Eigen::ColMajor >& T ) {
...
bool GetSensorPose( const std::string& dataset, Eigen::Vector3d& transl, Eigen::Vector3d& euler,
                    tf::Transform& tf_sensor ) {
    tf::Quaternion quat;
...
bool LocateRosBagFilePaths( const std::string& folder_name,
...
bool ReadAllParameters( po::options_description& desc, int& argc, char*** argv ) {
    ...
    desc.add_options()( "help", "produce help message" )(
        "map_type_name",
        po::value< string >( &map_type_name )->default_value( std::string( "default" ) ),
        "type of map to use e.g. ndt_map or ndt_dl_map (default it default)" )(
        "registration_type_name", po::value< string >( &registration_type_name )
                                      ->default_value( std::string( "ndt_d2d_"
                                                                    "reg" ) ),
        "type of map to use e.g. ndt_d2d_reg or ndt_dl_reg (default it default)" )(
        "visualize", "visualize the output" )( "use-odometry", "use initial guess from odometry" )(
        ...
        "reader-type", po::value< string >( &bag_reader_type )->default_value( "velodyne_reader" ),
        "e.g. velodyne_reader or pcl_reader" )(
        "data-set", po::value< string >( &dataset )->default_value( std::string( "default" ) ),
        "choose which dataset that is currently used, this option will assist with assigning the "
        "sensor pose" )( "dir-name", po::value< string >( &map_dir_name ), "where to look for ros "
                                                                           "bags" )(
        "output-dir-name",
        po::value< string >( &output_dir_name )->default_value( "/home/daniel/.ros/maps" ),
        "where to save the pieces of the map (default it ./map)" )(
        "map-size-xy", po::value< double >( &map_size_xy )->default_value( 83. ),
        "size of submaps" )( "map-size-z", po::value< double >( &map_size_z )->default_value( 6.0 ),
                             "size of submaps" )( "itrs",
                                                  po::value< int >( &itrs )->default_value( 30 ),
                                                  "number of iteration in the registration" )(
        "fuse-incomplete",
        "fuse in registration estimate even if iterations ran out. may be useful in combination "
        "with low itr numbers" )( "filter-fov", "cutoff part of the field of view" )(
        "hori-max", po::value< double >( &hori_max )->default_value( 2 * M_PI ),
        "the maximum field of view angle horizontal" )(
        "hori-min", po::value< double >( &hori_min )->default_value( -hori_max ),
        ...
        "Dt", po::value< double >( &motion_params.Dt )->default_value( 1. ), "forward uncertainty "
                                                                             "on rotation" )(
        "Cd", po::value< double >( &motion_params.Cd )->default_value( 1. ), "side uncertainty on "
                                                                             "distance traveled" )(
        "Ct", po::value< double >( &motion_params.Ct )->default_value( 1. ),
        "side uncertainty on "
        "rotation" )( "Td", po::value< double >( &motion_params.Td )->default_value( 1. ),
                      "rotation uncertainty on distance traveled" )(
        "Tt", po::value< double >( &motion_params.Tt )->default_value( 1. ),
        "rotation uncertainty "
        "on rotation" )( "min_dist", po::value< double >( &min_dist )->default_value( 0.2 ),
                         "minimum distance traveled before adding cloud" )(
        "min_rot_in_deg", po::value< double >( &min_rot_in_deg )->default_value( 5 ),
        ...
        "velodyne_config_file", po::value< std::string >( &velodyne_config_file )
        ...
        "tf world frame" )( "velodyne-packets-topic",
                            po::value< std::string >( &velodyne_packets_topic )
                                ->default_value( std::string( "/velodyne_packets" ) ),
                            "velodyne packets topic used" )(
        "velodyne_frame_id",
        po::value< std::string >( &velodyne_frame_id )->default_value( std::string( "/velodyne" ) ),
        "frame_id of the laser sensor" )( "alive", "keep the mapper/visualization running even "
                                                   "though it is completed (e.g. to take screen "
                                                   "shots etc." )(
        "nb_neighbours", po::value< int >( &nb_neighbours )->default_value( 2 ),
        "number of neighbours used in the registration" )(
        "min-range", po::value< double >( &min_range )->default_value( 1.0 ), "minimum range used "
                                                                              "from scanner" )(
        "max-range", po::value< double >( &max_range )->default_value( 30 ),
        "minimum range used from scanner" )( "save-map", "saves the graph map at the end of "
                                                         "execution" )(
        "nb_scan_msgs", po::value< int >( &nb_scan_msgs )->default_value( 1 ),
        "number of scan messages that should be loaded at once from the bag" )(
        "disable-keyframe-update", "use every scan to update map rather than update map upon "
                                   "distance traveled" )(
        "keyframe-min-distance", po::value< double >( &min_keyframe_dist )->default_value( 0.5 ),
        "minimum range used from scanner" )(
        "keyframe-min-rot-deg",
        po::value< double >( &min_keyframe_dist_rot_deg )->default_value( 15 ),
        "minimum range used from scanner" )( "gt-mapping", "disable registration and use ground "
                                                           "truth as input to mapping" )(
        "tf_topic", po::value< std::string >( &tf_topic )->default_value( std::string( "/tf" ) ),
        ...
        "vector y" )( "z", po::value< double >( &transl[2] )->default_value( 0. ),
                      "sensor pose - translation "
                      "vector z" )( "ex", po::value< double >( &euler[0] )->default_value( 0. ),
                                    "sensor pose - euler angle "
                                    "vector x" )(
        "ey", po::value< double >( &euler[1] )->default_value( 0. ), "sensor pose - euler angle "
                                                                     "vector y" )(
        "ez", po::value< double >( &euler[2] )->default_value( 0. ),
        "sensor pose - euler angle "
        "vector z" )( "skip-frame", po::value< unsigned int >( &skip_frame )->default_value( 20 ),
                      "sframes to skip before plot map etc." )(
        "sensor_time_offset", po::value< double >( &sensor_time_offset )->default_value( 0. ),
        "timeoffset of the scanner data" )( "registration3d", "registration3d" )(
        "check-consistency", "if consistency should be checked after registration" )(
        "do-soft-constraints", "do_soft_constraints_" )( "disable-registration", "Disable "
                                                                                 "Registration" )(
        "maxRotationNorm", po::value< double >( &maxRotationNorm_ )->default_value( 0.78539816339 ),
        "maxRotationNorm" )( "maxTranslationNorm",
                             po::value< double >( &maxTranslationNorm_ )->default_value( 0.4 ),
                             "maxTranslationNorm" )(
        "rotationRegistrationDelta",
        po::value< double >( &rotationRegistrationDelta_ )->default_value( M_PI / 6 ),
        "rotationRegistrationDelta" )(
        "translationRegistrationDelta",
        po::value< double >( &translationRegistrationDelta_ )->default_value( 1.5 ),
        "sensorRange" )( "resolution", po::value< double >( &resolution )->default_value( 0.4 ),
                         "resolution of the map" )(
        "resolution_local_factor",
        po::value< double >( &resolution_local_factor )->default_value( 1. ),
        "resolution factor of the local map used in the match and fusing step" )(
        "disable-submaps", "Adopt the sub-mapping technique which represent the global map as a "
                           "set of local submaps" )(
        "compound-radius", po::value< double >( &compound_radius_ )->default_value( 10.0 ),
        "Requires sub-mapping enabled, When creating new sub-lamps, information from previous map "
        "is transfered to the new map. The following radius is used to select the map objects to "
        "transfer" )( "interchange-radius",
                      po::value< double >( &interchange_radius_ )->default_value( 10.0 ),
                      "This radius is used to trigger creation or selection of which submap to "
                      "use" )( "use_pointtype_xyzir", "If the points to be processed should "
                                                      "contain ring and intensity information "
                                                      "(velodyne_pointcloud::PointXYZIR)" )(
        "min_nb_points_for_gaussian",
        po::value< int >( &min_nb_points_for_gaussian )->default_value( 6 ),
        ...
        "min_nb_points_set_uniform", "If the number of points of one cell is less than "
                                     "min_nb_points_for_gaussian, set the distribution to a "
                                     "uniform one (cov = Identity)" )(
        "nb_measurements", po::value< int >( &nb_measurements )->default_value( 1 ),
        "number of scans collecte at each read iteration" )(
        "max_nb_iters", po::value< int >( &max_nb_iters )->default_value( 30 ),
    ...
    if ( mapParPtr == NULL || regParPtr == NULL || graphParPtr == NULL ) return false;
    ...
    if ( gt_mapping ) base_link_id           = gt_base_link_id;
    ...
    if ( NDTD2DRegParamPtr ndt_reg_ptr =
             boost::dynamic_pointer_cast< NDTD2DRegParam >( regParPtr ) ) {
    ...
    {
        if ( NDTMapParamPtr ndt_map_ptr =
                 boost::dynamic_pointer_cast< NDTMapParam >( mapParPtr ) ) {
            ndt_map_ptr->resolution_ = resolution;
        }
        if ( NDTDLMapParamPtr ndt_map_ptr =
                 boost::dynamic_pointer_cast< NDTDLMapParam >( mapParPtr ) ) {
            ndt_map_ptr->resolution_ = resolution;
        }
    }
...
void processData() {
    ...
    ss << name << dataset << std::string( "__Sub=" ) << use_submap << "_sizexy=" << map_size_xy
       << "_Z=" << map_size_z << std::string( "_intrchR=" ) << interchange_radius_
       << std::string( "_compR=" ) << compound_radius_ << std::string( "_res=" ) << resolution
       << std::string( "_maxSensd=" ) << max_range << "_keyF=" << use_keyframe
       << "_d=" << min_keyframe_dist << "_deg=" << min_keyframe_dist_rot_deg;
    base_name += ss.str();
    bool dl = ( map_type_name == "ndt_dl_map" );
    base_name += "_dl=" + toString( dl ) + "_xyzir=" + toString( use_pointtype_xyzir ) + "_mpsu=" +
                 toString( min_nb_points_set_uniform ) + "_mnpfg=" +
                 toString( min_nb_points_for_gaussian ) + "kmnp" + toString( keep_min_nb_points );
    ...
    if ( gt_mapping ) tf_interp_link = gt_base_link_id;
    ...
    for ( int i = 0; i < ros_bag_paths.size(); i++ ) {
        ...
        ReadBagFileGeneric< PointT > reader( bag_reader_type, tf_interp_link, velodyne_config_file,
                                             bagfilename, velodyne_packets_topic, velodyne_frame_id,
                                             tf_world_frame, tf_topic, ros::Duration( 3600 ),
                                             &sensor_link, max_range, min_range, sensor_time_offset,
                                             nb_measurements );
        ...
        while ( found_scan ) {
            // end_of_bag_file=
            // vreader.readMultipleMeasurements(nb_scan_msgs,cloud_nofilter,tf_scan_source,tf_gt_base,tf_interp_link);
            ...
            if ( counter == 1 ) {
                ...
                fuser_ =
                    new GraphMapFuser( regParPtr, mapParPtr, graphParPtr, Tgt_base, sensor_offset );
                cout << "----------------------PARAMETERS FOR MAPPING--------------------------"
                     << endl;
            ...
            if ( !use_odometry ) { Tmotion.setIdentity(); }
            counter++;
            ...
            br.sendTransform( tf::StampedTransform( tf_fuser_pose, tplot, "/world", "/fuser_base_"
                                                                                    "link" ) );

            odom_pose = odom_pose * Tmotion;
            tf::Transform tf_odom_pose;
            tf::poseEigenToTF( odom_pose, tf_odom_pose );
            br.sendTransform( tf::StampedTransform( tf_odom_pose, tplot, "/world", "/odom_base_"
                                                                                   "link" ) );

            if ( visualize ) {
                br.sendTransform( tf::StampedTransform( tf_gt_base, tplot, "/world", "/state_base_"
                                                                                     "link" ) );
                br.sendTransform( tf::StampedTransform( tf_sensor_pose, tplot, "/fuser_base_link",
                                                        "/fuser_laser_link" ) );
            ...
            eval_files.Write( reader.getTimeStampOfLastSensorMsg(), Tgt_base, Todom_base,
                              fuser_pose, sensor_offset );
