  ...
  public:
    ...
    NDTFuserHMT( double map_resolution, double map_size_x_, double map_size_y_, double map_size_z_,
        ...
        if ( visualize_ ) {
            ...
            viewer->win3D->start_main_loop_own_thread();   // Very very ugly to start it here... FIX
                                                           // ME.
        }
#endif
        ...
        std::cout << "MAP: resolution: " << resolution << " size " << map_size_x << " "
                  << map_size_y << " " << map_size_z << " sr " << sensor_range << std::endl;
    }
    ~NDTFuserHMT() {
        delete map;
#ifndef NO_NDT_VIZ
    ...
    /**
     * @brief Set the initial position and set the first scan to the map and use tf for fixing the
     * first pose
     */
    void initialize( pcl::PointCloud< pcl::PointXYZ >& cloud, std::string& world_frame,
                     std::string& robot_frame, bool preLoad = false );
