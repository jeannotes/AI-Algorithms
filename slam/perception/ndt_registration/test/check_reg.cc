...
int main( int argc, char** argv ) {
    ...
    cout << "Cov(P2D) eigenvalues " << evals[0] << " " << evals[1] << " " << evals[2] << " "
         << evals[3] << " " << evals[4] << " " << evals[5] << " "
         << "\n";
    matcherD2D.covariance( cloud_fixed, cloud_offset, T, covD2D );
    Sol.compute( covD2D );
    evals = Sol.eigenvalues().real();
    cout << "Cov(D2D) eigenvalues " << evals[0] << " " << evals[1] << " " << evals[2] << " "
         << evals[3] << " " << evals[4] << " " << evals[5] << " "
         << "\n";
    ...
    if ( not converged ) cerr << "warning: matcher terminated before convergence\n";
