...
int main( int argc, char** argv ) {
    ...
    desc.add_options()( "help", "produce help message" )(
        "x", po::value< double >( &pose_increment_v( 0 ) )->default_value( 1. ),
        "x pos gt offset" )( "y",
                             po::value< double >( &pose_increment_v( 1 ) )->default_value( 1. ),
                             "y pos gt offset" )(
        "z", po::value< double >( &pose_increment_v( 2 ) )->default_value( 1. ),
        "z pos gt offset" )( "X",
                             po::value< double >( &pose_increment_v( 3 ) )->default_value( 0.1 ),
                             "x axis rot gt offset" )(
        "Y", po::value< double >( &pose_increment_v( 4 ) )->default_value( 0.1 ), "y axis rot gt "
                                                                                  "offset" )(
        "Z", po::value< double >( &pose_increment_v( 5 ) )->default_value( 0.1 ), "z axis rot gt "
                                                                                  "offset" )(
        "nb_clusters", po::value< int >( &nb_clusters )->default_value( 20 ),
        "number of clusters" )( "nb_points", po::value< int >( &nb_points )->default_value( 10 ),
                                "number of points per clusters" )(
        "std_dev", po::value< double >( &std_dev )->default_value( 0.1 ),
        "standard deviation of the points drawn from a normal distribution (here independent on "
        "the axes" )( "min", po::value< double >( &min )->default_value( -10 ), "minimum center "
                                                                                "point" )(
        "max", po::value< double >( &max )->default_value( 10 ), "maximum center point" )(
        "lazzy", "use lazzygrid" )( "p2preg", "calculate NDTMatchF2F using two pointclouds" )(
        ...
        "nf2f", "if f2f should NOT be evaluated" )( "usegt", "if the ground truth should be used "
                                                             "as initial estimate" );
    ...
    for ( int i = 0; i < nb_clusters; i++ ) {
        ...
        for ( int j = 0; j < nb_points; j++ ) {
            static_pc.push_back(
                pcl::PointXYZ( c_x + var_nor(), c_y + var_nor(), c_z + var_nor() ) );
            indices.push_back( index );
            index++;
        }
        all_indices.push_back( indices );
        corresp.push_back( std::pair< int, int >( i, nb_clusters - 1 - i ) );   // nb_clusters-1-i
                                                                                // -> To check the
                                                                                // da functions is
                                                                                // working.
    }
    std::vector< std::vector< size_t > > all_indices_moving(
        all_indices.size() );   // Reverse the correspondances (To check the da functions)
    std::reverse_copy( all_indices.begin(), all_indices.end(), all_indices_moving.begin() );

    for ( int i = 0; i < 1 /*nb_clusters*/; i++ ) { tmp_pc.push_back( pcl::PointXYZ( i, i, i ) ); }

    // Specify some offset...
    gt_transform = Eigen::Translation< double, 3 >( pose_increment_v( 0 ), pose_increment_v( 1 ),
                                                    pose_increment_v( 2 ) ) *
    ...
    if ( use_singleres ) resolutions.push_back( 1. );
    NDTMatcherD2D< pcl::PointXYZ, pcl::PointXYZ > matcher( use_irregular_grid, !use_singleres,
                                                           resolutions );
    ...
    if ( use_p2preg ) { matcher.match( moving_pc, static_pc, T_p2p ); }
    {
        ...
        if ( use_f2f ) { matcher.match( mov, ndt, T_f2f ); }
        if ( use_featurecorr ) {
    ...
    std::cout << "GT translation " << gt_transform.translation().transpose() << " (norm) "
              << gt_transform.translation().norm() << std::endl;
    ...
    if ( use_f2f ) {
        std::cout << "f2f translation " << T_f2f.translation().transpose() << " (norm) "
                  << T_f2f.translation().norm() << std::endl;
        std::cout << "f2f rotation " << T_f2f.rotation().eulerAngles( 0, 1, 2 ).transpose()
                  << " (norm) " << T_f2f.rotation().eulerAngles( 0, 1, 2 ).norm() << std::endl;
    }

    if ( use_featurecorr ) {
        std::cout << "feat translation " << T_feat.translation().transpose() << " (norm) "
                  << T_feat.translation().norm() << std::endl;
        std::cout << "feat rotation " << T_feat.rotation().eulerAngles( 0, 1, 2 ).transpose()
                  << " (norm) " << T_feat.rotation().eulerAngles( 0, 1, 2 ).norm() << std::endl;
    }

    if ( use_p2preg ) {
        std::cout << "p2preg translation " << T_p2p.translation().transpose() << " (norm) "
                  << T_p2p.translation().norm() << std::endl;
