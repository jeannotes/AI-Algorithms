...
void setViewerPose( pcl::visualization::PCLVisualizer& viewer,
    ...
    Eigen::Vector3f look_at_vector =
        pcl::getRotationOnly( viewer_pose ) * Eigen::Vector3f( 0, 0, 1 ) + pos_vector;
...
int main( int argc, char** argv ) {
    ...
    std::vector< int > pcd_filename_indices =
        pcl::console::parse_file_extension_argument( argc, argv, "wrl" );
    if ( !pcd_filename_indices.empty() ) {
        ...
        */
        scene_sensor_pose = Eigen::Affine3f( Eigen::Translation3f(
                                point_cloud.sensor_origin_[0], point_cloud.sensor_origin_[1],
    ...
    range_image.createFromPointCloud( point_cloud, angular_resolution, pcl::deg2rad( 360.0f ),
                                      pcl::deg2rad( 180.0f ), scene_sensor_pose, coordinate_frame,
    ...
    // PointCloudColorHandlerCustom<PointType> point_cloud_color_handler (point_cloud_ptr, 150, 150,
    // 150);
