...
void NDTMatcherP2D::generateScoreDebug( const char* out, pcl::PointCloud< pcl::PointXYZ >& fixed,
    ...
    for ( int q = resolutions.size() - 1; q >= 0; q-- ) {
        ...
        for ( double x = lmin; x < lmax; x += lstep ) {
            ...
            perception_oru::transformPointCloudInPlace( T, cloud );
...
bool NDTMatcherP2D::match( pcl::PointCloud< pcl::PointXYZ >& target,
    ...
    for ( int i = resolutions.size() - 1; i >= 0; i-- ) {
        ...
        // for(current_resolution = 2; current_resolution >= 0.5; current_resolution =
        ...
        T   = Temp * T;
        // transform moving
        perception_oru::transformPointCloudInPlace( Temp, moving );
#ifdef DO_DEBUG_PROC
...
bool NDTMatcherP2D::match( NDTMap& targetNDT, pcl::PointCloud< pcl::PointXYZ >& source,
                           Eigen::Transform< double, 3, Eigen::Affine, Eigen::ColMajor >& T ) {
    ///////////
    double lfc1, lfc2, lfd3;
    double integral, outlier_ratio, support_size;
    ...
    int itr_ctr      = 0;
    double step_size = 1;
    Eigen::Matrix< double, 6, 1 > pose_increment_v, pose_increment_reg_v, score_gradient,
        scg;   // column vectors
...
// std::cerr<<"iteration "<<itr_ctr<<" pose norm "<<(pose_increment_v.norm())<<" score
// "<<score<<std::endl;
...
//	cout<<"dg    "<<pose_increment_v.dot(score_gradient)<<endl;
        ...
        */
        ...
        prevCloud = perception_oru::transformPointCloud< pcl::PointXYZ >( T, source );
        scoreP    = score;
        score     = scorePointCloud( prevCloud, targetNDT );
        if ( score < score_best ) {
            Tbest      = T;
        ...
        // cout<<"iteration "<<itr_ctr<<" pose norm "<<(pose_increment_v.norm())<<" score_prev
        // "<<scoreP<<" scoreN "<<score<<endl;
...
bool NDTMatcherP2D::update_score_gradient( Eigen::Matrix< double, 6, 1 >& score_gradient,
                                           Eigen::Vector3d& transformed, Eigen::Matrix3d& Cinv ) {
    ...
    if ( factor < -120 ) { return false; }
    factor = lfd2 * exp( factor );
    if ( factor > 1 || factor < 0 || factor * 0 != 0 ) { return false; }
    factor *= lfd1;
...
void NDTMatcherP2D::update_hessian( Eigen::Matrix< double, 6, 6 >& Hessian,
    ...
    for ( int i = 0; i < Hessian.rows(); i++ ) {
        for ( int j = 0; j < Hessian.cols(); j++ ) {
            ...
            Hessian( i, j ) +=
                factor * ( -lfd2 * transformed.dot( CxdXdI ) * transformed.dot( CxdXdJ ) +
                           transformed.dot( CxSecondOrder ) + Jest.col( j ).dot( CxdXdI ) );
        }
    }
}
...
double NDTMatcherP2D::scorePointCloud( pcl::PointCloud< pcl::PointXYZ >& source,
    ...
    for ( unsigned int i = 0; i < source.points.size(); i++ ) {
        ...
        std::vector< NDTCell* > cells =
            targetNDT.getCellsForPoint( source.points[i], current_resolution );
...
void NDTMatcherP2D::derivativesPointCloud(
    pcl::PointCloud< pcl::PointXYZ >& source, NDTMap& targetNDT,
    ...
    for ( unsigned int i = 0; i < source.points.size(); i++ ) {
        ...
        //
        {
            ...
            if ( !update_score_gradient( score_gradient, transformed, Cinv ) ) { continue; }

            // update hessian matrix
            if ( computeHessian ) { update_hessian( Hessian, transformed, Cinv ); }
            cell = NULL;
        }
    }
...
double NDTMatcherP2D::lineSearchMT(
    Eigen::Matrix< double, 6, 1 >& score_gradient_init, Eigen::Matrix< double, 6, 1 >& increment,
    pcl::PointCloud< pcl::PointXYZ >& sourceCloud,
    Eigen::Transform< double, 3, Eigen::Affine, Eigen::ColMajor >& globalT, NDTMap& targetNDT ) {
    // default params
    ...
    if ( dginit >= 0.0 ) {
        ...
        if ( dginit >= 0.0 ) {
            //    cout << "MoreThuente::cvsrch - Non-descent direction (dginit = " << dginit << ")"
            //    << endl;
            // stp = recoverystep;
    ...
    while ( 1 ) {
        ...
        cloudHere = perception_oru::transformPointCloud( ps, sourceCloud );
...
int NDTMatcherP2D::MoreThuente::cstep( double& stx, double& fx, double& dx, double& sty, double& fy,
...
NDTMatcherP2D::subsample( pcl::PointCloud< pcl::PointXYZ >& original ) {
    std::string subsampleType = "GRID";
    if ( subsampleType == "NONE" ) { return original; }
