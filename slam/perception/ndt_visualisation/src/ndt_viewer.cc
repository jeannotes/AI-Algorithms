...
int main( int argc, char** argv ) {
    ...
    desc.add_options()( "help", "produce help message" )(
        "file-name", po::value< std::string >( &base_name ),
        "location of the ndt map you want to "
        "view" )( "resolution", po::value< double >( &resolution )->default_value( 1. ),
    ...
    while ( viewer->win3D->isOpen() ) {
        ...
        if ( viewer->win3D->keyHit() ) {
            ...
            if ( key == 'o' ) { viewer->plotNDTSAccordingToOccupancy( -1, &ndmap ); }
            if ( key == 'c' ) { viewer->plotNDTSAccordingToClass( -1, &ndmap ); }
            if ( key == 'p' ) { viewer->plotNDTSAccordingToCost( -1, 10000, &ndmap ); }
            if ( key == 't' ) { viewer->plotNDTTraversabilityMap( &ndmap ); }
            if ( key == 'h' ) { printUsage(); }
