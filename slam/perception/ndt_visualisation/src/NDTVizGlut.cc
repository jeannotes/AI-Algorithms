...
void win_reshape_( int w, int h ) {
    glut3d_ptr->win_reshape( w, h );
}
void win_redraw_() {
    glut3d_ptr->win_redraw();
}
void win_key_( unsigned char key, int x, int y ) {
    glut3d_ptr->win_key( key, x, y );
}
void win_mouse_( int button, int state, int x, int y ) {
    glut3d_ptr->win_mouse( button, state, x, y );
}
void win_motion_( int x, int y ) {
    glut3d_ptr->win_motion( x, y );
}
void win_idle_() {
    glut3d_ptr->win_idle();
}
void win_close_() {
    glut3d_ptr->win_close();
}
...
void* glthread( void* pParam ) {
    ...
    if ( glut3d_ptr->getFullScreen() ) { glutFullScreen(); }
...
void NDTVizGlut::update_cam() {
    ...
    gluLookAt( cp[0], cp[1], cp[2], fp[0], fp[1], fp[2], up[0], up[1], up[2] );
}

void NDTVizGlut::win_key( unsigned char key, int x, int y ) {
    this->pressed_keys.push_back( key );
}

// Mouse callback
void NDTVizGlut::win_mouse( int button, int state, int x, int y ) {
    //    std::cerr << "win_mouse - b:" << button << " s: " << state << "[" << x << "," << y << "]"
    //    << std::endl;
...
void NDTVizGlut::win_motion( int x, int y ) {
...
void NDTVizGlut::win_reshape( int width, int height ) {
...
void NDTVizGlut::process_events() {
    //     if (do_save_inc)
    //	  save_inc();
    //     glutMainLoopEvent();
}
...
// Run the GUI
int NDTVizGlut::win_run( int* argc, char** argv ) {
...
void NDTVizGlut::setCameraPointingToPoint( double x, double y, double z ) {
    camera->setFocalPoint( Eigen::Vector3f( x, y, z ) );
    update_cam();
}

void NDTVizGlut::setCameraPosition( double x, double y, double z ) {
    camera->setPosition( Eigen::Vector3f( x, y, z ) );
    update_cam();
}

void NDTVizGlut::switchCamera( const std::string& type ) {
...
void NDTVizGlut::setAspectRatioFactor( float factor ) {
    aspect_ratio_factor = factor;
}

const NDTVizGlutCamera* NDTVizGlut::getCameraConstPtr() const {
    return camera;
}

NDTVizGlutCamera* NDTVizGlut::getCameraPtr() {
    return camera;
}
