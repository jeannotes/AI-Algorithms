...
class NDTViz {
  public:
    ...
    void repaint() { win3D->repaint(); }

    void clear() { win3D->clearScene(); }

    void clearTrajectoryPoints() { gl_points.clear(); }

    void addTrajectoryPoint( float x, float y, float z, float R = 1.0, float G = 1.0,
                             float B = 1.0 ) {
        gl_points.push_back( x, y, z, R, G, B );
    }
    void displayTrajectory() { win3D->addObject( &gl_points ); }
    ...
    void displayParticles() { win3D->addObject( &gl_particles ); }
    ...
    void displayPoseCov() { win3D->addObject( &gl_posecov ); }

    void addGrid( size_t nbCells, double cellSize, double lineWidth = 1., double R = 1.0,
                  double G = 0.4, double B = 0., double A = 0.8 ) {
        double length = nbCells * cellSize;
        double offset = length * 0.5;
        for ( size_t i = 0; i <= nbCells; i++ ) {
            gl_grid.appendLine( i * cellSize - offset, -offset, 0, i * cellSize - offset,
                                nbCells * cellSize - offset, 0 );

            for ( size_t i = 0; i <= nbCells; i++ ) {
                gl_grid.appendLine( -offset, i * cellSize - offset, 0, nbCells * cellSize - offset,
                                    i * cellSize - offset, 0 );
            }
        }
    ...
    /**
      ...
      */
    void addScan( Eigen::Vector3d orig, pcl::PointCloud< pcl::PointXYZ >& cloud, double R = 1.0,
                  double G = 1.0, double B = 1.0 ) {
        gl_laserlines.clear();
        for ( unsigned int i = 0; i < cloud.points.size(); i += 2 ) {
            gl_laserlines.appendLine( orig( 0 ), orig( 1 ), orig( 2 ), cloud.points[i].x,
    ...
    void
    addPolygon( std::vector< Eigen::Vector2d, Eigen::aligned_allocator< Eigen::Vector2d > >& pts,
        ...
        for ( unsigned int i = 0; i < pts.size(); i++ ) {
            ...
            gl_polygonboundary.appendLine( pts[i]( 0 ), pts[i]( 1 ), 0., pts[j]( 0 ), pts[j]( 1 ),
                                           0. );
        }
    ...
    void
    addPathLine( std::vector< Eigen::Vector2d, Eigen::aligned_allocator< Eigen::Vector2d > >& pts,
                 double R = 1.0, double G = 1.0, double B = 1.0 ) {
        gl_pathline.clear();
        if ( pts.size() < 2 ) return;
        for ( unsigned int i = 0; i < pts.size() - 1; i++ ) {
            unsigned int j = i + 1;
            gl_pathline.appendLine( pts[i]( 0 ), pts[i]( 1 ), 0., pts[j]( 0 ), pts[j]( 1 ), 0. );
        }
    ...
    void addPointCloud( pcl::PointCloud< pcl::PointXYZ >& cloud, double R = 1.0, double G = 1.0,
                        double B = 1.0 ) {
        gl_pointcloud.setPointSize( 2.5 );
        for ( unsigned int i = 0; i < cloud.points.size(); i += 2 ) {
            gl_pointcloud.push_back( cloud.points[i].x, cloud.points[i].y, cloud.points[i].z, R, G,
                                     B );
        }
        win3D->addObject( &gl_pointcloud );
    }

    void plotNDTMap( perception_oru::NDTMap* map, double R = 1.0, double G = 1.0, double B = 1.0,
                     bool heightCoding = false, bool setCameraPos = true ) {
        ...
        for ( unsigned int i = 0; i < global_ndts.size(); i++ ) {
            ...
            double hval = 1.0;
            if ( heightCoding ) { hval = fabs( m[2] + 1.5 ) / 6.0; }
        ...
        for ( unsigned int i = 0; i < global_ndts.size(); i++ ) {
            ...
            if ( global_ndts[i]->getOccupancy() > occupancy ) {
                ...
                switch ( global_ndts[i]->getClass() ) {
                    case perception_oru::NDTCell::HORIZONTAL: color = cFlat; break;
                    case perception_oru::NDTCell::VERTICAL: color   = cVert; break;
                    case perception_oru::NDTCell::INCLINED: color   = cInclined; break;
                    case perception_oru::NDTCell::ROUGH: color      = cRough; break;
                    default: color                                  = cUnknown;
                }
    ...
    void plotNDTSAccordingToOccupancy( float occupancy, perception_oru::NDTMap* map ) {
        if ( win3D == NULL ) return;
        std::vector< perception_oru::NDTCell* > global_ndts;
        global_ndts = map->getAllCells();
        //	    fprintf(stderr," NUM NDT: %u ", global_ndts.size());
    ...
    void plotLocalNDTMap( pcl::PointCloud< pcl::PointXYZ >& cloud, double resolution, double R = 0,
                          double G = 1, double B = 0, bool heightCoding = true ) {
        if ( win3D == NULL ) return;
        ...
        for ( unsigned int i = 0; i < ndts.size(); i++ ) {
            ...
            objEllip.setLocation( m[0], m[1], m[2] );
            double hval = 1.0;
            if ( heightCoding ) { hval = fabs( m[2] + 1.5 ) / 20.0; }
    ...
    void plotLocalConflictNDTMap( perception_oru::NDTMap* map,
                                  pcl::PointCloud< pcl::PointXYZ >& cloud, double resolution,
                                  double R = 1, double G = 0, double B = 0,
                                  bool heightCoding = false, double maxz = 0 ) {
        if ( win3D == NULL ) return;
    ...
    /*     mrpt::opengl::CTexturedPlanePtr gl_img 		=
     * mrpt::opengl::CTexturedPlane::Create(0.5,-0.5,-0.5,0.5); */
    ...
    /* 	    float r = ((x >= 3.0/8.0) & (x < 5.0/8.0))*(4. * x - 3./2.)+((x >= 5./8.) & (x <
     * 7./8.))+(x >= 7./8.) * (-4. * x + 9./2.); */
    /* 	    float g = ((x >= 1./8.) & (x < 3./8.))*(4. * x - 1./2.)+((x >= 3./8.) & (x <
     * 5./8.))+((x >= 5./8.) & (x < 7./8.))*(-4. * x + 7./2.); */
    /* 	    float b = (x < 1./8.)*(4. * x + 1./2.)+((x >= 1./8.) & (x < 3./8.))+((x >= 3./8.) & (x
     * < 5./8.))*(-4. * x + 5./2.); */
    /* 	    size_t color=((unsigned char)(r*255))*255*255 + ((unsigned char)(g*255))*255 +
     * ((unsigned char)(b*255)); */
