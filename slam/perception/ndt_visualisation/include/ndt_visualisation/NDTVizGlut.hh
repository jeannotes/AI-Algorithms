...
inline void checkOpenGLError() {
    ...
    if ( ( openglErr = glGetError() ) != GL_NO_ERROR ) {
        const std::string sErr =
            std::string( "OpenGL error: " ) + std::string( (char*) gluErrorString( openglErr ) );
    ...
    void draw() {
        ...
        if ( m_eigVal( 0, 0 ) != 0.0 && m_eigVal( 1, 1 ) != 0.0 &&
            ...
            if ( dim == 2 ) {
                ...
                glPushMatrix();
                glMultMatrixf( mat );
                glScalef( m_eigVal( 0, 0 ) * m_quantiles, m_eigVal( 1, 1 ) * m_quantiles,
...
class NDTVizGlutFixedCamera : public NDTVizGlutCamera {
  ...
  public:
    ...
    Eigen::Vector3f getFocalPoint() const { return focal_point_; }

    Eigen::Vector3f getUpVector() const { return Eigen::Vector3f( 0., 0., 1. ); }

    void setFocalPoint( const Eigen::Vector3f& fp ) { focal_point_ = fp; }
...
class NDTVizGlutXYOrbitCamera : public NDTVizGlutCamera {
  ...
  public:
    ...
    Eigen::Vector3f getPosition() const {
        Eigen::Vector3f p =
            Eigen::Vector3f( distance * sin( pitch ) * cos( yaw ),
                             distance * sin( pitch ) * sin( yaw ), distance * cos( pitch ) );
    ...
    void update_motion( int x, int y ) {
        ...
        switch ( last_button_ ) {
            case GLUT_LEFT_BUTTON:
                yaw += -dx;
                pitch += dy;
                if ( pitch > M_PI / 2. ) pitch  = M_PI / 2. - 0.0001;
                if ( pitch < -M_PI / 2. ) pitch = -M_PI / 2. + 0.0001;
                break;
...
class NDTVizGlut {
  public:
    ...
    virtual void start_main_loop() { glutMainLoop(); }
...
// void win_mouse_(int button, int state, int x, int y) { glut3d_ptr->win_mouse(button, state, x,
// y); }
