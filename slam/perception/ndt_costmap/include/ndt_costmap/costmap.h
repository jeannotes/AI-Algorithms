...
class NDTCostmap {
  ...
  public:
    NDTCostmap( NDTMap* map ) {
        ...
        if ( map_ != NULL ) {
            ...
            if ( grid_ != NULL ) {
                ...
                for ( int i = 0; i < sx; ++i ) { levels[i] = new LevelSet[sy]; }
                alloc = true;
            } else {
                alloc = false;
            }
    ...
    ~NDTCostmap() {
        if ( alloc ) {
            for ( int i = 0; i < sx; ++i ) { delete[] levels[i]; }
            delete[] levels;
        }
    }
        ...
        for ( int iy = 0; iy < sy; ++iy ) {
            for ( int iz = 0; iz < sz - nheight_cells; ++iz ) {   //
                ...
                if ( ndcell->getOccupancy() > occ && ndcell->hasGaussian_ ) {
                    if ( ndcell->getClass() == NDTCell::HORIZONTAL ||
                         ndcell->getClass() == NDTCell::INCLINED ) {
                        bool space = true;
                        ...
                        for ( int j = 1; j < nheight_cells; ++j ) {
                            ...
                            if ( ndcell_up == NULL ) {
                                ...
                                space =
                                    !( ndcell_up->getOccupancy() > occ || ndcell_up->hasGaussian_ );
                                if ( !space ) break;
                            }
                        }
                        if ( space ) {
        ...
        for ( int iy = 0; iy < sy; ++iy ) {
            for ( int j = 0; j < levels[ix][iy].clear_levels.size(); ++j ) {
                ...
                if ( level_index_neighbours.size() > 0 ) {
                    ...
                    levels[ix][iy].map_ids.push_back(
                        levels[xy_ids_neighbours[0].first][xy_ids_neighbours[0].second]
                            .map_ids[level_index_neighbours[0]] );
                    maps[map_ids_neighbours[0]].image.at< float >( ix, iy ) = 1;
                    maps[map_ids_neighbours[0]].mean_height =
                        ( maps[map_ids_neighbours[0]].mean_height *
                    ...
                    if ( level_index_neighbours.size() == 2 ) {
                        ...
                        if ( map_ids_neighbours[0] > map_ids_neighbours[1] ) {
                            merge_maps.insert( std::pair< int, int >( map_ids_neighbours[0],
                                                                      map_ids_neighbours[1] ) );
                        }
                        if ( map_ids_neighbours[0] < map_ids_neighbours[1] ) {
                            merge_maps.insert( std::pair< int, int >( map_ids_neighbours[1],
                                                                      map_ids_neighbours[0] ) );
                        }
                    }
                    if ( level_index_neighbours.size() == 3 ) {
                        ...
                        if ( map_ids_neighbours[1] < map_ids_neighbours[2] ) {
                            merge_maps.insert( std::pair< int, int >( map_ids_neighbours[2],
                                                                      map_ids_neighbours[1] ) );
                        }
                        if ( map_ids_neighbours[0] < map_ids_neighbours[2] ) {
                            merge_maps.insert( std::pair< int, int >( map_ids_neighbours[2],
    ...
    for ( int ix = 0; ix < sx; ++ix ) {
        for ( int iy = 0; iy < sy; ++iy ) {
            for ( int j = 0; j < levels[ix][iy].clear_levels.size(); ++j ) {
                ...
                if ( found ) {
                    maps[idx].mean_height = maps[idx].mean_height * maps[idx].n_samples +
                                            levels[ix][iy].clear_levels[j];
                    maps[idx].n_samples++;
...
/* bool NDTCostmap::toMessage(ndt_costmap::TraversabilityMapMsg msg, double max_h, std::string
 * frame_id){ */
/*   if(!alloc) return; */
