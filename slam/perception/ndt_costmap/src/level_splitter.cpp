  ...
  public:
    ...
    LevelSplit( std::string map_file_name_, std::string map_prefix_, double resolution_ ) {
        ...
        for ( int z = 0; z < z_or; ++z ) {
            ...
            perception_oru::LazyGrid* localMapGrid = new perception_oru::LazyGrid(
                gsx, gsy, 3 * resolution,   // gsz,
                resolution, resolution, resolution, cx, cy, 0.0, prototype );
...
int main( int argc, char** argv ) {
    ...
    desc.add_options()( "help", "produce help message" )(
        "file-name", boost::program_options::value< std::string >( &map_file_name_ ), "ndt-map to "
                                                                                      "split" )(
        "prefix",
        boost::program_options::value< std::string >( &map_prefix_ )->default_value( "map" ),
        "prefix_for_sequence" )(
        "resolution", boost::program_options::value< double >( &resolution_ )->default_value( 1. ),
        "resolution of the map (necessary due to bug in loading, should be fixed soon)" );
