...
class Initializer {
  ...
  private:
    ...
    int CheckRT( const cv::Mat& R, const cv::Mat& t, const vector< cv::KeyPoint >& vKeys1,
                 ...
                 vector< bool >& vbInliers, const cv::Mat& K, vector< cv::Point3f >& vP3D,
    ...
    vector< bool > mvbMatched1;   ///< 记录Reference Frame的每个特征点在Current
                                  /// Frame是否有匹配的特征点
    ...
    vector< vector< size_t > >
        mvSets;   ///< 二维容器，外层容器的大小为迭代次数，内层容器大小为每次迭代算H或F矩阵需要的点
};
