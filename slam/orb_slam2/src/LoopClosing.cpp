...
bool LoopClosing::DetectLoop() {
    ...
    // Each candidate expands a covisibility group (keyframes connected to the loop candidate in the
    // covisibility graph)
    ...
    for ( size_t i = 0, iend = vpCandidateKFs.size(); i < iend; i++ ) {
        ...
        for ( size_t iG = 0, iendG = mvConsistentGroups.size(); iG < iendG; iG++ ) {
            ...
            if ( bConsistent ) {
                ...
                if ( !vbConsistentGroup
                         [iG] ) {   // 这里作者原本意思是不是应该是vbConsistentGroup[i]而不是vbConsistentGroup[iG]呢？（wubo???）
        ...
        // If the group is not consistent with any previous group insert with consistency counter
        // set to zero
...
bool LoopClosing::ComputeSim3() {
    ...
    while ( nCandidates > 0 && !bMatch ) {
        for ( int i = 0; i < nInitialCandidates; i++ ) {
            ...
            // If RANSAC returns a Sim3, perform a guided matching and optimize with all
            // correspondences
            if ( !Scm.empty() ) {
                ...
                // [sR t;0 1]
                cv::Mat R =
                    pSolver->GetEstimatedRotation();   // 候选帧pKF到当前帧mpCurrentKF的R（R12）
                cv::Mat t =
                    pSolver
                        ->GetEstimatedTranslation();   // 候选帧pKF到当前帧mpCurrentKF的t（t12），当前帧坐标系下，方向由pKF指向当前帧
                const float s =
                    pSolver
                        ->GetEstimatedScale();   // 候选帧pKF到当前帧mpCurrentKF的变换尺度s（s12）
                ...
                const int nInliers =
                    Optimizer::OptimizeSim3( mpCurrentKF, pKF, vpMapPointMatches, gScm, 10,
    ...
    matcher.SearchByProjection( mpCurrentKF, mScw, mvpLoopMapPoints, mvpCurrentMatchedPoints,
                                10 );   // 搜索范围系数为10
...
void LoopClosing::CorrectLoop() {
...

    {
        ...
        // Correct all MapPoints obsrved by current keyframe and neighbors, so that they align with
        ...
        for ( KeyFrameAndPose::iterator mit = CorrectedSim3.begin(), mend = CorrectedSim3.end();
            ...
            // Update keyframe pose with corrected Sim3. First transform Sim3 to SE3 (scale
            // translation)
    ...
    // After the MapPoint fusion, new links in the covisibility graph will appear attaching both
    // sides of the loop
    ...
    Optimizer::OptimizeEssentialGraph( mpMap, mpMatchedKF, mpCurrentKF, NonCorrectedSim3,
                                       CorrectedSim3, LoopConnections, mbFixScale );
    ...
    // We need to propagate the correction through the spanning tree
    {
        ...
        if ( !mbStopGBA ) {
            ...
            while ( !lpKFtoCheck.empty() ) {
                ...
                for ( set< KeyFrame* >::const_iterator sit = sChilds.begin(); sit != sChilds.end();
                      sit++ ) {
                    KeyFrame* pChild = *sit;
