...
void MapPoint::AddObservation( KeyFrame* pKF, size_t idx ) {
    ...
    // in rgbd, this means, we find corresponding point in right image
...
void MapPoint::ComputeDistinctiveDescriptors() {
    ...
    {
        unique_lock< mutex > lock1( mMutexFeatures );
        if ( mbBad )   // see SetBadFlag
            return;
        observations = mObservations;
        // in AddObservation, whached by which frame
        // map<KeyFrame*,size_t>
    }
    ...
    for ( map< KeyFrame *, size_t >::iterator mit = observations.begin(), mend = observations.end();
        ...
        if ( !pKF->isBad() ) vDescriptors.push_back( pKF->mDescriptors.row( mit->second ) );
        /*
         * the discriptors is construct from keyframe constructor
         * this is for initialization, and the for-loop is going through current frame's 3D
         * points(feature points)
         * mit->second is the sequence of descriptor
         */
    }
...
void MapPoint::UpdateNormalAndDepth() {
    ...
    cv::Mat Pos;
    {
        ...
        observations = mObservations;   // 获得观测到该3d点的所有关键帧 in AddObservation
    ...
    for ( map< KeyFrame *, size_t >::iterator mit = observations.begin(), mend = observations.end();
        ...
        cv::Mat normali = mWorldPos - Owi;   // mWorldPos is set from mappoint construction
        normal =
            normal +
            normali / cv::norm( normali );   // 对所有关键帧对该点的观测方向归一化为单位向量进行求和
        n++;
    }

    cv::Mat PC =
        Pos - pRefKF->GetCameraCenter();   // 参考关键帧相机指向3D点的向量（在世界坐标系下的表示）
...

    {
        ...
        mfMinDistance =
            mfMaxDistance / pRefKF->mvScaleFactors[nLevels - 1];   // 观测到该点的距离上限
