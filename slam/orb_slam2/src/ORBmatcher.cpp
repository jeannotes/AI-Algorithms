...
int ORBmatcher::SearchByProjection( Frame& F, const vector< MapPoint* >& vpMapPoints,
    ...
    for ( size_t iMP = 0; iMP < vpMapPoints.size(); iMP++ ) {
        ...
        // 通过投影点(投影到当前帧,见isInFrustum())以及搜索窗口和预测的尺度进行搜索,
        // 找出附近的兴趣点
        const vector< size_t > vIndices = F.GetFeaturesInArea(
            pMP->mTrackProjX, pMP->mTrackProjY, r * F.mvScaleFactors[nPredictedLevel],
...
bool ORBmatcher::CheckDistEpipolarLine( const cv::KeyPoint& kp1, const cv::KeyPoint& kp2,
    ...
    // 求出kp1在pKF2上对应的极线
    const float a = kp1.pt.x * F12.at< float >( 0, 0 ) + kp1.pt.y * F12.at< float >( 1, 0 ) +
                    F12.at< float >( 2, 0 );
    const float b = kp1.pt.x * F12.at< float >( 0, 1 ) + kp1.pt.y * F12.at< float >( 1, 1 ) +
                    F12.at< float >( 2, 1 );
    const float c = kp1.pt.x * F12.at< float >( 0, 2 ) + kp1.pt.y * F12.at< float >( 1, 2 ) +
                    F12.at< float >( 2, 2 );
...
int ORBmatcher::SearchByBoW( KeyFrame* pKF, Frame& F, vector< MapPoint* >& vpMapPointMatches ) {
    ...
    const DBoW2::FeatureVector& vFeatVecKF =
        pKF->mFeatVec;   // nodes and indexes of local features(i-th feature)
    ...
    while ( KFit != KFend && Fit != Fend ) {
        if ( KFit->first ==
            ...
            for ( size_t iKF = 0; iKF < vIndicesKF.size(); iKF++ ) {
                ...
                const cv::Mat& dKF =
                    pKF->mDescriptors.row( realIdxKF );   // 取出KF中该特征对应的描述子
                ...
                for ( size_t iF = 0; iF < vIndicesF.size(); iF++ ) {
                    ...
                    const cv::Mat& dF =
                        F.mDescriptors.row( realIdxF );   // 取出F中该特征对应的描述子

                    const int dist = DescriptorDistance( dKF, dF );   // 求描述子的距离

                    if ( dist <
                         bestDist1 ) {   // dist < bestDist1 < bestDist2，更新bestDist1 bestDist2
                        bestDist2 = bestDist1;
                        bestDist1 = dist;
                        bestIdxF  = realIdxF;
                    } else if ( dist <
                ...
                if ( bestDist1 <= TH_LOW ) {   // 匹配距离（误差）小于阈值
                    ...
                    if ( static_cast< float >( bestDist1 ) <
                        ...
                        if ( mbCheckOrientation ) {
                            ...
                            float rot =
                                kp.angle - F.mvKeys[bestIdxF].angle;   // 该特征点的角度变化值
...
int ORBmatcher::Fuse( KeyFrame* pKF, const vector< MapPoint* >& vpMapPoints, const float th ) {
    ...
    for ( int i = 0; i < nMPs; i++ ) {
        ...
        const float radius =
            th *
            pKF->mvScaleFactors
                [nPredictedLevel];   // 步骤2：根据MapPoint的深度确定尺度，从而确定搜索范围
        ...
        int bestIdx  = -1;
        for ( vector< size_t >::const_iterator vit = vIndices.begin(), vend = vIndices.end();
              vit != vend; vit++ ) {   // 步骤3：遍历搜索范围内的features
            const size_t idx = *vit;
...
int ORBmatcher::Fuse( KeyFrame* pKF, cv::Mat Scw, const vector< MapPoint* >& vpPoints, float th,
    ...
    for ( int iMP = 0; iMP < nPoints; iMP++ ) {
        ...
        int bestDist = INT_MAX;
        int bestIdx  = -1;
        for ( vector< size_t >::const_iterator vit = vIndices.begin(); vit != vIndices.end();
              vit++ ) {
            const size_t idx   = *vit;
            const int& kpLevel = pKF->mvKeysUn[idx].octave;
...
int ORBmatcher::SearchBySim3( KeyFrame* pKF1, KeyFrame* pKF2, vector< MapPoint* >& vpMatches12,
    ...
    for ( int i1 = 0; i1 < N1; i1++ ) {
        ...
        if ( bestDist <= TH_HIGH ) { vnMatch1[i1] = bestIdx; }
    }
    ...
    for ( int i2 = 0; i2 < N2; i2++ ) {
        ...
        if ( bestDist <= TH_HIGH ) { vnMatch2[i2] = bestIdx; }
    }
...
int ORBmatcher::SearchByProjection( Frame& CurrentFrame, const Frame& LastFrame, const float th,
    ...
    // this time's camera in last camera coordinate, compare with baseline
    // 判断前进还是后退
    const bool bForward =
        tlc.at< float >( 2 ) > CurrentFrame.mb && !bMono;   // 非单目情况，如果Z大于基线，则表示前进
    const bool bBackward = -tlc.at< float >( 2 ) > CurrentFrame.mb &&
                           !bMono;   // 非单目情况，如果Z小于基线，则表示前进
        ...
        if ( pMP ) {
            if ( !LastFrame.mvbOutlier[i] ) {
                ...
                if ( bestDist <= TH_HIGH ) {
                    ...
                    if ( mbCheckOrientation ) {
                        float rot =
                            LastFrame.mvKeysUn[i].angle - CurrentFrame.mvKeysUn[bestIdx2].angle;
                        if ( rot < 0.0 ) rot += 360.0f;
                        int bin                        = round( rot * factor );
                        if ( bin == HISTO_LENGTH ) bin = 0;
                        assert( bin >= 0 && bin < HISTO_LENGTH );
