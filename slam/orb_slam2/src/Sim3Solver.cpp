...
cv::Mat Sim3Solver::iterate( int nIterations, bool& bNoMore, vector< bool >& vbInliers,
                             int& nInliers ) {
    bNoMore   = false;
...
void Sim3Solver::ComputeSim3( cv::Mat& P1, cv::Mat& P2 ) {
    ...
    N = ( cv::Mat_< float >( 4, 4 ) << N11, N12, N13, N14, N12, N22, N23, N24, N13, N23, N33, N34,
          N14, N24, N34, N44 );
    ...
    ( evec.row( 0 ).colRange( 1, 4 ) ).copyTo( vec );   // extract imaginary part of the quaternion
                                                        // (sin*axis)
    ...
    if ( !mbFixScale ) {
        ...
        for ( int i = 0; i < aux_P3.rows; i++ ) {
            for ( int j = 0; j < aux_P3.cols; j++ ) { den += aux_P3.at< float >( i, j ); }
        }
...
void Sim3Solver::Project( const vector< cv::Mat >& vP3Dw, vector< cv::Mat >& vP2D, cv::Mat Tcw,
                          cv::Mat K ) {
...
void Sim3Solver::FromCameraToImage( const vector< cv::Mat >& vP3Dc, vector< cv::Mat >& vP2D,
                                    cv::Mat K ) {
    const float& fx = K.at< float >( 0, 0 );
    const float& fy = K.at< float >( 1, 1 );
    const float& cx = K.at< float >( 0, 2 );
