...
cv::Mat Tracking::GrabImageRGBD( const cv::Mat& imRGB, const cv::Mat& imD,
    ...
    // step 1: change image to grayscale
    if ( mImGray.channels() == 3 ) {
        if ( mbRGB )
            cvtColor( mImGray, mImGray, CV_RGB2GRAY );
        else
...
void Tracking::Track() {
    ...
    if ( mState == NOT_INITIALIZED ) {
        ...
        if ( !mbOnlyTracking ) {
            ...
            if ( mState == OK ) {
                ...
                if ( mVelocity.empty() || mCurrentFrame.mnId < mnLastRelocFrameId + 2 ) {
                    ...
                    bOK = TrackReferenceKeyFrame();   // return matches >= 10
                } else {
                    // 根据恒速模型设定当前帧的初始位姿
            ...
            if ( mState == LOST ) {
                ...
                if ( !mbVO ) {
                    ...
                    // mbVO为1，则表明此帧匹配了很少的3D
                    // map点，少于10个，要跪的节奏，既做跟踪又做定位
                    ...
                    if ( bOKMM && !bOKReloc ) {
                        ...
                        bOKReloc ) {   // 只要重定位成功整个跟踪过程正常进行（定位与跟踪，更相信重定位）
                        mbVO = false;
                    }
        ...
        if ( !mbOnlyTracking ) {
            ...
            // retrieve
            // a local map and therefore we do not perform TrackLocalMap(). Once the system
            // relocalizes
    ...
    if ( !mCurrentFrame.mTcw.empty() ) {
        ...
        cv::Mat Tcr =
            mCurrentFrame.mTcw * mCurrentFrame.mpReferenceKF->GetPoseInverse();   // Twc.clone();
...
/*
 * map initialization for rgbd & stereo
 * we have depth info, could get MapPoints
 */
void Tracking::StereoInitialization() {
    if ( mCurrentFrame.N > 500 ) {
        ...
        for ( int i = 0; i < mCurrentFrame.N; i++ ) {
            // https://blog.csdn.net/xiaoxiaowenqiang/article/details/79280735 ref
            float z = mCurrentFrame.mvDepth[i];
            if ( z > 0 ) {
                // 步骤4.1：通过反投影得到该特征点的3D坐标
                cv::Mat x3D = mCurrentFrame.UnprojectStereo( i );
                // 步骤4.2：将3D点构造为MapPoint
                // x3D.copyTo(mWorldPos);
                MapPoint* pNewMP = new MapPoint( x3D, pKFini, mpMap );   // pKFini is key frame
 ...
 *
 * Local
 * Mapping线程可能会将关键帧中某些MapPoints进行替换，由于tracking中需要用到mLastFrame，这里检查并更新上一帧中被替换的MapPoints
...
void Tracking::CheckReplacedInLastFrame() {
    for ( int i = 0; i < mLastFrame.N; i++ ) {
        ...
        if ( pMP ) {
            ...
            if ( pRep ) { mLastFrame.mvpMapPoints[i] = pRep; }
        }
    }
}
...
bool Tracking::TrackReferenceKeyFrame() {
    ...
    mCurrentFrame.SetPose(
        mLastFrame.mTcw );   // 用上一次的Tcw设置初值，在PoseOptimization可以收敛快一些
...
void Tracking::UpdateLastFrame() {
    ...
    if ( mnLastKeyFrameId == mLastFrame.mnId || mSensor == System::MONOCULAR ) return;
    ...
    for ( int i = 0; i < mCurrentFrame.N; i++ ) {
        if ( mCurrentFrame.mvpMapPoints[i] ) {
            ...
            if ( !mCurrentFrame.mvbOutlier[i] ) {
                ...
                if ( !mbOnlyTracking ) {
                    // 该MapPoint被其它关键帧观测到过
                    if ( mCurrentFrame.mvpMapPoints[i]->Observations() > 0 ) mnMatchesInliers++;
                } else
...
bool Tracking::NeedNewKeyFrame() {
    ...
    if ( nKFs < 2 ) thRefRatio = 0.4f;   // 关键帧只有一帧，那么插入关键帧的阈值设置很低
    if ( mSensor == System::MONOCULAR ) thRefRatio = 0.9f;

    // MapPoints中和地图关联的比例阈值
    float thMapRatio                         = 0.35f;
    if ( mnMatchesInliers > 300 ) thMapRatio = 0.20f;
...
void Tracking::UpdateLocalMap() {
    ...
    mpMap->SetReferenceMapPoints( mvpLocalMapPoints );   // set mvpReferenceMapPoints
...
void Tracking::UpdateLocalKeyFrames() {
    ...
    map< KeyFrame*, int > keyframeCounter;   //	mvpReferenceMapPoints
    for ( int i = 0; i < mCurrentFrame.N; i++ ) {
        if ( mCurrentFrame.mvpMapPoints[i] ) {
            MapPoint* pMP = mCurrentFrame.mvpMapPoints[i];
            if ( !pMP->isBad() ) {
    ...
    for ( vector< KeyFrame * >::const_iterator itKF    = mvpLocalKeyFrames.begin(),
                                               itEndKF = mvpLocalKeyFrames.end();
          itKF != itEndKF; itKF++ ) {
        // Limit the number of keyframes, if it is over 80, just break!
