...
bool Initializer::Initialize( const Frame& CurrentFrame, const vector< int >& vMatches12,
    ...
    for ( int i = 0; i < N; i++ ) { vAllIndices.push_back( i ); }
    ...
    // 计算homograpy并打分
    thread threadH( &Initializer::FindHomography, this, ref( vbMatchesInliersH ), ref( SH ),
                    ref( H ) );
    // 计算fundamental matrix并打分
    thread threadF( &Initializer::FindFundamental, this, ref( vbMatchesInliersF ), ref( SF ),
                    ref( F ) );
 ...
 * @brief 计算单应矩阵
 *
 * 假设场景为平面情况下通过前两帧求取Homography矩阵(current frame 2 到 reference frame
 * 1),并得到该模型的评分
 */
 ...
 * @brief 计算基础矩阵
 *
 * 假设场景为非平面情况下通过前两帧求取Fundamental矩阵(current frame 2 到 reference frame
 * 1),并得到该模型的评分
 */
 ...
 * - Multiple View Geometry in Computer Vision - model selection 4.7.1 RANSAC
 */
float Initializer::CheckFundamental( const cv::Mat& F21, vector< bool >& vbMatchesInliers,
                                     float sigma ) {
    const int N = mvMatches12.size();
...
bool Initializer::ReconstructH( vector< bool >& vbMatchesInliers, cv::Mat& H21, cv::Mat& K,
    ...
    // fail for points seen with low parallax)
