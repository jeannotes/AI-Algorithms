...
void Frame::UpdatePoseMatrices() {
    ...
    // attention, this means camera coordinate in the world-reference, its direction should be from world center to carera center
    mOw = -mRcw.t() * mtcw;
}
...
void Frame::ComputeStereoFromRGBD(const cv::Mat &imDepth) {
    ...
    for (int i = 0; i < N; i++) {
        ...
        if (d > 0) {
            mvDepth[i] = d;
            mvuRight[i] = kpU.pt.x - mbf / d; // get the corresponding x coordinate in right picture in P105
        }
    }
}
...
cv::Mat Frame::UnprojectStereo(const int &i) {
    ...
    if (z > 0) {
        ...
        /*
         * something to add now, mRwc*x3Dc is world coordinate, but this world coordinate is still small
         * because camera has moved, so we need to add how much camera has moved, actually that is mOw
         * mOw is from world center to this time's camera center(camera coordinate in world coordinate)
         */
    } else
        return cv::Mat();
}
