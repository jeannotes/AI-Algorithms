...
Frame::Frame( const cv::Mat& imGray, const cv::Mat& imDepth, const double& timeStamp,
    ...
    // get the corresponding x coordinate in right picture in P105
...
void Frame::AssignFeaturesToGrid() {
    int nReserve = 0.5f * N / ( FRAME_GRID_COLS * FRAME_GRID_ROWS );   // ? why 1/2, should be 1
...
void Frame::UpdatePoseMatrices() {
    ...
    // attention, this means camera coordinate in the world-reference, its direction should be from
    // world center to carera center
    mOw = -mRcw.t() * mtcw;
}
...
bool Frame::PosInGrid( const cv::KeyPoint& kp, int& posX, int& posY ) {
    ...
    // like triangulation, now we get the point in which grid
...
void Frame::ComputeStereoMatches() {
    ...
    const float minZ =
        mb;                 // NOTE bug mb没有初始化，mb的赋值在构造函数中放在ComputeStereoMatches函数的后面
    const float minD = 0;   // 最小视差, 设置为0即可
    const float maxD =
        mbf / minZ;   // 最大视差, 对应最小深度 mbf/minZ = mbf/mb = mbf/(mbf/fx) = fx (wubo???)
    ...
    for ( int iL = 0; iL < N; iL++ ) {
        ...
        if ( bestDist < thOrbDist ) {
            ...
            // kpL.pt.x对应金字塔最底层坐标，将最佳匹配的特征点对尺度变换到尺度对应层 (scaleduL,
            // scaledvL) (scaleduR0, )
            ...
            IL =
                ...
                IL.at< float >( w, w ) *
                    cv::Mat::ones(
                        IL.rows, IL.cols,
                        CV_32F );   //窗口中的每个元素减去正中心的那个元素，简单归一化，减小光照强度影响
            ...
            if ( bestincR == -L ||
                 bestincR ==
                     L )   // 整个滑动窗口过程中，SAD最小值不是以抛物线形式出现，SAD匹配失败，同时放弃求该特征点的深度
                continue;
            ...
            float bestuR =
                mvScaleFactors[kpL.octave] * ( (float) scaleduR0 + (float) bestincR + deltaR );
            ...
            if ( disparity >= minD && disparity < maxD ) {   // 最后判断视差是否在范围内
                ...
                vDistIdx.push_back(
                    pair< int, int >( bestDist, iL ) );   // 该特征点SAD匹配最小匹配偏差
            }
        }
    }
...
void Frame::ComputeStereoFromRGBD( const cv::Mat& imDepth ) {
    ...
    for ( int i = 0; i < N; i++ ) {
        ...
        if ( d > 0 ) {
            mvDepth[i] = d;
            mvuRight[i] =
                kpU.pt.x - mbf / d;   // get the corresponding x coordinate in right picture in P105
        }
    }
}
...
cv::Mat Frame::UnprojectStereo( const int& i ) {
    ...
    if ( z > 0 ) {
        ...
        /*
         * something to add now, mRwc*x3Dc is world coordinate, but this world coordinate is still
         * small
         * because camera has moved, so we need to add how much camera has moved, actually that is
         * mOw
         * mOw is from world center to this time's camera center(camera coordinate in world
         * coordinate)
         */
    } else
        return cv::Mat();
