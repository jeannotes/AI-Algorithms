...
class EXPORT TemplatedVocabulary {
  public:
     ...
     * @param L depth levels
     */
    virtual void create( const std::vector< std::vector< TDescriptor > >& training_features, int k,
                         int L );
  ...
  protected:
     ...
     */
    void getFeatures( const vector< vector< TDescriptor > >& training_features,
     ...
     * @param levelsup
     */
    virtual void transform( const TDescriptor& feature, WordId& id, WordValue& weight,
                            NodeId* nid = NULL, int levelsup = 0 ) const;
...
template < class TDescriptor, class F >
TemplatedVocabulary< TDescriptor, F >::TemplatedVocabulary( int k, int L, WeightingType weighting,
                                                            ScoringType scoring )
...
template < class TDescriptor, class F >
TemplatedVocabulary< TDescriptor, F >& TemplatedVocabulary< TDescriptor, F >::
operator=( const TemplatedVocabulary< TDescriptor, F >& voc ) {
    this->m_k         = voc.m_k;
    this->m_L         = voc.m_L;
...
void TemplatedVocabulary< TDescriptor, F >::getFeatures(
    ...
    for ( vvit = training_features.begin(); vvit != training_features.end(); ++vvit ) {
        ...
        for ( vit = vvit->begin(); vit != vvit->end(); ++vit ) { features.push_back( &( *vit ) ); }
    }
}
...
void TemplatedVocabulary< TDescriptor, F >::initiateClustersKMpp(
    ...
    // 2.
    // 对于数据集中的每一个点x，计算它与最近聚类中心(指已选择的聚类中心)的距离D(x)并保存在一个数组里，
    //    然后把这些距离加起来得到Sum(D(x))。
    // 3.
...
template < class TDescriptor, class F >
inline double TemplatedVocabulary< TDescriptor, F >::score( const BowVector& v1,
...
template < class TDescriptor, class F >
void TemplatedVocabulary< TDescriptor, F >::transform( const TDescriptor& feature, WordId& word_id,
                                                       WordValue& weight, NodeId* nid,
                                                       int levelsup ) const {
...
template < class TDescriptor, class F >
void TemplatedVocabulary< TDescriptor, F >::getWordsFromNode( NodeId nid,
...
void TemplatedVocabulary< TDescriptor, F >::saveToBinaryFile( const std::string& filename ) const {
    ...
    unsigned int size_node = sizeof( m_nodes[0].parent ) + F::L * sizeof( char ) +
                             sizeof( _weight ) + sizeof( bool );   // F::L
