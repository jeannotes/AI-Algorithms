...
CQuat::CQuat( CMat3& Cnb ) {
    ...
    if ( Cnb.e02 - Cnb.e20 < 0 ) { q2 = -q2; }
    if ( Cnb.e10 - Cnb.e01 < 0 ) { q3 = -q3; }
...
CMat3::CMat3( CVect3& att ) {
    double si = sin( att.i ), ci = cos( att.i ), sj = sin( att.j ), cj = cos( att.j ),
...
CMat3 askew( CVect3& v ) {
    return CMat3( 0, -v.k, v.j, v.k, 0, -v.i, -v.j, v.i, 0 );
}
...
int svd3( CMat3& A, CMat3& u, CMat3& s, CMat3& v ) {
    CMat3 B  = ( ~A ) * A;
    double a = -( B.e00 + B.e11 + B.e22 ), b = B.e00 * B.e11 + B.e00 * B.e22 + B.e11 * B.e22 -
                                               B.e12 * B.e12 - B.e01 * B.e01 - B.e02 * B.e02,
           c = -( B.e00 * B.e11 * B.e22 - B.e00 * B.e12 * B.e12 - B.e01 * B.e01 * B.e22 +
                  2 * B.e01 * B.e02 * B.e12 - B.e02 * B.e02 * B.e11 ),
           p = b - a * a / 3, q = ( 2 * a * a * a - 9 * a * b + 27 * c ) / 27,
           delta = q * q / 4 + p * p * p / 27, x1, x2, x3, tmp;
    int rootstate;
    if ( delta < -100 * EPS ) {
        double T = -q * sqrt( -27 * p ) / ( 2 * p * p ), theta = acosEx( T ),
               x0 = 2 * sqrt( -p / 3 );
        x1        = x0 * cos( theta / 3 );
        x2        = x0 * cos( theta / 3 - PI * 2.0 / 3 );
        x3        = x0 * cos( theta / 3 + PI * 2.0 / 3 );
        rootstate = 3;
    ...
    if ( rootstate == 1 ) {
        ...
        C    = x1 * I33 - B;
        vt1  = *( (CVect3*) &C.e00 ) * *( (CVect3*) &C.e10 );
        nvt1 = norm( vt1 );
        vt2  = *( (CVect3*) &C.e10 ) * *( (CVect3*) &C.e20 );
        nvt2 = norm( vt2 );
        vt3  = *( (CVect3*) &C.e20 ) * *( (CVect3*) &C.e00 );
        nvt3 = norm( vt3 );
        if ( nvt1 > nvt2 && nvt1 > nvt3 )
            v1 = vt1;
        else if ( nvt2 > nvt1 && nvt2 > nvt3 )
            v1 = vt2;
        else
            v1 = vt3;
        C      = x2 * I33 - B;
    ...
    v = CMat3( v1.i / nv1, v2.i / nv2, v3.i / nv3, v1.j / nv1, v2.j / nv2, v3.j / nv3, v1.k / nv1,
               v2.k / nv2, v3.k / nv3 );
    s = diag( CVect3( sqrt( x1 ), sqrt( x2 ), sqrt( x3 ) ) );
    u = A * v * CMat3( sign( s.e00 ) == 0 ? 0 : 1 / s.e00, 0, 0, 0,
                       sign( s.e11 ) == 0 ? 0 : 1 / s.e11, 0, 0, 0,
                       sign( s.e22 ) == 0 ? 0 : 1 / s.e22 );
    return sign( det( A ) );
}
...
CVect CMat::operator()( char RorC, int i ) {
    ...
    if ( RorC == 'R' ) {
        v.row = 1;
        v.clm = v.rc = clm;
        for ( double *p = v.dd, *p1 = &dd[i * clm], *pEnd = p1 + clm; p1 < pEnd; p++, p1++ )
            *p = *p1;
    } else if ( RorC == 'C' ) {
        v.row = v.rc = row;
        v.clm        = 1;
        for ( double *p = v.dd, *p1 = &dd[i], *pEnd = &dd[rc]; p1 < pEnd; p++, p1 += clm ) *p = *p1;
    }
    return v;
}
...
void CKalman::TimeUpdate( double ts ) {
    CMat Fk = Ft * ts;
    for ( double *p = Fk.dd, *pEnd = &Fk.dd[Fk.rc]; p < pEnd; p += Fk.row + 1 ) {
        *p += 1.0;   // Fk = I+Ft*ts
    }
    Xk = Fk * Xk;
    Pk = Fk * Pk * ( ~Fk ) + Qt * ts;
...
void CSINS::Update( CVect3* wm, CVect3* vm, int nSamples, double ts ) {
    ...
    imu.dvbm    = Ka * imu.dvbm - db * nts;   // calibration
...
void CSINS::etm( CMat3& Maa, CMat3& Mav, CMat3& Map, CMat3& Mva, CMat3& Mvv, CMat3& Mvp, CMat3& Mpv,
                 CMat3& Mpp ) {
    double tl = eth.tl, secl = 1.0 / eth.cl, secl2 = secl * secl, wN = eth.wnie.j, wU = eth.wnie.k,
           vE = vn.i, vN = vn.j;
    double f_RMh = eth.f_RMh, f_RNh = eth.f_RNh, f_clRNh = eth.f_clRNh, f_RMh2 = f_RMh * f_RMh,
           f_RNh2 = f_RNh * f_RNh;
    CMat3 Avn     = askew( vn ), Mp1( 0, 0, 0, -wU, 0, 0, wN, 0, 0 ),
...
int CIMUFile::Load( CVect3* wm, CVect3* vm, int n ) {
    ...
    if ( feof( f ) ) return 0;
    t = t + n * ts;
    if ( fmod( t + 0.01 * ts, 50.0 ) < n * ts ) printf( "%.4lf\n", t );
    return n;
}
...
CMat3 AlignCoarse( CVect3 wmm, CVect3 vmm, double latitude ) {
    ...
    T21 = wbib.i / cl - T31 * tl, T22 = wbib.j / cl - T32 * tl, T23 = wbib.k / cl - T33 * tl;
