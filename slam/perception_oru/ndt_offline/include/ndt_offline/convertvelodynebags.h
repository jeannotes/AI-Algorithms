...
class ConvertVelodyneBagsToPcl {
  public:
     ...
     */
    ConvertVelodyneBagsToPcl( std::string outbag_name, std::string calibration_file,
                              std::string bagfilename, std::string velodynetopic,
                              std::string tf_pose_id, std::string fixed_frame_id = "/world",
                              std::string tftopic               = "/tf",
                              ros::Duration dur                 = ros::Duration( 3600 ),
                              tf::StampedTransform* sensor_link = NULL,
                              double velodyne_max_range = 130.0, double velodyne_min_range = 2.0,
                              double sensor_time_offset = 0.0, int height = 64, int width = 1000 )
    ...
    bool ConvertToPclBag( tf::Transform& sensor_pose ) {
        ...
        if ( m.getTopic() == velodynetopic_ ) {
            velodyne_msgs::VelodyneScan::ConstPtr scan =
            ...
            if ( scan != NULL ) {
                ...
                std::cout << "Frame:" << ++counter << ", size:" << cloud->points.size()
                          << std::endl;
    ...
    int addToImages( const velodyne_rawdata::VPointCloud& pnts, int width, int height,
                     int startpixelx, int startpixely, const tf::Transform& T ) {
        double resolution_factor = width / view_width_;
        for ( size_t i = 0; i < pnts.size(); i++ ) {
            int y = height - pnts.points[i].ring - 1;
            tf::Point pt;
            pt.setValue( pnts.points[i].x - T.getOrigin()[0], pnts.points[i].y - T.getOrigin()[1],
                         pnts.points[i].z - T.getOrigin()[2] );
            ...
            if ( scale_intensity ) intensity = intensity / 255.;
    ...
    bool ConvertToCompleteImages( tf::Transform& sensor_pose ) {
        if ( image_clean_ ) {
            ...
            for ( size_t next = 0; next < global_scan->packets.size(); ++next ) {
                ...
                if ( odosync->getTransformationForTime( timestamp_of_last_sensor_message, t1,
                                                        tf_pose_id_, T ) ) {
                    pcl_ros::transformPointCloud( pnts, conv_points, T );
                    this->addToImages( conv_points, width_, height_, 0, 0, T );
                } else {
    ...
    bool ConvertToImages( tf::Transform& sensor_pose ) {
        ...
        if ( m.getTopic() == velodynetopic_ ) {
            ...
            if ( scan != NULL ) {
                ...
                out_msg.image = intensity_img_;
                outbag.write( "/velodyne_intensity_img", timestamp_of_last_sensor_message,
                              out_msg );
            }
        }
    ...
    void CloseOutputBag() { outbag.close(); }
