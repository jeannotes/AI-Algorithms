...
void filter_fov_fun( pcl::PointCloud< pcl::PointXYZ >& cloud,
                     pcl::PointCloud< pcl::PointXYZ >& cloud_nofilter, double hori_min,
                     double hori_max ) {
...
int main( int argc, char** argv ) {
    ...
    desc.add_options()( "help", "produce help message" )( "visualize", "visualize the output" )(
        "use-odometry", "use initial guess from odometry" )( "match2d", "use 2d-matcher" )(
        "no-step-control", "use step control in the optimization (default=false)" )(
        "nosubmaps", "run the standard fuser" )( "pre-load", "loads maps from the map directory if "
                                                             "available" )(
        "base-name", po::value< string >( &base_name ), "prefix for all generated files" )(
        "dir-name", po::value< string >( &dirname ),
        "where to look for SCANs" )( "map-dir-name", po::value< string >( &map_dirname ),
                                     "where to save the pieces of the map (default it ./map)" )(
        "size-xy", po::value< double >( &size_xy )->default_value( 30. ), "size of the central map "
                                                                          "xy" )(
        "size-z", po::value< double >( &size_z )->default_value( 0.8 ), "height of the map" )(
        "resolution", po::value< double >( &resolution )->default_value( 0.1 ),
        "resolution of the "
        "map" )( "resolution_local_factor",
                 po::value< double >( &resolution_local_factor )->default_value( 0.2 ),
                 "resolution factor of the local map used in the match and fusing step" )(
        "sensor_cutoff", po::value< double >( &sensor_cutoff )->default_value( 1000 ),
        "ignore ranges longer than this value" )(
        "itrs", po::value< int >( &itrs )->default_value( 30 ),
        "resolution of the map" )( "baseline", "run also the baseline registration algorithms" )(
        ...
        "the minimum field of view angle horizontal" )( "COOP", "if parameters from the COOP data "
                                                                "set should be used (sensorpose)" )(
        ...
        "Dd", po::value< double >( &motion_params.Dd )->default_value( 0.005 ),
        "forward uncertainty on distance traveled" )(
        "Dt", po::value< double >( &motion_params.Dt )->default_value( 0.01 ),
        "forward uncertainty on rotation" )(
        "Cd", po::value< double >( &motion_params.Cd )->default_value( 0.001 ),
        "side uncertainty on distance traveled" )(
        "Ct", po::value< double >( &motion_params.Ct )->default_value( 0.01 ),
        "side uncertainty "
        "on rotation" )( "Td", po::value< double >( &motion_params.Td )->default_value( 0.001 ),
                         "rotation uncertainty on distance traveled" )(
        "Tt", po::value< double >( &motion_params.Tt )->default_value( 0.005 ),
        "rotation uncertainty on rotation" )(
        "min_dist", po::value< double >( &min_dist )->default_value( 0.2 ),
        "minimum distance traveled before adding cloud" )(
        "min_rot_in_deg", po::value< double >( &min_rot_in_deg )->default_value( 5 ),
        "minimum rotation before adding cloud" )( "tf_base_link",
                                                  po::value< std::string >( &tf_base_link )
                                                      ->default_value( std::string( "/odom_base_"
                                                                                    "link" ) ),
                                                  "tf_base_link" )(
        "tf_sensor_link",
        po::value< std::string >( &tf_sensor_link )->default_value( std::string( "/sensor_link" ) ),
        "tf_sensor_link : tf of where the sensor is" )(
        "tf_gt_link",
        po::value< std::string >( &tf_gt_link )->default_value( std::string( "/state_base_link" ) ),
        "tf ground truth link" )( "velodyne_config_file",
                                  po::value< std::string >( &velodyne_config_file )
                                      ->default_value( std::string( "velo32."
                                                                    "yaml" ) ),
                                  "configuration file for the scanner" )(
        "tf_world_frame",
        po::value< std::string >( &tf_world_frame )->default_value( std::string( "/odom" ) ),
        "tf world frame" )( "velodyne_packets_topic",
        ...
        ( "alive",
          "keep the mapper/visualization running even though it is completed (e.g. to take screen "
          "shots etc." )( "nb_neighbours", po::value< int >( &nb_neighbours )->default_value( 2 ),
                          "number of neighbours used in the registration" )(
            "max_range", po::value< double >( &max_range )->default_value( 70. ),
            "maximum range used from scanner" )(
            "min_range", po::value< double >( &min_range )->default_value( 1.5 ),
            "minimum range used from scanner" )( "save_map", "saves the map at the end of "
                                                             "execution" )(
            ...
            "use_gt_as_interp_link",
            "use gt when performing point interplation while unwrapping "
            "the velodyne scans" )( "save_clouds", "save all clouds that are added to the map" )(
            "tf_topic",
            po::value< std::string >( &tf_topic )->default_value( std::string( "/tf" ) ),
            "tf topic to listen to" )( "x", po::value< double >( &transl[0] )->default_value( 0. ),
                                       "sensor pose - translation vector x" )(
            "y", po::value< double >( &transl[1] )->default_value( 0. ), "sensor pose - "
                                                                         "translation vector y" )(
            "z", po::value< double >( &transl[2] )->default_value( 0. ), "sensor pose - "
                                                                         "translation vector z" )(
            "ex", po::value< double >( &euler[0] )->default_value( 0. ), "sensor pose - euler "
                                                                         "angle vector x" )(
            "ey", po::value< double >( &euler[1] )->default_value( 0. ), "sensor pose - euler "
                                                                         "angle vector y" )(
            "ez", po::value< double >( &euler[2] )->default_value( 0. ), "sensor pose - euler "
                                                                         "angle vector z" )(
    ...
    if ( !vm.count( "map-dir-name" ) ) { map_dirname = "map"; }
    ...
    base_name +=
        motion_params.getDescString() + std::string( "_res" ) + toString( resolution ) +
        std::string( "_SC" ) + toString( do_soft_constraints ) + std::string( "_mindist" ) +
        toString( min_dist ) + std::string( "_sensorcutoff" ) + toString( sensor_cutoff ) +
        std::string( "_stepcontrol" ) + toString( step_control ) + std::string( "_neighbours" ) +
        toString( nb_neighbours ) + std::string( "_rlf" ) + toString( resolution_local_factor );

    std::cout << resolution << " " << size_xy << " " << size_xy << " " << size_z << " "
              << sensor_cutoff << " " << visualize << " match2d " << match2d << " " << use_multires
              << " " << fuse_incomplete << " itrs " << itrs << " " << base_name << " beHMT "
              << beHMT << " " << map_dirname << " step_control " << step_control << " "
              << do_soft_constraints << " nb_neighb " << nb_neighbours << " "
              << resolution_local_factor << " min range " << min_range << std::endl;
    ...
    std::cout << resolution << " " << size_xy << " " << size_xy << " " << size_z << " "
              ...
              << beHMT << " " << map_dirname << " step_control " << step_control << " "
              << do_soft_constraints << " nb_neighb " << nb_neighbours << " "
              << resolution_local_factor << std::endl;

    // 	exit(0);
    std::string file_to_log( "/home/malcolm/Documents/log_fuser/log_fuser_pointcloud_offline.txt" );
    perception_oru::ndt_fuser::NDTFuserHMTLogger ndtslammer(
        file_to_log, resolution, size_xy, size_xy, size_z, sensor_cutoff, visualize, match2d,
        use_multires, fuse_incomplete, itrs, base_name, beHMT, map_dirname, step_control,
        do_soft_constraints, nb_neighbours, resolution_local_factor );
    //     ros::Time::init();
    ...
    if ( !gt_file.is_open() || !est_file.is_open() || !odom_file.is_open() ) {
        //       ROS_ERROR_STREAM("Failed to open : " << gt_file << " || " << est_file << " || " <<
        //       odom_file);
    }

    std::string tf_interp_link = tf_base_link;
    if ( use_gt_as_interp_link ) { tf_interp_link = tf_gt_link; }

    for ( int i = 0; i < scanfiles.size(); i++ ) {
        std::string bagfilename = scanfiles[i];
        fprintf( stderr, "Opening %s\n", bagfilename.c_str() );
        perception_oru::ndt_offline::PointCloudBagReader< pcl::PointXYZ > vreader(
            velodyne_config_file, bagfilename,
            velodyne_packets_topic,   //"/velodyne_packets"
            tf_base_link, tf_sensor_link, tf_world_frame, tf_topic, ros::Duration( 3600 ),
            &sensor_link, 0, 0.02, min_range, true, false );
        ...
        while ( vreader.readMultipleMeasurements( nb_scan_msgs, cloud ) ) {
            ...
            if ( counter == 689 ) {
                std::cout << "Saving map of cell : " << ndtslammer.map->getAllCells().size()
                          << std::endl;
            ...
            if ( counter == 0 ) {
                ...
                std::cout << "initializing ..." << vreader.getLastPose().matrix() << " sensor "
                          << vreader.getSensorPose().matrix() << "cloud " << cloud.size()
                          << std::endl;
                ...
                Eigen::Affine3d sens =
                    Eigen::Translation< double, 3 >( 0, 0, 1 ) *
                ...
                std::cout << std::endl
                          << "Robot pose " << vreader.getLastPose().matrix() << std::endl;

                // 				ndtslammer.initialize(Tgt,cloud,preload);
                std::cout << "initializing done" << std::endl;
                // 				exit(0);
                std::cout << "Saving map of cell : " << ndtslammer.map->getAllCells().size()
                          << " with cloud " << cloud.size() << std::endl;
                ndtslammer.print();
                // 				exit(0);
                // Told = Tgt;//for gt

                if ( save_clouds ) { saveCloud( counter - 1, cloud ); }
                ...
                if ( !use_odometry ) {
                    ...
                    // added_motion.matrix() << std::endl << std::endl;
                    Eigen::Vector3d added_motion_euler =
                        added_motion.rotation().eulerAngles( 0, 1, 2 );

                    if ( added_motion.translation().norm() > min_dist ||
                         fabs( added_motion_euler[2] ) > ( min_rot_in_deg * M_PI / 180.0 ) ) {
                        // 						laserpub.publish<sensor_msgs::LaserScan>(*(vreader.getLastLaserScan()));
                        Eigen::Affine3d Todo = ndtslammer.update( added_motion, cloud,
                                                                  vreader.getTimeStampOfLastMsg() );

                        added_motion.setIdentity();

                        std::cout << std::endl
                                  << "Updated " << cloud.size() << " motion " << Tmotion.matrix()
                        ...
                        // 							std::cout << "FRAME " << mesg.header.frame_id <<
                        // std::endl;
                        // 							exit(0);
                        // 							mesg = vreader.last_pointcloud;
                        //
                        ...
                        // 							mesg.header.stamp = ros::Time::now();
                        // 							laserpub.publish<sensor_msgs::PointCloud2>(mesg);
                        // 							sensor_msgs::PointCloud2::ConstPtr mesg_laser =
                        // vreader.getLastMsgScan();
                        // 							sensor_msgs::LaserScan mes_laser_tmp =
                        // *mesg_laser;
                ...
                // 				if(counter == 15){
                // 					std::cout << "Saving map of cell : " <<
                // ndtslammer.map->getAllCells().size()
                // << std::endl;
                // 					if (ndtslammer.wasInit() && ndtslammer.map != NULL) {
                // 						ndtslammer.map->writeToJFF("map.jff");
                // 						std::cout << "Done." << std::endl;
                ...
                // 			sensorpose_est_file << frame_time << " " << transformToEvalString(Todo *
                // Ts);
            }
        }
