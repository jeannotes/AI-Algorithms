...
class view_bag {
  ...
  public:
    ...
    view_bag( ros::NodeHandle param_nh ) {
        ...
        *odom_sub_ = nh_.subscribe< nav_msgs::Odometry >( odometry_topic, 10,
                                                          &view_bag::odom_callback, this );
    ...
    void odom_callback( const nav_msgs::Odometry::ConstPtr& msg_in ) {
        ...
        Todom = Eigen::Translation3d( msg_in->pose.pose.position.x, msg_in->pose.pose.position.y,
                                      msg_in->pose.pose.position.z ) *
                qd;
