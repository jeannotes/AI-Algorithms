...
void NDTOccupancyMap< PointT >::loadPointCloud( const Eigen::Vector3d& origin,
    ...
    */
    ...
    while ( it != pc.points.end() ) {
        ...
        for ( unsigned int i = 0; i < N; i++ ) {
            ...
            // fprintf(stderr,"Requesting point for (%.3f %.3f %.3f) target = (%.3f %.3f %.3f)....
            // ",pt.x,pt.y,pt.z,it->x, it->y, it->z );
            NDTCell< PointT >* ptCell =
                dynamic_cast< NDTCell< PointT >* >( index_->getCellForPoint( pt ) );
            // fprintf(stderr,"Done\n");
            if ( ptCell != NULL ) {
                // fprintf(stderr,"Updating point for (%.3f %.3f %.3f) target = (%.3f %.3f %.3f)....
                // ",pt.x,pt.y,pt.z,it->x, it->y, it->z );
                ptCell->updateEmpty();
            } else {
...
*/
template < typename PointT >
void NDTOccupancyMap< PointT >::addPointCloud( const Eigen::Vector3d& origin,
                                               const pcl::PointCloud< PointT >& pc ) {
    if ( isFirstLoad_ ) { loadPointCloud( origin, pc ); }
...
template < typename PointT >
void NDTOccupancyMap< PointT >::loadPointCloud(
    const pcl::PointCloud< PointT >& pc, const std::vector< std::vector< size_t > >& indices ) {
    loadPointCloud( pc );
...
template < typename PointT >
void NDTOccupancyMap< PointT >::loadDepthImage( const cv::Mat& depthImage,
...
template < typename PointT >
pcl::PointCloud< PointT > NDTOccupancyMap< PointT >::loadDepthImageFeatures(
    const cv::Mat& depthImage, std::vector< cv::KeyPoint >& keypoints, size_t& supportSize,
    double maxVar, DepthCamera< PointT >& cameraParams, bool estimateParamsDI, bool nonMean ) {
...
std::string NDTOccupancyMap< PointT >::getMyIndexStr() const {
    ...
    if ( tr != NULL ) { return std::string( "OctTree" ); }
    LazyGrid< PointT >* gr = dynamic_cast< LazyGrid< PointT >* >( index_ );
...
template < typename PointT >
std::vector< NDTCell< PointT >* > NDTOccupancyMap< PointT >::getCellsForPoint( const PointT pt,
                                                                               double radius ) {
    // assert(false);
...
void NDTOccupancyMap< PointT >::debugToVRML( const char* fname, pcl::PointCloud< PointT >& pc ) {
    ...
    for ( size_t i = 0; i < pc.points.size(); i++ ) {
        ...
        if ( this->getCellForPoint( pc.points[i], link ) ) {
            ...
            fprintf( fout, "%lf %lf %lf\n\t%lf %lf %lf\n\t", pc.points[i].x, pc.points[i].y,
                     pc.points[i].z, centerCell.x, centerCell.y, centerCell.z );
            n_lines++;
        }
    }

    fprintf( fout, "]\n\t}\n\tcoordIndex [\n\t" );
    for ( int i = 0; i < n_lines; i++ ) { fprintf( fout, "%d, %d, -1\n\t", 2 * i, 2 * i + 1 ); }
    fprintf( fout, "]\n}\n}\n" );
...
template < typename PointT >
std::vector< NDTCell< PointT >* > NDTOccupancyMap< PointT >::pseudoTransformNDT(
...
template < typename PointT >
NDTCell< PointT >* NDTOccupancyMap< PointT >::getCellIdx( unsigned int idx ) {
    CellVector< PointT >* cl = dynamic_cast< CellVector< PointT >* >( index_ );
 ...
 */
template < typename PointT >
std::vector< perception_oru::NDTCell< PointT >* >
NDTOccupancyMap< PointT >::getDynamicCells( unsigned int Timescale, float threshold ) {
    std::vector< NDTCell< PointT >* > ret;
    typename SpatialIndex< PointT >::CellVectorItr it = index_->begin();
    while ( it != index_->end() ) {
        NDTCell< PointT >* cell = dynamic_cast< NDTCell< PointT >* >( *it );
