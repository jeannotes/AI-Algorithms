...
void OctTree< PointT >::setParameters( double _BIG_CELL_SIZE, double _SMALL_CELL_SIZE,
                                       int _MAX_POINTS, int _MAX_DEPTH ) {
    // defaults
    ...
    if ( !this->leaf_ ) {
        for ( unsigned int it = 0; it < 8; it++ ) {
            if ( this->children_[it] != NULL ) {
                this->children_[it]->setParameters( _BIG_CELL_SIZE, _SMALL_CELL_SIZE, _MAX_POINTS,
                                                    _MAX_DEPTH );
            }
        }
    }
...
size_t OctTree< PointT >::getIndexForPoint( const PointT& pt ) const {
    ...
    size_t ret = 0;
    if ( pt.x < myCell_->getCenter().x ) { ret += 4; }
    if ( pt.y < myCell_->getCenter().y ) { ret += 2; }
    if ( ret % 4 == 0 ) {
        if ( pt.z < myCell_->getCenter().z ) { ret += 1; }
    } else {
        if ( pt.z > myCell_->getCenter().z ) { ret += 1; }
    }
    return ret;
...
OctTree< PointT >::OctTree() {
    ...
    this->leaf_   = true;
    this->depth_  = 0;
    for ( unsigned int it = 0; it < 8; it++ ) { this->children_[it] = NULL; }
    myCell_      = NULL;
    leafsCached_ = false;
    this->setParameters();
}
...
/** parametrized constructor
  */
template < typename PointT >
OctTree< PointT >::OctTree( PointT center, double xsize, double ysize, double zsize,
                            NDTCell< PointT >* type, OctTree< PointT >* _parent,
                            unsigned int _depth ) {
    parent_ = _parent;
    leaf_   = true;
    depth_  = _depth;
    for ( unsigned int it = 0; it < 8; it++ ) { children_[it] = NULL; }
...
Cell< PointT >* OctTree< PointT >::addPoint( const PointT& point_c ) {
    ...
    if ( std::isnan( point.x ) || std::isnan( point.y ) || std::isnan( point.z ) ) { return NULL; }
    leafsCached_ = false;
    if ( this->leaf_ ) {
        double xs, ys, zs;
        ...
        if ( myCell_->points_.size() < (unsigned int) MAX_POINTS && cellSize <= BIG_CELL_SIZE ) {
            if ( !myCell_->isInside( point ) ) {
                // DBG(1,"OctTree: addPoint (%lf,%lf,%lf) not in
            ...
            for ( int it = 0; it < 8; it++ ) {
                ...
                children_[it] = new OctTree< PointT >( newCenter, xs / 2, ys / 2, zs / 2, myCell_,
                                                       this, depth_ + 1 );
                children_[it]->setParameters( BIG_CELL_SIZE, SMALL_CELL_SIZE, MAX_POINTS,
                                              MAX_DEPTH );
            }
            // add current points
...
OctTree< PointT >* OctTree< PointT >::getLeafForPoint( const PointT& point ) {
    if ( this->leaf_ && myCell_ != NULL ) {
        if ( myCell_->isInside( point ) ) { return this; }
    } else {
        size_t ind = getIndexForPoint( point );
        if ( children_[ind] != NULL ) { return children_[ind]->getLeafForPoint( point ); }
    }
    return NULL;
}
        ...
        if ( cur != NULL ) {
            if ( cur->isLeaf() ) {
                ...
                for ( int i = 0; i < 8; i++ ) {
                    ...
                    if ( tmp != NULL ) { next.push_back( tmp ); }
                }
            }
        }
...
SpatialIndex< PointT >* OctTree< PointT >::clone() const {
    ...
    if ( myCell_ != NULL ) { tr->setCellType( myCell_ ); }
    return tr;
}
...
/** returns all the neighboring cells within a radius
   \param point the location around which we are looking for neighbors. The point must be inside the
   boundaries of a current leaf!
   \param radius the neighbor tolerance
   \param cells output
  */
template < typename PointT >
void OctTree< PointT >::getNeighbors( const PointT& point, const double& radius,
    ...
    while ( mparent != NULL ) {
        ...
        for ( unsigned int nt = 0; nt < toExpand.size(); nt++ ) {
            ...
            if ( toExpand[nt]->isLeaf() && d.norm() < radius ) {
...
Cell< PointT >* OctTree< PointT >::getClosestLeafCell( const PointT& point ) {
    if ( this->leaf_ && myCell_ != NULL ) {
        if ( myCell_->isInside( point ) ) { return myCell_; }
    } else {
        size_t ind = getIndexForPoint( point );
        if ( children_[ind] != NULL ) {
            ...
            if ( index >= 0 && index < 8 ) { return children_[index]->getClosestLeafCell( point ); }
        }
    }
    return myCell_;
}
...
NDTCell< PointT >* OctTree< PointT >::getClosestNDTCell( const PointT& point ) {
    ...
    size_t ind = getIndexForPoint( point );
    if ( children_[ind] != NULL ) { return children_[ind]->getClosestNDTCell( point ); }
