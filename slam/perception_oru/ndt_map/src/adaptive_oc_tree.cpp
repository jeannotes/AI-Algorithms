...
void AdaptiveOctTree< PointT >::setParameters( bool _useDornikHansen, bool _useFlatness,
                                               double _RSS_THRESHOLD, double _DH_SIGNIFICANCE_LVL,
                                               double _MIN_CELL_SIZE, double _FLAT_FACTOR,
                                               double _BIG_CELL_SIZE, double _SMALL_CELL_SIZE ) {
...
AdaptiveOctTree< PointT >::AdaptiveOctTree() : OctTree< PointT >() {
    if ( !parametersSet ) { setParameters(); }
}
...
AdaptiveOctTree< PointT >::AdaptiveOctTree( pcl::PointXYZ center, double xsize, double ysize,
                                            double zsize, NDTCell< PointT >* type,
                                            OctTree< PointT >* _parent, unsigned int _depth )
...
double AdaptiveOctTree< PointT >::computeDornikHansen( NDTCell< PointT >* cell ) {
    ...
    for ( unsigned int dim = 0; dim < 3; dim++ ) {
        ...
        alpha = a + b1 * b1 * c;
        chi   = ( b2 - 1 - b1 * b1 ) * 2 * k;
        z2    = ( pow( ( chi / ( 2 * alpha ) ), ( 1. / 3. ) ) - 1 + ( 1. / ( 9 * alpha ) ) ) *
             sqrt( 9 * alpha );
    ...
    if ( pval > DH_SIGNIFICANCE_LVL ) {
        ...
        std::cout << "final split was at (" << dx << "," << dy << "," << dz << "); pval is " << pval
                  << std::endl;
    }
    return 0;
}
...
/**
  ...
  */
template < typename PointT >
std::vector< OctTree< PointT >* >
AdaptiveOctTree< PointT >::splitTree( OctTree< PointT >* octLeaf ) {
    ...
    if ( octLeaf->isLeaf() ) {
        ...
        for ( unsigned int it = 0; it < 8; it++ ) {
            ...
            octLeaf->children_[it] = new OctTree< PointT >(
                newCenter, xs / 2, ys / 2, zs / 2, octLeaf->myCell_, this, this->depth_ + 1 );
...
SpatialIndex< PointT >* AdaptiveOctTree< PointT >::clone() {
    if ( this->myCell_ == NULL ) { return new AdaptiveOctTree(); }
    double sx, sy, sz;
    this->myCell_->getDimensions( sx, sy, sz );
    AdaptiveOctTree< PointT >* tr =
        new AdaptiveOctTree< PointT >( this->myCell_->getCenter(), sx, sy, sz, this->myCell_ );
    return tr;
}
}
