...
void NDTCell::setParameters( double _EVAL_ROUGH_THR, double _EVEC_INCLINED_THR, double _EVAL_FACTOR,
                             int _MIN_NB_POINTS_FOR_GAUSSIAN, bool _CLEAR_MIN_NB_POINTS,
...
*/
void NDTCell::updateSampleVariance( const Eigen::Matrix3d& cov2, const Eigen::Vector3d& m2,
                                    unsigned int numpointsindistribution, bool updateOccupancyFlag,
                                    float max_occu, unsigned int maxnumpoints ) {
    ...
    if ( this->hasGaussian_ ) {
...
#if 1
        double w1 =
            ( (double) N / (double) ( numpointsindistribution * ( N + numpointsindistribution ) ) );
        ...
        Eigen::Matrix3d csum3 =
            csum1 + csum2 + N * d1 * d1.transpose() + numpointsindistribution * d2 * d2.transpose();
#endif
        ...
        if ( updateOccupancyFlag ) {
            ...
            double logoddlikoccu =
                numpointsindistribution * log( ( likoccval ) / ( 1.0 - likoccval ) );
...
inline void computeCovFromPoints(
    Eigen::Matrix3d& cov_, const Eigen::Vector3d& mean_,
    const std::vector< pcl::PointXYZ, Eigen::aligned_allocator< pcl::PointXYZ > >& points_ ) {
    Eigen::MatrixXd mp;
...
void NDTCell::computeGaussianSimple() {
    if ( points_.size() < MIN_NB_POINTS_FOR_GAUSSIAN ) {
        ...
        if ( !MIN_NB_POINTS_SET_UNIFORM ) { return; }
    } else {
        computeMeanFromPoints( mean_, points_ );
...
*/
void NDTCell::computeGaussian( int mode, unsigned int maxnumpoints, float occupancy_limit,
    ...
    if ( logoddlikoccu >
         0.4 ) {   /// We have to trust that it is occupied if we have measurements from the cell!
    ...
    if ( ( hasGaussian_ == false && points_.size() < MIN_NB_POINTS_FOR_GAUSSIAN ) ) {
        ...
        if ( !MIN_NB_POINTS_SET_UNIFORM ) return;
        if ( !CLEAR_MIN_NB_POINTS ) clear_points = false;
        min_nb_points                            = true;
    }
    ...
    if ( !hasGaussian_ ) {
        ...
        /////////////////////////////////////////////////////////////////////////////
                ...
                CELL_UPDATE_MODE_COVARIANCE_INTERSECTION ) {   /// Update using new information
        /////////////////////////////////////////////////////////////////////////////
        /////////////////////////////////////////////////////////////////////////////
        Eigen::Vector3d m2;
        m2 << 0, 0, 0;
        ...
        c3 =
            covSum_ + c2 + w1 * ( w2 * meanSum_ - ( T2 ) ) * ( w2 * meanSum_ - ( T2 ) ).transpose();
        ...
        for ( unsigned int i = 0; i < points_.size(); i++ ) {
            ...
            double sigma_dist = 0.5 * ( ( dist ) / 20.0 );   /// test for distance based sensor
                                                             /// noise
            double snoise = sigma_dist + sensor_noise;
            double model_trust_given_meas =
                0.49 +
                0.5 * exp( -0.5 * ( ( Xm - X ).norm() * ( Xm - X ).norm() ) / ( snoise * snoise ) );
...
void NDTCell::writeJFFVector( FILE* jffout, Eigen::Vector3d& vec ) {
    ...
    for ( int i = 0; i < 3; i++ ) { dtemp[i] = vec.coeff( i ); }
...
int NDTCell::loadJFFMatrix( FILE* jffin, Eigen::Matrix3d& mat ) {
    ...
    if ( fread( &dtemp, sizeof( double ), 6, jffin ) <= 0 ) return -1;
...
int NDTCell::loadJFFVector( FILE* jffin, Eigen::Vector3d& vec ) {
    ...
    if ( fread( &dtemp, sizeof( double ), 3, jffin ) <= 0 ) return -1;
...
int NDTCell::loadJFFEventData( FILE* jffin, TEventData& evdata ) {
    ...
    if ( fread( &ocval, sizeof( uint8_t ), 1, jffin ) <= 0 ) return -1;
    if ( fread( &ftemp, sizeof( float ), 4, jffin ) <= 0 ) return -1;
    if ( fread( &evnts, sizeof( uint64_t ), 1, jffin ) <= 0 ) return -1;
...
int NDTCell::loadFromJFF( FILE* jffin ) {
    pcl::PointXYZ center;
    if ( fread( &center, sizeof( pcl::PointXYZ ), 1, jffin ) <= 0 ) return -1;
    this->setCenter( center );

    double dimensions[3];
    if ( fread( &dimensions, sizeof( double ), 3, jffin ) <= 0 ) return -1;
    this->setDimensions( dimensions[0], dimensions[1], dimensions[2] );

    Eigen::Matrix3d temp_matrix;
    Eigen::Vector3d temp_vector;
    if ( loadJFFMatrix( jffin, temp_matrix ) < 0 ) return -1;
    this->setCov( temp_matrix );

    if ( loadJFFVector( jffin, temp_vector ) < 0 ) return -1;
    ...
    if ( fread( &dtemp, sizeof( double ), 2, jffin ) <= 0 ) return -1;
    if ( fread( &itemp, sizeof( int ), 3, jffin ) <= 0 ) return -1;
    if ( fread( &ftemp, sizeof( float ), 4, jffin ) <= 0 ) return -1;
...
*/
double NDTCell::computeMaximumLikelihoodAlongLine( const pcl::PointXYZ& p1, const pcl::PointXYZ& p2,
                                                   Eigen::Vector3d& out ) {
