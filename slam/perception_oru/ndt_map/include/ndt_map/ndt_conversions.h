    ...
    for ( int cell_idx = 0; cell_idx < map_vector.size(); cell_idx++ ) {
        if ( map_vector[cell_idx] != NULL ) {   // we send intialized cells
            ...
            if ( map_vector[cell_idx]->hasGaussian_ ) {   // we only send a cell with gaussian
                ...
                for ( int i = 0; i < 3; i++ ) {
                    for ( int j = 0; j < 3; j++ ) { cell.cov_matrix.push_back( cov( i, j ) ); }
                }
 ...
 *
 */
inline bool fromMessage( LazyGrid*& idx, NDTMap*& map, ndt_map::NDTMapMsg msg,
                         std::string& frame_name, bool dealloc = false ) {
    if ( !( msg.x_cell_size == msg.y_cell_size &&
            msg.y_cell_size == msg.z_cell_size ) ) {   // we assume that voxels are cubes
 ...
 *
 */
inline bool toOccupancyGrid( NDTMap* ndt_map, nav_msgs::OccupancyGrid& occ_grid, double resolution,
                             std::string frame_id ) {   // works only for 2D case
...
inline bool toOccupancyGrid( const boost::shared_ptr< perception_oru::NDTMap >& ndt_map,
                             nav_msgs::OccupancyGrid& occ_grid, double resolution,
                             std::string frame_id ) {   // works only for 2D case
    double size_x, size_y, size_z;
    int size_x_cell_count, size_y_cell_count;
    double cen_x, cen_y, cen_z;
