...
class GraphMap {
  public:
    /*!
     * \brief UpdateGraph is  high level interface to graph class. It allows for automatic selection
     * and creation of mapnodes.
     */
    /*!
     * \brief AutomaticMapInterchange Automatically interchange between or create new map upon
     * certain conditions
     * \param Tnow pre: Pose is described in the local mapNode frame. Post: Pose is described in the
     * current local mapNode frame and is properly updated if a new frame has been selected
     * \param cov contain the movement uncertainty accumulated since last node
     * \param T_world_to_local pre: Not used: Post T_world_to_local contain the mapping from world
     * to the current local mapNode frame.
     */

    virtual void AddMapNode( const MapParamPtr& mapparam, const Eigen::Affine3d& diff,
                             const Matrix6d& cov );   //
