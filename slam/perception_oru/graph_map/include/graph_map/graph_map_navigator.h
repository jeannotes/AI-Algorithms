...
class GraphMapNavigator : public GraphMap {
  public:
    GraphMapNavigator( const Eigen::Affine3d& nodepose, const MapParamPtr& mapparam,
    ...
    bool SwitchToClosestMapNode( Affine3d& Tnow, const Matrix6d& cov,
                                 Affine3d& T_world_to_local_map, const double radius );
    //!
    ...
    //! \param Tnow  is the target pose in the global frame
    //! \param new_current After the method is invoked, new_current will contain true if a new map
    //! node was selected as(closest )map node and assigned to current_node;
    //! \return
    //!
    bool SwitchToClosestMapNode( Affine3d& Tnow, bool& new_current );
    bool AutomaticMapInterchange( Affine3d& Tnow, const Matrix6d& cov,
                                  Affine3d& T_world_to_local_map, bool& changed_map_node,
