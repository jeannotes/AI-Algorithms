...
class GraphMapFuser {
  public:
    GraphMapFuser() {}
    GraphMapFuser( string maptype, string registratorType, const Eigen::Affine3d& init_pose,
                   const Affine3d& sensorPose );   // Ros friendly constructor to read parameters
                                                   // from ros-par-server
    GraphMapFuser( RegParamPtr regParam, MapParamPtr mapParam, GraphParamPtr graph_param,
                   const Eigen::Affine3d& init_pose, const Eigen::Affine3d& sensorPose );
    ...
    void SetMotionParameters( const MotionModel2d& motion_param ) {
        motion_model_2d_ = motion_param;
    }
    template < class PointT >
    bool ProcessFrame( pcl::PointCloud< PointT >& cloud, Eigen::Affine3d& Tnow,
                       const Eigen::Affine3d& Tmotion );   // cloud is the current scan in robot
                                                           // frame,  Tnow is the current pose in
                                                           // world frame
    // void ProcessFrame(pcl::PointCloud<pcl::PointXYZ> &cloud, Eigen::Affine3d &Tnow, const
    // Eigen::Affine3d &Tmotion); //cloud is the current scan in robot frame,  Tnow is the current
    // pose in world frame
