...
void segmentPointCurvature( const pcl::PointCloud< velodyne_pointcloud::PointXYZIR >& cloud,
    ...
    for ( int i = 0; i < cloud.size(); i++ ) {
        ...
        if ( p.ring > N_SCANS ) N_SCANS = p.ring;
    }
    ...
    for ( int i = 0; i < ring_clouds.size(); i++ ) { *laserCloud += ring_clouds[i]; }
    ...
    for ( int i = 0; i < N_SCANS; i++ ) {
        /////////////////
        // ROS_INFO_STREAM("scanStartInd[i] " << scanStartInd[i] << " scanEndInd[i] " <<
        // scanEndInd[i]);
        ...
        for ( int k = scanStartInd[i] + 10; k < scanEndInd[i] - 10; k++ ) {
            ...
            if ( diff1 > M_PI / 2. ) continue;
            if ( diff2 > M_PI / 2. ) continue;
            if ( diff1 < 0.001 ) continue;
            if ( diff2 < 0.001 ) continue;

            //      if (diff1*10 < diff2) {
            //        pointsNaNToEdge.push_back(laserCloud->points[k]);
            //        ROS_INFO_STREAM("++ori_prev : " << ori_prev << " ori : " << ori << " ori_next
            //        : " << ori_next << " diff1 : " << diff1 << " diff2 : " << diff2 << " ring : "
            //        << int(laserCloud->points[k].intensity));
            ...
            for ( int k = ep; k >= sp; k-- ) {
                ...
                if ( cloudNeighborPicked[ind] == 0 && cloudCurvature[ind] > 0.1 ) {
                    ...
                    if ( largestPickedNum <= 2 ) {
                        ...
                        cornerPointsLessSharp.push_back(
                            pcl::PointXYZ( laserCloud->points[ind].x, laserCloud->points[ind].y,
                                           laserCloud->points[ind].z ) );
