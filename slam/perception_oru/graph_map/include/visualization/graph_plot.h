...
class GraphPlot {
  public:
    static void sendMapToRviz( mean_vector& mean, cov_vector& cov, ros::Publisher* mapPublisher,
                               string frame, int color,
                               const Affine3d& offset = Affine3d::Identity(), string ns = "ndt",
                               int markerType = visualization_msgs::Marker::SPHERE );
    static void SendLocalMapToRviz( perception_oru::NDTMap* mapPtr, int color,
                                    const Affine3d& offset = Affine3d::Identity() );
    static void SendGlobalMapToRviz( perception_oru::NDTMap* mapPtr, int color,
                                     const Affine3d& offset = Affine3d::Identity() );
    static void SendGlobal2MapToRviz( perception_oru::NDTMap* mapPtr, int color,
                                      const Affine3d& offset = Affine3d::Identity() );
    static void SendGlobal2MapToRviz( std::vector< perception_oru::NDTCell* > cells, int color,
                                      const Affine3d& offset = Affine3d::Identity() );
    ...
    static void PlotMap( MapTypePtr map, int color, const Affine3d& offset = Affine3d::Identity(),
                         PlotMarker marker = sphere, std::string ns = "" );
    static void PlotMap( NDTMap& map, int color, const Affine3d& offset = Affine3d::Identity(),
                         PlotMarker marker = sphere, std::string ns = "ndtmap",
                         double point_size = 0.1 );

  private:
    static void PublishMapAsPoints( mean_vector& mean, int color, double scale,
                                    const Eigen::Affine3d& offset, std::string ns = "pts",
                                    int id = 0 );
    static void CovarToMarker( const Eigen::Matrix3d& cov, const Eigen::Vector3d& mean,
                               visualization_msgs::Marker& marker );
    static void GetAllCellsMeanCov( const perception_oru::NDTMap* mapPtr, cov_vector& cov,
                                    mean_vector& mean );
    static void GetAllCellsMeanCov( std::vector< perception_oru::NDTCell* > cells, cov_vector& cov,
                                    mean_vector& mean );
    static void makeRightHanded( Eigen::Matrix3d& eigenvectors, Eigen::Vector3d& eigenvalues );
    static void computeShapeScaleAndOrientation3D( const Eigen::Matrix3d& covariance,
  ...
  private:
    ...
    class ColorGradient {
      private:
        struct ColorPoint {   // Internal class used to store colors at different points in the
                              // gradient.
