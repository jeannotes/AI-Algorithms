...
void NDTDLMapType::update( const Eigen::Affine3d& Tsensor, pcl::PointCloud< pcl::PointXYZ >& cloud,
                           bool simple ) {   // update map, cloud is the scan, Tsensor is the pose
                                             // where the scan was aquired.
...
void NDTDLMapType::update( const Eigen::Affine3d& Tsensor,
    ...
    if ( initialized_ && enable_mapping_ ) {
        ...
        map_flat_->addPointCloudMeanUpdate( Tsensor.translation(), surfPointsLessFlat, localMapSize,
                                            1e5, 25, 2 * sizez_, 0.06 );
        map_edge_->addPointCloudMeanUpdate( Tsensor.translation(), cornerPointsLessSharp,
                                            localMapSize, 1e5, 25, 2 * sizez_, 0.06 );
...
void NDTDLMapType::InitializeMap( const Eigen::Affine3d& Tsensor,
                                  pcl::PointCloud< pcl::PointXYZ >& cloudFlat,
                                  pcl::PointCloud< pcl::PointXYZ >& cloudEdge ) {
    cout << "initialize map" << endl;
    map_flat_->addPointCloud( Tsensor.translation(), cloudFlat, 0.1, 100.0, 0.1 );
    map_flat_->computeNDTCells( CELL_UPDATE_MODE_SAMPLE_VARIANCE, 1e5, 255, Tsensor.translation(),
                                0.1 );

    map_edge_->addPointCloud( Tsensor.translation(), cloudEdge, 0.1, 100.0, 0.1 );
    map_edge_->computeNDTCells( CELL_UPDATE_MODE_SAMPLE_VARIANCE, 1e5, 255, Tsensor.translation(),
                                0.1 );
}

bool NDTDLMapType::CompoundMapsByRadius( MapTypePtr target, const Affine3d& T_source,
                                         const Affine3d& T_target, double radius ) {
    return true;
