...
void ScanPlot::plot( const sensor_msgs::LaserScan& scan, const scanRepresentation plot_type,
                     const string PlotOptsX, const string PlotOptsY, double alpha ) {
...
void ScanPlot::scanToPairVector( const sensor_msgs::LaserScan& scan, const scanRepresentation rep,
        ...
        case RangeAngleDerivative:
            for ( int i = 0; i < scan.ranges.size(); i++ ) {
                angle = -( scan.angle_min +
                           (double) i / (double) ( scan.ranges.size() ) *
                               (double) ( scan.angle_max - scan.angle_min ) );
                range = scan.ranges[i];
            ...
            for ( int i = 0; i < scan.ranges.size(); i++ ) {
                angle = -( scan.angle_min +
                           (double) i / (double) ( scan.ranges.size() ) *
                               (double) ( scan.angle_max - scan.angle_min ) );
                range = scan.ranges[i];
                data.push_back( std::make_pair( range * cos( angle + M_PI / 2 ),
                                                range * sin( angle + M_PI / 2 ) ) );
            }
    }
}
...
void ScanPlot::derivative( pair_vector_double& data_in, pair_vector_double& derivative ) {
    ...
    for ( int i = 1; i < data_in.size() - 1; i++ ) {
        double dR = ( (double) data_in[i + 1].second - (double) data_in[i - 1].second ) /
                    ( (double) data_in[i + 1].first - (double) data_in[i - 1].first );
