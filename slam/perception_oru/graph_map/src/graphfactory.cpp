...
MapTypePtr GraphFactory::CreateMapType( MapParamPtr mapparam ) {
    if ( NDTMapParamPtr ndt2MapParam = boost::dynamic_pointer_cast< NDTMapParam >(
             mapparam ) ) {   // perform typecast and check if not null conversion
        cout << "Graphfactory: Created map of type: \"" << ndt_map_type_name << "\"" << endl;
        return MapTypePtr( new NDTMapType( ndt2MapParam ) );
    } else if ( NDTDLMapParamPtr ndtdlMapParam = boost::dynamic_pointer_cast< NDTDLMapParam >(
                    mapparam ) ) {   // perform typecast and check if not null conversion
        cout << "Graphfactory: Created map of type: \"" << ndtdl_map_type_name << "\"" << endl;
        return MapTypePtr( new NDTDLMapType( ndtdlMapParam ) );
    } else if ( TemplateMapParamPtr template_par =
                    boost::dynamic_pointer_cast< TemplateMapParam >( mapparam ) ) {
...
GraphMapPtr GraphFactory::CreateGraph( const Eigen::Affine3d& nodepose, MapParamPtr& mapparam,
                                       GraphParamPtr graphparam ) {
    if ( mapparam != NULL ) {
        cout << "Graphfactory: Creating graph" << endl;
        GraphMapPtr graphPtr =
            GraphMapPtr( new GraphMapNavigator( nodepose, mapparam, graphparam ) );
        return graphPtr;
    } else {
...
GraphMapNavigatorPtr GraphFactory::CreateGraphNavigator( const Eigen::Affine3d& nodepose,
...
MapNodePtr GraphFactory::CreateMapNode( const Eigen::Affine3d& pose,
                                        const MapParamPtr& mapparam ) {   // Create a node
    cout << "Graphfactory: Creating map node" << endl;
    return boost::shared_ptr< MapNode >( new MapNode( pose, mapparam ) );
}

RegTypePtr GraphFactory::CreateRegistrationType( const Affine3d& sensor_pose,
                                                 RegParamPtr regparam ) {
    if ( NDTD2DRegParamPtr ndt_reg_ptr =
             boost::dynamic_pointer_cast< NDTD2DRegParam >( regparam ) ) {
    ...
    else if ( NDTDLRegTypeParamPtr ndt_reg_ptr = boost::dynamic_pointer_cast< NDTDLRegTypeParam >(
                  regparam ) ) {   // perform typecast and check if not null conversion
...
RegParamPtr GraphFactory::CreateRegParam( string regType ) {
    if ( regType.compare( ndt_d2d_reg_type_name ) == 0 ) {
        ...
        cout << "Graphfactory: Creating parameters for registration type: \"" << ndtdl_map_type_name
             << "\"" << endl;
...
FactorPtr GraphFactory::CreateMapNodeFactor( MapNodePtr prevMapPose, MapNodePtr nextMapPose,
                                             const Eigen::Affine3d& diff, const Matrix6d& covar ) {
    FactorPtr factorptr =
