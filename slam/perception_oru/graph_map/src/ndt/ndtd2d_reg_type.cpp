...
{
NDTD2DRegType::NDTD2DRegType( const Affine3d& sensor_pose, RegParamPtr paramptr )
    : registrationType( sensor_pose, paramptr ) {
    NDTD2DRegParamPtr param_ptr =
        boost::dynamic_pointer_cast< NDTD2DRegParam >( paramptr );   // Should not be NULL
    if ( param_ptr != NULL ) {
...
bool NDTD2DRegType::Register( MapTypePtr maptype, Eigen::Affine3d& Tnow,
                              pcl::PointCloud< pcl::PointXYZ >& cloud, Matrix6d cov ) {
    ...
    perception_oru::NDTMap ndlocal(
        new perception_oru::LazyGrid( resolution_ * resolutionLocalFactor_ ) );
    ...
    if ( matchSuccesfull ) {   // if succesfull match, make sure the registration is consistent
        Eigen::Affine3d diff =
            Tnow.inverse() * Tinit;   // difference between prediction and registration
        ...
        if ( checkConsistency_ && diff.translation().norm() > maxTranslationNorm_ ) {
            ...
            cerr << "movement=" << diff.translation().norm() << "m  >  "
                 << diff.translation().norm() << endl;
            return false;
...
bool NDTD2DRegType::Register( MapTypePtr maptype, Eigen::Affine3d& Tnow,
                              ...
                              Matrix6d cov ) {
    cerr << "TODO: implement update for point type PointXYZIR - will convert to PointXYZ for now"
         << endl;
    pcl::PointCloud< pcl::PointXYZ > cloud_xyz;
    pcl::copyPointCloud( cloud, cloud_xyz );
    Register( maptype, Tnow, cloud_xyz, cov );
}
bool NDTD2DRegType::RegisterMap2Map( MapTypePtr map_prev, MapTypePtr map_next,
                                     Eigen::Affine3d& Tdiff, double match_score ) {
    NDTMap *prev, *next;
