...
std::string registrationType::ToString() {
    ...
    if ( enableRegistration_ ) {
        ...
        ss << "max registration distances(translation,rotation): (" << maxTranslationNorm_ << ","
           << maxRotationNorm_ << ")" << endl;
        ss << "max registration deltas(translationRegistrationDelta,rotationRegistrationDelta): ("
           << translationRegistrationDelta_ << "," << rotationRegistrationDelta_ << ")" << endl;
...
void registrationParameters::GetParametersFromRos() {
    ...
    */
    if ( render_GT_map ) enableRegistration_ = false;
}
}
}
