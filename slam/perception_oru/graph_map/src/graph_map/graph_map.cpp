...
GraphMap::GraphMap( const Affine3d& nodepose, const MapParamPtr& mapparam,
    ...
    factors_.clear();
    prevNode_    = NULL;
    currentNode_ = GraphFactory::CreateMapNode( nodepose, mapparam );   // The first node to be
                                                                        // added
    ...
    // double min_size=mapparam->sizex_< mapparam->sizey_? mapparam->sizex_ : mapparam->sizey_;
    // //select min map length
...
void GraphMap::AddMapNode( const MapParamPtr& mapparam, const Affine3d& diff,
                           const Matrix6d& cov ) {   // Add node with link uncertainty
    ...
    factors_.push_back(
        sd );   // Add connection between current and new node with link diff and covariance
...
string GraphMap::ToString() {
    ...
    for ( int i = 0; i < nodes_.size(); i++ ) {
        if ( i == 0 ) ss << "Node positions:" << endl;

        NodePtr ptr              = nodes_[i];
        Eigen::Vector3d position = ptr->GetPose().translation();
        ss << "node " << i << " (x,y,z):(" << position( 0 ) << "," << position( 1 ) << ","
           << position( 2 ) << ")" << endl;
    }
    if ( currentNode_ != NULL ) ss << "Detailed info node 0:" << endl << currentNode_->ToString();

    return ss.str();
}

Affine3d GraphMap::GetNodePose( int nodeNr ) {
    if ( nodeNr < GraphSize() ) return nodes_[nodeNr]->GetPose();
}
...
NodePtr GraphMap::GetNode( int nodeNr ) {
    if ( nodeNr < GraphSize() ) { return nodes_[nodeNr]; }
}
