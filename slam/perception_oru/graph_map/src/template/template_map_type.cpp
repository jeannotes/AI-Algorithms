...
TemplateMapType::TemplateMapType( MapParamPtr paramptr ) : MapType( paramptr ) {
    TemplateMapParamPtr param =
        boost::dynamic_pointer_cast< TemplateMapParam >( paramptr );   // Should not be NULL
    if ( param != NULL ) {
...
void TemplateMapType::update( const Eigen::Affine3d& Tsensor,
                              pcl::PointCloud< pcl::PointXYZ >& cloud ) {   // update map, cloud is
                                                                            // the scan, Tsensor is
                                                                            // the pose where the
                                                                            // scan was aquired.
...
bool TemplateMapType::CompoundMapsByRadius( MapTypePtr target, const Affine3d& T_source,
...
void TemplateMapParam::GetParametersFromRos() {
    ...
    nh.param< std::string >( "Super_important_map_parameter", SuperImportantMapParameter,
                             "parhaps not so important..." );
}
}
}
