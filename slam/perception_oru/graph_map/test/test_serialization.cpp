...
class testNDTMapSerialization {
  public:
    testNDTMapSerialization() {
        ...
        NDTMap* map2;
        LoadNdtMap( &map2 );
        cout << "cells shared=" << map2->getAllCells().size() << ","
             << map2->getAllCellsShared().size() << "," << map2->getAllInitializedCells().size()
             << endl;
  ...
  private:
    void CreateNdtMap( NDTMap** map ) {
        ...
        ( *map )->addPointCloudMeanUpdate( Eigen::Vector3d( 0, 0, 0 ), cloud, localMapSize, 1e5, 25,
                                           2 * 3, 0.06 );
        // (*map)->computeNDTCells();
