...
class NDTMatcherNode {
  protected:
    ...
    std::vector< pcl::PointCloud< pcl::PointXYZ >,
                 Eigen::aligned_allocator< pcl::PointCloud< pcl::PointXYZ > > >
        pcl_buffer_;
  ...
  public:
...

    {
        ...
        points2_sub_ = nh_.subscribe( "kinect_head/camera/rgb/points", 10,
                                      &NDTMatcherNode::points2Callback, this );
        pcl_buffer_.resize( 2 );
        incremental_pose_.setIdentity();
    }

    ~NDTMatcherNode() { delete matcher; }
    ...
    void points2Callback( const sensor_msgs::PointCloud2::ConstPtr& msg_in ) {
        ...
        if ( !ret ) ROS_INFO( "Registration failed!" );
