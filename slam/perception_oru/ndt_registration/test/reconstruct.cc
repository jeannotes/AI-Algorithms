...
int main( int argc, char** argv ) {
    ...
    while ( getline( &line, &len, fin ) > 0 ) {
        int n = sscanf( line, "%lf,%lf,%lf %lf", &q1, &q2, &q3, &q4 );
        if ( n != 4 ) {
            cout << "wrong format of pose at : " << line << endl;
            break;
        }
        if ( !getline( &line, &len, fin ) > 0 ) break;
        n = sscanf( line, "%lf,%lf,%lf", &xd, &yd, &zd );
        if ( n != 3 ) {
            cout << "wrong format of pose at : " << line << endl;
            break;
        }
        ...
        *( pcName.rbegin() ) = '\0';
        ...
        Tloc  = Tprev.inverse();
        Tprev = Eigen::Translation< double, 3 >( xd, yd, zd ) *
                Eigen::AngleAxis< double >( q4, Eigen::Vector3d( q1, q2, q3 ) );
        Tloc = Tloc * Tprev;

        transformPointCloudInPlace( Tloc, thisone );
        cout << "old pose is t:" << Tprev.translation().transpose() << " r "
             << Tprev.rotation().eulerAngles( 0, 1, 2 ).transpose() << endl;
        cout << "local pose is t:" << Tloc.translation().transpose() << " r "
             << Tloc.rotation().eulerAngles( 0, 1, 2 ).transpose() << endl;

        if ( !first ) {
            // register
            Treg.setIdentity();
            matcher.match( thisone, prev, Treg );
            cout << "registration pose is t:" << Treg.translation().transpose() << " r "
                 << Treg.rotation().eulerAngles( 0, 1, 2 ).transpose() << endl;
            Tglob = Tglob * Tloc * Treg;
            cout << "new global pose t:" << Tglob.translation().transpose() << " r "
                 << Tglob.rotation().eulerAngles( 0, 1, 2 ).transpose() << endl;
        } else {
            Tglob = Tloc;
            first = false;
        }
