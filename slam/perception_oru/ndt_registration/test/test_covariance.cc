...
int main( int argc, char** argv ) {
    ...
    *( prefix.rbegin() ) = '\0';
    ...
    while ( getline( &line, &len, fin ) > 0 ) {
        int n =
            sscanf( line, "%lf %lf %lf %lf %lf %lf %lf %lf", &ts, &xd, &yd, &zd, &x, &y, &z, &w );
        if ( n != 8 ) {
        ...
        */
        ...
        Eigen::Transform< double, 3, Eigen::Affine, Eigen::ColMajor > P =
            prevPose.inverse() * currPose;

        cout << "testing scan " << ctr - 1 << " to " << ctr << endl;
#if DEBUG_COVARIANCE
        cout << "prev pose    : " << prevPose.translation().transpose() << " rpy "
             << prevPose.rotation().eulerAngles( 0, 1, 2 ).transpose() << endl;
        cout << "curr pose    : " << currPose.translation().transpose() << " rpy " << r << " " << p
             << " " << y << endl;
        cout << "gt difference: " << P.translation().transpose() << " rpy "
             << P.rotation().eulerAngles( 0, 1, 2 ).transpose() << endl;
#endif
        ...
        for ( int i = 0; i < N_SAMPLES; i++ ) {
            ...
            */
...
#if DEBUG_COVARIANCE
            cout << "curr pose: " << currPose.translation().transpose() << " " << r + randRoll
                 << " " << p + randPitch << " " << y + randYaw << endl;
            cout << "diff: " << Pr.translation().transpose() << " rpy "
                 << Pr.rotation().eulerAngles( 0, 1, 2 ).transpose() << endl;
#endif
            bool ret;
            ...
            /*
                    ...
                    logger<<"pnt2ndt("<<ctr<<","<<i+1<<",:) =
               ["<<offset.translation().transpose()<<"
               "<<offset.rotation().eulerAngles(0,1,2).transpose()<<"];\n";
                    ...
                    offset = P.inverse()*(R*Pr);
                    //that's the error! log it.
                    logger<<"icp("<<ctr<<","<<i+1<<",:) = ["<<offset.translation().transpose()<<"
               "<<offset.rotation().eulerAngles(0,1,2).transpose()<<"];\n";
            */
            // NDT2NDT
            // transform curr scan by Pr
            ...
            fclose( fout );
            cout << "registration: " << R.translation().transpose() << " rpy "
                 << R.rotation().eulerAngles( 0, 1, 2 ).transpose() << endl;
#endif
            // compute P[-](Pr[+]R)
            offset = P.inverse() * ( R * Pr );
            // that's the error! log it.
            logger << "ndt2ndt(" << ctr << "," << i + 1 << ",:) = ["
                   << offset.translation().transpose() << " "
                   << offset.rotation().eulerAngles( 0, 1, 2 ).transpose() << "];\n";
            //	    cout<<offset.translation().transpose()<<"
            //"<<offset.rotation().eulerAngles(0,1,2).transpose()<<endl;
        }

        Eigen::Matrix< double, 6, 6 > cov;
        //	matcherP2F.covariance(prev,curr,P,cov);
        //	logger<<"COVpnt2ndt("<<ctr<<",:,:) =
        //["<<cov.row(0)<<";\n"<<cov.row(1)<<";\n"<<cov.row(2)<<";\n"<<cov.row(3)<<";\n"<<cov.row(4)<<";\n"<<cov.row(5)<<"];\n";
