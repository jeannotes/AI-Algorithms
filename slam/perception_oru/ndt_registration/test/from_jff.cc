...
int main( int argc, char** argv ) {
    ...
    istringstream roll_c( argv[4] ), pitch_c( argv[5] ), yaw_c( argv[6] ), xoffset_c( argv[1] ),
        yoffset_c( argv[2] ), zoffset_c( argv[3] );
    ...
    perception_oru::NDTMap< pcl::PointXYZI > ndmap(
        new perception_oru::LazyGrid< pcl::PointXYZI >( 0.4 ) ),
        local_map( new perception_oru::LazyGrid< pcl::PointXYZI >( 0.4 ) );
    ndmap.loadFromJFF( argv[7] );
    local_map.loadFromJFF( argv[8] );

    // lslgeneric::NDTMatcherD2D_2D<pcl::PointXYZI,pcl::PointXYZI> matcherD2D(false, false,
    // resolutions);
    perception_oru::NDTMatcherD2D< pcl::PointXYZI, pcl::PointXYZI > matcherD2D( false, false,
                                                                                resolutions );
