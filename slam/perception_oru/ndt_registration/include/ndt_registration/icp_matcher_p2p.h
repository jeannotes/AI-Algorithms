...
//! cov code from https://sites.google.com/site/saimanoj18/
void calculate_ICP_COV( pcl::PointCloud< pcl::PointXYZ >& data_pi,
                        pcl::PointCloud< pcl::PointXYZ >& model_qi, Eigen::Matrix4d& transform,
    ...
    a = yaw;
    b = pitch;
    c = roll;   // important // According to the rotation matrix I used and after verification, it
                // is Yaw Pitch ROLL = [a,b,c]== [R] matrix used in the MatLab also :)
    ...
    for ( size_t s = 0; s < data_pi.points.size(); ++s ) {
        ...
        *************************************************************/
        ...
        double d2J_dx2, d2J_dydx, d2J_dzdx, d2J_dadx, d2J_dbdx, d2J_dcdx, d2J_dxdy, d2J_dy2,
            d2J_dzdy, d2J_dady, d2J_dbdy, d2J_dcdy, d2J_dxdz, d2J_dydz, d2J_dz2, d2J_dadz, d2J_dbdz,
            d2J_dcdz, d2J_dxda, d2J_dyda, d2J_dzda, d2J_da2, d2J_dbda, d2J_dcda, d2J_dxdb, d2J_dydb,
            d2J_dzdb, d2J_dadb, d2J_db2, d2J_dcdb, d2J_dxdc, d2J_dydc, d2J_dzdc, d2J_dadc, d2J_dbdc,
            d2J_dc2;
        ...
        // In this way, matlab can be used to generate these terms for various objective functions
        // of ICP
        // and they can simply be copied to the C++ files and with appropriate changes to ICP
        // estimation,
        ...
        d2J_db2 =
            ...
            ( pix * cos( b ) + piz * cos( c ) * sin( b ) + piy * sin( b ) * sin( c ) ) *
                ( 2 * pix * cos( b ) + 2 * piz * cos( c ) * sin( b ) +
                  2 * piy * sin( b ) * sin( c ) ) -
            ( 2 * piz * cos( b ) * cos( c ) - 2 * pix * sin( b ) + 2 * piy * cos( b ) * sin( c ) ) *
        ...
        d2J_dX2_temp << d2J_dx2, d2J_dydx, d2J_dzdx, d2J_dadx, d2J_dbdx, d2J_dzdx, d2J_dxdy,
            d2J_dy2, d2J_dzdy, d2J_dady, d2J_dbdy, d2J_dcdy, d2J_dxdz, d2J_dydz, d2J_dz2, d2J_dadz,
            d2J_dbdz, d2J_dcdz, d2J_dxda, d2J_dyda, d2J_dzda, d2J_da2, d2J_dbda, d2J_dcda, d2J_dxdb,
            d2J_dydb, d2J_dzdb, d2J_dadb, d2J_db2, d2J_dcdb, d2J_dxdc, d2J_dydc, d2J_dzdc, d2J_dadc,
            d2J_dbdc, d2J_dc2;
    ...
    // Now its time to calculate d2J_dZdX , where Z are the measurements Pi and Qi, X =
    // [x,y,z,a,b,c]

    // n is the number of correspondences
    int n = data_pi.points.size();

    /*  Here we check if the number of correspondences between the source and the target point
     clouds are greater than 200.
      if yes, we only take the first 200 correspondences to calculate the covariance matrix
      You can try increasing it but if its too high, the system may run out of memory and give an
     exception saying
    ...
    */
    ...
    if ( n > 200 )
        n = 200;   ////////////****************************IMPORTANT CHANGE********but may not
                   /// affect*****************/////////////////////////////////////////

    std::cout << "\nNumber of Correspondences used for ICP's covariance estimation = " << n
              << std::endl;
    ...
    for ( int k = 0; k < n; ++k ) {   // row
        ...
        d2J_dpix_db =
            ...
            sin( b ) * ( 2 * pix * cos( b ) + 2 * piz * cos( c ) * sin( b ) +
                         2 * piy * sin( b ) * sin( c ) ) -
            2 * cos( b ) * ( z - qiz - pix * sin( b ) + piz * cos( b ) * cos( c ) +
                             piy * cos( b ) * sin( c ) ) +
            cos( a ) * cos( b ) *
                ( 2 * piz * cos( a ) * cos( b ) * cos( c ) - 2 * pix * cos( a ) * sin( b ) +
                  2 * piy * cos( a ) * cos( b ) * sin( c ) ) -
        ...
        d2J_dpiy_db =
            ...
            ( cos( c ) * sin( a ) - cos( a ) * sin( b ) * sin( c ) ) *
                ( 2 * piz * cos( a ) * cos( b ) * cos( c ) - 2 * pix * cos( a ) * sin( b ) +
                  2 * piy * cos( a ) * cos( b ) * sin( c ) ) -
            2 * sin( b ) * sin( c ) * ( z - qiz - pix * sin( b ) + piz * cos( b ) * cos( c ) +
                                        piy * cos( b ) * sin( c ) ) -
            cos( b ) * sin( c ) * ( 2 * pix * cos( b ) + 2 * piz * cos( c ) * sin( b ) +
                                    2 * piy * sin( b ) * sin( c ) ) +
            2 * cos( a ) * cos( b ) * sin( c ) *
                ( x - qix - piy * ( cos( c ) * sin( a ) - cos( a ) * sin( b ) * sin( c ) ) +
                  piz * ( sin( a ) * sin( c ) + cos( a ) * cos( c ) * sin( b ) ) +
        ...
        d2J_dpiy_dc =
            ...
            ( cos( c ) * sin( a ) - cos( a ) * sin( b ) * sin( c ) ) *
                ( 2 * piy * ( sin( a ) * sin( c ) + cos( a ) * cos( c ) * sin( b ) ) +
                  2 * piz * ( cos( c ) * sin( a ) - cos( a ) * sin( b ) * sin( c ) ) ) +
            2 * cos( b ) * cos( c ) * ( z - qiz - pix * sin( b ) + piz * cos( b ) * cos( c ) +
                                        piy * cos( b ) * sin( c ) ) +
            cos( b ) * sin( c ) * ( 2 * piy * cos( b ) * cos( c ) - 2 * piz * cos( b ) * sin( c ) );
        ...
        d2J_dpiz_db =
            ...
            ( cos( a ) * sin( c ) - cos( c ) * sin( a ) * sin( b ) ) *
                ( 2 * piz * cos( b ) * cos( c ) * sin( a ) - 2 * pix * sin( a ) * sin( b ) +
                  2 * piy * cos( b ) * sin( a ) * sin( c ) ) -
            2 * cos( c ) * sin( b ) * ( z - qiz - pix * sin( b ) + piz * cos( b ) * cos( c ) +
                                        piy * cos( b ) * sin( c ) ) -
            cos( b ) * cos( c ) * ( 2 * pix * cos( b ) + 2 * piz * cos( c ) * sin( b ) +
                                    2 * piy * sin( b ) * sin( c ) ) +
            2 * cos( a ) * cos( b ) * cos( c ) *
                ( x - qix - piy * ( cos( c ) * sin( a ) - cos( a ) * sin( b ) * sin( c ) ) +
                  piz * ( sin( a ) * sin( c ) + cos( a ) * cos( c ) * sin( b ) ) +
        ...
        d2J_dpiz_dc =
            ...
            cos( b ) * cos( c ) *
                ( 2 * piy * cos( b ) * cos( c ) - 2 * piz * cos( b ) * sin( c ) ) -
            2 * cos( b ) * sin( c ) * ( z - qiz - pix * sin( b ) + piz * cos( b ) * cos( c ) +
                                        piy * cos( b ) * sin( c ) );
        ...
        d2J_dqix_db =
            ...
            2 * pix * cos( a ) * sin( b ) - 2 * piz * cos( a ) * cos( b ) * cos( c ) -
            2 * piy * cos( a ) * cos( b ) * sin( c );
        ...
        d2J_dqiy_db =
            ...
            2 * pix * sin( a ) * sin( b ) - 2 * piz * cos( b ) * cos( c ) * sin( a ) -
            2 * piy * cos( b ) * sin( a ) * sin( c );
        ...
        d2J_dZdX_temp << d2J_dpix_dx, d2J_dpiy_dx, d2J_dpiz_dx, d2J_dqix_dx, d2J_dqiy_dx,
            d2J_dqiz_dx, d2J_dpix_dy, d2J_dpiy_dy, d2J_dpiz_dy, d2J_dqix_dy, d2J_dqiy_dy,
            d2J_dqiz_dy, d2J_dpix_dz, d2J_dpiy_dz, d2J_dpiz_dz, d2J_dqix_dz, d2J_dqiy_dz,
            d2J_dqiz_dz, d2J_dpix_da, d2J_dpiy_da, d2J_dpiz_da, d2J_dqix_da, d2J_dqiy_da,
            d2J_dqiz_da, d2J_dpix_db, d2J_dpiy_db, d2J_dpiz_db, d2J_dqix_db, d2J_dqiy_db,
            d2J_dqiz_db, d2J_dpix_dc, d2J_dpiy_dc, d2J_dpiz_dc, d2J_dqix_dc, d2J_dqiy_dc,
            d2J_dqiz_dc;

        d2J_dZdX.block< 6, 6 >( 0, 6 * k ) = d2J_dZdX_temp;
    }

    // By reaching here both the matrices d2J_dX2 and d2J_dZdX are calculated and lets print those
    // values out;
    ...
    /**************************************
     *
     * Here we create the matrix cov(z) as mentioned in Section 3.3 in the paper, "Covariance of ICP
     * with 3D Point to Point and Point to Plane Error Metrics"
     *
     * ************************************/
...
//! Class wrapping pcl icp registration and covariance estimation
class ICPMatcherP2P {
  public:
    bool match( pcl::PointCloud< pcl::PointXYZ >& fixed, pcl::PointCloud< pcl::PointXYZ >& moving,
        ...
        pcl::PointCloud< pcl::PointXYZ >::ConstPtr f(
            new pcl::PointCloud< pcl::PointXYZ >( fixed ) );
        pcl::PointCloud< pcl::PointXYZ >::ConstPtr m(
            new pcl::PointCloud< pcl::PointXYZ >( moving ) );
        ...
        std::cout << "Tout : " << ndt_generic::affine3dToStringRotMat( Tout ) << std::endl;
    ...
    bool covariance( pcl::PointCloud< pcl::PointXYZ >& target,
                     ...
                     Eigen::MatrixXd& cov ) {
        calculate_ICP_COV( target, source, T.matrix(), cov );
        if ( cov.allFinite() ) return true;
        return false;
    }

    const pcl::PointCloud< pcl::PointXYZ >& getFinalPC() const { return final_; }
