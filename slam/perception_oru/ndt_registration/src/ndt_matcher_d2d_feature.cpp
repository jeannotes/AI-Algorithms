...
{
bool NDTMatcherFeatureD2D::covariance(
    NDTMap& targetNDT, NDTMap& sourceNDT,
...
NDTMatcherFeatureD2D::scoreNDT( std::vector< NDTCell* >& sourceNDT, NDTMap& targetNDT,
    ...
    for ( unsigned int j = 0; j < _corr.size(); j++ ) {
        ...
        if ( _corr[j].second >= (int) sourceNDT.size() ) {
            std::cout << "second correspondance : " << _corr[j].second << ", " << sourceNDT.size()
                      << std::endl;
        }
        if ( sourceNDT[i] == NULL ) { std::cout << "sourceNDT[i] == NULL!" << std::endl; }
    ...
    if ( _trimFactor == 1. ) {
        ...
        //	std::nth_element (scores.begin(), scores.begin()+index, scores.end(), sort_scores());
        ////boost::bind(&std::pair<unsigned int, double>::second, _1) <
        // boost::bind(&std::pair<unsigned int, double>::second, _2));
        std::nth_element( scores.begin(), scores.begin() + index, scores.end(),
                          boost::bind( &std::pair< unsigned int, double >::second, _1 ) <
                              boost::bind( &std::pair< unsigned int, double >::second, _2 ) );
        std::fill( _goodCorr.begin(), _goodCorr.end(), false );
        //	std::cout << "_goodCorr.size() : " << _goodCorr.size() << " scores.size() : " <<
        // scores.size() << " index : " << index << std::endl;
...
double NDTMatcherFeatureD2D::derivativesNDT( const std::vector< NDTCell* >& sourceNDT,
    ...
    for ( unsigned int j = 0; j < _corr.size(); j++ ) {
        ...
        if ( cell->hasGaussian_ ) {
            ...
            if ( !this->update_gradient_hessian( score_gradient, Hessian, transformed, Cinv, sh,
                                                 computeHessian ) ) {
                continue;
            }
            score_here += sh;
