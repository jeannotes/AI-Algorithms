...
void NDTMatcherD2D::init( bool _isIrregularGrid, bool useDefaultGridResolutions,
...
bool NDTMatcherD2D::match( pcl::PointCloud< pcl::PointXYZ >& target,
...
#endif
    {
        ...
        for ( int r_ctr = resolutions.size() - 1; r_ctr >= 0; r_ctr-- ) {
            ...
            time_load += ( tv_end.tv_sec - tv_start.tv_sec ) * 1000. +
            ...
            time_match += ( tv_end.tv_sec - tv_start.tv_sec ) * 1000. +
                          ( tv_end.tv_usec - tv_start.tv_usec ) / 1000.;
...
// T = T*Temp;
            ...
            std::cout << "rotation   : " << Temp.rotation().eulerAngles( 0, 1, 2 ).transpose()
                      << std::endl;
    ...
    if ( useInitialGuess ) { T = T * Tinit; }
    gettimeofday( &tv_end0, NULL );
    time_combined = ( tv_end0.tv_sec - tv_start0.tv_sec ) * 1000. +
                    ( tv_end0.tv_usec - tv_start0.tv_usec ) / 1000.;
    // std::cout<<"load: "<<time_load<<" match "<<time_match<<" combined
    // "<<time_combined<<std::endl;
    return ret;
}

bool NDTMatcherD2D::match( NDTMap& targetNDT, NDTMap& sourceNDT,
                           Eigen::Transform< double, 3, Eigen::Affine, Eigen::ColMajor >& T,
                           bool useInitialGuess ) {
    // locals
    ...
    while ( !convergence ) {
        ...
        if ( minCoeff < 0 ) {   //|| evals.minCoeff()) // < 10e-5*evals.maxCoeff())
            if ( regularize ) {
                ...
                regularizer =
                    regularizer + minCoeff > 0 ? regularizer : 0.001 * maxCoeff - minCoeff;
                ...
                if ( score_here > score_best ) { T = Tbest; }
                // de-alloc nextNDT
        ...
        if ( score_gradient.norm() <= DELTA_SCORE ) {
            ...
            if ( score_here > score_best ) {
                //		std::cout<<"crap iterations, best was "<<score_best<<" last was
                //"<<score_here<<std::endl;
                T = Tbest;
            }
            // de-alloc nextNDT
        ...
        //	pose_increment_v.block<3,1>(0,0) =
        //-Hessian.block<3,3>(0,0).ldlt().solve(score_gradient.block<3,1>(0,0));
        //	pose_increment_v.block<3,1>(3,0) =
        //-Hessian.block<3,3>(3,3).ldlt().solve(score_gradient.block<3,1>(3,0));
        /*
            Eigen::SelfAdjointEigenSolver<Eigen::Matrix<double,6,6> > Sol (Hessian);
            Eigen::Matrix<double,6,1> evals = Sol.eigenvalues().real();
            if(evals.minCoeff() < 0 ) {
        ...
        */
        ...
        // pose_increment_v =
        // Hessian.jacobiSvd(Eigen::ComputeFullU|Eigen::ComputeFullV).solve(-score_gradient);
        ...
        */
        ...
        Hessian.jacobiSvd(Eigen::ComputeFullU|Eigen::ComputeFullV).solve(-score_gradient);
            ...
            double pnorm = sqrt(pose_increment_v(0)*pose_increment_v(0) +
        pose_increment_v(1)*pose_increment_v(1)
        ...
        */
        ...
        // std::cout<<"\%iteration "<<itr_ctr<<" pose norm "<<(pose_increment_v.norm())<<" score
        // "<<score_here<<" step "<<step_size<<std::endl;
        /*
        pose_increment_v(2) = 0;
        pose_increment_v(3) = 0;
        pose_increment_v(4) = 0;
        ...
        */
        ...
        TR.setIdentity();
        TR = Eigen::Translation< double, 3 >( pose_increment_v( 0 ), pose_increment_v( 1 ),
                                              pose_increment_v( 2 ) ) *
        ...
        T = TR * T;
        //	std::cout<<"incr(:,"<<itr_ctr+1<<") = ["<<pose_increment_v.transpose()<<"]';\n";
        //	std::cout<<"pose(:,"<<itr_ctr+2<<") = ["<<T.translation().transpose()<<"
        //"<<T.rotation().eulerAngles(0,1,2).transpose()<<"]';\n";
...
bool NDTMatcherD2D::update_gradient_hessian_local(
    Eigen::MatrixXd& score_gradient, Eigen::MatrixXd& Hessian, const Eigen::Vector3d& x,
    const Eigen::Matrix3d& B, const double& likelihood, const Eigen::Matrix< double, 3, 6 >& _Jest,
    const Eigen::Matrix< double, 18, 6 >& _Hest, const Eigen::Matrix< double, 3, 18 >& _Zest,
    const Eigen::Matrix< double, 18, 18 >& _ZHest, bool computeHessian ) {
    // vars for gradient
    ...
    if ( computeHessian ) {
        Hessian += factor *
                   ( 2 * _Jest.transpose() * B * _Jest + 2 * _xtBH - _xtBZhBx -
                     2 * _xtBZBJ.transpose() - 2 * _xtBZBJ + _xtBZBZBx + _xtBZBZBx.transpose() -
                     lfd2 * _Q * _Q.transpose() / 2 );   // + Eigen::Matrix<double,6,6>::Identity();
    }
    return true;
}

bool NDTMatcherD2D::update_gradient_hessian( Eigen::MatrixXd& score_gradient,
                                             Eigen::MatrixXd& Hessian, const Eigen::Vector3d& x,
                                             const Eigen::Matrix3d& B, const double& likelihood,
                                             bool computeHessian ) {
    /*
        double lmax = 1;
    ...
    */
    ...
    if ( computeHessian ) {
        Hloc =
            factor * ( 2 * Jest.transpose() * B * Jest + 2 * xtBH - xtBZhBx -
                       2 * xtBZBJ.transpose() - 2 * xtBZBJ + xtBZBZBx + xtBZBZBx.transpose() -
                       lfd2 * Q * Q.transpose() / 2 );   // + Eigen::Matrix<double,6,6>::Identity();
...
void NDTMatcherD2D::computeDerivativesLocal( Eigen::Vector3d& x, Eigen::Matrix3d C1,
    ...
    //_Zest
    myBlock << 0, -C1( 0, 2 ), C1( 0, 1 ), -C1( 0, 2 ), -2 * C1( 1, 2 ), -C1( 2, 2 ) + C1( 1, 1 ),
        C1( 0, 1 ), -C1( 2, 2 ) + C1( 1, 1 ), 2 * C1( 1, 2 );
    _Zest.block< 3, 3 >( 0, 9 ) = myBlock;
    myBlock << 2 * C1( 0, 2 ), C1( 1, 2 ), -C1( 0, 0 ) + C1( 2, 2 ), C1( 1, 2 ), 0, -C1( 0, 1 ),
        -C1( 0, 0 ) + C1( 2, 2 ), -C1( 0, 1 ), -2 * C1( 0, 2 );
    _Zest.block< 3, 3 >( 0, 12 ) = myBlock;
    myBlock << -2 * C1( 0, 1 ), -C1( 1, 1 ) + C1( 0, 0 ), -C1( 1, 2 ), -C1( 1, 1 ) + C1( 0, 0 ),
        2 * C1( 0, 1 ), C1( 0, 2 ), -C1( 1, 2 ), C1( 0, 2 ), 0;
    ...
    if ( computeHessian ) {
        ...
        //_ZHest
        myBlock << 0, -C1( 0, 1 ), -C1( 0, 2 ), -C1( 0, 1 ), 2 * C1( 2, 2 ) - 2 * C1( 1, 1 ),
            -4 * C1( 1, 2 ), -C1( 0, 2 ), -4 * C1( 1, 2 ), 2 * C1( 1, 1 ) - 2 * C1( 2, 2 );
        _ZHest.block< 3, 3 >( 9, 9 ) = myBlock;

        myBlock << 0, C1( 0, 0 ) - C1( 2, 2 ), C1( 1, 2 ), C1( 0, 0 ) - C1( 2, 2 ), 2 * C1( 0, 1 ),
            2 * C1( 0, 2 ), C1( 1, 2 ), 2 * C1( 0, 2 ), -2 * C1( 0, 1 );
        _ZHest.block< 3, 3 >( 9, 12 ) = myBlock;

        myBlock << 0, C1( 1, 2 ), C1( 0, 0 ) - C1( 1, 1 ), C1( 1, 2 ), -2 * C1( 0, 2 ),
            2 * C1( 0, 1 ), C1( 0, 0 ) - C1( 1, 1 ), 2 * C1( 0, 1 ), 2 * C1( 0, 2 );
        _ZHest.block< 3, 3 >( 9, 15 ) = myBlock;

        myBlock << 2 * C1( 2, 2 ) - 2 * C1( 0, 0 ), -C1( 0, 1 ), -4 * C1( 0, 2 ), -C1( 0, 1 ), 0,
            -C1( 1, 2 ), -4 * C1( 0, 2 ), -C1( 1, 2 ), 2 * C1( 0, 0 ) - 2 * C1( 2, 2 );
        _ZHest.block< 3, 3 >( 12, 12 ) = myBlock;

        myBlock << -2 * C1( 1, 2 ), C1( 0, 2 ), 2 * C1( 0, 1 ), C1( 0, 2 ), 0,
            C1( 1, 1 ) - C1( 0, 0 ), 2 * C1( 0, 1 ), C1( 1, 1 ) - C1( 0, 0 ), 2 * C1( 1, 2 );
        _ZHest.block< 3, 3 >( 12, 15 ) = myBlock;

        myBlock << 2 * C1( 1, 1 ) - 2 * C1( 0, 0 ), -4 * C1( 0, 1 ), -C1( 0, 2 ), -4 * C1( 0, 1 ),
            2 * C1( 0, 0 ) - 2 * C1( 1, 1 ), -C1( 1, 2 ), -C1( 0, 2 ), -C1( 1, 2 ), 0;
...
void NDTMatcherD2D::computeDerivatives( Eigen::Vector3d& x, Eigen::Matrix3d C1,
                                        bool computeHessian ) {
    Jest( 0, 4 ) = x( 2 );
    Jest( 0, 5 ) = -x( 1 );
    Jest( 1, 3 ) = -x( 2 );
...
double NDTMatcherD2D::scoreNDT( std::vector< NDTCell* >& sourceNDT, NDTMap& targetNDT ) {
    ...
    for ( unsigned int i = 0; i < sourceNDT.size(); i++ ) {
        ...
        std::vector< NDTCell* > cells =
            targetNDT.getCellsForPoint( point, n_neighbours );   // targetNDT.getAllCells(); //
        for ( unsigned int j = 0; j < cells.size(); j++ ) {
            ...
            if ( cell->hasGaussian_ ) {
                ...
                if ( !exists ) { continue; }
                double l =
                    ( transformed ).dot( Cinv * ( transformed ) );   // + (eps).dot(Cinv*(eps));
                if ( l * 0 != 0 ) { continue; }
                // if(l > 120) continue;
...
double NDTMatcherD2D::scoreNDT_OM( NDTMap& sourceNDT, NDTMap& targetNDT ) {
    ...
    while ( it != lz->end() ) {
        ...
        // SWITCHME
        std::vector< NDTCell* > all_cells =
            targetNDT.getCellsForPoint( point, n_neighbours, false );
        ...
        for ( unsigned int j = 0; j < all_cells.size(); j++ ) {
            ...
            double cell_dist   = std::sqrt( pow( cen1.x - cen2.x, 2 ) + pow( cen1.y - cen2.y, 2 ) +
                                          pow( cen1.z - cen2.z, 2 ) );
            double lambda = 1 - cell_dist / ( cell->getDiagonal() );
            lambda        = lambda < 0 ? 0 : importance_free * lambda;
            // std::cout<<"lam "<<lambda<<" comp "<<-lambda*((1-o1)*(1-o2) - o1*(1-o2)
            // -o2*(1-o1))<<std::endl;
            ...
            // std::cout<<"j "<<j<<" sc "<<source->consistency_score<<" ll "<<ll<<" incr
            // "<<ll*((1-o1)*(1-o2) - o1*(1-o2) -o2*(1-o1))<<std::endl;
            source->consistency_score +=
                ll * ( ( 1 - o1 ) * ( 1 - o2 ) - o1 * ( 1 - o2 ) - o2 * ( 1 - o1 ) );
        }
        it++;
    }
    // cleanup
...

double NDTMatcherD2D::scoreNDTPositive(
    ...
    for ( unsigned int i = 0; i < sourceNDT.size(); i++ ) {
        ...
        if ( cell->hasGaussian_ ) {
            ...
            if ( fabsf( sh ) > 1e-10 ) { NUMBER_OF_ACTIVE_CELLS++; }
            score_here += ( 1.0 - sh );
...
double NDTMatcherD2D::derivativesNDT( const std::vector< NDTCell* >& sourceNDT,
...
#pragma omp for
        for ( unsigned int i = 0; i < sourceNDT.size(); i++ ) {
            ...
            meanMoving = sourceNDT[i]->getMean();
            CMoving    = sourceNDT[i]->getCov();
            computeDerivativesLocal( meanMoving, CMoving, _Jest, _Hest, _Zest, _ZHest,
                                     computeHessian );
            ...
            for ( unsigned int j = 0; j < cells.size(); j++ ) {
                ...
                if ( cell->hasGaussian_ ) {
                    ...
                    // if(l > 120) continue;
                    double sh = -lfd1 * ( exp( -lfd2 * l / 2 ) );
                    if ( !update_gradient_hessian_local( score_gradient_omp_loc, Hessian_omp_loc,
                                                         transformed, Cinv, sh, _Jest, _Hest, _Zest,
                                                         _ZHest, computeHessian ) ) {
                        continue;
                    }
                    score_here_loc += sh;
                    cell = NULL;
            ...
            Hessian_omp.block( 0, n_dimensions * thread_id, n_dimensions, n_dimensions ) +=
                Hessian_omp_loc;
    ...
    for ( unsigned int i = 0; i < sourceNDT.size(); i++ ) {
        ...
        for ( int j = 0; j < cells.size(); j++ ) {
            ...
            if ( cell->hasGaussian_ ) {
                ...
                // ["<<cell->getMean().transpose()<<"]';\n";
                // std::cout<<"C1 = ["<<CMoving<<"];\n C2 = ["<<CFixed<<"];\n";
                //		    if(!update_gradient_hessian(score_gradient, Hessian, transformed, Cinv,
                // sh, computeHessian))
                if ( !update_gradient_hessian( score_gradient, Hessian, transformed, Cinv, sh,
                                               computeHessian ) ) {
                    // delete cell;
                    continue;
                }
...
double NDTMatcherD2D::lineSearchMT( Eigen::Matrix< double, 6, 1 >& increment,
                                    std::vector< NDTCell* >& sourceNDT, NDTMap& targetNDT ) {
    // default params
    ...
    if ( dginit >= 0.0 ) {
        std::cout << "MoreThuente::cvsrch - wrong direction (dginit = " << dginit << ")"
                  << std::endl;
        ...
        if ( dginit >= 0.0 ) {
            for ( unsigned int i = 0; i < sourceNDTHere.size(); i++ ) {
                if ( sourceNDTHere[i] != NULL ) delete sourceNDTHere[i];
            }
            return recoverystep;
        }
    } else {
    ...
    while ( 1 ) {
        ...
        */
        ...
        if ( ( stp == stpmax ) && ( f <= ftest1 ) && ( dg <= dgtest ) ) info = 5;   // stp=stpmax

        if ( ( stp == stpmin ) && ( ( f > ftest1 ) || ( dg >= dgtest ) ) ) info = 4;   // stp=stpmin

        if ( nfev >= maxfev ) info = 3;   // max'd out on fevals

        if ( brackt && ( stmax - stmin <= xtol * stmax ) ) info = 2;   // bracketed soln
        ...
        if ( stage1 && ( f <= fx ) && ( f > ftest1 ) ) {
            ...
            infoc = MoreThuente::cstep( stx, fxm, dgxm, sty, fym, dgym, stp, fm, dgm, brackt, stmin,
                                        stmax );
            ...
            // VALGRIND_CHECK_VALUE_IS_DEFINED(dg);
            infoc =
                MoreThuente::cstep( stx, fx, dgx, sty, fy, dgy, stp, f, dg, brackt, stmin, stmax );
        }
...
int NDTMatcherD2D::MoreThuente::cstep( double& stx, double& fx, double& dx, double& sty, double& fy,
                                       double& dy, double& stp, double& fp, double& dp,
...
/*
void NDTMatcherD2D<PointSource,PointTarget>::generateScoreDebug(const char* out,
pcl::PointCloud<pcl::PointXYZ>& fixed, pcl::PointCloud<pcl::PointXYZ>& moving) {
...
bool NDTMatcherD2D::covariance( NDTMap& targetNDT, NDTMap& sourceNDT,
                                Eigen::Transform< double, 3, Eigen::Affine, Eigen::ColMajor >& T,
                                Eigen::MatrixXd& cov ) {
    ...
    std::vector< NDTCell* > targetNDTN = targetNDT.pseudoTransformNDT(
        T );   // WHY T? // HENRIK T-> TR    //    std::vector<NDTCell*> targetNDTN =
               // targetNDT.pseudoTransformNDT(TR); // WHY T? // HENRIK T-> TR.
    ...
    for ( int i = 0; i < sourceNDTN.size(); i++ ) {
        ...
        if ( !targetNDT.getCellForPoint( point, cell ) ) { continue; }
        if ( cell == NULL ) { continue; }
        if ( cell->hasGaussian_ ) {
            meanFixed   = cell->getMean();
            ...
            if ( factor < -120 ) { continue; }
            factor = exp( lfd2 * factor ) / 2;
            if ( factor > 1 || factor < 0 || factor * 0 != 0 ) { continue; }
            ...
            G.setZero();
            for ( int q = 3; q < 6; q++ ) {
                G( q ) = -transformed.transpose() * Q * Zest.block< 3, 3 >( 0, 3 * q ) * Cinv *
                         transformed;
                G( q ) = G( q ) -
                         transformed.transpose() * Cinv * Zest.block< 3, 3 >( 0, 3 * q ) * Q *
                             transformed;
            }
            ...
            for ( int j = 0; j < targetNDTN.size(); j++ ) {
                if ( targetNDTN[j]->getMean() == meanFixed ) {
                    Jdpdz.row( j + sourceNDTN.size() ) =
                        Jdpdz.row( j + sourceNDTN.size() ) + G.transpose();
                    continue;
                }
            }
    ...
    std::cout << "cov 2 : " << cov.inverse() * fabsf( scoreNDT( sourceNDTN, targetNDT ) * 2 / 3 )
              << std::endl;
    cov = cov.inverse();
    ...
    for ( unsigned int q = 0; q < sourceNDTN.size(); q++ ) { delete sourceNDTN[q]; }
    sourceNDTN.clear();
    for ( unsigned int q = 0; q < targetNDTN.size(); q++ ) { delete targetNDTN[q]; }
    targetNDTN.clear();

    if ( cov.allFinite() ) return true;
    return false;
}
