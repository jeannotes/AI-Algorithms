...
void NDTMatcherD2D_2D::init( bool _isIrregularGrid, bool useDefaultGridResolutions,
    ...
    current_resolution = 0.1;   // Argggg!!! This is very important to have initiated! (one day
                                // debugging later) :-) TODO do we need to set it to anything
                                // better?
...
bool NDTMatcherD2D_2D::match( pcl::PointCloud< pcl::PointXYZ >& target,
    ...
    std::cout << "load: " << time_load << " match " << time_match << " combined " << time_combined
              << std::endl;
...
bool NDTMatcherD2D_2D::match( NDTMap& targetNDT, NDTMap& sourceNDT,
    ...
    // Eigen::MatrixXd Hessian(6,6), score_gradient(6,1), H(3,3); //column vectors,
    // pose_increment_v(6,1)
    ...
    while ( !convergence ) {
        ...
        if ( score_here < score_best ) {
            ...
            // temporarily commented  std::cout<<"best score "<<score_best<<" at
            // "<<itr_ctr<<std::endl;
        }
        ...
        // tillfälligt bortkommenterad   std::cout<<"Hh(:,:,"<<itr_ctr+1<<")  =  ["<<
        // H<<"];\n"<<std::endl;				  //
        // tillfälligt bortkommenterad    std::cout<<"gradh (:,"<<itr_ctr+1<<")=
        // ["<<scg.transpose()<<"];"<<std::endl;         //
        ...
        */
        ...
        if ( itr_ctr > 0 ) { convergence = ( ( pose_increment_v.norm() ) < DELTA_SCORE ); }
        if ( itr_ctr > ITR_MAX ) {
            convergence = true;
            ret         = false;
        }
...
bool NDTMatcherD2D_2D::update_gradient_hessian_local_2d(
    ...
    const Eigen::Matrix3d& B, const double& likelihood, const Eigen::Matrix< double, 3, 3 >& _Jest,
    const Eigen::Matrix< double, 9, 3 >& _Hest, const Eigen::Matrix< double, 3, 9 >& _Zest,
...
bool NDTMatcherD2D_2D::update_gradient_hessian_2d( Eigen::MatrixXd& score_gradient,
                                                   Eigen::MatrixXd& Hessian,
                                                   const Eigen::Vector3d& x,
                                                   const Eigen::Matrix3d& B,
                                                   const double& likelihood, bool computeHessian ) {
    xtBJ.setZero();
    xtBZBx.setZero();
    Q.setZero();
    JtBJ.setZero();
    ...
    if ( computeHessian ) {
        Hessian += factor *
                   ( 2 * Jest.transpose() * B * Jest + 2 * xtBH - xtBZhBx - 2 * xtBZBJ.transpose() -
...
void NDTMatcherD2D_2D::computeDerivativesLocal_2d( Eigen::Vector3d& x, Eigen::Matrix3d C1,
    ...
    //_Zest
    _Zest.block< 3, 3 >( 0, 6 ) << -2 * C1( 0, 1 ), -C1( 1, 1 ) + C1( 0, 0 ), -C1( 1, 2 ),
        -C1( 1, 1 ) + C1( 0, 0 ), 2 * C1( 0, 1 ), C1( 0, 2 ), -C1( 1, 2 ), C1( 0, 2 ), 0;

    if ( computeHessian ) {
        _Hest.block< 3, 1 >( 6, 2 ) << -x( 0 ), -x( 1 ), 0;
        _ZHest.block< 3, 3 >( 6, 6 ) << 2 * C1( 1, 1 ) - 2 * C1( 0, 0 ), -4 * C1( 0, 1 ),
            -C1( 0, 2 ), -4 * C1( 0, 1 ), 2 * C1( 0, 0 ) - 2 * C1( 1, 1 ), -C1( 1, 2 ), -C1( 0, 2 ),
            -C1( 1, 2 ), 0;
    }
}

void NDTMatcherD2D_2D::computeDerivatives_2d( Eigen::Vector3d& x, Eigen::Matrix3d C1,
                                              bool computeHessian ) {
    Jest( 0, 2 ) = -x( 1 );
    Jest( 1, 2 ) = x( 0 );
    ...
    Eigen::Matrix3d myBlock;
    //_Zest
    Zest.block< 3, 3 >( 0, 6 ) << -2 * C1( 0, 1 ), -C1( 1, 1 ) + C1( 0, 0 ), -C1( 1, 2 ),
        -C1( 1, 1 ) + C1( 0, 0 ), 2 * C1( 0, 1 ), C1( 0, 2 ), -C1( 1, 2 ), C1( 0, 2 ), 0;

    if ( computeHessian ) {
        Hest.block< 3, 1 >( 6, 2 ) << -x( 0 ), -x( 1 ), 0;
        ZHest.block< 3, 3 >( 6, 6 ) << 2 * C1( 1, 1 ) - 2 * C1( 0, 0 ), -4 * C1( 0, 1 ),
            -C1( 0, 2 ), -4 * C1( 0, 1 ), 2 * C1( 0, 0 ) - 2 * C1( 1, 1 ), -C1( 1, 2 ), -C1( 0, 2 ),
            -C1( 1, 2 ), 0;
    }
}
...
double NDTMatcherD2D_2D::derivativesNDT_2d( const std::vector< NDTCell* >& sourceNDT,
...
#pragma omp for
        for ( unsigned int i = 0; i < sourceNDT.size(); i++ ) {
            ...
            computeDerivativesLocal_2d( meanMoving, CMoving, _Jest, _Hest, _Zest, _ZHest,
                                        computeHessian );
            ...
            for ( unsigned int j = 0; j < cells.size(); j++ ) {
                ...
                if ( cell->hasGaussian_ ) {
                    ...
                    if ( !update_gradient_hessian_local_2d( score_gradient_omp_loc, Hessian_omp_loc,
                                                            transformed, Cinv, sh, _Jest, _Hest,
                                                            _Zest, _ZHest, computeHessian ) ) {
                        continue;
                    }
                    score_here_loc += sh;
                    cell = NULL;
    ...
    for ( unsigned int i = 0; i < sourceNDT.size(); i++ ) {
        ...
        for ( int j = 0; j < cells.size(); j++ ) {
            ...
            if ( cell->hasGaussian_ ) {
                ...
                // sh, computeHessian))
                if ( !update_gradient_hessian_2d( score_gradient, Hessian, transformed, Cinv, sh,
                                                  computeHessian ) ) {
                    // delete cell;
                    continue;
                }
...
int NDTMatcherD2D_2D::MoreThuente::cstep( double& stx, double& fx, double& dx, double& sty,
                                          double& fy, double& dy, double& stp, double& fp,
                                          double& dp, bool& brackt, double stmin, double stmax ) {
    int info = 0;
