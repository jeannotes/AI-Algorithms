...
* camera postion. It also log the position and orientation of each pose along with a time stamp a
* file
* \author Malcolm
*/
class NDTFuserHMTLogger : public perception_oru::NDTFuserHMT {
  ...
  public:
    NDTFuserHMTLogger( const std::string& file_out_logger, double map_resolution,
                       double map_size_x_, double map_size_y_, double map_size_z_,
                       ...
                       std::string prefix_ = "", bool beHMT_ = true,
                       std::string hmt_map_dir_ = "map", bool _step_control = true,
                       bool doSoftConstraints_ = false, int nb_neighbours = 2,
                       double resolutionLocalFactor = 1. )
        : NDTFuserHMT( map_resolution, map_size_x_, map_size_y_, map_size_z_, sensor_range_,
                       visualize_, be2D_, doMultires_, fuseIncomplete_, max_itr, prefix_, beHMT_,
                       hmt_map_dir_, _step_control, doSoftConstraints_, nb_neighbours,
                       resolutionLocalFactor ),
          _file_out_logger( file_out_logger ){};
  ...
  private:
    ...
    double getRobustYawFromAffine3d( const Eigen::Affine3d& a ) {
        // To simply get the yaw from the euler angles is super sensitive to numerical errors which
        // will cause roll and pitch to have angles very close to PI...
