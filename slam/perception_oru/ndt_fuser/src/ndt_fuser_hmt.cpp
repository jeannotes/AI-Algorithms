...
{
void NDTFuserHMT::initialize( Eigen::Affine3d initPos, pcl::PointCloud< pcl::PointXYZ >& cloud,
                              bool preLoad ) {
    ...
    //#else
    if ( beHMT ) {
        map = new perception_oru::NDTMapHMT(
            resolution, Tnow.translation()( 0 ), Tnow.translation()( 1 ), Tnow.translation()( 2 ),
        ...
        if ( preLoad ) {
            ...
            map->initialize( Tnow.translation()( 0 ), Tnow.translation()( 1 ),
                             Tnow.translation()( 2 ), map_size_x, map_size_y, map_size_z );
        }
    }
    //#endif
    map->addPointCloud( Tnow.translation(), cloud, 0.1, 100.0, 0.1 );
    // map->addPointCloudMeanUpdate(Tnow.translation(),cloud,localMapSize, 1e5, 1250, map_size_z/2,
    // 0.06);
    ...
    if ( visualize ) {
        ...
        viewer->addTrajectoryPoint( Tnow.translation()( 0 ), Tnow.translation()( 1 ),
                                    Tnow.translation()( 2 ) + 1, 1, 0, 0 );
        viewer->addTrajectoryPoint( Todom.translation()( 0 ), Todom.translation()( 1 ),
                                    Todom.translation()( 2 ) + 0.5, 0, 1, 0 );
        viewer->displayTrajectory();
        viewer->setCameraPointing( Tnow.translation()( 0 ), Tnow.translation()( 1 ),
                                   Tnow.translation()( 2 ) + 3 );
        viewer->repaint();
#endif
    }
}
 ...
 *
 */
Eigen::Affine3d NDTFuserHMT::update( Eigen::Affine3d Tmotion,
    ...
    t0 = getDoubleTime();
    /// Create local map
    perception_oru::NDTMap ndlocal(
        new perception_oru::LazyGrid( resolution * resolution_local_factor ) );
    ...
    if ( disableRegistration ) {
        ...
        map->addPointCloudMeanUpdate( spose.translation(), cloud, localMapSize, 1e5, 25,
                                      2 * map_size_z, 0.06 );
        if ( visualize ) {   //&&ctr%20==0)
            ...
            viewer->addTrajectoryPoint( Tnow.translation()( 0 ), Tnow.translation()( 1 ),
                                        Tnow.translation()( 2 ) + 0.2, 0, 1, 0 );
            viewer->addTrajectoryPoint( Todom.translation()( 0 ), Todom.translation()( 1 ),
                                        Todom.translation()( 2 ) + 0.2, 0.5, 0, 0.5 );
            viewer->displayTrajectory();
            viewer->setCameraPointing( Tnow.translation()( 0 ), Tnow.translation()( 1 ),
                                       Tnow.translation()( 2 ) + 3 );
            viewer->repaint();
    ...
    if ( doMultires ) {
        ...
        // add distros
        double cx, cy, cz;
        if ( !map->getCentroid( cx, cy, cz ) ) { fprintf( stderr, "Centroid NOT Given-abort!\n" ); }
        ...
        if ( matcher2D.match( *map, ndlocal, Tinit, true ) || fuseIncomplete ) {
            ...
            if ( ( diff.translation().norm() > max_translation_norm ||
                ...
                if ( diff_fuse.translation().norm() > translation_fuse_delta ||
                    ...
                    // std::cout<<"F: "<<spose.translation().transpose()<<"
                    // "<<spose.rotation().eulerAngles(0,1,2).transpose()<<std::endl;
                    t4 = getDoubleTime();
                    // TSV: originally this!
                    // map->addPointCloudMeanUpdate(spose.translation(),cloud,localMapSize, 1e5,
                    // 1250, map_size_z/2, 0.06);
                    map->addPointCloudMeanUpdate( spose.translation(), cloud, localMapSize, 1e5, 5,
                                                  2 * map_size_z, 0.06 );
                    ...
                    // map->computeNDTCells(CELL_UPDATE_MODE_SAMPLE_VARIANCE, 1e5, 255,
                    // spose.translation(), 0.1);
                    // t4 = getDoubleTime();
                    // std::cout<<"match: "<<t3-t2<<" addPointCloud: "<<t5-t4<<" ndlocal "<<t1-t0<<"
                    // total: "<<t5-t0<<std::endl;
                    Tlast_fuse = Tnow;
                    if ( visualize ) {   //&&ctr%20==0)
#ifndef NO_NDT_VIZ
                        if ( ctr % 30 == 0 ) {
                        ...
                        viewer->addTrajectoryPoint( Tnow.translation()( 0 ),
                                                    Tnow.translation()( 1 ),
                                                    Tnow.translation()( 2 ) + 0.2, 0, 1, 0 );
                        viewer->addTrajectoryPoint( Todom.translation()( 0 ),
                                                    Todom.translation()( 1 ),
                                                    Todom.translation()( 2 ) + 0.2, 0.5, 0, 0.5 );
                        viewer->displayTrajectory();
                        viewer->setCameraPointing( Tnow.translation()( 0 ), Tnow.translation()( 1 ),
...
// viewer->win3D->process_events();
        ...
        if ( doSoftConstraints ) {
            ...
            Eigen::MatrixXd local_cov = motionModel2D.getCovMatrix6(
                Tmotion, 1., 1., 1. );   // 0.0000000000001, 0.0000000000001, 0.0000000000001);

            Eigen::MatrixXd local_cov_pos = local_cov.block( 0, 0, 3, 3 );

            // Convert it into the clobal frame.
            Eigen::MatrixXd global_cov_pos =
                Tinit.rotation() * local_cov_pos * Tinit.rotation().transpose();
            if ( visualize ) {
#ifndef NO_NDT_VIZ
            ...
            // // Force initialization to be only yaw. Set roll, pitch and height to zero + put in a
            // covariance with relative low variance in height, roll and pitch...
            ...
            std::cout << matcherSC.nb_match_calls << " successes : " << matcherSC.nb_success_reg
                      << std::endl;
        } else {
            ...
            if ( ( diff.translation().norm() > max_translation_norm ||
                ...
                if ( diff_fuse.translation().norm() > translation_fuse_delta ||
                    ...
                    if ( visualize ) {   //&&ctr%20==0)
                        ...
                        viewer->addTrajectoryPoint( Tnow.translation()( 0 ),
                                                    Tnow.translation()( 1 ),
                                                    Tnow.translation()( 2 ) + 2.2, 0, 1, 0 );
                        viewer->addTrajectoryPoint( Todom.translation()( 0 ),
                                                    Todom.translation()( 1 ),
                                                    Todom.translation()( 2 ) + 2.2, 0.5, 0, 0.5 );
                        viewer->displayTrajectory();
                        viewer->setCameraPointing( Tnow.translation()( 0 ), Tnow.translation()( 1 ),
                                                   Tnow.translation()( 2 ) + 3 );
                        viewer->repaint();
