...
void NDTFuser::initialize( Eigen::Affine3d initPos, pcl::PointCloud< pcl::PointXYZ >& cloud ) {
    ...
    Tnow = initPos;
    map->initialize( Tnow.translation()( 0 ), Tnow.translation()( 1 ), Tnow.translation()( 2 ),
...
Eigen::Affine3d NDTFuser::update( Eigen::Affine3d Tmotion,
...
// viewer->plotLocalNDTMap(cloud,resolution);
            ...
            /*
               std::cout<<"load: "<<t2-t1<<" match: "<<t3-t2<<" fuse: "<<t4-t3<<" total:
               "<<t4-t1<<std::endl;
        ...
        if ( matcher.match( *map, ndlocal, Tinit, true ) ) {
            ...
            if ( ( diff.translation().norm() > max_translation_norm ||
                ...
                if ( diff_fuse.translation().norm() > translation_fuse_delta ||
                     diff_fuse.rotation().eulerAngles( 0, 1, 2 ).norm() >
                         2 * rotation_fuse_delta ) {
                    t3 = getDoubleTime();
                    map->addPointCloud( spose.translation(), cloud, 0.06, 2.5 );
                    t4 = getDoubleTime();
                    map->computeNDTCells( CELL_UPDATE_MODE_SAMPLE_VARIANCE, 1e5, 255,
...
// viewer->plotLocalNDTMap(cloud,resolution);
            ...
            std::cout << "load: " << t2 - t1 << " match: " << t3 - t2 << " fuse: " << t4 - t3
                      << " total: " << t4 - t1 << std::endl;
            /*FILE *ftmp = fopen("tmp.txt","a");
              fprintf(ftmp,"%lf, ",t4-t3);
              fclose(ftmp);*/
        } else {
