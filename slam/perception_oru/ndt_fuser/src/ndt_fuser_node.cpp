...
class view_bag {
  ...
  public:
    ...
    view_bag( ros::NodeHandle param_nh ) : nb_added_clouds_( 0 ) {
        ...
        fuser = new perception_oru::NDTFuserHMT( resolution, size_x, size_y, size_z, sensor_range,
                                                 visualize, match2D, false, false, 30, map_name,
                                                 beHMT, map_dir, true, do_soft_constraints );

        fuser->setMotionParams( motion_params );
        fuser->setSensorPose( sensor_pose_ );

        if ( !matchLaser ) {
            points2_sub_ = new message_filters::Subscriber< sensor_msgs::PointCloud2 >(
                nh_, points_topic, 20 );
            if ( useOdometry ) {
                odom_sub_ = new message_filters::Subscriber< nav_msgs::Odometry >(
                    nh_, odometry_topic, 200 );
                ...
                sync_po_->registerCallback(
                    boost::bind( &view_bag::points2OdomCallback, this, _1, _2 ) );
            } else {
                points2_sub_->registerCallback(
                    boost::bind( &view_bag::points2Callback, this, _1 ) );
            }
        } else {
            laser_sub_ =
            ...
            if ( useOdometry && !renderGTmap ) {
                ...
                sync_lo_->registerCallback(
                    boost::bind( &view_bag::laserOdomCallback, this, _1, _2 ) );
            } else if ( !renderGTmap ) {
        ...
        if ( plotGTTrack ) {
            gt_sub =
                nh_.subscribe< nav_msgs::Odometry >( gt_topic, 10, &view_bag::gt_callback, this );
        }

        initPoseSet = false;
    }

    ~view_bag() { delete fuser; }

    void processFrame( pcl::PointCloud< pcl::PointXYZ >& cloud_in, Eigen::Affine3d Tmotion ) {
        m.lock();
        ...
        if ( nb_added_clouds_ == 0 ) {
            ...
            if ( initPoseFromGT ) {
                ...
                ROS_INFO( "Init pose is (%lf,%lf,%lf) form tf", pose_.translation()( 0 ),
                          pose_.translation()( 1 ), pose_.rotation().eulerAngles( 0, 1, 2 )( 0 ) );

                perception_oru::ndt_fuser::initSensorPose( *fuser, sensor_frame,
                ...
                ROS_INFO( "Init pose from launch file is (%lf,%lf,%lf)", pose_.translation()( 0 ),
                          pose_.translation()( 1 ), pose_.rotation().eulerAngles( 0, 1, 2 )( 0 ) );
                fuser->initialize( pose_, cloud );
            }
            nb_added_clouds_++;
