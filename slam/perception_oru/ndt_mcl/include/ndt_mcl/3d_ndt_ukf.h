...
class UKF3D {
  public:
    ...
    UKF3D() : N_( 6 ) { allocateSigmas(); }

    void setParams( const UKF3D::Params& params ) { params_ = params; }
    ...
    int getNbSigmaPoints() const { return 2 * N_ + 1; }

    std::vector< double > getMeanWeights() const;
    std::vector< double > getCovWeights() const;

    Eigen::Affine3d getMean() const { return T_; }

    Eigen::MatrixXd getCov() const { return P_; }
    ...
    double getLambda() const { return params_.alpha * params_.alpha * ( N_ + params_.kappa ) - N_; }

    double getXsi() const { return sqrt( N_ + getLambda() ); }

    const Eigen::VectorXd& getSigma( int i ) const { return sigmas_[i]; }
...
class NDTUKF3D {
  public:
    ...
    void computeMeasurements( pcl::PointCloud< pcl::PointXYZ >& cloud,
                              const Eigen::Affine3d& Tsensor, Eigen::MatrixXd& meassurements,
                              Eigen::VectorXd& raw_ranges );
    ...
    //    void updateAndPredictEff(Eigen::Affine3d Tmotion, pcl::PointCloud<pcl::PointXYZ> &cloud,
    //    double subsample_level);

    Eigen::Affine3d getMean() const { return ukf_.getMean(); }

    void initializeFilter( const Eigen::Affine3d& T, double varx, double vary, double varz,
                           double varR, double varP, double varY ) {
    ...
    void setParamsUKF( const UKF3D::Params& params ) { ukf_.setParams( params ); }
    ...
    const pcl::PointCloud< pcl::PointXYZ >& getFilterRaw() const { return pc_filtered_raw_; }

    const pcl::PointCloud< pcl::PointXYZ >& getFilterPred() const { return pc_filtered_pred_; }
