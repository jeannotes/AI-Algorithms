...
std::vector< double > UKF3D::getMeanWeights() const {
    ...
    for ( int i = 0; i < this->getNbSigmaPoints(); i++ ) { ret.push_back( getWeightMean( i ) ); }
    return ret;
}
...
std::vector< double > UKF3D::getCovWeights() const {
    ...
    for ( int i = 0; i < this->getNbSigmaPoints(); i++ ) { ret.push_back( getWeightCov( i ) ); }
    return ret;
}

double UKF3D::getWeightMean( int idx ) const {
    const double lambda = this->getLambda();
    if ( idx == 0 ) { return ( lambda / ( N_ + lambda ) ); }
...
void UKF3D::update( const Eigen::MatrixXd& pred_ranges, const Eigen::VectorXd& raw_ranges ) {
    ...
    Eigen::MatrixXd K_t =
        J_t.transpose() *
        S.colPivHouseholderQr().solve( Eigen::MatrixXd::Identity( S.rows(), S.cols() ) );
...
void UKF3D::updateSeq( const Eigen::MatrixXd& pred_ranges, const Eigen::VectorXd& raw_ranges ) {
    ...
    while ( i < raw_ranges.size() && counter < params_.nb_updates ) {
        ...
        if ( i + nb_ranges >= raw_ranges.size() ) { nb_ranges = raw_ranges.size() - i - 1; }
...
void UKF3D::assignMinDiagVariances() {
    ...
    for ( int i = 0; i < N_; i++ ) {
        ...
        if ( P_( i, i ) < min_var ) { P_( i, i ) = min_var; }
    }
}
...
std::vector< int > UKF3D::filterMeasurements( Eigen::MatrixXd& pred_ranges,
    ...
    for ( int i = 0; i < raw_ranges.size(); i++ ) {
        ...
        for ( int j = 0; j < this->getNbSigmaPoints(); j++ ) {
            ...
            if ( diff > params_.range_filter_max_dist ) { use = false; }
        }
        if ( use ) { idx.push_back( i ); }
    }
...
void NDTUKF3D::predict( Eigen::Affine3d Tmotion ) {
    ...
    for ( int i = 0; i < 6; i++ ) { Q( i, i ) = m( i ) + motion_model_offset[i]; }
...
void NDTUKF3D::updateAndPredict( Eigen::Affine3d Tmotion, pcl::PointCloud< pcl::PointXYZ >& cloud,
                                 const Eigen::Affine3d& Tsensor ) {
...
void NDTUKF3D::computeMeasurements( pcl::PointCloud< pcl::PointXYZ >& cloud,
                                    const Eigen::Affine3d& Tsensor, Eigen::MatrixXd& measurements,
                                    Eigen::VectorXd& raw_ranges ) {
    std::vector< Eigen::Vector3d > dirs;
    std::vector< double > r;
    ndt_generic::computeDirectionsAndRangesFromPointCloud( cloud, Tsensor.translation(), dirs, r );
    ...
    for ( int i = 0; i < sigmas.size(); i++ ) {
        ...
        for ( int j = 0; j < d.size(); j++ ) { d[j] = rot * dirs[j]; }
