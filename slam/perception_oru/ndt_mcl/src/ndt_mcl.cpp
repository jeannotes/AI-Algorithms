...
void NDTMCL::updateAndPredict( Eigen::Affine3d Tmotion, pcl::PointCloud< pcl::PointXYZ >& cloud ) {
    ...
    /** Arla Motion model
      ...
      pf.predict(mcl::pose(tr[0],tr[1],rot[2]), mcl::pose(tr[0]*0.2 + 0.005,tr[1]*0.1+
     0.005,rot[2]*0.2+0.001));
      }else if(tr[0]>=0){
      pf.predict(mcl::pose(tr[0],tr[1],rot[2]), mcl::pose(tr[0] + 0.01,tr[1] +
     0.01,rot[2]*0.5+0.001));
      }else{
      pf.predict(mcl::pose(tr[0],tr[1],rot[2]), mcl::pose(tr[0] + 0.02,tr[1] +
     0.01,rot[2]*0.8+0.001));
      }
     **/
    pf.predict( mcl::pose( tr[0], tr[1], rot[2] ),
                mcl::pose( tr[0] * 0.1 + 0.005, tr[1] * 0.1 + 0.005, rot[2] * 0.1 + 0.001 ) );
    ...
    if ( forceSIR ) {
        ...
        double varP = 0;
        for ( int i = 0; i < pf.NumOfParticles; i++ ) {
            varP += ( pf.Particles[i].p - 1.0 / pf.NumOfParticles ) *
                    ( pf.Particles[i].p - 1.0 / pf.NumOfParticles );
        }
