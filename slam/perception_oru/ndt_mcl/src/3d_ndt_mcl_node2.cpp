...
*
* The initialization is based now on ground truth pose information (should be replaced with manual
* input).
*
* The visualization depends on mrpt-gui
*
* More details about the algorithm:
* Jari Saarinen, Henrik Andreasson, Todor Stoyanov and Achim J. Lilienthal, Normal Distributions
* Transform Monte-Carlo Localization (NDT-MCL)
* IEEE/RSJ International Conference on Intelligent Robots and Systems November 3-8, 2013, Tokyo Big
* Sight, Japan
*
...
* Known issues: in launch file (or hard coded parameters) you have to set the same resolution for
* NDT map as is saved -- otherwise it wont work
*/
...
typedef message_filters::sync_policies::ApproximateTime< sensor_msgs::PointCloud2,
                                                         nav_msgs::Odometry >
    PointsOdomSync;
// TODO typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::PointCloud2,
// geometry_msgs::PoseStamped> PointsPoseSync;
...
class NDTMCL3DNode {
  private:
    ...
    std::string tf_base_link, tf_sensor_link, tf_gt_link, points_topic, odometry_topic,
        odometry_frame, scan_topic, scan2_topic;
  ...
  public:
    NDTMCL3DNode( ros::NodeHandle param_nh )
        : data_( new velodyne_rawdata::RawData() ), data2_( new velodyne_rawdata::RawData() ) {
        //////////////////////////////////////////////////////////
        ...
        //////////////////////////////////////////////////////////
        {
            ...
            param_nh.param< int >( "cloud_min_size", cloud_min_size_,
                                   2000 );   // minimum amount of points (to avoid some problems
                                             // with lost packages...).
        }
        ...
        double pose_init_x, pose_init_y, pose_init_z, pose_init_r, pose_init_p, pose_init_t;
        double sensor_pose_x, sensor_pose_y, sensor_pose_z, sensor_pose_r, sensor_pose_p,
            sensor_pose_t;
        ...
        if ( use_sensor_pose ) {
            ...
            hasSensorPose = true;
            sensorPoseT =
                Eigen::Translation< double, 3 >( sensor_pose_x, sensor_pose_y, sensor_pose_z ) *
        ...
        param_nh.param< std::string >( "output_map_file_name", output_map_name,
                                       std::string( "ndt_mapper_output.ndmap" ) );
        ...
        ROS_INFO_STREAM( "Loaded map: " << mapName << " containing " << ndmap.getAllCells().size()
                                        << " cells" );
        ...
        param_nh.param< int >( "SIR_max_iters_wo_resampling", ndtmcl->SIR_max_iters_wo_resampling,
                               25 );
        ...
        param_nh.param< std::string >( "tf_laser_link", tf_sensor_link,
                                       std::string( "/velodyne_link" ) );
        ...
        if ( scan_topic != std::string( "" ) ) {
            ROS_INFO_STREAM(
                "A scan topic is specified [will use that along with tf] : " << scan_topic );
            scan_sub_ = nh_.subscribe< velodyne_msgs::VelodyneScan >(
                scan_topic, 10, &NDTMCL3DNode::scan_callback, this );
            if ( scan2_topic != std::string( "" ) ) {
                scan2_sub_ = nh_.subscribe< velodyne_msgs::VelodyneScan >(
                    scan2_topic, 10, &NDTMCL3DNode::scan2_callback, this );
                ROS_INFO_STREAM(
                    "A scan2 topic is specified [will use that along with tf] : " << scan_topic );
            }
        } else {
            points2_sub_ =
                new message_filters::Subscriber< sensor_msgs::PointCloud2 >( nh_, points_topic, 1 );
            odom_sub_ =
        ...
        if ( do_visualize ) { ndt_viz.win3D->start_main_loop_own_thread(); }
        ...
        heartbeat_slow_visualization_ = nh_.createTimer(
            ros::Duration( 1.0 ), &NDTMCL3DNode::publish_visualization_slow, this );
        heartbeat_fast_visualization_ = nh_.createTimer(
            ros::Duration( 0.1 ), &NDTMCL3DNode::publish_visualization_fast, this );
        ...
        if ( est_filename != std::string( "" ) ) { est_file_.open( est_filename.c_str() ); }
        if ( est2d_filename != std::string( "" ) ) { est2d_file_.open( est2d_filename.c_str() ); }

        if ( !gt_file_.is_open() || !gt2d_file_.is_open() || !est_file_.is_open() ||
             !est2d_file_.is_open() ) {
            //               ROS_ERROR_STREAM("Failed to open : " << est_file_ << " | " <<
            //               est2d_file_ << " | " << gt_file_);
        }

        if ( gt_topic != std::string( "" ) ) {
            ROS_INFO_STREAM( "Subscribing to : " << gt_topic );
            gt_sub = nh_.subscribe< nav_msgs::Odometry >( gt_topic, 10, &NDTMCL3DNode::gt_callback,
                                                          this );
        }
    }
    ~NDTMCL3DNode() {
        if ( gt_file_.is_open() ) gt_file_.close();
        if ( gt2d_file_.is_open() ) gt2d_file_.close();
        if ( est_file_.is_open() ) est_file_.close();
        if ( est2d_file_.is_open() ) est2d_file_.close();
    ...
    void publish_visualization_fast( const ros::TimerEvent& event ) {
        if ( do_pub_particles_markers_ ) {
            mcl_m.lock();
            marker_pub_.publish( ndt_visualisation::markerParticlesNDTMCL3D( *ndtmcl, -1, "particle"
                                                                                          "s" ) );
            mcl_m.unlock();
        }
    }
    ...
    void publish_visualization_slow( const ros::TimerEvent& event ) {
        if ( do_pub_ndt_markers_ ) {
            ...
            //                                    graph->getT(), 1, "nd_global_map_last",
            //                                    markers_ndt);
    ...
    void visualize() {
        ...
        if ( do_visualize ) {
            ...
            ndt_viz.addTrajectoryPoint( mean.translation()[0], mean.translation()[1],
                                        mean.translation()[2] + 0.5, 1.0, 0, 0 );
            ...
            ndt_viz.win3D->setCameraPointingToPoint( mean.translation()[0], mean.translation()[1],
                                                     3.0 );
            ndt_viz.repaint();
            //}
        }
    ...
    void initialize() {
        ...
        ndtmcl->initializeFilter( tr[0], tr[1], tr[2], rot[0], rot[1], rot[2], 0.5, 0.5, 0.1,
                                  2.0 * M_PI / 180.0, 2.0 * M_PI / 180.0, 2.0 * M_PI / 180.0,
                                  this->numParticles );
        //        ndtmcl->initializeFilter(tr[0], tr[1],tr[2],rot[0],rot[1],rot[2],0.005, 0.005,
        //        0.001, 0.02*M_PI/180.0, 0.02*M_PI/180.0 ,0.02*M_PI/180.0, this->numParticles);
        ndt_viz.plotNDTSAccordingToOccupancy( -1, &ndtmcl->map );
        isFirstLoad = false;
    }

    // Simply to get an easier interface to tf.
    bool getTransformationForTime( const ros::Time& t0, const std::string& frame_id,
                                   tf::Transform& T ) {
        bool success = false;
        try {
            success =
                tf_listener.waitForTransform( odometry_frame, frame_id, t0, ros::Duration( 2.0 ) );
    ...
    // Simply to get an easier interface to tf.
    bool getRelativeTransformationForTime( const ros::Time& t0, const ros::Time& t1,
                                           const std::string& frame_id, tf::Transform& T ) {
        bool success = false;
        try {
            success = tf_listener.waitForTransform( frame_id, t0, frame_id, t1, odometry_frame,
                                                    ros::Duration( 2.0 ) );
        } catch ( tf::TransformException& ex ) {
            ROS_WARN_THROTTLE( 100, "%s", ex.what() );
            return false;
    ...
    bool getTransformationAffine3d( const ros::Time& t0, const std::string& frame_id,
                                    Eigen::Affine3d& T ) {
        tf::Transform tf_T;
    ...
    // Access the scans directly. This to better handle the time offsets / perform interpolation.
    ...
    void scan_callback( const velodyne_msgs::VelodyneScan::ConstPtr& scanMsg ) {
        ...
        // If we are using the dual scan setup then we need to have the timestamp of the previous
        // one in order to get syncronized pointclouds
        if ( use_dual_scan ) {
        ...
        // Get the current odometry position (need to check the incremental odometry meassure since
        // last call).
        {
            tf::Transform T;
        ...
        if ( Tm.translation().norm() < 0.01 &&
             fabs( Tm.rotation().eulerAngles( 0, 1, 2 )[2] ) < ( 0.5 * M_PI / 180.0 ) ) {
            ROS_INFO_STREAM( "Not enough distance / rot traversed : "
                             << Tm.translation().norm() << " / "
                             << fabs( Tm.rotation().eulerAngles( 0, 1, 2 )[2] ) );
            mcl_m.unlock();
            return;
        }

        // We're interessted in getting the pointcloud in the vehicle frame (that is to transform it
        // using the sensor pose offset).
        ...
        for ( size_t next = 0; next < scanMsg->packets.size(); ++next ) {
            ...
            tf::Transform T;
            if ( !getRelativeTransformationForTime( t0, t1, tf_base_link, T ) ) { continue; }
...
// Visualization...
    ...
    void scan2_callback( const velodyne_msgs::VelodyneScan::ConstPtr& scanMsg ) {
        ...
        for ( size_t next = 0; next < scanMsg->packets.size(); ++next ) {
            ...
            pcl_ros::transformPointCloud( pnts, conv_points, Tcloud );
            for ( size_t i = 0; i < conv_points.size(); i++ ) {
                cloud.push_back( pcl::PointXYZ( conv_points.points[i].x, conv_points.points[i].y,
                                                conv_points.points[i].z ) );
            }
            pnts.clear();
            conv_points.clear();
    ...
    void callback( const sensor_msgs::PointCloud2::ConstPtr& cloud_in,
        ...
        Todo = Eigen::Translation3d( odo_in->pose.pose.position.x, odo_in->pose.pose.position.y,
                                     odo_in->pose.pose.position.z ) *
               qd;
    ...
    bool sendROSOdoMessage( Eigen::Affine3d mean, ros::Time ts ) {
        ...
        transform.setOrigin(
            tf::Vector3( mean.translation()[0], mean.translation()[1], mean.translation()[2] ) );
    ...
    void gt_callback( const nav_msgs::Odometry::ConstPtr& msg_in ) {
        ...
        gt_pose = Eigen::Translation3d( msg_in->pose.pose.position.x, msg_in->pose.pose.position.y,
