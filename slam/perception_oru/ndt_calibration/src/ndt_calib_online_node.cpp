...
class PoseInterpolationTF : public PoseInterpolationInteface {
  public:
    PoseInterpolationTF( const std::string& fixedframe, tf::Transformer& transformer )
        : fixedframe_( fixedframe ), transformer_( transformer ) {}

    bool getTransformationForTime( ros::Time t0, const std::string& frame_id, Eigen::Affine3d& T ) {
        std::string str;
        if ( !transformer_.canTransform( fixedframe_, frame_id, t0, &str ) ) { return false; }
...
class NDTCalibOnlineNode {
  ...
  public:
    // Constructor
    NDTCalibOnlineNode( ros::NodeHandle param_nh )
        : tf_listener_( ros::Duration( 3600 ) ), pose_interp_( "/world", tf_listener_ ) {
        ...
        laser_sub_ = nh_.subscribe< sensor_msgs::LaserScan >(
            laser_topic, 1, &NDTCalibOnlineNode::laserCallback, this );
        points2_sub_ = nh_.subscribe< sensor_msgs::PointCloud2 >(
            points_topic, 1, &NDTCalibOnlineNode::points2Callback, this );
    }

    // The T is given in global vehicle coordinates (not any relative ones as commonly done in the
    // registration / fuser approaches.
    void processFrame( pcl::PointCloud< pcl::PointXYZ >& cloud, Eigen::Affine3d T,
                       ros::Time stamp ) {
        if ( !initialized ) {
        ...
        if ( rot.norm() < min_rotation_ ) { return; }
        ...
        if ( pairs_.size() > min_nb_calibration_pairs_ ) {
            if ( pairs_.size() > max_nb_calibration_pairs_ ) {
                pairs_.erase( pairs_.begin(),
                              pairs_.begin() + pairs_.size() - max_nb_calibration_pairs_ );
            }
            this->calibrate();
        }
    ...
    void laserCallback( const sensor_msgs::LaserScan::ConstPtr& msg ) {
        ...
        // TODO, check if the projector class could be used for individual interpolation of laser
        // beams.
    ...
    void calibrate() {
        ...
        opt.interpPairPoses( delta_sensor_time_offset );   // We already moved the interpolation
                                                           // points while adding the clouds to the
                                                           // pairs, the sensortime reported will be
                                                           // the provided on added with the output.

        ROS_INFO_STREAM( "initial sensor pose est : " << affine3dToString( Ts ) << " dt : "
                                                      << sensor_offset_t_ + delta_sensor_time_offset
                                                      << " error : " << opt.getScore6d( Ts ) );
        if ( calib_ ) {
            ...
            std::cout << " new sensor pose est : " << affine3dToString( Ts )
                      << " dt : " << sensor_offset_t_ + delta_sensor_time_offset
                      << " error : " << opt.getScore6d( Ts ) << std::endl;
        }
    }
