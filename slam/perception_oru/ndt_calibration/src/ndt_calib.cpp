...
//////////////////////////////////////////////////////////
double scoreICP( const NDTCalibScanPairs& pairs, const Eigen::Affine3d& Ts ) {
    double e = 0;
    for ( unsigned int i = 0; i < pairs.size(); ++i ) { e += pairs[i].scoreICP( Ts ); }
    return e;
}

double scoreEstSensorPose( const NDTCalibScanPairs& pairs, const Eigen::Affine3d& Ts ) {
    double e = 0;
...
double scoreEstSensorPoseRel( const NDTCalibScanPairs& pairs, const Eigen::Affine3d& Ts ) {
    double e = 0;
    for ( unsigned int i = 0; i < pairs.size(); ++i ) { e += pairs[i].scoreEstSensorPoseRel( Ts ); }
    return e;
}

double scoreNDT( const NDTCalibScanPairs& pairs, const Eigen::Affine3d& Ts ) {
    double e = 0;
    for ( unsigned int i = 0; i < pairs.size(); ++i ) { e += pairs[i].scoreNDT( Ts ); }
    return e;
}

/////////////////////////////////////////////////////////////////////
//! Optimization error functions
double score_f( const gsl_vector* v, void* params ) {
    NDTCalibOptimize* opt;
    opt = (NDTCalibOptimize*) params;

    Eigen::VectorXd x( opt->getNbParameters() );
    for ( int i = 0; i < x.size(); i++ ) { x[i] = gsl_vector_get( v, i ); }

    return opt->getScore( x );
}

void score_df( const gsl_vector* v, void* params, gsl_vector* df ) {
...
void score_fdf( const gsl_vector* x, void* params, double* f, gsl_vector* df ) {
    *f = score_f( x, params );
    score_df( x, params, df );
}

/// This is the most fundamental score. We must end up here independent on how many parameters we
/// try to optimize, note that the time offset has to be updated before this point
/// (interpPairPoses).
double NDTCalibOptimize::getScore6d( const Eigen::Affine3d& T ) const {
    switch ( _scoreType ) {
        case SCORE_ICP: return scoreICP( _pairs, T );
        case SCORE_EST_SENSOR_POSE: return scoreEstSensorPose( _pairs, T );
        case SCORE_EST_SENSOR_POSE_REL: return scoreEstSensorPoseRel( _pairs, T );
        case SCORE_NDT: return scoreNDT( _pairs, T );
        default: std::cerr << "Error: unknown score type : " << _scoreType << std::endl; return -1;
    }
}
...
void NDTCalibOptimize::assignParameters( const Eigen::VectorXd& params, Eigen::Affine3d& Ts,
                                         double& t ) {
    ...
    for ( int i = 0; i < 6; i++ ) { pose_params[i] = all_params[i]; }
...
void NDTCalibOptimize::setInitialParameters( const Eigen::Affine3d& Ts, const double& t ) {
    ...
    for ( size_t i = 0; i < 6; i++ ) { _initParameters[i] = x[i]; }
    _initParameters[6]                                    = t;
}
...
void NDTCalibOptimize::interpPose( double time, Eigen::Affine3d& T ) {
    ...
    if ( !_poseInterp.getTransformationForTime( t, _poseFrameId, T ) ) {
        std::cout << "[FAIL] : couldn't find transformation for time " << t
                  << " at frame : " << _poseFrameId << std::endl;
    }
}
...
bool NDTCalibOptimize::calibrate( Eigen::Affine3d& Ts, double& sensorTimeOffset ) {
    ...
    for ( int i = 0; i < nb_params; i++ ) { gsl_vector_set( x, i, init_offset[i] ); }
    ...
    for ( int i = 0; i < nb_params; i++ ) { x_vec[i] = gsl_vector_get( s->x, i ); }
