  ...
  private:
    ...
    void BuildMap() {
        ...
        for ( size_t i = 0; i < ndts.size(); i++ ) {
            ...
            // if(floor_height - floor_deviation<ndts[i]->getMean()[2] && floor_height +
            // floor_deviation>ndts[i]->getMean()[2]){
            double ccx, ccy, ccz;
            ...
            //}
            // if(!found && ndts[i]->getMean()[2]<floor_deviation+floor_height &&
            // ndts[i]->getMean()[2]>floor_height-floor_deviation ){
        ...
        for ( size_t k = 0; k < floors.size(); k++ ) {
            ...
            for ( double lx = ccx - sx / 2.0 + occ_resolution / 2.0;
                  lx <= ccx + sx / 2.0 - occ_resolution / 2.0; lx += occ_resolution ) {
                for ( double ly = ccy - sy / 2.0 + occ_resolution / 2.0;
                      ly <= ccy + sy / 2.0 - occ_resolution / 2.0; ly += occ_resolution ) {
                    double lccz = ccz;
                    int X       = floor( ( lx + sx / 2.0 ) / occ_resolution + 0.5 ) +
                            height / 2.0;   // round((lx - (occ_resolution - xs) / 2.0) /
                                            // occ_resolution);
                    int Y = floor( ( ly + sy / 2.0 ) / occ_resolution + 0.5 ) +
                            width / 2.0;   // round((ly - (occ_resolution - ys) / 2.0) /
                                           // occ_resolution);
                    ...
                    // "<<floors[j][k]->computeMaximumLikelihoodAlongLine(p1, p2, out)<<" "<<X<<"
                    // "<<Y);

                    // if(floors[k]->hasGaussian_){//computeMaximumLikelihoodAlongLine(p1, p2, out)
                    // > lik_tr){
        ...
        if ( map_file.is_open() ) {
            ...
            for ( int w = width - 1; w >= 0; w-- ) {
                for ( int h = 0; h < height; h++ ) map_file << maps[w][h] << " ";
                map_file << std::endl;
            }
            map_file.close();
        } else
