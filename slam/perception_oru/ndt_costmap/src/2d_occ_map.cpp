...
class map_converter {
  ...
  public:
    map_converter( int argc, char** argv ) {
        ...
        desc.add_options()( "help", "produce help message" )(
            "ndt-file", boost::program_options::value< std::string >( &ndt_map_name ),
            "location of the ndt map you want to conver" )(
            "ndt-resolution",
            boost::program_options::value< double >( &ndt_resolution )->default_value( 1. ),
            "resolution of the map" )(
            "occ-file", boost::program_options::value< std::string >( &occ_map_name ),
            "name of new occupancy map (do not put extension)" )(
            "occ-resolution",
            boost::program_options::value< double >( &occ_resolution )->default_value( 1. ),
            "desired resolution" )(
            "lik_tr", boost::program_options::value< double >( &lik_tr )->default_value( 0.01 ),
            "likelihood treshold the lower value the more conservative map" );
        boost::program_options::variables_map vm;
        boost::program_options::store(
            boost::program_options::parse_command_line( argc, argv, desc ), vm );
        boost::program_options::notify( vm );

        if ( !vm.count( "ndt-file" ) || !vm.count( "ndt-resolution" ) || !vm.count( "occ-file" ) ||
             !vm.count( "occ-resolution" ) ) {
  ...
  private:
    int LoadMap() {
        ...
        if ( ndtMap->loadFromJFF( jffin ) < 0 ) { return -1; }
        return 0;
    }
    ...
    void BuildMap() {
        ...
        for ( double current_x = -xs / 2 + occ_resolution; current_x < xs / 2 - occ_resolution;
              current_x += occ_resolution ) {
            // std::cout << "first loop \n";
            int w = 0;
            for ( double current_y = -ys / 2 + occ_resolution; current_y < ys / 2 - occ_resolution;
                  current_y += occ_resolution ) {
                // std::cout << "H:"<<h<< " W:"<<w<<"\n";
                ...
                if ( ndtMap->getCellAtPoint( p, check_cell ) ) {
                    ...
                    if ( check_cell->getOccupancy() < 0.0 ) {
                        ...
                        if ( check_cell->getLikelihood( p ) > lik_tr ) {
                            ...
                            // double
                            // d=sqrt((m[0]-current_x)*(m[0]-current_x)+(m[1]-current_y)*(m[1]-current_y));
    ...
    void SaveMap() {
        ...
        if ( map_file.is_open() ) {
            ...
            for ( int w = width - 1; w >= 0; w-- ) {
                for ( int h = 0; h < height; h++ ) { map_file << map[w][h] << " "; }
                map_file << std::endl;
            }
            map_file.close();
        } else
