...
visualization_msgs::Marker correspondenceMarkers( const Correspondences& correspondences,
    ...
    BOOST_FOREACH ( const Correspondence& c, correspondences ) {
        const tf::Vector3 curr_pt( c.first->getPosition().x, c.first->getPosition().y,
                                   0.0 );   // Current
        const tf::Vector3 ref_pt( c.second->getPosition().x, c.second->getPosition().y,
                                  0.0 );   // Ref
...
// the two scans
visualization_msgs::Marker interestPointMarkersFrameId( const InterestPointVec& pts,
                                                        const Eigen::Affine3d& T, const unsigned id,
                                                        const std::string& frame_id ) {
    visualization_msgs::Marker m;
...
visualization_msgs::Marker interestPointMarkersFrameId( const InterestPointVec& pts,
    ...
    T = Eigen::Translation3d( pose.position.x, pose.position.y, pose.position.z ) * qd;
...
visualization_msgs::Marker interestPointSupportMarkers( const InterestPointVec& pts,
                                                        const geometry_msgs::Pose& pose,
                                                        const unsigned id ) {
    tf::Transform trans;
    tf::poseMsgToTF( pose, trans );
...
visualization_msgs::Marker poseArrowMarker( const geometry_msgs::Pose& pose, const int id,
                                            const int color, const std::string& frame ) {
    visualization_msgs::Marker m;
    ndt_visualisation::assignDefault( m );
...
visualization_msgs::Marker poseArrowMarkerEigen( const Eigen::Affine3d& pose, const int id,
                                                 const int color, const std::string& frame ) {
...
visualization_msgs::Marker posePointsMarkerNDTFeatureFrames(
    const std::vector< ndt_feature::NDTFeatureFrame >& frames, const int id, const int color,
    const std::string& frame, bool useOdom ) {
    visualization_msgs::Marker m;
    ndt_visualisation::assignDefault( m );
    ndt_visualisation::assignColor( m, color );
...
void publishMarkerNDTFeatureFrames( const std::vector< ndt_feature::NDTFeatureFrame >& frames,
...
visualization_msgs::Marker featureMatchesMarkerNDTFeatureFrames(
    const std::vector< ndt_feature::NDTFeatureFrame >& frames,
    const std::vector< std::pair< size_t, size_t > >& matches, const int id, const int color,
    const std::string& frame, bool useOdom ) {
...
void publishMarkerNDTFeatureNodes( const NDTFeatureGraphInterface& graph,
                                   ros::Publisher& marker_pub ) {
    int id = 0;
    for ( size_t i = 0; i < graph.getNbNodes(); i++ ) {
        marker_pub.publish( poseArrowMarkerEigen( graph.getNodeInterface( i ).getPose(), id, 2,
                                                  std::string( "nodes_pose" ) ) );
        id++;
    }
}

visualization_msgs::Marker markerNDTFeatureLinks( const NDTFeatureGraphInterface& graph,
                                                  const int id, const int color,
                                                  const std::string& frame, bool skipOdom ) {
    visualization_msgs::Marker m;
    ndt_visualisation::assignDefault( m );
    ndt_visualisation::assignColor( m, color );
    m.ns      = frame;
    ...
    for ( size_t i = 0; i < graph.getNbLinks(); i++ ) {
        if ( skipOdom ) {
            if ( graph.getLinkInterface( i ).getScore() < 0 ) { continue; }
        }
        geometry_msgs::Pose pose1, pose2;
...
void publishMarkerNDTFeatureLinks( const NDTFeatureGraphInterface& graph,
                                   ros::Publisher& marker_pub ) {
