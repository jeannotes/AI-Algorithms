...
class NDTFuserNode {
  protected:
    ...
    std::string points_topic, laser_topic, map_dir, map_name, odometry_topic, world_frame,
        robot_frame, sensor_frame, fuser_frame, init_pose_frame, gt_topic, bag_name;
    double size_x, size_y, size_z, resolution, sensor_range, min_laser_range_;
    bool visualize, match2D, matchLaser, beHMT, useOdometry, plotGTTrack, initPoseFromGT,
        initPoseFromTF, initPoseSet, renderGTmap;
  ...
  public:
    ...
    NDTFuserNode( ros::NodeHandle param_nh ) : nb_added_clouds_( 0 ) {
        ...
        _gaussian_pub =
            param_nh.advertise< visualization_msgs::Marker >( "gaussian_that_gave_corners", 10 );
        _gaussian_pub2 =
            param_nh.advertise< visualization_msgs::Marker >( "gaussian_that_gave_corners2", 10 );
        _corner_orientation_pub =
            param_nh.advertise< visualization_msgs::Marker >( "corner_orientation", 10 );
        _ndt_corner_pub =
            param_nh.advertise< visualization_msgs::Marker >( "corner_ndt_marker", 10 );
        ...
        if ( tf_pose_frame_ != std::string( "" ) ) { use_tf_listener_ = true; }
        ...
        fuser = new perception_oru::ndt_fuser::NDTFuserHMTLogger(
            "/home/malcolm/Documents/log_fuser/log_fuser.txt", resolution, size_x, size_y, size_z,
        ...
        if ( !matchLaser ) {
            ...
            if ( useOdometry ) {
                ...
                sync_po_->registerCallback(
                    boost::bind( &NDTFuserNode::points2OdomCallback, this, _1, _2 ) );
            } else {
                points2_sub_->registerCallback(
                    boost::bind( &NDTFuserNode::points2Callback, this, _1 ) );
            }
        } else {
            ...
            if ( useOdometry && !renderGTmap ) {
                ...
                sync_lo_->registerCallback(
                    boost::bind( &NDTFuserNode::laserOdomCallback, this, _1, _2 ) );
            } else if ( !renderGTmap ) {
                std::cout << "which1" << std::endl;
                laser_sub_->registerCallback(
                    boost::bind( &NDTFuserNode::laserCallback, this, _1 ) );
            } else {
                std::cout << "which2" << std::endl;
                // this render map directly from GT
                odom_sub_ =
                    new message_filters::Subscriber< nav_msgs::Odometry >( nh_, gt_topic, 10 );
                sync_lo_ = new message_filters::Synchronizer< LaserOdomSync >(
                    LaserOdomSync( SYNC_FRAMES ), *laser_sub_, *odom_sub_ );
                sync_lo_->registerCallback(
                    boost::bind( &NDTFuserNode::laserOdomCallback, this, _1, _2 ) );
        ...
        if ( plotGTTrack ) {
            gt_sub = nh_.subscribe< nav_msgs::Odometry >( gt_topic, 10, &NDTFuserNode::gt_callback,
                                                          this );
        }
    ...
    ~NDTFuserNode() { delete fuser; }

    void processFrame( pcl::PointCloud< pcl::PointXYZ >& cloud, Eigen::Affine3d Tmotion ) {
        m.lock();
        if ( nb_added_clouds_ == 0 ) {
            ROS_INFO( "initializing fuser map. Init pose from GT? %d, TF? %d", initPoseFromGT,
                      initPoseFromTF );
            if ( initPoseFromGT ) {
                ...
                ROS_INFO( "Init pose is (%lf,%lf,%lf)", pose_.translation()( 0 ),
                          pose_.translation()( 1 ), pose_.rotation().eulerAngles( 0, 1, 2 )( 0 ) );
                fuser->initialize( pose_, cloud );
                std::cout << "Saving map of cell : " << fuser->map->getAllCells().size()
                          << std::endl;
                // 	  		exit(0);
            ...
            if ( ( Tmotion.translation().norm() < 0.01 &&
                   Tmotion.rotation().eulerAngles( 0, 1, 2 )( 2 ) < 0.01 ) &&
                 useOdometry ) {
...
#endif
        tf_.sendTransform(
            tf::StampedTransform( transform, ros::Time::now(), world_frame, fuser_frame ) );

        // Extract Corners

        extractCorners();

        std::cout << "Exporting the map of size "
                  << fuser->map->getAllInitializedCellsShared().size() << std::endl;
    ...
    void drawCorners(
        const std::vector< perception_oru::ndt_feature_finder::NDTCornerBundle >& corners ) {
        ...
        for ( auto it = corners.begin(); it != corners.end(); ++it ) {
            ...
            for ( auto it_ndt = ( *it ).getCells1().begin(); it_ndt != ( *it ).getCells1().end();
                  ++it_ndt ) {
            ...
            for ( auto it_ndt = ( *it ).getCells2().begin(); it_ndt != ( *it ).getCells2().end();
                  ++it_ndt ) {
                Eigen::Vector3d mean = ( *it_ndt )->getMean();
                auto angle           = perception_oru::ndt_feature_finder::NDTCellAngle( **it_ndt );
    ...
    void
    drawOrientation( const perception_oru::ndt_feature_finder::NDTCornerBundle& corner_bundle ) {
        ...
        int i = 0;
        for ( auto it = corner_bundle.getOrientations().begin();
              it != corner_bundle.getOrientations().end(); ++it ) {
    ...
    void points2Callback( const sensor_msgs::PointCloud2::ConstPtr& msg_in ) {
        ...
        ROS_INFO_STREAM( "transf: " << transf.translation().transpose() << " "
                                    << transf.rotation().eulerAngles( 0, 1, 2 ) );
        if ( nb_added_clouds_ == 0 ) {
            last_tf_frame_ = transf;
            Tm.setIdentity();
        } else {
            Tm = last_tf_frame_.inverse() * transf;
            ROS_INFO_STREAM( "delta from last update: "
                             << Tm.translation().transpose() << " "
                             << Tm.rotation().eulerAngles( 0, 1, 2 )[2] );
            last_tf_frame_ = transf;
        }
    ...
    void points2OdomCallback( const sensor_msgs::PointCloud2::ConstPtr& msg_in,
        ...
        this_odom =
            Eigen::Translation3d( odo_in->pose.pose.position.x, odo_in->pose.pose.position.y,
        ...
        if ( nb_added_clouds_ == 0 ) {
            ...
            std::cout << "delta from last update: " << Tm.translation().transpose() << " "
                      << Tm.rotation().eulerAngles( 0, 1, 2 )[2] << std::endl;
            // if(Tm.translation().norm()<0.2 &&
            // fabs(Tm.rotation().eulerAngles(0,1,2)[2])<(5*M_PI/180.0)) {
            //    message_m.unlock();
            //    return;
            //}
        }
    ...
    void laserOdomCallback( const sensor_msgs::LaserScan::ConstPtr& msg_in,
        ...
        // std::cout<<"AT: "<<this_odom.translation().transpose()<<"
        // "<<this_odom.rotation().eulerAngles(0,1,2)[2] << std::endl;
    ...
    void gt_callback( const nav_msgs::Odometry::ConstPtr& msg_in ) {
        ...
        if ( visualize ) {
#ifndef NO_NDT_VIZ
            fuser->viewer->addTrajectoryPoint( gt_pose.translation()( 0 ),
                                               gt_pose.translation()( 1 ),
                                               gt_pose.translation()( 2 ) + 0.2, 1, 1, 1 );
