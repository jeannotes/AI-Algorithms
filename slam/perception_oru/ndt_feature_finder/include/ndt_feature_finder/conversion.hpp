...
inline cv::Point2i scalePoint( const cv::Point2d& p, double max, double min,
    ...
    if ( center.x >= size_image_max || center.y >= size_image_max || center.x <= 0 ||
         center.y <= 0 ) {
        std::cout << "center " << center << " size_ image " << size_image_max << " min " << min
                  << " max " << max << " for point " << p << std::endl;
    }
...
inline void drawCell( const perception_oru::NDTCell& cell, cv::Mat& dst, int scale = 50 ) {
    ...
    // 			std::cout << "CENTER " << center.x << " " << center.y << " for " <<mean(0) << " " <<
    // mean(1)<< std::endl;
    ...
    // 			std::cout << "Size : " << cv::Size(eigen_val, eigen_val1) << " " << evals(0) << " "
    // <<
    // evals(1) << std::endl;
    ...
    cv::ellipse( dst, center, cv::Size( eigen_val, eigen_val1 ), rotation_degree, 0, 360,
                 cv::Scalar( 255, 255, 255 ), -1 );
}

inline void drawCell( const perception_oru::NDTCell& cell, cv::Mat& dst, double max, double min,
                      double size_image_max ) {
    Eigen::Vector3d mean  = cell.getMean();
    Eigen::Vector3d evals = cell.getEvals();
    Eigen::Matrix3d evecs = cell.getEvecs();
    ...
    cv::Point2d center =
        scalePoint( cv::Point2d( mean( 0 ), mean( 1 ) ), max, min, size_image_max );
    // The 3 was found in the rviz visualization package
    // 			center.x = mean(0) * scale;
    // 			center.y = mean(1) * scale;

    // 			std::cout << "CENTER " << center.x << " " << center.y << " for " <<mean(0) << " " <<
    // mean(1)<< " max min " << max << "  " << min << " size image max " << size_image_max <<
    // std::endl;
...
inline void toCvMat( const perception_oru::NDTMap& map, cv::Mat& dst, double size_image_max,
    ...
    for ( size_t i = 0; i < cells.size(); ++i ) {
        ...
        if ( max < mean( 0 ) ) { max = mean( 0 ); }
        if ( max < mean( 1 ) ) { max = mean( 1 ); }
        if ( min > mean( 0 ) ) { min = mean( 0 ); }
        if ( min > mean( 1 ) ) { min = mean( 1 ); }
    }
    ...
    // Should it be in meters ?
    map.getGridSizeInMeters( cx, cy, cz );
    std::cout << " Size grid " << cx << " " << cy << " " << cz << " and max min " << max << " "
              << min << std::endl;
