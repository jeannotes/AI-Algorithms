...
class NDTCorner {
  ...
  public:
     ...
     */
    bool cellIsCorner( const perception_oru::NDTMap& map, const perception_oru::NDTCell& cell,
                       const std::vector< boost::shared_ptr< perception_oru::NDTCell > >& allCells,
                       NDTCornerBundle& corner );

    /**
     * @brief calculate and return a vector of all corners in ndt_map. Return the direction of the
     * smallest angle of the corner
     */
    std::vector< NDTCornerBundle > getAllCorners( const perception_oru::NDTMap& map );

    /**
     * @brief calculate and return a vector of all corners in ndt_map. Use the robot pose to detect
    ...
    /**
     * @brief return all the cells around cell that possess a gaussian and are maximum the
     * resolution of the map away.
     */
    // 			void getClosestCells(const perception_oru::NDTMap& map, const
    // perception_oru::NDTCell&
    // cell, int neig_size, std::vector< boost::shared_ptr< perception_oru::NDTCell > >&
    // cells_gaussian, std::vector< boost::shared_ptr< perception_oru::NDTCell > >&
    // cells_initialized) const ;
     ...
     */
    // 			int getBiggestEigenVector2D(const perception_oru::NDTCell& cell, Eigen::Vector3d&
    // eigenval, Eigen::Matrix3d& eigenvec) const;
    ...
    // 			const std::vector< Eigen::Vector3d >& getAccurateCorners() const {return
    // _corners_position;}
    // 			std::vector< cv::Point2d >& getAccurateCvCorners(){return _opencv_corners_position;}
    // 			const std::vector< cv::Point2d >& getAccurateCvCorners() const {return
    // _opencv_corners_position;}
    // 			const std::vector<std::pair<double,double> >& getAngles() const
    // {assert(_angles.size()
    // ==_corners.size()); return _angles;}
    // 			std::vector<std::pair<double,double> >& getAngles(){assert(_angles.size() ==
    // _corners.size()); return _angles;}

    // 			void exportCorners(std::ostream& out){
    // 				for(size_t i = 0 ; i < _opencv_corners.size() ; ++i){
    // 					std::cout << _opencv_corners[i].x << " " << _opencv_corners[i].y <<
    // std::endl;
  ...
  private:
    ...
    getCellsPointingToward( std::vector< perception_oru::NDTCell* >& neighbor,
                            const perception_oru::NDTCell& cell ) const;
    bool AlignedNDT( const std::vector< perception_oru::NDTCell* >& neighbor,
                     const perception_oru::NDTCell& cell );
};
}
}
