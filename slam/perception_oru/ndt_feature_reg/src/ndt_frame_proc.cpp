...
{
void NDTFrameProc::addFrameIncremental( NDTFrame* f, bool skipMatching, bool ndtEstimateDI,
                                        bool match_full, bool match_no_association ) {
    if ( !match_full ) { detectKeypoints( f ); }
    ...
    NDTFrameProc::EigenTransform tr;
    tr.setIdentity();
    if ( frames.size() == 1 ) {
    ...
    if ( !( match_full || match_no_association ) ) {
        ...
        if ( !skipMatching ) {
            ...
            lslgeneric::NDTMatcherFeatureD2D matcher_feat( corr, trim_factor );
            matcher_feat.match( frames[i - 1]->ndt_map, frames[i]->ndt_map, tr,
                                true );   // true = use initial guess
...
void NDTFrameProc::trimNbFrames( size_t maxNbFrames ) {
    if ( frames.size() > maxNbFrames ) {
        ...
        transformVector.erase( transformVector.begin(),
                               transformVector.end() -
                                   1 );   // Just to keep a 1-1 coorespondance with frames.
    }
}

void NDTFrameProc::processFrames( bool skipMatching, bool ndtEstimateDI, bool match_full,
                                  bool match_no_association ) {
    ...
    if ( !match_full ) { detectKeypoints( frames[0] ); }
    ...
    for ( size_t i = 1; i < frames.size(); i++ ) {
        if ( !match_full ) { detectKeypoints( frames[i] ); }
        ...
        /*
            transformVector.push_back(tr);
            cv::Mat display;
            drawMatches(frames[i-1]->img, frames[i-1]->kpts, frames[i]->img, frames[i]->kpts,
           pe.inliers, display);
...
void NDTFrameProc::detectKeypoints( NDTFrame* f ) const {
    if ( img_scale == 1. ) {
...
void NDTFrameProc::calcDescriptors( NDTFrame* f ) const {
    if ( img_scale == 1. ) {
...
NDTFrameProc::createColoredFeaturePC( NDTFrame& f, pcl::PointXYZRGB color ) {
...
{
     // Load the file.
     f.depth_img = cv::imread(fileName, CV_LOAD_IMAGE_ANYDEPTH); // CV_LOAD_IMAGE_ANYDEPTH is
important to load the 16bits image
...
template <typename PointT>
bool
NDTFrameProc<PointT>::setupDepthToPC(const std::string &fileName, double fx, double fy, double cx,
double cy, const std::vector<double> &dist, double ds)
{
...
template <typename PointT>
void
NDTFrameProc<PointT>::setupDepthToPC(const cv::Mat &depthImg, double fx, double fy, double cx,
double cy, const std::vector<double> &dist, double ds)
{
