...
{
// ICP wrapper
// bool NDTMapBuilder::matchICP(pcl::PointCloud<pcl::PointXYZ> &target,
...
bool NDTMapBuilder::addScan( pcl::PointCloud< pcl::PointXYZ > scan, int id ) {
    ...
    if ( id - 1 < 0 || id - 1 > vertices.size() ) return false;
    ...
    for ( int q = 0; q < 4; q++ ) {
        ...
        //	cout<<"T init "<<T.translation().transpose()<<" r
        //"<<T.rotation().eulerAngles(0,1,2).transpose()<<endl;
        Tndt.setIdentity();
        ...
        std::cout << "T fin " << Tout.translation().transpose() << " r "
                  << Tout.rotation().eulerAngles( 0, 1, 2 ).transpose() << std::endl;
...
void NDTMapBuilder::saveG2OlogFile( const char* fname ) {
    ...
    for ( int i = 0; i < edges.size(); i++ ) {
        ...
        fprintf( fout, "EDGE_SE3:QUAT %d %d %lf %lf %lf %lf %lf %lf %lf ", edges[i].idFirst,
                 edges[i].idSecond, t( 0 ), t( 1 ), t( 2 ), q.x(), q.y(), q.z(), q.w() );

        Eigen::Matrix< double, 6, 6 > information = ( edges[i].covariance + reg * eye ).inverse();
        for ( int p = 0; p < 6; p++ ) {
            for ( int q = p; q < 6; q++ ) { fprintf( fout, "%lf ", information( p, q ) ); }
        }
        fprintf( fout, "\n" );
    }
    fclose( fout );
...
void NDTMapBuilder::printNodePositions() {
    ...
    for ( int i = 0; i < vertices.size(); i++ ) {
        std::cout << vertices[i].pose.translation().transpose() << " "
                  << vertices[i].pose.rotation().eulerAngles( 0, 1, 2 ) << std::endl;
    }
}
}
