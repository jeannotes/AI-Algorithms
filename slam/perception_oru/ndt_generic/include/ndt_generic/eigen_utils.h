...
Eigen::Affine3d getAffine3dMean( const std::vector< Eigen::Affine3d >& Ts ) {
    ...
    // This works nice to average the rotation. However if there are different weights (e.g. the
    // factor is not constant this is *NOT* a suitable approach(!)
...
Eigen::Affine3d getAffine3dMeanWeightsUsingQuat( const std::vector< Eigen::Affine3d >& Ts,
    ...
    // According the eigen they are not sorted in any way... simply check the for the largest eigen
    // vector.
    int idx;
...
Eigen::Vector3d getWeightedPoint( const Eigen::Vector3d& pt1, const Eigen::Matrix3d& cov1,
                                  const Eigen::Vector3d& pt2, const Eigen::MatrixXd& cov2 ) {
...
Eigen::Affine3d getWeightedPose( const Eigen::Affine3d& pose1, const Eigen::MatrixXd& cov1,
                                 const Eigen::Affine3d& pose2, const Eigen::MatrixXd& cov2 ) {
