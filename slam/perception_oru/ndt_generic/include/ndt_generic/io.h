...
{
// Provides a vector of files (or if the provided path string is a file then add it into the
// vector).
...
void extract_file_names( const std::string& path, const std::string& ext,
    ...
    for ( boost::filesystem::recursive_directory_iterator i( _path ); i != end; ++i ) {
        ...
        if ( !ext.empty() && cp.extension() != ext ) continue;
...
void loadCloud( const std::string& base_name_pcd, int counter,
    ...
    std::string pcd_file = base_name_pcd + std::string( "cloud" ) +
                           ndt_generic::toString( counter ) + std::string( ".pcd" );
...
std::vector< double > loadDoubleVecTextFile( const std::string& fileName ) {
    ...
    while ( !ifs.eof() ) {
        ...
        double val;
        if ( sscanf( line.c_str(), "%lf", &val ) == 1 ) { vec.push_back( val ); }
    }
    return vec;
}
...
void saveDoubleVecTextFile( const std::vector< double >& vec, const std::string& fileName ) {
    ...
    for ( unsigned int i = 0; i < vec.size(); i++ ) { ofs << vec[i] << std::endl; }
    ofs.close();
}
...
class CreateEvalFiles {
  public:
    ...
    void Write( const ros::Time frame_time, const Eigen::Affine3d& Tgtbase,
                const Eigen::Affine3d& Todombase, const Eigen::Affine3d& Tfuserpose,
                const Eigen::Affine3d& sensoroffset ) {
        if ( !enable_ ) return;
        ...
        sensorpose_est_file << frame_time << " "
                            << transformToEvalString( Tfuserpose * sensoroffset );
        gt_file.flush();
        odom_file.flush();
        est_file.flush();
