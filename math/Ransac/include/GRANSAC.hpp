...
class RANSAC {
  private:
    ...
    std::shared_ptr< T > m_BestModel;   // Pointer to the best model, valid only after Estimate() is
                                        // called
    ...
    std::vector< std::mt19937 > m_RandEngines;   // Mersenne twister high quality RNG that support
  ...
  public:
    ...
    const std::vector< std::shared_ptr< AbstractParameter > >& GetBestInliers( void ) {
        return m_BestInliers;
    };

    bool Estimate( std::vector< std::shared_ptr< AbstractParameter > > Data ) {
        if ( Data.size() <= t_NumParams ) {
            std::cout << "[ WARN ]: RANSAC - Number of data points is too less. Not doing anything."
                      << std::endl;
            return false;
        }
        ...
        std::vector< std::vector< std::shared_ptr< AbstractParameter > > > InliersAccum(
            m_MaxIterations );
...
#pragma omp parallel for
        for ( int i = 0; i < m_MaxIterations; ++i ) {
            ...
            std::vector< std::shared_ptr< AbstractParameter > > RemainderSamples =
                m_Data;   // Without the chosen random samples

            std::shuffle( RemainderSamples.begin(), RemainderSamples.end(),
                          m_RandEngines[omp_get_thread_num()] );   // To avoid picking the same
                                                                   // element more than once
            std::copy( RemainderSamples.begin(), RemainderSamples.begin() + t_NumParams,
                       RandomSamples.begin() );
            RemainderSamples.erase( RemainderSamples.begin(),
                                    RemainderSamples.begin() + t_NumParams );

            std::shared_ptr< T > RandomModel = std::make_shared< T >( RandomSamples );

            // Check if the sampled model is the best so far
            std::pair< VPFloat, std::vector< std::shared_ptr< AbstractParameter > > > EvalPair =
                RandomModel->Evaluate( RemainderSamples, m_Threshold );
