...
// Could be anything from a point (that make a 2D line or 3D plane or image correspondences) to a
// line
...
class AbstractModel {
  ...
  public:
    ...
    virtual std::pair< VPFloat, std::vector< std::shared_ptr< AbstractParameter > > > Evaluate(
        std::vector< std::shared_ptr< AbstractParameter > > EvaluateParams, VPFloat Threshold ) = 0;

    virtual std::array< std::shared_ptr< AbstractParameter >, t_NumParams > GetModelParams( void ) {
